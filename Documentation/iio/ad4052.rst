...
is ADC conversion ready signal exposed as a interrupt.
...
The ``interrupt-names`` ``gp1`` sets the role of Data Ready signal. If it is
not, the driver will not wait for the data ready assertion, and will result in
reading a sample before it's ready, particularly in oversampling mode.
