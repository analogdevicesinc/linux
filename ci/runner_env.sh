...
assert_job_summary () {
	local msg="Errors and warnings are annotated on the changed files, in each job step, as well as aggregated on the Summary page."$_n"Some messages may not cause the step to fail, but still need to be checked."$_n
	local fail=""
	local warn=""

	for job_var_name in $(compgen -A variable job_); do
		local job_name="${job_var_name#job_}"
		local json="${!job_var_name}"
		...
		[[ -n "$fail_" ]] && fail+=$_n"  job $job_name:"$_n"$fail_"
		...
		[[ -n "$warn_" ]] && warn+=$_n"  job $job_name:"$_n"$warn_"
	done

	if [[ -n "$fail" ]]; then
		if [[ -n "$warn" ]]; then
			echo "::error::Some jobs didn't pass strict checks:$fail"$_n$_n"And non-strict checks:$warn"$_n$_n"$msg"
		else
			echo "::error::Some jobs didn't pass strict checks:$fail"$_n$_n"$msg"
		fi
		exit 1
	else
		if [[ -n "$warn" ]]; then
			echo "::warning::Some jobs didn't pass non-strict checks:$warn"$_n$_n"$msg"
		fi
	fi
}
