...
artifacts_structure() {
	...
	echo "git_branch=${BUILD_SOURCEBRANCHNAME}" >> ${timestamp}/rpi_archives_properties.txt
	echo "git_sha=${GIT_SHA}" >> ${timestamp}/rpi_archives_properties.txt
	echo "git_sha_date=${GIT_SHA_DATE}" >> ${timestamp}/rpi_archives_properties.txt
...
artifacts_swdownloads() {
	...
	scp -2 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o HostKeyAlgorithms=+ssh-dss \
		-v -i ${KEY_FILE} -r rpi_archives_properties.txt ${DEST_SERVER}/rpi-6.6.y
