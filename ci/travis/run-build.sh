...
build_default() {
	...
	# xilinx toolchain (that comes with vivado 2022.2), the build system will detect that
	# GCC_PLUGINS are enabled, as the toolchan "supports" them, but the build will fail to
	# build them... As we want to be able to compile the kernel with xilinx toolchain (do we?)
	# we need to explicitly disable GCC_PLUGINS in our defconfigs (as xilinx is also doing BTW).
