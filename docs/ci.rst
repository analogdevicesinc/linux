...
In this sections there are instructions to bring-up your own continuous integration,
...
* | ``mirror/jic23/iio/master``: Linux IIO Subsystem (Cameron's branch)
  | https://git.kernel.org/pub/scm/linux/kernel/git/jic23/iio.git/log/?h=master
...
The continuous intergrations has some features that are described in this section.

Warning and error handling
~~~~~~~~~~~~~~~~~~~~~~~~~~

Each tool will have it's own outputs and the standard to consider something
...
* ``fail``: Indicates that an warning or error deemed strict was raised.
  Is collected at the assert job and ends the run with failure.
* ``warn``: Indicates that an warning or error non-deemed strict was raised.
...
It allows to manipulate the source code depending on the run conditions, and can be used
...
so ensure to filter on the scripts themself which file they manipulate.
...
.. shell::
   ...
   ~/linux
   $act pull_request --remote-name private
...
Additional arguments are added from the :git-linux:`.actrc` on invoke.
...
The :git-doctools:`act-report.sh <ci/scripts/act-report.sh>`
bash script is a suggested method to tune down the verbosity while parsing GitHub Actions
annotations to human readable, for example:

.. shell::

   ~/linux
   $act-report
    [build_llvm_x86_64/build.yml/build] Set up job
    [build_gcc_x86_64/build.yml/build] Set up job
    [build_gcc_aarch64/build.yml/build] Set up job
    [ ... ]
    [checks/checks.yml/checks] Main Check patch
    [build_gcc_arm/build.yml/build] Main Make defconfig
    [build_gcc_arm/build.yml/build] Main Imply driver config and apply cocci/bash
    [checks/checks.yml/checks] warning drivers/iio/adc/ad4000.c:41

        Argument '_sign' is not used in function-like macro
        +#define __AD4000_DIFF_CHANNEL(_sign, _real_bits, _storage_bits, ... \
        {                                                                    \
        .type = IIO_VOLTAGE,                                                 \
        .indexed = 1,
    [ ... ]
