.. imported from: https://wiki.analog.com/resources/tools-software/linux-drivers/iio-pll/ad9162
...
The AD9161/AD91621 are high performance, 11-bit/16-bit digital-to-analog
converters (DACs) that supports data rates to 6 GSPS. The DAC core is based on a
quad-switch architecture coupled with a 2x interpolator filter that enables an
effective DAC update rate of up to 12 GSPS in some modes. The high dynamic range
and bandwidth makes these DACs ideally suited for the most demanding high speed
...
.. list-table::
   ...
   * - :git-linux:`drivers/iio/frequency/ad9162.c`
     - `WIP <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/iio/frequency/ad9162.c>`__
     -
     -
...
.. list-table::
   ...
   * - driver
     ...
     -
   * - driver
     - :git-linux:`drivers/iio/frequency/ad916x_amp.c <drivers/iio/amplifiers/ad916x_amp.c>`
     -
   * - API driver
     - :git-linux:`drivers/iio/frequency/ad916x`
     -
   * - driver
   ...
   * - include
     ...
     -
   * - Documentation
     - :git-linux:`ad916x.yaml <Documentation/devicetree/bindings/iio/frequency/ad916x.yaml>`
     -
   * - Documentation
     - :git-linux:`ad916x-amp.yaml <Documentation/devicetree/bindings/iio/amplifiers/ad916x-amp.yaml>`
     -
...
.. list-table::
   ...
   * - Function
     ...
     -
   * - dtsi
     - :git-linux:`adi-fmcomms11.dtsi <arch/arm/boot/dts/adi-fmcomms11.dtsi>`
     -
   * - dtsi
     - :git-linux:`adi-fmcomms11-RevA.dtsi <arch/arm/boot/dts/adi-fmcomms11-RevA.dtsi>`
     -
   * - dts
     - :git-linux:`rpi-cn0511-overlay.dts <rpi-5.4.y:arch/arm/boot/dts/overlays/rpi-cn0511-overlay.dts>`
     -
   * - dts
     - :git-linux:`zynq-zc706-adv7511-fmcomms11.dts <arch/arm/boot/dts/zynq-zc706-adv7511-fmcomms11.dts>`
     -
   * - dts
     - :git-linux:`zynq-zc706-adv7511-fmcomms11-RevA.dts <arch/arm/boot/dts/zynq-zc706-adv7511-fmcomms11-RevA.dts>`
     -
...
For detailed information about the required/optional devicetree properties look
at
:git-linux:`ad916x.yaml <Documentation/devicetree/bindings/iio/frequency/ad916x.yaml>`
and
:git-linux:`ad916x-amp.yaml <Documentation/devicetree/bindings/iio/amplifiers/ad916x-amp.yaml>`.
...
The driver has two main ways of functioning depending on the
:git-linux:`adi,standalone-probe <Documentation/devicetree/bindings/iio/frequency/ad916x.yaml#L49>`
devicetree property. If set, the driver will probe in standalone mode which
means that it won’t link with the :ref:`axi-dac-dds-hdl`. This means that
different attributes will be seen depending on the working mode. The next ABI is
the same independent of the working mode.
...
Range is: −fclk/2 ≤ fCARRIER < +fclk/2 or 0 ≤ fCARRIER < fclk if FIR85 is
enabled
...
This mode can be used together with
:git-linux:`adi,dc-test-en <Documentation/devicetree/bindings/iio/frequency/ad916x.yaml#L42>`
for running the device as a signal generator. The present attributes are:
...
Set the DC test word data. This will be the data source of the digital path when
:git-linux:`adi,dc-test-en <Documentation/devicetree/bindings/iio/frequency/ad916x.yaml#L42>`
is enabled. We can control the signal amplitude with this attribute.
...
This setting allows for runtime calibration of the device internal temperature
sensor (mostly useful for evaluation and testing). For that, a valid temperature
in milli degrees °C should be written to this attribute. Internally, the driver
will get the raw temperature value. With this pair of values, the driver can
then calculate the parameters needed to get the processed temperature from the
sensor…
...
As said, in this mode the driver is used in conjunction with the
:ref:`axi-dac-dds-hdl`.
...
The AD9166 comes with an integrated differential to single-ended output buffer
amplifier, matched to a 50Ω load up to 10 GHz. This amplifier has it’s own SPI
interface and thus, it has it’s own device driver.
