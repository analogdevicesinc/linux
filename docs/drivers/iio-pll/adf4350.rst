.. imported from: https://wiki.analog.com/resources/tools-software/linux-drivers/iio-pll/adf4350
...
- :adi:`ADF4350 Eval Board <EVAL-ADF4350EB1Z>`
- :adi:`ADF4351 Eval Board <EVAL-ADF4351EB1Z>`
...
.. list-table::
   ...
   * - Source
     ...
     -
     -
   * - `git <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/iio/frequency/adf4350.c>`__
     - `Yes <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/iio/frequency/adf4350.c>`__
     -
     -
...
.. list-table::
   ...
   * - Function
     - File
   * - driver
     - `drivers/iio/frequency/adf4350.c <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/iio/frequency/adf4350.c>`__
   * - include
     - `include/linux/iio/frequency/adf4350.h <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/linux/iio/frequency/adf4350.h>`__
   * - Documentation
     - `Documentation/ABI/testing/sysfs-bus-iio-frequency-adf4350 <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/Documentation/ABI/testing/sysfs-bus-iio-frequency-adf4350>`__
   * - Documentation
     - `Documentation/devicetree/bindings/iio/frequency/adf4350.txt <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/Documentation/devicetree/bindings/iio/frequency/adf4350.txt>`__
...
Stores PLL Y frequency in Hz. Reading returns the actual frequency in Hz. When
setting a new frequency, the PLL requires some time to lock. If available, the
user can read out_altvoltageY_locked in order to check whether the PLL has
locked or not.
...
Stores PLL Y frequency resolution/channel spacing in Hz. The value given
directly influences the MODULUS used by the fractional-N PLL. It is assumed that
the algorithm that is used to compute the various dividers, is able to generate
proper values for multiples of channel spacing.
...
Sets PLL Y REFin frequency in Hz. In some clock chained applications, the
reference frequency used by the PLL may change during runtime. This attribute
allows the user to adjust the reference frequency accordingly. The value written
has no effect until out_altvoltageY_freq is updated.
...
If available, this attribute allows the user to power down the PLL and it’s
RFOut buffers. This is in particular useful during REFin changes.
...
If available, this attribute allows the user to determine whether the PLL has
locked by reading ‘1’ or not ‘0’.
