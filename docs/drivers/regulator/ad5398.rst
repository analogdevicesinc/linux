.. imported from: https://wiki.analog.com/resources/tools-software/linux-drivers/regulator/ad5398
...
AD5398 and AD5821 is a single 10-bit DAC with 120mA output current sink
capability. I2C bus is connected to enable/disable output and set the proper
output current value. This driver adapts to voltage and current regulator
...
.. list-table::
   ...
   * - Source
     ...
     -
     -
   * - `git <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/regulator/ad5398.c>`__
     - `Yes <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/regulator/ad5398.c>`__
     -
     -
...
.. list-table::
   ...
   * - Function
     - File
   * - driver
     - `drivers/regulator/ad5398.c <https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/regulator/ad5398.c>`__
...
Select voltage and current regulator framework support and enable AD5398/AD5821
driver. Don’t forget to select virtual regulator consumer and userspace
regulator consumer as module. Because AD5398 driver is loaded after I2C bus
finish initialization at kernel booting, built-in consumers may be loaded
earlier and find supply in AD5398 driver is not available.
...
The platform info defined in bf537-stamp board file is for ad5398 and ad5821
only. If you want to customize this driver for other compatible current
regulator DAC, you should revise the platform information accordingly.

- Define device supply name exported by the driver.

:git-linux:`ad5398_consumer{} <arch/blackfin/mach-bf537/boards/stamp.c>`

- Define supply value range.

:git-linux:`ad5398_regulator_data{} <arch/blackfin/mach-bf537/boards/stamp.c>`

- Define supply name in platform_data for the virtual consumer to match and use.

:git-linux:`ad5398_virt_consumer_device{} <arch/blackfin/mach-bf537/boards/stamp.c>`

- Define supply name for the user space consumer to match and use.

:git-linux:`ad5398_bulk_data{} <arch/blackfin/mach-bf537/boards/stamp.c>`
:git-linux:`ad5398_userspace_comsumer_data{} <arch/blackfin/mach-bf537/boards/stamp.c>`
:git-linux:`ad5398_userspace_consumer_device{} <arch/blackfin/mach-bf537/boards/stamp.c>`
...
.. tip::
   ...
   To control ADC to output a given current, you only need to set the value to
   sysfs entry min_microamps in virtual consumer platform device. The driver
   adjusts the output according to the min current value received.
