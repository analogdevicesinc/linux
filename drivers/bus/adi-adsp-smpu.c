...
/*
 * System Memory Protection Unit (SMPU) for ADI ADSP processors
 *
 * Copyright (C) 2026 - Analog Devices, Inc.
 * Author: Ozan Durgut <ozan.durgut@analog.com>
 *
 * The SMPU is the memory firewall found on several Analog Devices
 * ADSP/SHARC-FX processors (SC59x, SC846, SC594): it restricts accesses to
 * L2 SRAM / DDR / boot-ROM windows by bus-master transaction ID. Register
 * layout and the 8-region-per-instance limit are identical across these
 * chips; only the SEC interrupt ID differs, so it is carried by the
 * devicetree interrupts property rather than hardcoded here.
 *
 ...
 * so walk every instance and service the ones that fired.
...
static int smpu_summary_show(struct seq_file *s, void *data)
{
	...
	seq_puts(s, "ADSP System Memory Protection Unit\n");
...
static int adi_smpu_probe(struct platform_device *pdev)
{
	...
	/*
	 * The shared violation interrupt covers all instances. The protection
	 * policy itself is programmed by secure firmware; the driver only
	 * reports the violations it raises.
	 */
...
static const struct of_device_id adi_smpu_match[] = {
	...
	{ .compatible = "adi,sc846-smpu" },
	{ .compatible = "adi,sc594-smpu" },
	{ }
};
...
MODULE_DESCRIPTION("System Memory Protection Unit for ADI ADSP SoCs");
