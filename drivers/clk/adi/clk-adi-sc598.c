...
static void sc598_clock_probe(struct device_node *np)
{
	...
	clks[ADSP_SC598_CLK_CGU1_PLL_IN] =
	    clk_register_divider(NULL, "cgu1_df", "cgu1_in_sel",
				 CLK_SET_RATE_PARENT, cgu1 + CGU_CTL, 0, 1, 0,
				 &cdu_lock);
	clks[ADSP_SC598_CLK_3PLL_PLL_IN] =
	    clk_register_divider(NULL, "3pll_df", "cgu1_in_sel",
				 CLK_SET_RATE_PARENT, pll3, 3, 1, 0, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CGU1_VCO_OUT] =
	    sc5xx_cgu_pll("cgu1_vco", "cgu1_df", cgu1 + CGU_CTL, CGU_MSEL_SHIFT,
			  CGU_MSEL_WIDTH, 0, true, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CGU0_DDIV] =
	    cgu_divider("cgu0_ddiv", "cgu0_pllclk", cgu0 + CGU_DIV, 16, 5, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU0_ODIV] =
	    cgu_divider("cgu0_odiv", "cgu0_pllclk", cgu0 + CGU_DIV, 22, 7, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU0_S0SELDIV] =
	    cgu_divider("cgu0_s0seldiv", "sysclk_0", cgu0 + CGU_DIV, 5, 3, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU0_S1SELDIV] =
	    cgu_divider("cgu0_s1seldiv", "sysclk_0", cgu0 + CGU_DIV, 13, 3, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU0_S1SELEXDIV] =
	    cgu_divider("cgu0_s1selexdiv", "cgu0_pllclk", cgu0 + CGU_DIVEX, 16,
			8, 0, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CGU1_DDIV] =
	    cgu_divider("cgu1_ddiv", "cgu1_pllclk", cgu1 + CGU_DIV, 16, 5, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_ODIV] =
	    cgu_divider("cgu1_odiv", "cgu1_pllclk", cgu1 + CGU_DIV, 22, 7, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_S0SELDIV] =
	    cgu_divider("cgu1_s0seldiv", "sysclk_1", cgu1 + CGU_DIV, 5, 3, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_S1SELDIV] =
	    cgu_divider("cgu1_s1seldiv", "sysclk_1", cgu1 + CGU_DIV, 13, 3, 0,
			&cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_S0SELEXDIV] =
	    cgu_divider("cgu1_s0selexdiv", "cgu1_pllclk", cgu1 + CGU_DIVEX, 0,
			8, 0, &cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_S1SELEXDIV] =
	    cgu_divider("cgu1_s1selexdiv", "cgu1_pllclk", cgu1 + CGU_DIVEX, 16,
			8, 0, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CGU0_OCLK] =
	    cgu_gate("oclk_0", "cgu0_odiv", cgu0 + CGU_SCBF_DIS, 3, &cdu_lock);
	clks[ADSP_SC598_CLK_CGU0_DCLK] =
	    cgu_gate("dclk_0", "cgu0_ddiv", cgu0 + CGU_SCBF_DIS, 2, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CGU1_OCLK] =
	    cgu_gate("oclk_1", "cgu1_odiv", cgu1 + CGU_SCBF_DIS, 3, &cdu_lock);
	clks[ADSP_SC598_CLK_CGU1_DCLK] =
	    cgu_gate("dclk_1", "cgu1_ddiv", cgu1 + CGU_SCBF_DIS, 2, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_CDU_DDR_SEL] =
	    cdu_mux("cdu_ddr_sel", cdu + CDU_CFG3, cdu_ddr_sels, &cdu_lock);
	...
	clks[ADSP_SC598_CLK_SHARC1] =
	    cdu_gate("sharc1", "sharc1_sel", cdu + CDU_CFG1, CLK_IS_CRITICAL,
		     &cdu_lock);
	clks[ADSP_SC598_CLK_ARM] =
	    cdu_gate("arm", "arm_sel", cdu + CDU_CFG2, CLK_IS_CRITICAL,
		     &cdu_lock);
	clks[ADSP_SC598_CLK_CDU_DDR] =
	    cdu_gate("cdu_ddr", "cdu_ddr_sel", cdu + CDU_CFG3, 0, &cdu_lock);
