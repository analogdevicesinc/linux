...
struct ltc6948_state {
	...
	u8 buf[15] __aligned(ARCH_DMA_MINALIGN);
};
			    ...
			    unsigned long long freq, unsigned long long *round_rate)
{
	...
	if (fract) {
		/* Fract mode max fpfd = 76.1 MHz */
		rdiv = DIV_ROUND_UP(parent_rate, MAX_FPFD_FRACT);
		fpfd = parent_rate / rdiv;
...
static int ltc6948_pll_fract_n_get_rate(struct ltc6948_state *st, unsigned long parent_rate,
					u64 *freq)
{
...
static void ltc6948_clock_disable(struct clk_hw *hw)
{
	struct ltc6948_state *st = to_clk_priv(hw)->st;
	int ret;
...
{
	debugfs_create_file_unsafe("status", 0444, dentry, to_clk_priv(hw)->st,
				   &ltc6948_debugfs_fops);
}
...
static int ltc6948_probe(struct spi_device *spi)
{
	...
	ret = regmap_write(st->regmap, LTC6948_REG_HW(0x2), LTC6948_MSK_POR |
			   LTC6948_MSK_OMUTE | LTC6948_MSK_MTCAL);
	if (ret)
		return ret;

	fsleep(10);
	ret = regmap_write(st->regmap, LTC6948_REG_HW(0x2), LTC6948_MSK_OMUTE |
			   LTC6948_MSK_MTCAL);
	if (ret)
		return ret;

	ret = regmap_read(st->regmap, LTC6948_REG_HW(0xE), &val);
	if (ret < 0)
		return ret;
	...
	case 4:
		...
		break;
	default:
		return dev_err_probe(&spi->dev, -EINVAL, "Invalid part id\n");
	}
	...
	dev_dbg(&spi->dev, "LTC6948-%u Rev.%u successfully initialized\n", st->part_id, st->rev);
