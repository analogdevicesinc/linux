...
static int ad9517_clk_register(struct ad9517_state *st, unsigned int num)
{
	...
	if (num % 2 == 0) {
		...
		ret = devm_clk_hw_register(&st->spi->dev,
	...
	ret = devm_clk_hw_register(&st->spi->dev, &output->hw);
	if (ret)
		return ret;

	st->clk_hw_data->hws[num] = &output->hw;
...
static const struct ad9517_chan ad9516_chan[] = {
...
static const struct ad9517_chan ad9517_chan[] = {
	AD9517_CHAN(0, AD9517_ADDRESS_LVPECL(0)),
	AD9517_CHAN(1, AD9517_ADDRESS_LVPECL(1)),
...
struct ad9517_device_info {
	...
	const struct ad9517_chan *channels;
};
...
static int ad9517_check_part_id(struct spi_device *spi, unsigned int id)
{
...
{
	const struct ad9517_device_info *info;
	struct device *dev = &spi->dev;
	struct clk *ref_clk, *clkin;
	struct ad9517_state *st;
	int cnt, ret, conf;
	unsigned int id;
	bool spi3wire;

	id = (unsigned int)device_get_match_data(dev);
	if (!id)
		return -ENODEV;
	...
	ret = ad9517_check_part_id(spi, id);
	if (ret < 0)
		return ret;

	info = &ad9517_device_info[ret];

	cnt = sizeof(st->clk_hw_data) + info->num_channels * sizeof(struct clk_hw);
	st->clk_hw_data = devm_kzalloc(dev, cnt, GFP_KERNEL);
	if (!st->clk_hw_data)
		return -ENOMEM;

	memcpy(st->regs, ad9517_default_regs, sizeof(st->regs));

	ref_clk = devm_clk_get_optional(dev, "refclk");
	if (IS_ERR(ref_clk))
		return ret;
	...
	for (cnt = 0; cnt < info->num_channels; cnt++)
		st->output[cnt].address = info->channels[cnt].address;

	ret = ad9517_setup(st, info->num_channels);
	if (ret < 0)
		return ret;

	for (cnt = 0; cnt < info->num_channels; cnt++) {
		ret = ad9517_clk_register(st, cnt);
		if (ret)
			return ret;
	}

	st->clk_hw_data->num = info->num_channels;
...
#define AD9517_DRV_DATA(type, part_id) \
	(void *)((type << 8) | part_id)

static const struct of_device_id clk_ad9517_of_match[] = {
	{ .compatible = "adi,ad9516-0", .data = AD9517_DRV_DATA(AD9516, 0x01) },
	{ .compatible = "adi,ad9516-1", .data = AD9517_DRV_DATA(AD9516, 0x41) },
	{ .compatible = "adi,ad9516-2", .data = AD9517_DRV_DATA(AD9516, 0x81) },
	{ .compatible = "adi,ad9516-3", .data = AD9517_DRV_DATA(AD9516, 0x43) },
	{ .compatible = "adi,ad9516-4", .data = AD9517_DRV_DATA(AD9516, 0xc3) },
	{ .compatible = "adi,ad9516-5", .data = AD9517_DRV_DATA(AD9516, 0xc1) },
	{ .compatible = "adi,ad9517-0", .data = AD9517_DRV_DATA(AD9517, 0x11) },
	{ .compatible = "adi,ad9517-1", .data = AD9517_DRV_DATA(AD9517, 0x51) },
	{ .compatible = "adi,ad9517-2", .data = AD9517_DRV_DATA(AD9517, 0x91) },
	{ .compatible = "adi,ad9517-3", .data = AD9517_DRV_DATA(AD9517, 0x53) },
	{ .compatible = "adi,ad9517-4", .data = AD9517_DRV_DATA(AD9517, 0xD3) },
	{ .compatible = "adi,ad9518-0", .data = AD9517_DRV_DATA(AD9518, 0x21) },
	{ .compatible = "adi,ad9518-1", .data = AD9517_DRV_DATA(AD9518, 0x61) },
	{ .compatible = "adi,ad9518-2", .data = AD9517_DRV_DATA(AD9518, 0xa1) },
	{ .compatible = "adi,ad9518-3", .data = AD9517_DRV_DATA(AD9518, 0x63) },
	{ .compatible = "adi,ad9518-4", .data = AD9517_DRV_DATA(AD9518, 0xe3) },
	{ }
};
MODULE_DEVICE_TABLE(of, clk_ad9517_of_match);

static struct spi_driver ad9517_driver = {
	.driver = {
		.name	= "ad9517",
		.of_match_table = clk_ad9517_of_match,
	},
	.probe = ad9517_probe,
};
...
MODULE_DESCRIPTION("Analog Devices AD9517 and similar clock generators");
