...
{
	if (chan->hw_2d)
		return sg->hw->x_len * sg->hw->y_len;
	else
		return sg->hw->x_len;
}
...
{
	struct device *dev = &desc->chan->vchan.chan.dev->device;
	...
	unsigned long flags)
{
	...
	if (chan->hw_2d) {
		desc->sg[0].hw->x_len = xt->sgl[0].size;
		desc->sg[0].hw->y_len = xt->numf;
	} else {
		desc->sg[0].hw->x_len = xt->sgl[0].size * xt->numf;
		desc->sg[0].hw->y_len = 1;
	}
