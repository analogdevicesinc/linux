...
#else
static inline void adv_trace(const char *fmt, ...) { }
#endif /* adv_trace */
				   ...
				   bool rgb, bool hdmi_mode)
{
	...
	config.avi_infoframe.active_aspect = HDMI_ACTIVE_ASPECT_PICTURE;
	config.avi_infoframe.picture_aspect = HDMI_PICTURE_ASPECT_NONE;
...
static int adv7511_irq_process(struct adv7511 *adv7511, bool process_hpd)
{
	...
	regmap_write(adv7511->regmap, ADV7511_REG_INT(1), irq1 & 0xBF);
...
static void adv7511_hdcp_disable(struct adv7511 *adv7511)
{
	...
	/* Disable Black Image */
	regmap_update_bits(adv7511->regmap, ADV7511_REG_BLACK_IMAGE,
			   ADV7511_BLACK_IMAGE_CTRL, 0);
...
static void adv7511_hdcp_bksv_work(struct work_struct *work)
{
	...
	/* handle BKSVs in case of repeater sink */
		...
		ret = regmap_read(adv7511->regmap, ADV7511_REG_BCAPS, &reg);
		hdcp->repeater_sink = (bool)(ADV7511_BCAPS_REPEATER & reg);
		ret = regmap_read(adv7511->regmap, ADV7511_REG_BKSV_COUNT,
				    &reg);
		ksv_count = reg & GENMASK(6, 0);

		if (hdcp->repeater_sink && ksv_count) {
			hdcp->ksv_count += ksv_count;
			...
			ret = regmap_bulk_read(adv7511->regmap_edid, 0x00,
					       hdcp->ksv_fifo +
					       (hdcp->ksv_count - ksv_count) * DRM_HDCP_KSV_LEN,
					       ksv_count * DRM_HDCP_KSV_LEN);
		}
	}
	regmap_write(adv7511->regmap, ADV7511_REG_INT(1), ADV7511_INT1_BKSV);
	/* Enable BKSV ready interrupt */
	regmap_update_bits(adv7511->regmap, ADV7511_REG_INT_ENABLE(1),
			   ADV7511_INT1_BKSV, ADV7511_INT1_BKSV);
	adv_trace("Clear BKSV IRQ\n");
	mutex_unlock(&hdcp->mutex);
err:
	mutex_unlock(&hdcp->mutex);
}
			     ...
			     struct drm_connector *connector)
{
	...
	kfree(edid);
	kfree(adv7511->hdcp.ksv_fifo);
	adv7511->hdcp.ksv_fifo = NULL;
...
static int adv7511_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	adv7511->regmap_edid = devm_regmap_init_i2c(adv7511->i2c_edid, &adv7511_edid_regmap_config);
