...
DEFINE_SIMPLE_ATTRIBUTE(go_command_fops, NULL, adm1266_set_go_command_op,
			"%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(read_state_fops, adm1266_get_state_op, NULL, "%llu\n");
...
static void adm1266_debug_init(struct adm1266_data *data)
{
	...
	debugfs_create_file("go_command", 0200, root, data,
			    &go_command_fops);
	debugfs_create_file("read_state", 0400, root, data,
			    &read_state_fops);
}
