...
static void i2c_pmbus_try_get_dmabuf(struct i2c_msg *msg)
{
	unsigned char *dma_buf;

	dma_buf = kzalloc(PMBUS_BLOCK_MAX + 3, GFP_KERNEL);
	if (!dma_buf)
		return;
		   ...
		   u8 *data_w, u8 *data_r)
{
	...
	i2c_pmbus_try_get_dmabuf(&msgs[0]);
	i2c_pmbus_try_get_dmabuf(&msgs[1]);
...
cleanup:
	if (msgs[0].flags & I2C_M_DMA_SAFE)
		kfree(msgs[0].buf);
	if (msgs[1].flags & I2C_M_DMA_SAFE)
		kfree(msgs[1].buf);
