...
#define FIFO_STATUS_SDI_ALMOST_FULL	BIT(1)
#define FIFO_STATUS_IBI_ALMOST_FULL	BIT(2)
					   ...
					   u32 pending)
{
	...
	writel(IRQ_PENDING_CMDR_PENDING, master->regs + REG_IRQ_PENDING);
...
static irqreturn_t adi_i3c_master_irq(int irq, void *data)
{
	...
	if (clear)
		writel_relaxed(clear, master->regs + REG_IRQ_PENDING);
