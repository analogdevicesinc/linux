...
static int ad4052_update_xfers(struct iio_dev *indio_dev,
					 ...
					 unsigned int val)
{
	...
	return ad4052_update_xfers(indio_dev, chan);
}
					...
					const struct iio_chan_spec *chan)
{
	 ...
	 * offload scan types.
	 */
	if (iio_dev_opaque->currentmode & INDIO_BUFFER_HARDWARE)
...
static int ad4052_probe(struct spi_device *spi)
{
	...
	if (ret == -ENODEV) {
		...
		if (ret)
			return dev_err_probe(dev, ret,
					     "Failed to init sampling frequency\n");
	...
	ret = ad4052_update_xfers(indio_dev, indio_dev->channels);
	if (ret)
		return ret;
...
static void ad4052_remove(struct spi_device *spi)
{
	struct ad4052_state *st = dev_get_drvdata(&spi->dev);
