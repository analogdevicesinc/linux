...
/* TIMER_CONFIG */
#define AD4052_FS_BURST_AUTO_MSK	GENMASK(7, 4)

#define AD4052_SPI_VENDOR		0x0456

#define AD4050_MAX_AVG			0x7
#define AD4052_MAX_AVG			0xB
#define AD4052_CHECK_OVERSAMPLING(x, y)	((y) < 0 || (y) > BIT((x) + 1))
#define AD4052_MAX_RATE(x)		((x) == AD4052_500KSPS ? 500000 : 2000000)
#define AD4052_CHECK_RATE(x, y)		((y) > AD4052_MAX_RATE(x) || (y) <= 0)
...
irqreturn_t ad4052_irq_handler_thresh(int irq, void *private)
{
...
irqreturn_t ad4052_irq_handler_drdy(int irq, void *private)
{
...
static const int ad4052_sample_rate_avail[] = {
	2000000, 1000000, 300000, 100000, 33300, 10000, 3000,
	500, 333, 250, 200, 166, 140, 125, 111
};
			     ...
			     int *type, int *len, long mask)
{
	...
	iio_device_claim_direct_scoped(return -EBUSY, indio_dev) {
		...
		case IIO_CHAN_INFO_SAMP_FREQ:
			offset = st->chip->grade == AD4052_500KSPS ? 2 : 0;
			*vals = &ad4052_sample_rate_avail[offset];
			*len = ARRAY_SIZE(ad4052_sample_rate_avail) - offset;
			*type = IIO_VAL_INT;
...
static int ad4052_set_sampling_freq(const struct ad4052_state *st, unsigned int val)
{
	const int *vals;
	int offset, len;
	...
	vals = &ad4052_sample_rate_avail[offset];
	len = ARRAY_SIZE(ad4052_sample_rate_avail) - offset;
	val = find_closest_descending(val, vals, len);

	val = FIELD_PREP(AD4052_FS_BURST_AUTO_MSK, val);
	return regmap_write(st->regmap, AD4052_REG_TIMER_CONFIG, val);
}

static int ad4052_get_sampling_freq(const struct ad4052_state *st, int *val)
{
	int buf, ret;
	...
	*val = ad4052_sample_rate_avail[FIELD_GET(AD4052_FS_BURST_AUTO_MSK, buf)];
...
static int __ad4052_read_chan_raw(struct ad4052_state *st, int *val)
{
	...
	struct spi_transfer t_adc = {
		.bits_per_word = st->xfer.bits_per_word,
		.len = st->xfer.len,
		.rx_buf = &st->d32,
	};
	...
	ret = spi_sync_transfer(spi, &t_adc, 1);
	if (ret)
		return ret;
...
static ssize_t ad4052_buffer_sampling_freq_show(struct device *dev,
						...
						char *buf)
{
	...
	freq = DIV_ROUND_CLOSEST_ULL(NANO, st->cnv_wf.period_length_ns);

	return sysfs_emit(buf, "%d\n", freq);
}

static ssize_t ad4052_buffer_sampling_freq_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t len)
{
	...
	if (AD4052_CHECK_RATE(st->chip->grade, freq))
		return -EINVAL;

	ret = __ad4052_set_sampling_freq(st, freq);
	if (ret)
		return ret;
...
static const struct iio_dev_attr *ad4052_attributes[] = {
	NULL,
...
static int ad4052_probe(struct spi_device *spi)
{
	...
	ad4052_attributes[0] = *indio_dev->buffer->attrs;
	indio_dev->buffer->attrs = ad4052_attributes;
