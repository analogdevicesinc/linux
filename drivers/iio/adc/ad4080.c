...
static int ad4080_probe(struct spi_device *spi)
{
	...
	if (st->info->num_channels > 1) {
		u32 reg_values[AD4080_MAX_CHANNELS];

		ret = device_property_read_u32_array(dev, "reg", reg_values,
						     st->info->num_channels);
		if (ret)
			return dev_err_probe(dev, ret,
					     "missing reg entries for multi-channel device\n");

		for (ch = 1; ch < st->info->num_channels; ch++) {
			st->spi[ch] = spi_new_ancillary_device(spi, reg_values[ch]);
			if (IS_ERR(st->spi[ch]))
				return PTR_ERR(st->spi[ch]);
