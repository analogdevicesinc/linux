				      ...
				      const char *buf, size_t len)
{
	...
	axiadc_write(axi_adc_st, ADI_REG_CNTRL_3, 0x2);

	// enable the sync
	if (st->num_lanes == 1)
		axiadc_write(axi_adc_st, ADI_REG_CNTRL, 0x108);
	else
		axiadc_write(axi_adc_st, ADI_REG_CNTRL, 0x208);
...
static int ad4080_post_setup(struct iio_dev *indio_dev)
{
	...
	unsigned int reg_cntrl = axiadc_read(axi_adc_st, ADI_REG_CNTRL);
	...
	axiadc_write(axi_adc_st, ADI_REG_CNTRL_3, 0x2); // in the default mode the CNV is not used as a start of transfer flag
	return 0;
}
...
static int ad4080_setup(struct ad4080_state *st)
{
	...
	regmap_write(st->regmap, AD4080_REG_GPIO_CONFIG_B,
