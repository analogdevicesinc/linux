...
#define AD4134_ODR_DEFAULT			1000000
...
static int ad4134_set_dig_fil(struct iio_dev *dev, const struct iio_chan_spec *chan, unsigned int filter);
static int ad4134_get_dig_fil(struct iio_dev *dev, const struct iio_chan_spec *chan);
static ssize_t ad4134_ext_info_write(struct iio_dev *indio_dev,uintptr_t private, const struct iio_chan_spec *chan, const char *buf, size_t len);
static ssize_t ad4134_ext_info_read(struct iio_dev *indio_dev, uintptr_t private, const struct iio_chan_spec *chan, char *buf);
...
static struct iio_chan_spec_ext_info ad4134_ext_info[] = {
	...
	IIO_ENUM_AVAILABLE("filter_type", IIO_SHARED_BY_ALL, &ad7134_flt_type_iio_enum),
	 ...
	 {
	 .name = "odr_set_freq",
	 .read = ad4134_ext_info_read,
	 .write = ad4134_ext_info_write,
...
static ssize_t ad4134_ext_info_read(struct iio_dev *indio_dev,
				    uintptr_t private,
...
static int _ad4134_set_odr(struct ad4134_state *st, unsigned int odr)
{
	...
	state_trigger.phase = state_odr.duty_cycle - DIV_ROUND_CLOSEST_ULL(PICO * 12, st->sys_clk_rate);
...
static ssize_t ad4134_ext_info_write(struct iio_dev *indio_dev,
				     uintptr_t private,
				     const struct iio_chan_spec *chan,
...
static int ad4134_setup(struct ad4134_state *st)
{
	...
	if (IS_ERR(st->trigger_pwm))
		pr_err("%s: %d: Failed to find trigger PWM\n", __FUNCTION__, __LINE__);
...
static int ad4134_adc_channel_init(struct iio_dev *indio_dev)
{
	struct ad4134_state *st = iio_priv(indio_dev);
	...
	for_each_set_bit(bit, &st->channels_mask, st->num_channels) {
		...
		chan->info_mask_shared_by_type = BIT(IIO_CHAN_INFO_SAMP_FREQ) |	BIT(IIO_CHAN_INFO_SCALE);
...
static int ad4134_probe(struct spi_device *spi)
{
	...
	if (!of_property_read_string(node, "ad7134,adc-frame", (const char **)&s)
	&& !strcmp(s, "ad7134_16"))
		{
			st->_realbits = 16;
			st->_wordbits = 16;
	...
	else if (!of_property_read_string(node, "ad7134,adc-frame", (const char **)&s)
	...
	&& !strcmp(s, "ad7134_24CRC"))
		...
		dev_info(&indio_dev->dev, "ad4134_Frame_used: %u realbits, %u wordbits, %u shift, %u output_frame\n", st->_realbits, st->_wordbits, st->_shift, st->output_frame);
	...
	ret = ad4134_adc_channel_init(indio_dev);
	if (ret < 0) {
