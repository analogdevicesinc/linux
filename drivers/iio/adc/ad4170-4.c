...
static void ad4170_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
{
	...
	if (!iio_device_claim_direct(indio_dev)) {
		dev_err(&st->spi->dev, "failed to set GPIO: %d", -EBUSY);
		return;
	}

	if (!!value)
		ret = regmap_set_bits(st->regmap, AD4170_GPIO_OUTPUT_REG, BIT(offset));
	else
		ret = regmap_clear_bits(st->regmap, AD4170_GPIO_OUTPUT_REG, BIT(offset));

	iio_device_release_direct(indio_dev);
	if (ret)
		dev_err(&st->spi->dev, "failed to set GPIO: %d", ret);
}
...
static irqreturn_t ad4170_trigger_handler(int irq, void *p)
{
	...
	iio_push_to_buffers_with_timestamp(indio_dev, st->bounce_buffer,
