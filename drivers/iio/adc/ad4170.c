...
/* For the moment this structure uses the internal 16MHz clk, needs to be adapted to external clk source*/
				      ...
				      unsigned int channel)
{
	...
	//setup->afe.ref_select = AD4170_REFIN_REFOUT; ///done
...
static int ad4170_set_channel_pga(struct iio_dev *indio_dev,struct ad4170_state *st, unsigned int channel,
				  int val, int val2)
{
	struct ad4170_chan_info *chan_info = &st->chan_info[channel];
	struct ad4170_setup *setup = &chan_info->setup;
...
{
	int i = 0;
	long unsigned int valid_mask = 0x0;
