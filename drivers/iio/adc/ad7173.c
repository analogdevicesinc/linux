...
static int ad7173_gpio_get(struct gpio_chip *chip, unsigned int offset)
{
...
static void ad7173_gpio_set(struct gpio_chip *chip, unsigned int offset,
	int value)
{
	struct ad7173_state *st = gpiochip_to_ad7173(chip);
...
static int ad7173_gpio_direction_input(struct gpio_chip *chip,
	unsigned int offset)
{
	struct ad7173_state *st = gpiochip_to_ad7173(chip);
	unsigned int mask;
...
static int ad7173_gpio_direction_output(struct gpio_chip *chip,
...
static void ad7173_gpio_free(struct gpio_chip *chip, unsigned int offset)
{
	struct ad7173_state *st = gpiochip_to_ad7173(chip);
	unsigned int mask;
	...
	struct device_node *np)
{
	...
	for_each_available_child_of_node(chan_node, child) {
		...
		*chan = ad7173_channel_template;
		...
		chan->channel = ain[0];
		chan->channel2 = ain[1];
		chan->differential = of_property_read_bool(child,
							   "adi,bipolar");
		if (chan->differential)
