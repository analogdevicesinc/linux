...
#define AD7192_COMM_ADDR_MASK	GENMASK(5, 3) /* Register Address */
...
#define AD7194_CH_AINPOS_MASK		GENMASK(7, 4)
#define AD7194_CH_AINPOS(x)		FIELD_PREP(AD7194_CH_AINPOS_MASK, ((x) - 1))
					  /* Positive Input Mask */
#define AD7194_CH_AINNEG_MASK		GENMASK(3, 0)
#define AD7194_CH_AINNEG(x)		FIELD_PREP(AD7194_CH_AINNEG_MASK, ((x) - 1))
					  /* Negative Input Mask */
#define AD7194_CH_DIFF_ADDR(pos, neg)	(AD7194_CH_AINPOS(pos) | AD7194_CH_AINNEG(neg))
					  /* Differential Channel Address Mask */
#define AD7194_CH_TEMP			0x100 /* Temp sensor */
#define AD7194_CH_AIN_MASK		GENMASK(7, 4)
#define AD7194_CH_AIN_PREP(x)		FIELD_PREP(AD7194_CH_AIN_MASK, ((x) - 1))
#define AD7194_CH_AIN(x)		(AD7194_CH_AIN_PREP(x) | 0x400)
					  /* AINx - AINCOM */
...
static int ad7192_compute_f_order(struct ad7192_state *st, bool sinc3_enabled, bool chop_enabled)
{
	...
	if (!avg_factor_selected && !chop_enabled)
		return 1;
...
static int ad7192_get_f_order(struct ad7192_state *st)
{
	...
	sinc3_enabled = FIELD_GET(AD7192_MODE_SINC3, st->mode);
	chop_enabled = FIELD_GET(AD7192_CONF_CHOP, st->conf);

	return ad7192_compute_f_order(st, sinc3_enabled, chop_enabled);
}

static int ad7192_compute_f_adc(struct ad7192_state *st, bool sinc3_enabled, bool chop_enabled)
{
	unsigned int f_order;

	f_order = ad7192_compute_f_order(st, sinc3_enabled, chop_enabled);

	return DIV_ROUND_CLOSEST(st->fclk, f_order * AD7192_MODE_RATE(st->mode));
}
					  ...
					  struct device_node *np)
{
	...
	if (num_channels > AD7194_CH_DIFF_NR) {
		dev_err(dev, "Number of differential channels out of range.\n");
		return -EINVAL;
	}
	...
	for_each_available_child_of_node(np, child) {
		...
		if (ain[0] > AD7194_CH_NR || ain[1] > AD7194_CH_NR) {
			dev_err(dev, "Input channel out of range.\n");
			ret = -EINVAL;
			goto out_error;
		}
