...
static int ad7768_get_dec_rate(struct ad7768_state *st, unsigned int *dec_rate)
{
	...
	case SINC5_X16:
		*dec_rate = 16;
		break;
	case SINC3:
		ret = ad_sd_read_reg(&st->sd, AD7768_REG_SINC3_DEC_RATE_MSB,
				     1, &regval);
		if (ret)
			return ret;
		...
		sinc3_dec <<= 8;
		ret = ad_sd_read_reg(&st->sd, AD7768_REG_SINC3_DEC_RATE_LSB,
				     1, &regval);
		if (ret)
			return ret;
			       ...
			       unsigned int dec_rate)
{
	...
	/* Programmable decimation rate */
		...
		ret = ad_sd_write_reg(&st->sd, AD7768_REG_SINC3_DEC_RATE_MSB,
				      1, writeval);
		if (ret)
			return ret;

		writeval = (dec_rate & GENMASK(7, 0));
		ret = ad_sd_write_reg(&st->sd, AD7768_REG_SINC3_DEC_RATE_LSB,
				      1, writeval);
	} else {
		ret = -EINVAL;
	}
				     ...
				     unsigned int *filter)
{
	...
	if (ad7768_fixed_clk_div_search(st, freq, &index) < 1) {
		...
		for (i = 0; i < ARRAY_SIZE(ad7768_mclk_divs); i++) {
			tmp_dec_rate = DIV_ROUND_CLOSEST(temp,
							 ad7768_mclk_divs[i]);
...
static IIO_DEVICE_ATTR(in_voltage_filter_low_pass_3db_frequency_available,
		       0444, ad7768_show_bw_available, NULL, 0);

static struct attribute *ad7768_attributes[] = {
	&iio_dev_attr_in_voltage_filter_low_pass_3db_frequency_available.dev_attr.attr,
	NULL
};
