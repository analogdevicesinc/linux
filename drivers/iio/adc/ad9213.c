...
#if 1
	status = clk_set_rate(adc->jesd_clk, rx_lane_rate_kbps);
	if (status < 0) {
		dev_err(&adc->spi->dev, "Failed to set lane rate to %lu kHz: %d\n",
			rx_lane_rate_kbps, status);
	}
#endif
...
#if 0
	ret = clk_prepare_enable(adc->sample_clk);
	if (ret)
		return ret;
#endif
...
static int ad9213_probe(struct spi_device *spi)
{
	...
	ret = ad9081_request_clks(indio_dev);
	if (ret)
		return ret;
	...
	adc->jesd_L = 16;
	of_property_read_u32(np, "adi,lanes-per-link", &adc->jesd_L);
	adc->jesd_M = 2;
	of_property_read_u32(np, "adi,converters-per-link", &adc->jesd_M);
	adc->jesd_F = 2;
	of_property_read_u32(np, "adi,octets-per-frame", &adc->jesd_F);
	adc->jesd_S = 16;
	of_property_read_u32(np, "adi,samples-per-converter-per-frame", &adc->jesd_S);
	adc->jesd_HD = 0;
	of_property_read_u32(np, "adi,high-density", &adc->jesd_HD);
	adc->jesd_N = 16;
	of_property_read_u32(np, "adi,converter-resolution", &adc->jesd_N);
	adc->jesd_NP = 16;
	of_property_read_u32(np, "adi,bits-per-sample", &adc->jesd_NP);
	adc->jesd_CS = 0;
	of_property_read_u32(np, "adi,control-bits-per-sample", &adc->jesd_CS);
	adc->jesd_K = 32;
	of_property_read_u32(np, "adi,frames-per-multiframe", &adc->jesd_K);
	adc->jesd_subclass = 0;
	of_property_read_u32(np, "adi,subclass", &adc->jesd_subclass);
