			     ...
			     unsigned int write_val, unsigned int *read_val)
{
	...
	if (read_val)
		return regmap_read(adc->regmap, reg, read_val);

	return regmap_write(adc->regmap, reg, write_val);
}
...
static int ad9213_probe(struct spi_device *spi)
{
	...
	adc->syncinb_cmos_en = device_property_read_bool(&spi->dev,
							 "adi,syncinb-cmos-enable");
