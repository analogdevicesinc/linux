...
static int ad9213_multi_chip_sync(struct iio_dev *indio_dev)
{
	...
	ad9213_write(adc, 0x1627, 0x00);
	ad9213_write(adc, 0x1628, 0x01);
	ad9213_write(adc, 0x1629, 0x00);

	/* MCS_SAMPLE_CLK_PERIOD */
	/* fCLK = 6 GHz */
	ad9213_write(adc, 0x162D, 0x00);
	ad9213_write(adc, 0x162E, 0x00);
	ad9213_write(adc, 0x162F, 0x64);
	ad9213_write(adc, 0x1630, 0x00);

	/* MCS_SYSREF_IGNORE_COUNT  */
	ad9213_write(adc, 0x1521, 0x64);

	/* MCS_SYSREF_AVGING_COUNT */
	ad9213_write(adc, 0x162A, 0xE8);
	ad9213_write(adc, 0x162B, 0x03);

	/* MCS_MODE */
	ad9213_write(adc, 0x1621, 0x09);

	/* MCS_PHASE_SLIP_MODE */
	ad9213_write(adc, 0x1636, 0x01);

	/* MCS_CTRL */
	ad9213_write(adc, 0x1622, 0x02);

	/* SYSREF_RESYNC */
	ad9213_write(adc, 0x693, 0x01);

	/* USER_CTRL_TRANSFER */
	ad9213_write(adc, 0x1600, 0x01);
...
static int ad9213_setup(struct ad9213 *adc)
{
	...
	//ad9213_write(adc, AD9213_REG_DDC_CTRL, 0x00);

	/* k) Write Register 0x631 = 0x02. DDC decimate by 8. */
	//ad9213_write(adc, AD9213_REG_DDC_DEC_CTRL,
		...
		struct jesd204_link *lnk)
{
	...
	dev_info(&adc->spi->dev, "XXXX AD9213 PLL %s\n",
...
static int ad9213_phy_read_raw(struct iio_dev *indio_dev,
			       ...
			       long m)
{
	unsigned long long llval = 10250000000;
