			      ...
			      u32 setting)
{
	...
	st->load_bias_idx = setting;

	if (tx)
		ret = regmap_write(st->regmap, st->dev_addr |
				   ADAR1000_TX_BIAS_RAM_CTL | ADAR1000_RAM_ACCESS_TX,
				   ADAR1000_BIAS_RAM_FETCH  | setting);
	else
		ret = regmap_write(st->regmap, st->dev_addr |
			      ...
			      u32 setting, void * const bias)
{
	...
	if (tx) {
		...
		st->tx_bias[setting] = *(struct adar1000_tx_bias_setting*)bias;
	} else {
		...
		st->rx_bias[setting] = *(struct adar1000_rx_bias_setting*)bias;
	}
...
static int adar1000_bias_parse(struct adar1000_state *st, bool tx, const char *buf, void *value_bias)
{
	int ret;
	char *line, *ptr = (char*) buf;
	int val, tmp[10], i = 0, j = 0, cnt;
	size_t len = 0;
	u64 readin;
				       ...
				       const char *buf, size_t len)
{
	...
	case BIAS_SET_SAVE: {
		...
		ret = adar1000_bias_parse(st, chan->output == 1, buf, value_bias);
		if (ret < 0)
			return ret;
...
#define _ADAR1000_RAM_INFO(_name, _ident) { \
	.name = _name, \
...
static const struct iio_chan_spec_ext_info adar1000_rx_ext_info[] = {
	_ADAR1000_RAM_INFO("beam_pos_load", BEAM_POS_LOAD),
	_ADAR1000_RAM_INFO("beam_pos_save", BEAM_POS_SAVE),
