...
#define ADEMA_REG_DATAPATH_ALPHA(x)	(0x03D + (x))
...
/* Long-frame response byte offsets (position of MSB of Vx_WAV inside the
 * 20/32-byte response). Byte 0 of the response is CMD_ECHO.
 ...
 *      response.
 *
 * This is expensive but keeps behaviour identical to the register_read /
 * register_write API of the NoOS driver.
 */
...
static int adema_lookup_datarate(u32 xtal_hz, u32 fs, u8 *reg)
{
	unsigned int i;
	u32 scale;
	...
	scale = xtal_hz / ADEMA_XTAL_HZ_NOMINAL;
	if (!scale)
		scale = 1;

	for (i = 0; i < ARRAY_SIZE(adema_datarates); i++) {
		if (adema_datarates[i].fs * scale == fs) {
...
static int adema_set_sampling_freq(struct adema_state *st, u32 fs)
{
	...
	ret = adema_lookup_datarate(st->xtal_hz, fs, &datarate);
	if (ret)
		return ret;
...
/* --- phase offset (13-bit signed magnitude) --- */
...
static int adema_set_phase(struct adema_state *st, unsigned int ch, s32 val)
{
	...
	raw = val & GENMASK(12, 0);
	ret = regmap_write(st->regmap, ADEMA_REG_PHASE_OFFSET_HI(ch),
			   (raw >> 8) & 0x1F);
	if (ret)
		return ret;
	return regmap_write(st->regmap, ADEMA_REG_PHASE_OFFSET_LO(ch), raw & 0xFF);
}
			       ...
			       const char *buf, size_t len)
{
	...
	case ADEMA_EXT_INFO_DC_BLOCK_ALPHA:
		...
		if (ret)
			return ret;
		ret = regmap_write(st->regmap, ADEMA_REG_DATAPATH_ALPHA(ch), uval);
		if (ret)
			return ret;
		ret = adema_config_lock(st);
		break;
...
static const int adema_sampling_freqs[] = {
			    ...
			    long info)
{
	switch (info) {
	case IIO_CHAN_INFO_SAMP_FREQ:
		*vals = adema_sampling_freqs;
		*length = ARRAY_SIZE(adema_sampling_freqs);
		*type = IIO_VAL_INT;
...
static void adema_unpack_long_frame(struct adema_state *st)
{
	...
	for (i = 0; i < st->chip_info->num_channels; i++) {
		...
		u32 raw = ((u32)p[0] << 16) | ((u32)p[1] << 8) | p[2];
...
/* ------------------------------------------------------------------------ */
/*  SPI Offload path (Phase 2 scaffolding)                                  */
/* ------------------------------------------------------------------------ */
...
static int adema_setup_offload(struct iio_dev *indio_dev,
			       struct adema_state *st)
{
	...
	if (ret)
		return dev_err_probe(dev, ret, "cannot set up DMA buffer\n");

	indio_dev->setup_ops = &adema_offload_buffer_setup_ops;
	/* Drop timestamp: offload path streams raw frames directly. */
...
static int adema_init(struct adema_state *st)
{
	...
	ret = regmap_write(st->regmap, ADEMA_REG_DATARATE, 0x31);
	if (ret)
		return ret;
	st->sampling_freq = 32000;

	ret = adema_config_lock(st);
	if (ret)
		return ret;

	/* Issue ALIGN so channels start synchronously. */
...
static int adema_request_dready(struct iio_dev *indio_dev, struct adema_state *st)
{
	...
	/*
	 * Register the IRQ disabled: at 32 kSPS the DREADY line fires 32000
	 * times/second and we don't want any of that until userspace enables
	 * the buffer. The buffer's postenable/predisable hooks bracket
...
static int adema_probe(struct spi_device *spi)
{
	 ...
	 * if the controller does not advertise offload support.
	 */
	st->offload = devm_spi_offload_get(dev, spi, &adema_offload_config);
