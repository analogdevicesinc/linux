...
static int adrv9025_read_raw(struct iio_dev *indio_dev,
...
static int adrv9025_write_raw(struct iio_dev *indio_dev,
			      struct iio_chan_spec const *chan, int val,
			      int val2, long mask)
{
	switch (mask) {
...
EXPORT_SYMBOL(adrv9025_hdl_loopback);

static int adrv9025_post_setup(struct iio_dev *indio_dev)
{
...
int adrv9025_register_axi_converter(struct adrv9025_rf_phy *phy)
{
	...
	conv->write_raw = adrv9025_write_raw;
	conv->read_raw = adrv9025_read_raw;
	conv->post_setup = adrv9025_post_setup;
	conv->spi = spi;
	conv->phy = phy;
...
EXPORT_SYMBOL(adrv9025_register_axi_converter);
...
EXPORT_SYMBOL(adrv9025_spi_to_phy);
