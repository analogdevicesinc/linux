    ...
    */
    ...
    for (i = 0U; i < numTxAttenConfigs; ++i)
    {
        ...
        for (chanId = 0U; chanId <= ADI_ADRV903X_TX_CHAN_ID_MAX; ++chanId)
        {
            ...
            data1[7U] = addrLsb;
            data1[8U] = attenLsb;
            halError = adrv903x_SpiWrite(device->common.devHalInfo, data1, DATA_TRANSACTION_1_SIZE);
    ...
    {
        dataFF[2U] = latchMask;
        halError = adrv903x_SpiWrite(device->common.devHalInfo, dataFF, DATA_TRANSATCTION_FF_SIZE);
        recoveryAction = (adi_adrv903x_ErrAction_e) adi_common_hal_ErrCodeConvert(halError);
        if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE)
        {
            ADI_PARAM_ERROR_REPORT(&device->common,
    ...
    else
    {
        data2[2U] = latchMask;
        data2[5U] = latchMask;
        halError = adrv903x_SpiWrite(device->common.devHalInfo, data2, DATA_TRANSACTION_2_SIZE);
        recoveryAction = (adi_adrv903x_ErrAction_e) adi_common_hal_ErrCodeConvert(halError);
        if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE)
        {
            ADI_PARAM_ERROR_REPORT(&device->common,
