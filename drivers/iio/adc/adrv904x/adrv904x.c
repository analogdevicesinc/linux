...
// 		       adrv904x_phy_show, adrv904x_phy_store,
// 		       ADRV904X_INIT_CAL | (ADI_ADRV904X_IC_TX_ATTEN_CAL << 8));
				     ...
				     const char *buf, size_t len)
{
	...
	case LOEXT_FREQ:
		...
		ret = adi_adrv904x_LoFrequencySet(
			phy->kororDevice, &loConfig);
		if (ret)
			ret = adrv904x_dev_err(phy);
		break;
	default:
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	case LOEXT_FREQ:
		loConfig.loName = ADI_ADRV904X_LO0 + chan->channel;
		ret = adi_adrv904x_LoFrequencyGet(
			phy->kororDevice, &loConfig);
		if (ret)
			ret = adrv904x_dev_err(phy);
		break;
				 ...
				 int *val2, long m)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output) {
			...
			ret = adi_adrv904x_TxAttenGet(
				phy->kororDevice, 1 << chan->channel, &txAtten);
			if (ret) {
				ret = adrv904x_dev_err(phy);
				break;
			}
			...
			*val2 = (txAtten.txAttenuation_mdB % 1000) * 1000;
			...
			if (chan->channel <= CHAN_RX8) {
				...
				ret = adi_adrv904x_RxGainGet(
					phy->kororDevice, 1 << chan->channel, &rxGain);
				if (ret) {
					ret = adrv904x_dev_err(phy);
					break;
				}
...
static int adrv904x_probe(struct spi_device *spi)
{
	...
	if (!of_property_read_string(np, "adi,arm-dfe-firmware-name", &name)) {
		strncpy(phy->trxBinaryInfoPtr.dfeCpu.filePath, name, sizeof(phy->trxBinaryInfoPtr.dfeCpu.filePath));
	} else {
