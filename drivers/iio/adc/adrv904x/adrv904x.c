...
enum adrv904x_iio_dev_attr {
	ADRV904X_INIT_CAL,
	ADRV904X_CAL_MASK_RX,
	ADRV904X_CAL_MASK_TX,
	ADRV904X_CAL_MASK_ORX,
 ...
 */
static int adrv904x_TxLinkSamplingRateFind(adi_adrv904x_Device_t *device,
...
static IIO_DEVICE_ATTR(calibrate_rx_channel_mask, 0644, adrv904x_phy_show,
		       adrv904x_phy_store, ADRV904X_CAL_MASK_RX);

static IIO_DEVICE_ATTR(calibrate_tx_channel_mask, 0644, adrv904x_phy_show,
		       adrv904x_phy_store, ADRV904X_CAL_MASK_TX);

static IIO_DEVICE_ATTR(calibrate_orx_channel_mask, 0644, adrv904x_phy_show,
		       adrv904x_phy_store, ADRV904X_CAL_MASK_ORX);
...
static struct attribute *adrv904x_phy_attributes[] = {
	...
	&iio_dev_attr_calibrate_rx_channel_mask.dev_attr.attr,
	&iio_dev_attr_calibrate_tx_channel_mask.dev_attr.attr,
	&iio_dev_attr_calibrate_orx_channel_mask.dev_attr.attr,
				     ...
				     const char *buf, size_t len)
{
	...
	case TX_LOL:
		...
		if (enable) {
			/* Set TX to ORX mapping before enabling LOL calibration */
			ret = adi_adrv904x_TxToOrxMappingSet(phy->kororDevice, ADI_ADRV904X_TX_ORX_MAPPING_MODE_8BIT);
			if (ret) {
				ret = adrv904x_dev_err(phy);
				break;
			}
		...
		if (chan->output) {
			...
			*val2 = (txAtten.txAttenuation_mdB % 1000) * 1000;
			...
			if (chan->channel <= CHAN_RX8) {
				...
				adrv904x_gainindex_to_gain(phy, chan->channel,
...
static long adrv904x_bb_round_rate(struct clk_hw *hw, unsigned long rate,
				   unsigned long *prate)
{
	struct adrv904x_clock *clk_priv __maybe_unused = to_clk_priv(hw);
	...
	case TX_SAMPL_CLK:
		{
			...
			ret = adrv904x_TxLinkSamplingRateFind(phy->kororDevice,
...
static int adrv904x_jesd204_link_pre_setup(struct jesd204_dev *jdev,
					...
					enum jesd204_state_op_reason reason)
{
	...
	if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE) {
		pr_err("ERROR adi_adrv904x_HwReset failed in %s at line %d.\n", __func__,
		       __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}
...
static int adrv904x_jesd204_link_setup(struct jesd204_dev *jdev,
				       enum jesd204_state_op_reason reason)
{
	struct adrv904x_jesd204_priv *priv = jesd204_dev_priv(jdev);
 ...
 * Returns init cal.
 */
static ADI_API adi_adrv904x_InitCalibrations_e drv_cals_TrackingCalConvert(const adi_adrv904x_TrackingCalibrationMask_e trackingCal)
{
...
static int adrv904x_probe(struct spi_device *spi)
{
	...
	ret = adrv904x_TlsSet(HAL_TLS_ERR, errData);
