                                                     ...
                                                     const adi_adrv904x_SpiConfigSettings_t* const spiSettings)
{
    ...
    if (!ADI_COMMON_DEVICE_STATE_IS_OPEN(device->common))
    {
        ...
        recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_HwOpen(&device->common);
        if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
        {
...
ADI_API adi_adrv904x_ErrAction_e adi_adrv904x_HwClose(adi_adrv904x_Device_t* const device)
{
    ...
    if (ADI_COMMON_DEVICE_STATE_IS_OPEN(device->common))
    {
        recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_HwClose(&device->common);
        if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
        {
...
ADI_API adi_adrv904x_ErrAction_e adi_adrv904x_HwReset(adi_adrv904x_Device_t* const device)
{
    ...
    recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_HwReset(&device->common, RESETB_LEVEL_LOW);
    if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
    {
        ADI_API_ERROR_REPORT(&device->common, recoveryAction, "Issue with adi_adrv904x_hal_HwReset() setting Level Low");
        goto cleanup;
    }

    {
    static const uint8_t RESETB_WAIT_MS = 50U; /* move here to fix compiler warning */
    recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_Wait_ms(&device->common, RESETB_WAIT_MS);
}
        if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
    {
    ...
    recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_HwReset(&device->common, RESETB_LEVEL_HIGH);
    if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
    {
        ADI_API_ERROR_REPORT(&device->common, recoveryAction, "Issue with adi_adrv904x_hal_HwReset() setting Level High");
        goto cleanup;
    }

    /* Give SPI state machine time to come out of reset */
    recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_Wait_us(&device->common, WAIT_10_US);
    if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
    {
                                                         ...
                                                         const adi_adrv904x_Init_t* const    init)
{
    ...
    recoveryAction = (adi_adrv904x_ErrAction_e) adi_adrv904x_hal_Wait_ms(&device->common, 1U);
    if (recoveryAction != ADI_ADRV904X_ERR_ACT_NONE)
    {
        ADI_API_ERROR_REPORT(&device->common, recoveryAction, "HAL Wait(ms) Failed");
        goto cleanup;
