...
typedef enum
{
    ...
    HAL_TLS_END         /*!< Marks end of valid values; And a special case in adrv904x_TlsSet */
} adi_hal_TlsType_e;
 ...
 */
typedef struct adrv904x_LogStatusGet
{
    uint8_t                 interfaceEnabled;   /*!< Interface Enabled Flag */
...
extern int adrv904x_fseek (FILE * stream, long int offset, int origin);
extern size_t adrv904x_fread(void *ptr, size_t size, size_t count, FILE *stream);
extern size_t adrv904x_fwrite(const void * ptr, size_t size, size_t count, FILE *stream);
extern int adrv904x_fprintf(FILE *stream, const char *fmt, ...);
extern int adrv904x_fclose(FILE *stream);
extern long int adrv904x_ftell(FILE *stream);
extern int adrv904x_ferror(FILE *stream);
extern char *adrv904x_fgets(char *dst, int num, FILE *stream);
extern int adrv904x_fflush(FILE *stream);

extern FILE* adrv904x_fopen (adi_hal_Cfg_t *hal, const char * filename, const char *mode);
#define fopen(filename, mode) adrv904x_fopen(device->common.devHalInfo, filename, mode)

extern void *adrv904x_malloc(size_t size);
#define malloc(size) adrv904x_malloc(size)

extern void *adrv904x_calloc(size_t size, size_t nmemb);

extern void adrv904x_time(ktime_t *second);
extern struct tm* adrv904x_gmtime(adi_hal_Cfg_t *hal, const ktime_t *timer);
#define gmtime(timer) adrv904x_gmtime(device->common.devHalInfo, timer)
