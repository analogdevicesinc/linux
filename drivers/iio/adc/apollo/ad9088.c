				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (entry->out_value) {
		...
		case DBGFS_HSCI_ENABLE:
			...
			break;
		case DBGFS_FSRC_INFO:
			u8 fsrc[] = {ADI_APOLLO_FSRC_A0, ADI_APOLLO_FSRC_A1, ADI_APOLLO_FSRC_B0, ADI_APOLLO_FSRC_B1};
			u8 terminal[] = {ADI_APOLLO_RX, ADI_APOLLO_TX};
			adi_apollo_fsrc_inspect_t fsrc_inspect;

			for (i = 0; i < ARRAY_SIZE(fsrc); i++) {
				for (u8 j = 0; j < ARRAY_SIZE(terminal); j++) {
					ret = adi_apollo_fsrc_inspect(&phy->ad9088, terminal[j], fsrc[i], &fsrc_inspect);
					if (ret)
						break;
					len += snprintf(phy->dbuf + len, sizeof(phy->dbuf), "channel %d %s\n",
						       i, terminal[j] == ADI_APOLLO_RX ? "RX" : "TX");
					len += snprintf(phy->dbuf + len, sizeof(phy->dbuf), "fsrc_bypass %d enable %d\n",
						       fsrc_inspect.fsrc_bypass, fsrc_inspect.dp_cfg.enable);
					len += snprintf(phy->dbuf + len, sizeof(phy->dbuf), "int %llu frac_a %llu frac_b %llu\n",
						       fsrc_inspect.dp_cfg.fsrc_rate_int, fsrc_inspect.dp_cfg.fsrc_rate_frac_a, fsrc_inspect.dp_cfg.fsrc_rate_frac_b);
					len += snprintf(phy->dbuf + len, sizeof(phy->dbuf), "mode_1x %d delay %d\n",
						       fsrc_inspect.dp_cfg.mode_1x, fsrc_inspect.dp_cfg.fsrc_delay);
					len += snprintf(phy->dbuf + len, sizeof(phy->dbuf), "gain_reduction %d data_mult_dither_en %d split_4t4r %d\n\n",
						       fsrc_inspect.dp_cfg.gain_reduction, fsrc_inspect.dp_cfg.data_mult_dither_en, fsrc_inspect.dp_cfg.split_4t4r);
				}
			}
			break;
		default:
					    ...
					    enum jesd204_state_op_reason reason)
{
	...
	ret = ad9088_fsrc_setup(phy, ADI_APOLLO_RX);
	if (ret)
		return ret;
	ret = ad9088_fsrc_setup(phy, ADI_APOLLO_TX);
	if (ret)
		return ret;

	if (phy->trig_req_gpio && phy->trig_sync_en) {
		gpiod_set_value(phy->trig_req_gpio, 1);
		udelay(1);
		gpiod_set_value(phy->trig_req_gpio, 0);
	}
...
static int ad9088_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(phy->trig_req_gpio))
		return PTR_ERR(phy->trig_req_gpio);
