				       ...
				       const char *name, const adi_apollo_fsrc_inspect_t *fsrc)
{
	...
	len += snprintf(buf + len, size - len, "    fsrc_rate_int:    %llu\n", fsrc->dp_cfg.fsrc_rate_int);
	len += snprintf(buf + len, size - len, "    fsrc_rate_frac_a: %llu\n", fsrc->dp_cfg.fsrc_rate_frac_a);
	len += snprintf(buf + len, size - len, "    fsrc_rate_frac_b: %llu\n", fsrc->dp_cfg.fsrc_rate_frac_b);
	len += snprintf(buf + len, size - len, "    gain_reduction:   %u\n", fsrc->dp_cfg.gain_reduction);
	len += snprintf(buf + len, size - len, "    fsrc_delay:       %u\n", fsrc->dp_cfg.fsrc_delay);
...
int ad9088_fsrc_inspect(struct ad9088_phy *phy)
{
	...
	len += snprintf(phy->dbuf + len, sizeof(phy->dbuf) - len,
			"      For N/M format, use adi_api_utils_ratio_composition() in userspace\n");
