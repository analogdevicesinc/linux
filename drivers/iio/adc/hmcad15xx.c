...
/*
 * Compute the actual sampling frequency.
 *
 * Fs = input_clk / clk_div / valid_divisor
 *
 * For dual 8-bit mode (RES_14BIT), the HDL packs two 8-bit samples into
 * 16-bit words using a shift register, resulting in 50% valid duty cycle.
 * Combined with HMCAD clk_div=4, the effective rate is input_clk / 4 / 2.
 *
 * For 8-bit and 12-bit modes, the sampling rate equals input_clk / clk_div.
 */
static unsigned int hmcad15xx_get_sampling_freq(struct hmcad15xx_state *st)
{
	...
	/* Dual 8-bit (RES_14BIT=1) has 50% valid due to sample packing */
