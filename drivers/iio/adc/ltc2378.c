                            ...
                            int *val, int *val2, long info)
{
        ...
        case IIO_CHAN_INFO_RAW: 
                ret = ltc2378_read_channel(adc, chan->channel, val);
                if (ret)
                        return ret;
...
static int ltc2378_buffer(struct iio_dev *indio_dev, struct spi_message *msg)
{
        ...
        xfer->word_delay.unt = SPI_DELAY_UNIT_USECS;
...
static const struct iio_info ltc2378_iio_info = {
        ...
        .debugfs_reg_access = &ltc2378_reg_access,
};
...
static int ltc2378_set_samp_freq(struct ltc2378_adc *adc, int freq)
{
        ...
        cnv_state.period = (struct pwm_state){
...
static int ltc2378_probe(struct spi_device *spi)
{
        ...
        adc->vref = devm_regulator_get(&spi_dev, "vref"); // check with devicetree
        if (IS_ERR(adc->vref))
                return PTR_ER(adc->vref);
        ...
        ret_clk = devm_clk_get(&spi->dev, "ref_clk"); // check with devicetree
        if (IS_ERR(ref_clk))
