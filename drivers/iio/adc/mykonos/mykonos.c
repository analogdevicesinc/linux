...
static mykonosErr_t mykVerifyTxProfile(mykonosDevice_t *device, mykonosTxProfile_t *txProfile, uint32_t *txHsDigClk_kHz)
{
    ...
    *txHsDigClk_kHz = 0;
    ...
    if ((txProfile->iqRate_kHz < MIN_TX_IQRATE_KHZ) ||
        (txProfile->iqRate_kHz > MAX_TX_IQRATE_KHZ))
    {
        return MYKONOS_ERR_TXPROFILE_IQRATE;
    }

    if ((txProfile->rfBandwidth_Hz < MIN_TX_RFBW_HZ) ||
        (txProfile->rfBandwidth_Hz > MAX_TX_RFBW_HZ))
    {
    ...
    if((txProfile->dacDiv != DACDIV_2) &&
       (txProfile->dacDiv != DACDIV_2p5) &&
       (txProfile->dacDiv != DACDIV_4))
    {
        return MYKONOS_ERR_TXPROFILE_DACDIV;
    }

    *txHsDigClk_kHz = (txProfile->iqRate_kHz * txProfile->txFirInterpolation *
...
static mykonosErr_t mykVerifyRxProfile(mykonosDevice_t *device, mykonosRxProfType_t rxChannel, mykonosRxProfile_t *rxProfile, uint32_t *rxHsDigClk_kHz)
{
    ...
    *rxHsDigClk_kHz = 0;
    ...
    /********************************/
    if ((rxProfile->iqRate_kHz < MIN_RX_IQRATE_KHZ) ||
        (rxProfile->iqRate_kHz > MAX_RX_IQRATE_KHZ))
    {
        return MYKONOS_ERR_RXPROFILE_IQRATE;
    }

    /* check for Rx/Obs or Sniffer BW */
    if ((rxProfile->rfBandwidth_Hz < minBwHz) ||
        (rxProfile->rfBandwidth_Hz > maxBwHz))
    {
    ...
    if ((rxProfile->adcDiv !=1) &&
        (rxProfile->adcDiv !=2))
    {
        return MYKONOS_ERR_RXPROFILE_ADCDIV;
    }

    *rxHsDigClk_kHz = (rxProfile->iqRate_kHz * rxProfile->rxFirDecimation *
                       rxProfile->rhb1Decimation * rxProfile->rxDec5Decimation *
                       rxProfile->adcDiv);
...
mykonosErr_t MYKONOS_verifyDeviceDataStructure(mykonosDevice_t *device)
{
    if (device == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, 0, MYKONOS_ERR_CHECKDEVSTRUCT_NULLDEVPOINTER,
                       getMykonosErrorMessage(MYKONOS_ERR_CHECKDEVSTRUCT_NULLDEVPOINTER));
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, 0, MYKONOS_ERR_CHECKDEVSTRUCT_SPI,
                       getMykonosErrorMessage(MYKONOS_ERR_CHECKDEVSTRUCT_SPI));
    ...
    if (device->rx == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_CHECKDEVSTRUCT_RX,
                       getMykonosErrorMessage(MYKONOS_ERR_CHECKDEVSTRUCT_RX));
    ...
    if (device->tx == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_CHECKDEVSTRUCT_TX,
                       getMykonosErrorMessage(MYKONOS_ERR_CHECKDEVSTRUCT_TX));
...
mykonosErr_t MYKONOS_initialize(mykonosDevice_t *device)
{
    ...
    if (device->profilesValid & TX_PROFILE_VALID)
    {
        ...
        {
            case  1: break; /* keep bit as 0 in bitfield */
            case  2: txChannelSettings |= 0x20; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_TXHB2_INTERPOLATION,
        ...
        {
            case  1: break; /* keep bit as 0 in bitfield */
            case  2: txChannelSettings |= 0x10; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_TXHB1_INTERPOLATION,
        ...
        else
        {
            ...
            {
                case  1: txChannelSettings |= 0x01; break;
                case  2: txChannelSettings |= 0x02; break;
                case  4: txChannelSettings |= 0x03; break;
                default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_TXFIR_INTERPOLATION,
        ...
        {
            case   DACDIV_2: break; /* Keep bit [1]=0 */
            case DACDIV_2p5: break; /* Keep bit [1]=0, div 2.5 is set when Rx DEC5 filter enabled*/
            case   DACDIV_4: adcDacClockRateSelect |= 2; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_DACDIV,
                                    getMykonosErrorMessage(MYKONOS_ERR_INIT_INV_DACDIV));
    ...
    if (device->profilesValid & RX_PROFILE_VALID)
    {
        ...
        {
            case  4: break; /* keep bit as 0 in bitfield */
            case  5: rxChannelSettings |= 0x04; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_RXDEC5_DECIMATION,
        ...
        {
            case  1: break; /* keep bit as 0 in bitfield */
            case  2: rxChannelSettings |= 0x10; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_RXHB1_DECIMATION,
        ...
        else
        {
            ...
            {
                case 1: rxChannelSettings |= 0x01; break;
                case 2: rxChannelSettings |= 0x02; break;
                case 4: rxChannelSettings |= 0x03; break;
                default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_RXFIR_DECIMATION,
        ...
        {
            case  1: adcDacClockRateSelect |= 0; break;
            case  2: adcDacClockRateSelect |= 1; break;
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_ADCDIV,
                                    getMykonosErrorMessage(MYKONOS_ERR_INIT_INV_ADCDIV));
    ...
    if (device->profilesValid & ORX_PROFILE_VALID)
    {
        switch(device->obsRx->orxProfile->adcDiv)
        {
            case  1: break; /* Keep bit[4]=0 */
            case  2: adcDacClockRateSelect |= 0x10; break; /* Set bit[4]=1 */
            default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_OBSRX_ADCDIV,
    ...
    if (device->obsRx > 0)
    {
        ...
        if (device->profilesValid & SNIFF_PROFILE_VALID)
        {
            ...
            {
                case 1: break;
                case 2: sniffChannelSettings |= 0x10; break;
                default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_SNIFFER_RHB1,
            ...
            if (device->obsRx->snifferProfile->rxFir > 0)
            {
                ...
                {
                    case 1: sniffChannelSettings |= 0x01; break;
                    case 2: sniffChannelSettings |= 0x02; break;
                    case 4: sniffChannelSettings |= 0x03; break;
                    default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_SNIFFER_RFIR_DEC,
        ...
        if (device->profilesValid & ORX_PROFILE_VALID)
        {
            ...
            {
                case 1: break;
                case 2: orxChannelSettings |= 0x10; break;
                default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_ORX_RHB1,
            ...
            {/* if pointer to orx rxFIR is valid */
                ...
                {
                    case 1: orxChannelSettings |= 0x01; break;
                    case 2: orxChannelSettings |= 0x02; break;
                    case 4: orxChannelSettings |= 0x03; break;
                    default: CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INIT_INV_ORX_RFIR_DEC,
    ...
    */
    ...
    CMB_SPIWriteByte(device->spiSettings, MYKONOS_ADDR_CONFIGURATION_CONTROL_5,
                     ((rxRealIfData << 2) | (enRxHighRejDec5 ? 3 : 0)));
...
mykonosErr_t MYKONOS_waitForEvent(mykonosDevice_t *device, waitEvent_t waitEvent, uint32_t timeout_us)
{
    ...
    switch(waitEvent)
    {
        ...
        case ARMBUSY:/* wait for xD30[7]=0 */
             ...
             break;
        default:
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,
    ...
    if (waitEvent == INITARM_DONE)
    {
        CMB_setTimeout_ms(5000); /* timeout after 5 seconds */
        ...
        do
        {
            ...
            {
                CMB_writeToLog(ADIHAL_LOG_WARNING, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_WAITFOREVENT_TIMEDOUT_INITARMDONE,
                               getMykonosErrorMessage(MYKONOS_ERR_WAITFOREVENT_TIMEDOUT_INITARMDONE));
                return MYKONOS_ERR_WAITFOREVENT_TIMEDOUT_INITARMDONE;
            }
        }while(data  == 0 ); /* for INITARM_DONE, wait for SPI address 0x?? to have a positive value. */
    }
    else
    {
...
/* For SW verification tests, allow API to think all cals are complete*/
    ...
    else
    {
        data = 0xFF;
            ...
            if(CMB_hasTimeoutExpired() > 0)
            {
                CMB_writeToLog(ADIHAL_LOG_WARNING, device->spiSettings->chipSelectIndex,  errCode,
                               getMykonosErrorMessage(errCode));
                return errCode;
            }
...
mykonosErr_t MYKONOS_initDigitalClocks(mykonosDevice_t *device)
{
    ...
    /******************RF Synth Section************************/
    static const uint8_t icp_46p08[53] = {15, 16, 17, 18, 19, 20, 21, 23, 24, 25,
                             26, 27, 28, 29, 30, 32, 20, 21, 21, 22,
                             23, 23, 24, 25, 25, 26, 27, 27, 28, 29,
                             29, 30, 31, 32, 32, 33, 34, 34, 35, 36,
                             36, 37, 38, 39, 21, 21, 21, 22, 22, 22,
                             23, 23, 23};

    static const uint8_t icp_61p44[53] = {11, 12, 13, 13, 14, 15, 16, 17, 18, 18,
                             19, 20, 21, 22, 23, 24, 15, 15, 16, 16,
                             17, 17, 18, 18, 19, 19, 20, 20, 21, 21,
                             22, 22, 23, 24, 24, 25, 25, 26, 26, 27,
                             27, 28, 28, 29, 15, 16, 16, 16, 16, 17,
                             17, 17, 17};

    static const uint8_t icp_76p8[53]  = { 9,  9, 10, 11, 11, 12, 13, 13, 14, 15,
                             15, 16, 17, 17, 18, 19, 12, 12, 13, 13,
                             13, 14, 14, 15, 15, 15, 16, 16, 17, 17,
                             17, 18, 18, 19, 19, 20, 20, 20, 21, 21,
                             22, 22, 23, 23, 12, 12, 13, 13, 13, 13,
                             13, 14, 14};
    ...
    if (retVal != MYKONOS_ERR_OK){return retVal;}
    ...
    switch(hsDiv)
    {
        ...
        case 5:
            ...
            break;
        default:
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_CLKPLL_INV_HSDIV,
                    getMykonosErrorMessage(MYKONOS_ERR_CLKPLL_INV_HSDIV));
    ...
    switch(vcoDiv)
    {
        ...
        case VCODIV_3:
            ...
            break;
        default:
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_CLKPLL_INV_VCODIV,
                    getMykonosErrorMessage(MYKONOS_ERR_CLKPLL_INV_VCODIV));
    ...
    vcoVaractorRef = (vcoIndex <= 16) ? 10 : (vcoIndex > 16 && vcoIndex <= 29) ?
                      11 : (vcoIndex > 29 && vcoIndex <=  32) ?
                      12 : (vcoIndex > 32 && vcoIndex <= 44) ? 14 : 12;

    if ((scaledRefClk_kHz >= 40000) && (scaledRefClk_kHz < 53760))
    { /* Settings designed for 46.08 MHz PLL REFCLK */
        vcoCalOffset   = (vcoIndex <=  2) ? 14 : (vcoIndex >  2 && vcoIndex <=  5) ?
                         13 : (vcoIndex >  5 && vcoIndex <=  13) ?
                         12 : (vcoIndex > 13 && vcoIndex <= 16) ? 11 : 15;
    ...
    { /* Settings designed for 61.44 MHz PLL REFCLK */
        vcoCalOffset   = (vcoIndex <=  2) ? 14 : (vcoIndex >  2 && vcoIndex <=  5) ?
                          13 : (vcoIndex >  5 && vcoIndex <=  11) ?
                          12 : (vcoIndex > 11 && vcoIndex <= 16) ? 11 : 15;
    ...
    { /* Settings designed for 76.8 MHz PLL REFCLK */
        vcoCalOffset   = (vcoIndex <=  2) ? 14 : (vcoIndex >  2 && vcoIndex <=  5) ?
                          13 : (vcoIndex >  5 && vcoIndex <=  10) ?
                          12 : (vcoIndex > 10 && vcoIndex <= 16) ? 11 : 15;
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_SETRFPLL_INV_REFCLK,
                getMykonosErrorMessage(MYKONOS_ERR_SETRFPLL_INV_REFCLK));
...
mykonosErr_t MYKONOS_setRfPllFrequency(mykonosDevice_t *device, mykonosRfPllName_t pllName, uint64_t rfPllLoFrequency_Hz)
{
    ...
    switch(pllName)
    {
        ...
        case TX_PLL:      extData[1] = 0x01; break;
        case SNIFFER_PLL: extData[1] = 0x02; break;
        default:
        {
    ...
    if (retVal != MYKONOS_ERR_OK)
    {
        if (cmdStatusByte > 0)
        {
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_SETRFPLL_ARMERROR,
                    getMykonosErrorMessage(MYKONOS_ERR_SETRFPLL_ARMERROR));
...
mykonosErr_t MYKONOS_getRfPllFrequency(mykonosDevice_t *device, mykonosRfPllName_t pllName, uint64_t *rfPllLoFrequency_Hz)
{
    ...
    switch(pllName)
    {
        case CLK_PLL:     getClkPllFrequency = 1; break;
        case RX_PLL:      extData[1] = 0x00; break;
        case TX_PLL:      extData[1] = 0x01; break;
        case SNIFFER_PLL: extData[1] = 0x02; break;
        default:
    ...
    if (getClkPllFrequency > 0)
    {
        ...
        {
            case 0: refclk_Hz = (device->clocks->deviceClock_kHz * 1000); break;
            case 1: refclk_Hz = ((device->clocks->deviceClock_kHz * 1000) >> 1); break; /* div 2 */
            case 2: refclk_Hz = ((device->clocks->deviceClock_kHz * 1000) >> 2); break; /* div 4 */
            case 3: refclk_Hz = ((device->clocks->deviceClock_kHz * 1000) << 1); break; /* times 2 */
            default:
            {
        ...
        switch (vcoDivReg)
        {
            case 0: vcoDivTimes10 = 10; break;
            case 1: vcoDivTimes10 = 15; break;
            case 2: vcoDivTimes10 = 20; break;
            case 3: vcoDivTimes10 = 30; break;
        }
        ...
        //*rfPllLoFrequency_Hz = (((refclk_Hz * clkPllIntWord)  + (((refclk_Hz * clkPllFracWord / 1044480) + 1) >> 1)) * hsDiv * vcoDivTimes10 / 10);
...
mykonosErr_t MYKONOS_setTxPfirSyncClk(mykonosDevice_t *device)
{
    ...
    if (((device->profilesValid & TX_PROFILE_VALID) > 0) &&
         (device->tx->txProfile->txFir != NULL))
    {
        ...
        dpClk_kHz = (device->tx->txProfile->iqRate_kHz * device->tx->txProfile->txInputHbInterpolation *
                    device->tx->txProfile->txFirInterpolation * effectiveRows);
...
mykonosErr_t MYKONOS_setRxPfirSyncClk(mykonosDevice_t *device)
{
    ...
    if (((device->profilesValid & RX_PROFILE_VALID) > 0) &&
         (device->rx->rxProfile->rxFir != NULL))
    {
        switch(device->rx->rxProfile->rxFir->numFirCoefs)
        {
            case 24: effectiveRxNumRows = 1; break;
            case 48: effectiveRxNumRows = 2; break;
            case 72: effectiveRxNumRows = 4; break;
        }
        ...
        if ((slowestIqRate_kHz == 0) || (slowestIqRate_kHz > device->rx->rxProfile->iqRate_kHz))
        {
    ...
    if (((device->profilesValid & ORX_PROFILE_VALID) > 0) &&
        (device->obsRx->orxProfile->rxFir != NULL))
    {
        switch(device->obsRx->orxProfile->rxFir->numFirCoefs)
        {
            case 24: effectiveOrxNumRows = 1; break;
            case 48: effectiveOrxNumRows = 2; break;
            case 72: effectiveOrxNumRows = 4; break;
        }
    ...
    if (((device->profilesValid & SNIFF_PROFILE_VALID) > 0) &&
        (device->obsRx->snifferProfile->rxFir != NULL))
    {
        switch(device->obsRx->snifferProfile->rxFir->numFirCoefs)
        {
            case 24: effectiveSnrxNumRows = 1; break;
            case 48: effectiveSnrxNumRows = 2; break;
            case 72: effectiveSnrxNumRows = 4; break;
        }
...
mykonosErr_t MYKONOS_programFir(mykonosDevice_t *device, mykonosfirName_t filterToProgram, mykonosFir_t *firFilter)
{
    ...
    {
        case     TX1_FIR:  filterSelect = 0x01; numTapMultiple = 16; maxNumTaps = 96; break;
        case     TX2_FIR:  filterSelect = 0x02; numTapMultiple = 16; maxNumTaps = 96; break;
        case  TX1TX2_FIR:  filterSelect = 0x03; numTapMultiple = 16; maxNumTaps = 96; break;
        case     RX1_FIR:  filterSelect = 0x04; numTapMultiple = 24; maxNumTaps = 72; break;
        case     RX2_FIR:  filterSelect = 0x08; numTapMultiple = 24; maxNumTaps = 72; break;
        case  RX1RX2_FIR:  filterSelect = 0x0C; numTapMultiple = 24; maxNumTaps = 72; break;
        case OBSRX_A_FIR:  filterSelect = 0x10; numTapMultiple = 24; maxNumTaps = 72; break;
        case OBSRX_B_FIR:  filterSelect = 0x20; numTapMultiple = 24; maxNumTaps = 72; break;
        default:
    ...
    if ((firFilter->numFirCoefs % numTapMultiple == 0) && (firFilter->numFirCoefs > 0) &&
        (firFilter->numFirCoefs <= maxNumTaps))
    {
    ...
    if (filterToProgram == RX1_FIR || filterToProgram == RX2_FIR || filterToProgram == RX1RX2_FIR)
    {
        ...
        {
            case -12: filterGain = 0x00; break;
            case  -6: filterGain = 0x01; break;
            case   0: filterGain = 0x02; break;
            case   6: filterGain = 0x03; break;
            default :
    ...
    else if (filterToProgram == TX1_FIR || filterToProgram == TX2_FIR || filterToProgram == TX1TX2_FIR)
    {
        ...
        switch (firFilter->gain_dB){
            case 0  : filterGain = 0; break;
            case 6  : filterGain = 1; break;
            default :
...
mykonosErr_t MYKONOS_programRxGainTable(mykonosDevice_t *device, uint8_t *gainTablePtr, uint8_t numGainIndexesInTable, mykonosGainTable_t rxChannel)
{
    ...
    switch(rxChannel)
    {
        ...
        case LOOPBACK_GT:
            ...
            break;
        default:
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_RXGAINTABLE_INV_CHANNEL, getMykonosErrorMessage(MYKONOS_ERR_RXGAINTABLE_INV_CHANNEL));
            return MYKONOS_ERR_RXGAINTABLE_INV_CHANNEL;
    }

    if (retFlag)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_RXGAINTABLE_INV_GAIN_INDEX_RANGE, getMykonosErrorMessage(MYKONOS_ERR_RXGAINTABLE_INV_GAIN_INDEX_RANGE));
...
mykonosErr_t MYKONOS_setupRxAgc(mykonosDevice_t *device)
{
    ...
    if ((device->rx->rxAgcCtrl->peakAgc->apdHighThresh > 0x3F) ||
    ...
    if ((device->rx->rxAgcCtrl->peakAgc->apdLowThresh > 0x3F) ||
        (device->rx->rxAgcCtrl->peakAgc->apdHighThresh < device->rx->rxAgcCtrl->peakAgc->apdLowThresh))
    {
    ...
    if ((device->rx->rxAgcCtrl->peakAgc->hb2HighThresh > 0xFF) ||
    ...
    if ((device->rx->rxAgcCtrl->peakAgc->hb2LowThresh > 0xFF) ||
    ...
    if ((device->rx->rxAgcCtrl->peakAgc->hb2VeryLowThresh > 0xFF) ||
    ...
    else
    {
        ...
        hb2OvldCfgRegValue = (device->rx->rxAgcCtrl->peakAgc->hb2OverloadThreshCnt) |
                             (device->rx->rxAgcCtrl->peakAgc->hb2OverloadDurationCnt << 4) |
...
mykonosErr_t MYKONOS_setDefaultObsRxPath(mykonosDevice_t *device,  mykonosObsRxChannels_t defaultObsRxCh)
{
    ...
    if (device->obsRx == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_SETDEFOBSRXPATH_NULL_OBSRX_STRUCT,
                       getMykonosErrorMessage(MYKONOS_ERR_SETDEFOBSRXPATH_NULL_OBSRX_STRUCT));
        return MYKONOS_ERR_SETDEFOBSRXPATH_NULL_OBSRX_STRUCT;
    }
...
mykonosErr_t MYKONOS_setObsRxPathSource(mykonosDevice_t *device, mykonosObsRxChannels_t obsRxCh)
{
    ...
    if (((obsRxCh > OBS_RX2_SNIFFERLO) &&
        (obsRxCh != OBS_SNIFFER_A) &&
        (obsRxCh != OBS_SNIFFER_B) &&
        (obsRxCh != OBS_SNIFFER_C))
       )
    {
...
mykonosErr_t MYKONOS_setupObsRxAgc(mykonosDevice_t *device)
{
    ...
    /* Range check for agcGainUpdateCounter (22-bit) */
    if ((device->obsRx->orxAgcCtrl->agcGainUpdateCounter > 0x3FFFFF) || (device->obsRx->orxAgcCtrl->agcGainUpdateCounter < 1))
    {
    ...
    /* Range check on agcPeakWaitTime (5-bit) */
    if (device->obsRx->orxAgcCtrl->agcPeakWaitTime > 0x1F || device->obsRx->orxAgcCtrl->agcPeakWaitTime < 0x02)
    {
    ...
    /* Range check apdFastAttack and hb2FastAttack (1-bit)  */
    if (device->obsRx->orxAgcCtrl->peakAgc->apdFastAttack > 0x1 || device->obsRx->orxAgcCtrl->peakAgc->hb2FastAttack > 0x1)
    {
    ...
    /* Range check hb2HighThresh */
    if ((device->obsRx->orxAgcCtrl->peakAgc->hb2HighThresh > 0xFF) ||
    ...
    /* Range check hb2LowThresh */
    if ((device->obsRx->orxAgcCtrl->peakAgc->hb2LowThresh > 0xFF) ||
    ...
    /* Range check hb2VeryLowThresh */
    if ((device->obsRx->orxAgcCtrl->peakAgc->hb2VeryLowThresh > 0xFF) ||
...
mykonosErr_t MYKONOS_setTx1Attenuation(mykonosDevice_t *device, uint16_t tx1Attenuation_mdB)
{
    ...
    {
    case TXATTEN_0P05_DB: attenStepSizeDiv = 50; break;
    case TXATTEN_0P1_DB: attenStepSizeDiv = 100; break;
    case TXATTEN_0P2_DB: attenStepSizeDiv = 200; break;
    case TXATTEN_0P4_DB: attenStepSizeDiv = 400; break;
    default:
...
mykonosErr_t MYKONOS_enableTxNco(mykonosDevice_t *device, uint8_t enable, int32_t tx1ToneFreq_kHz, int32_t tx2ToneFreq_kHz)
{
    ...
    if (enable > 0)
    {
        ...
        if ((tx1ToneFreq_kHz > ((int32_t)(device->tx->txProfile->iqRate_kHz) / 2)) ||
            (tx1ToneFreq_kHz < ((int32_t)(device->tx->txProfile->iqRate_kHz) * -1 / 2)))
        {
        ...
        if ((tx2ToneFreq_kHz > ((int32_t)(device->tx->txProfile->iqRate_kHz) / 2)) ||
            (tx2ToneFreq_kHz < ((int32_t)(device->tx->txProfile->iqRate_kHz) * -1 / 2)))
        {
 ...
 *
 * \param device                  Pointer to the Mykonos device's data structure
 * \param powerThreshold  12-bit field. PA Protection Threshold sets the power level that, if detected in the TX data path, raises the PA error flags.
 * \param attenStepSize     7-bit field. Attenuation Step Size sets the size of the attenuation step when Tx Atten Control is Enabled. Resolution is 0.2dB/LSB.
 * \param avgDuration         4-bit field. Averaging Duration for the TX data path power measurement. Specified in number of cycles of the TX IQ Rate.
...
mykonosErr_t MYKONOS_setupPaProtection(mykonosDevice_t *device, uint16_t powerThreshold, uint8_t attenStepSize, uint8_t avgDuration, uint8_t stickyFlagEnable, uint8_t txAttenControlEnable)
{
    ...
    CMB_SPIWriteByte(device->spiSettings, MYKONOS_ADDR_PA_PROECTION_ATTEN_CONTROL, 0x00);
    ...
    CMB_SPIWriteByte(device->spiSettings, MYKONOS_ADDR_PA_PROECTION_ATTEN_CONTROL, paProtectAttenCntl);
...
/**
 * \brief Enables the Mykonos PA Protection block according to the parameters passed in MYKONOS_setupPaProection(...).
 *
 ...
 *
 * This function uses the 'averagingDuration' parameter provided in MYKONOS_setupPaProtection to set the number of samples to accumulate
 * to obtain an estimate for a TX channel specified by the 'channel' parameter. A 12-bit field estimating the channel power is returned in the
 * '*channelPower' pointer.
 *
 * <B>Dependencies</B>
 * - device->spiSettings
 * - device->spiSettings->chipSelectIndex
 ...
 * "1" indicates TX1 Error Flag is high
 * "2" indicates TX2 Error Flag is high
...
const char* getMykonosErrorMessage(mykonosErr_t errorCode)
{
    ...
    switch (errorCode)
    {
        case MYKONOS_ERR_OK: return "";
        case MYKONOS_ERR_INV_PARM: return "Mykonos: Invalid parameter\n";
        ...
        case MYKONOS_ERR_DESER_INV_EQ_PARM: return "Invalid deserializer EQ setting";
        case MYKONOS_ERR_DESER_INV_LANEPN_PARM: return "Invalid deserializer invert Lane PN setting\n";
        ...
        case MYKONOS_ERR_DEFRAMER_INV_K_OFFSET_PARAM: return "Invalid Deframer LMFC offset";
        case MYKONOS_ERR_DEFRAMER_INV_K_PARAM: return "Invalid Deframer K setting\n";
        ...
        case MYKONOS_ERR_INITARM_INV_DATARATE_PARM: return 0;
        ...
        case MYKONOS_ERR_LOADBIN_INVALID_BYTECOUNT: return 0;
        case MYKONOS_ERR_ARM_INVALID_BUILDCHKSUM: return "Verify ARM checksum failed\n";
        ...
        case MYKONOS_ERR_GETINITCALSTATUS_ARMERROR: return "MYKONOS_getInitCalStatus() returned an ARM error while getting the init cal status information";
        case MYKONOS_ERR_CFGCLGC_TXORX_PROFILE_INV: return "Tx and ObsRx profiles must be valid to use the CLGC feature in MYKONOS_configClgc()\n";
...
static mykonosErr_t MYKONOS_calculateScaledDeviceClk_kHz(mykonosDevice_t *device, uint32_t *scaledRefClk_kHz, uint8_t *deviceClkDiv)
{
    uint32_t deviceClock_kHz = device->clocks->deviceClock_kHz;
    ...
    if (device == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, 0, MYKONOS_ERR_NULL_DEVICE_PARAM,
                       getMykonosErrorMessage(MYKONOS_ERR_NULL_DEVICE_PARAM));
    ...
    if      (deviceClock_kHz >  20000 && deviceClock_kHz <=  40000) {*scaledRefClk_kHz = deviceClock_kHz << 1; *deviceClkDiv = 3;} /* x2 */
    else if (deviceClock_kHz >  40000 && deviceClock_kHz <=  80000) {*scaledRefClk_kHz = deviceClock_kHz;      *deviceClkDiv = 0;} /* x1 */
    else if (deviceClock_kHz >  80000 && deviceClock_kHz <= 160000) {*scaledRefClk_kHz = deviceClock_kHz >> 1; *deviceClkDiv = 1;} /* div2 */
    else if (deviceClock_kHz > 160000 && deviceClock_kHz <= 320000) {*scaledRefClk_kHz = deviceClock_kHz >> 2; *deviceClkDiv = 2;} /* div4 */
    else {
...
mykonosErr_t MYKONOS_setupSerializers(mykonosDevice_t *device)
{
    ...
    if (device->profilesValid & RX_PROFILE_VALID)
    {
        ...
        {
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SER_INV_M_PARM,
                           getMykonosErrorMessage(MYKONOS_ERR_SER_INV_M_PARM));
        ...
        {
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SER_INV_AMP_PARM,
                           getMykonosErrorMessage(MYKONOS_ERR_SER_INV_AMP_PARM));
        ...
        {
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SER_INV_L_PARM,
                           getMykonosErrorMessage(MYKONOS_ERR_SER_INV_L_PARM));
    ...
    if (device->obsRx->framer > 0 && (device->profilesValid & (ORX_PROFILE_VALID | SNIFF_PROFILE_VALID)))
    {
        ...
        if ((obsRxLaneRate_kHz != 0) &&
            ((obsRxLaneRate_kHz < 614400) || (obsRxLaneRate_kHz > 6144000))
           )
        {
            CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SER_INV_LANERATE_PARM,
                           getMykonosErrorMessage(MYKONOS_ERR_SER_INV_LANERATE_PARM));
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SER_INV_HSCLK_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_SER_INV_HSCLK_PARM));
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INITSER_INV_PROFILE,
                       getMykonosErrorMessage(MYKONOS_ERR_INITSER_INV_PROFILE));
    ...
    if (device->profilesValid & RX_PROFILE_VALID)
    {
        ...
        else if (hsDigClkdiv4_5_kHz > rxFramerPclk_kHz)
        {
            ...
            switch(tempPclkDiv)
            {
                case  2: rxPclkDiv = 3; break;
                case  4: rxPclkDiv = 4; break;
                case  8: rxPclkDiv = 5; break;
                case 16: rxPclkDiv = 6; break;
                default:
                {
        ...
        else if (hsDigClkdiv4_5_kHz < rxFramerPclk_kHz)
        {
            ...
            switch(tempPclkDiv)
            {
                case  2: rxPclkDiv = 1; break;
                case  4: rxPclkDiv = 0; break;
                default:
                {
                    CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_INV_RXFRAMER_PCLKDIV_PARM,
                                   getMykonosErrorMessage(MYKONOS_ERR_INV_RXFRAMER_PCLKDIV_PARM));
    ...
    else if (device->profilesValid & (ORX_PROFILE_VALID | SNIFF_PROFILE_VALID))
    {
        ...
        else if (hsDigClkdiv4_5_kHz > obsRxFramerPclk_kHz)
        {
            ...
            switch(tempPclkDiv)
            {
                case  2: obsRxPclkDiv = 3; break;
                case  4: obsRxPclkDiv = 4; break;
                case  8: obsRxPclkDiv = 5; break;
                case 16: obsRxPclkDiv = 6; break;
                default:
                {
        ...
        else if (hsDigClkdiv4_5_kHz < obsRxFramerPclk_kHz)
        {
            ...
            switch(tempPclkDiv)
            {
                case  2: obsRxPclkDiv = 1; break;
                case  4: obsRxPclkDiv = 0; break;
                default:
                {
                    CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_INV_OBSRXFRAMER_PCLKDIV_PARM,
                                   getMykonosErrorMessage(MYKONOS_ERR_INV_OBSRXFRAMER_PCLKDIV_PARM));
...
mykonosErr_t MYKONOS_setupDeserializers(mykonosDevice_t *device)
{
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_DESER_INV_M_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_DESER_INV_M_PARM));
    ...
    if ((L == 0) || (L == 3) || (L > 4))
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_DESER_INV_L_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_DESER_INV_L_PARM));
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_DESER_INV_EQ_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_DESER_INV_EQ_PARM));
    ...
    else if(hsclkRate_kHz > 6144000 && hsclkRate_kHz <= 12288000)
    {
        rxcdr_div = 2;
    }
    else if(hsclkRate_kHz > 12288000)
    {
        rxcdr_div = 4;
    }
    else
...
mykonosErr_t MYKONOS_setupJesd204bFramer(mykonosDevice_t *device)
{
    ...
    if (device->rx->framer->M != 1 && device->rx->framer->M != 2 && device->rx->framer->M !=4)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_FRAMER_INV_M_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_FRAMER_INV_M_PARM));
    ...
    if (ML == 42)
    {
        ...
        {
            case 3:  framerLaneXbar = 0x08; break;
            case 5:  framerLaneXbar = 0x20; break;
            case 6:  framerLaneXbar = 0x20; break;
            case 9:  framerLaneXbar = 0x80; break;
            case 10: framerLaneXbar = 0x80; break;
            case 12: framerLaneXbar = 0x80; break;
            default:
    ...
    else
    {
        switch((device->rx->framer->serializerLanesEnabled & 0x0F))
        {
            ...
            case 1:  framerLaneXbar = 0x00; break;
            case 2:  framerLaneXbar = 0x00; break;
            case 3:  framerLaneXbar = 0x04; break;
            case 4:  framerLaneXbar = 0x00; break;
            case 5:  framerLaneXbar = 0x10; break;
            case 6:  framerLaneXbar = 0x10; break;
            case 8:  framerLaneXbar = 0x00; break;
            case 9:  framerLaneXbar = 0x40; break;
            case 10: framerLaneXbar = 0x40; break;
            case 12: framerLaneXbar = 0x40; break;
            case 15: framerLaneXbar = 0xE4; break;
            default:
    ...
    subaddr[ 0] = 0x00; subdata[ 0] = (DID & 0xFF);                             /* Device ID */
    subaddr[ 1] = 0x01; subdata[ 1] = (BID & 0x0F);                             /* Bank ID */
    subaddr[ 2] = 0x02; subdata[ 2] = (LID0 & 0x1F);                             /* Lane 0 ID */
    subaddr[ 3] = 0x03; subdata[ 3] = (device->rx->framer->scramble <<7) | (FramerL & 0x1F); /* [7] Scramble, #Lanes[4:0]-1 */
    subaddr[ 4] = 0x04; subdata[ 4] = (FramerF & 0xFF);                         /* F[7:0]-1 */
    subaddr[ 5] = 0x05; subdata[ 5] = (K & 0x1F);                                 /* K[4:0]-1 */
    subaddr[ 6] = 0x06; subdata[ 6] = (FramerM & 0xFF);                         /* M[7:0]-1 */
    ...
    {
        /* Lane 1 ID */
        subaddr[14] = 0x1A; subdata[14] = LID1;

        CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID1 & 0x1F) + (device->rx->framer->scramble) +
                   (FramerL & 0x1F)+ (FramerF & 0xFF) +  (K & 0x1F) +
                   (FramerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) +
                   (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
                   (SUBCLASSV & 7) + (JESDV & 7);
        ...
        /* Checksum Lane 1 */
        subaddr[15] = 0x1D; subdata[15] = CheckSum & 0xFF;
    }
    else
    {
        /* Lane 1 ID */
        subaddr[14] = 0x12; subdata[14] = LID1;
...
mykonosErr_t MYKONOS_setupJesd204bObsRxFramer(mykonosDevice_t *device)
{
    ...
    subaddr[ 1] = 0x01; subdata[ 1] = (BID & 0x0F);                             /* Bank ID */
    subaddr[ 2] = 0x02; subdata[ 2] = (LID0 & 0x1F);                             /* Lane 0 ID */
    subaddr[ 3] = 0x03; subdata[ 3] = (device->obsRx->framer->scramble <<7) | (FramerL & 0x1F); /* [7] Scramble, #Lanes[4:0]-1 */
    subaddr[ 4] = 0x04; subdata[ 4] = (FramerF & 0xFF);                         /* F[7:0]-1 */
    subaddr[ 5] = 0x05; subdata[ 5] = (K & 0x1F);                                 /* K[4:0]-1 */
    subaddr[ 6] = 0x06; subdata[ 6] = (FramerM & 0xFF);                         /* M[7:0]-1 */
    subaddr[ 7] = 0x07; subdata[ 7] = ((CS & 0x3)<<6) | (N & 0x1F);             /* [7:6] = CS[1:0], N[4:0]-1 */
    ...
    CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID0 & 0x1F) + (device->obsRx->framer->scramble) + (FramerL & 0x1F)+ (FramerF & 0xFF) +  (K & 0x1F) +
      (FramerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) + (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
      (SUBCLASSV & 7) + (JESDV & 7);
    ...
    CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID1 & 0x1F) + (device->obsRx->framer->scramble) + (FramerL & 0x1F)+ (FramerF & 0xFF) +  (K & 0x1F) +
    (FramerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) + (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
    (SUBCLASSV & 7) + (JESDV & 7);
    ...
    CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID2 & 0x1F) + (device->obsRx->framer->scramble) + (FramerL & 0x1F)+ (FramerF & 0xFF) +  (K & 0x1F) +
      (FramerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) + (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
      (SUBCLASSV & 7) + (JESDV & 7);
    ...
    CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID3 & 0x1F) + (device->obsRx->framer->scramble) + (FramerL & 0x1F)+ (FramerF & 0xFF) +  (K & 0x1F) +
      (FramerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) + (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
      (SUBCLASSV & 7) + (JESDV & 7);
...
mykonosErr_t MYKONOS_enableRxFramerLink(mykonosDevice_t *device, uint8_t enable)
{
    ...
    if (enableLink)
    {
        ...
        CMB_SPIWriteField(device->spiSettings, MYKONOS_ADDR_SERIALIZER_CTL_1,
                          ((~device->rx->framer->serializerLanesEnabled) & 0x0F),
...
mykonosErr_t MYKONOS_enableObsRxFramerLink(mykonosDevice_t *device, uint8_t enable)
{
    ...
    if (enableLink)
    {
        ...
        CMB_SPIWriteField(device->spiSettings, MYKONOS_ADDR_SERIALIZER_CTL_1,
                          ((~device->obsRx->framer->serializerLanesEnabled) & 0x0F),
    ...
    else
    {
        ...
        CMB_SPIWriteField(device->spiSettings, MYKONOS_ADDR_SERIALIZER_CTL_1, device->obsRx->framer->serializerLanesEnabled, device->obsRx->framer->serializerLanesEnabled, 0);
    }
...
mykonosErr_t MYKONOS_setupJesd204bDeframer(mykonosDevice_t *device)
{
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_DEFRAMER_INV_M_PARM,
    ...
    subaddr[ 0] = 0x50; subdata[ 0] = DID & 0xFFU;                             /* Device ID */
    subaddr[ 1] = 0x51; subdata[ 1] = BID & 0x0F;                             /* Bank ID */
    subaddr[ 2] = 0x52; subdata[ 2] = LID0 & 0x1F;                             /* Lane 0 ID */
    subaddr[ 3] = 0x53; subdata[ 3] =  (device->tx->deframer->scramble << 7) | (DeframerL & 0x1F); /* [7] = Scramble Enable, #Lanes[4:0] */
    subaddr[ 4] = 0x54; subdata[ 4] = DeframerF & 0xFFU;                     /* F[7:0] */
    subaddr[ 5] = 0x55; subdata[ 5] = K & 0x1F;                             /* K[4:0] */
    subaddr[ 6] = 0x56; subdata[ 6] = DeframerM & 0xFFU;                     /* M[7:0] */
    subaddr[ 7] = 0x57; subdata[ 7] = ((CS & 0x3) << 6) | (N & 0x1F);         /* [7:6] = CS[1:0], N[4:0] */
    subaddr[ 8] = 0x58; subdata[ 8] = ((SUBCLASSV & 7) << 5) | (Np & 0x1F);    /* Np[4:0] */
    subaddr[ 9] = 0x59; subdata[ 9] = ((JESDV & 7) <<5) | (S & 0x1F);         /* S[4:0] */
    subaddr[10] = 0x5A; subdata[10] = ((HD & 1) << 7) | (CF & 0x1F);         /* [7]=HD, CF[4:0] */
    subaddr[11] = 0x5B; subdata[11] = 0x00;                                 /* reserved */
    subaddr[12] = 0x5C; subdata[12] = 0x00;                                    /* reserved */
    CheckSum = (DID & 0xFF) + (BID & 0xF) + (LID0 & 0x1F) + (device->tx->deframer->scramble) + (DeframerL & 0x1F)+ (DeframerF & 0xFF) +  (K & 0x1F) +
        (DeframerM & 0xFF) + (CS & 0x3) +  (N & 0x1F) + (Np & 0x1F) + (S & 0x1F) + (HD & 1) +  (CF & 0x1F) +
        (SUBCLASSV & 7) + (JESDV & 7);
    subaddr[13] = 0x5D; subdata[13] = CheckSum & 0xFFU;                     /* Checksum Lane 0 */
    subaddr[14] = 0x6D; subdata[14] = 0xA0U;                                 /* Bad Disparity setup */
    subaddr[15] = 0x6E; subdata[15] = 0xA0U;                                 /* Not in Table setup */
    subaddr[16] = 0x6F; subdata[16] = 0xA0U;                                 /* UnExpected K character setup */
    subaddr[17] = 0x75; subdata[17] = CTRLREG0;                             /* CTRLREG 0 */
    subaddr[18] = 0x76; subdata[18] = CTRLREG1;                             /* CTRLREG 1 (Bytes per frame) */
    subaddr[19] = 0x77; subdata[19] = CTRLREG2;                             /* CTRLREG 2 */
    subaddr[20] = 0x78; subdata[20] = 0x01;                                 /* # 4*K multiframes during ILAS */
    subaddr[21] = 0x7A; subdata[21] = 0xE0U;                                 /* Setup JESD interrupt sources */
    subaddr[22] = 0x7B; subdata[22] = 0x08U;                                 /* Sync assertion setup */
    subaddr[23] = 0x7C; subdata[23] = 0xFFU;                                 /* Error count threshold */
    subaddr[24] = 0x7D; subdata[24] = DeframerLaneEnable & 0xFFU;             /* Lane enable */
...
mykonosErr_t MYKONOS_readDeframerPrbsCounters(mykonosDevice_t *device, uint8_t counterSelect, uint32_t *prbsErrorCount)
{
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_READ_DEFRAMERPRBS_NULL_PARAM, getMykonosErrorMessage(MYKONOS_ERR_READ_DEFRAMERPRBS_NULL_PARAM));
        return MYKONOS_ERR_READ_DEFRAMERPRBS_NULL_PARAM;
    }

    if (counterSelect & ~0x03)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_DEFRAMER_INV_PRBS_CNTR_SEL_PARAM, getMykonosErrorMessage(MYKONOS_ERR_DEFRAMER_INV_PRBS_CNTR_SEL_PARAM));
...
mykonosErr_t MYKONOS_getInitCalStatus(mykonosDevice_t *device, mykonosInitCalStatus_t *initCalStatus)
{
    ...
    initCalStatus->calsDoneLifetime = ((uint32_t)(calStatusArray[3]) << 24) | ((uint32_t)(calStatusArray[2]) << 16) | ((uint32_t)(calStatusArray[1]) << 8) | ((uint32_t)(calStatusArray[0]));
    initCalStatus->calsDoneLastRun = ((uint32_t)(calStatusArray[7]) << 24) | ((uint32_t)(calStatusArray[6]) << 16) | ((uint32_t)(calStatusArray[5]) << 8) | ((uint32_t)(calStatusArray[4]));
    initCalStatus->calsMinimum = ((uint32_t)(calStatusArray[11]) << 24) | ((uint32_t)(calStatusArray[10]) << 16) | ((uint32_t)(calStatusArray[9]) << 8) | ((uint32_t)(calStatusArray[8]));
...
mykonosErr_t MYKONOS_getTxLolStatus(mykonosDevice_t *device, mykonosTxChannels_t txChannel, mykonosTxLolStatus_t *txLolStatus)
{
    ...
    txLolStatus->performanceMetric = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
...
mykonosErr_t MYKONOS_getTxQecStatus(mykonosDevice_t *device, mykonosTxChannels_t txChannel, mykonosTxQecStatus_t *txQecStatus)
{
    ...
    txQecStatus->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                   (((uint32_t)armReadBack[5]) <<  8) | ((uint32_t)armReadBack[4]);
    txQecStatus->performanceMetric = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                     (((uint32_t)armReadBack[ 9]) <<  8) | ((uint32_t)armReadBack[ 8]);
...
mykonosErr_t MYKONOS_initArm(mykonosDevice_t *device)
{
    ...
    switch(device->clocks->clkPllVcoDiv)
    {
        case VCODIV_1: vcoDivTimes10 = 10; break;
        case VCODIV_1p5: vcoDivTimes10 = 15; break;
        case VCODIV_2: vcoDivTimes10 = 20; break;
        case VCODIV_3: vcoDivTimes10 = 30; break;
        default:
        {
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_INITARM_INV_REGCLK,
...
mykonosErr_t MYKONOS_loadArmFromBinary(mykonosDevice_t *device, uint8_t *binary, uint32_t count)
{
    ...
    if(binary == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_LOADBIN_NULL_PARAM,
                       getMykonosErrorMessage(MYKONOS_ERR_LOADBIN_NULL_PARAM));
...
mykonosErr_t MYKONOS_checkArmState(mykonosDevice_t *device, mykonosArmState_t armStateCheck)
{
    ...
    do
    {
        MYKONOS_getRadioState(device, &armStatus);
...
mykonosErr_t MYKONOS_getArmVersion(mykonosDevice_t *device, uint8_t *majorVer, uint8_t *minorVer, uint8_t *rcVer)
{
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_GETARMVER_NULL_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_GETARMVER_NULL_PARM));
...
mykonosErr_t MYKONOS_configDpd(mykonosDevice_t *device)
{
    ...
    /* throw error if not in radioOff/IDLE state */
    if (((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_IDLE) &&
        ((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_READY))
    {
    ...
    if ((device->tx->dpdConfig->damping < 0) || (device->tx->dpdConfig->damping > 15))
    {
...
mykonosErr_t MYKONOS_getDpdStatus(mykonosDevice_t *device, mykonosTxChannels_t txChannel, mykonosDpdStatus_t *dpdStatus)
{
    ...
    switch(txChannel)
    {
        case TX1: channelSelect = 0; break;
        case TX2: channelSelect = 1; break;
        default:
        {
...
mykonosErr_t MYKONOS_restoreDpdModel(mykonosDevice_t *device, mykonosTxChannels_t txChannel, uint8_t *modelDataBuffer, uint32_t modelNumberBytes)
{
    ...
    /* throw error if not in radioOff/IDLE state */
     if (((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_IDLE) &&
         ((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_READY))
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_RESTDPDMOD_ARMSTATE,
                       getMykonosErrorMessage(MYKONOS_ERR_RESTDPDMOD_ARMSTATE));  /* radio not off or initialized */
...
mykonosErr_t MYKONOS_saveDpdModel(mykonosDevice_t *device, mykonosTxChannels_t txChannel, uint8_t *modelDataBuffer, uint32_t modelNumberBytes)
{
    ...
    /* throw error if not in radioOff/IDLE state */
     if (((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_IDLE) &&
         ((radioStatus & 0x03) != MYKONOS_ARM_SYSTEMSTATE_READY))
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_SAVDPDMOD_ARMSTATE,
                       getMykonosErrorMessage(MYKONOS_ERR_SAVDPDMOD_ARMSTATE));  /* radio not off or initialized */
...
mykonosErr_t MYKONOS_configClgc(mykonosDevice_t *device)
{
    ...
    if ((device->tx->clgcConfig->tx1DesiredGain < -10000) || (device->tx->clgcConfig->tx1DesiredGain > 10000) ||
        (device->tx->clgcConfig->tx2DesiredGain < -10000) || (device->tx->clgcConfig->tx2DesiredGain > 10000))
    {
    ...
    if ((device->tx->clgcConfig->tx1ControlRatio < 1) || (device->tx->clgcConfig->tx1ControlRatio > 100) ||
        (device->tx->clgcConfig->tx2ControlRatio < 1) || (device->tx->clgcConfig->tx2ControlRatio > 100))
    {
...
mykonosErr_t MYKONOS_configVswr(mykonosDevice_t *device)
{
    ...
    if ((device->tx->vswrConfig->tx1VswrSwitchGpio3p3Pin > 11) ||
        (device->tx->vswrConfig->tx2VswrSwitchGpio3p3Pin > 11))
    {
...
mykonosErr_t MYKONOS_readArmMem(mykonosDevice_t *device, uint32_t address, uint8_t *returnData, uint32_t bytesToRead, uint8_t autoIncrement)
{
    ...
    if (!(address >= MYKONOS_ADDR_ARM_START_PROG_ADDR && address <= MYKONOS_ADDR_ARM_END_PROG_ADDR ) && !(address >= MYKONOS_ADDR_ARM_START_DATA_ADDR && address <= MYKONOS_ADDR_ARM_END_DATA_ADDR))
    {
    ...
    if (!((address + bytesToRead - 1) >= MYKONOS_ADDR_ARM_START_PROG_ADDR && (address + bytesToRead - 1) <= MYKONOS_ADDR_ARM_END_PROG_ADDR ) && !((address + bytesToRead - 1) >= MYKONOS_ADDR_ARM_START_DATA_ADDR && (address + bytesToRead - 1) <= MYKONOS_ADDR_ARM_END_DATA_ADDR))
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex,  MYKONOS_ERR_READARMMEM_INV_ADDR_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_READARMMEM_INV_ADDR_PARM));
        return MYKONOS_ERR_READARMMEM_INV_ADDR_PARM;
...
mykonosErr_t MYKONOS_writeArmMem(mykonosDevice_t *device, uint32_t address, uint8_t *data, uint32_t byteCount)
{
    ...
    if (!((address + byteCount - 1) >= MYKONOS_ADDR_ARM_START_PROG_ADDR && (address + byteCount-1) <= MYKONOS_ADDR_ARM_END_PROG_ADDR ) && !((address + byteCount-1) >= MYKONOS_ADDR_ARM_START_DATA_ADDR && (address + byteCount-1) <= MYKONOS_ADDR_ARM_END_DATA_ADDR))
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_WRITEARMMEM_INV_ADDR_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_WRITEARMMEM_INV_ADDR_PARM));
        return MYKONOS_ERR_WRITEARMMEM_INV_ADDR_PARM;
...
mykonosErr_t MYKONOS_sendArmCommand(mykonosDevice_t *device, uint8_t opCode, uint8_t *extendedData, uint8_t extendedDataNumBytes)
{
    ...
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, device->spiSettings->chipSelectIndex, MYKONOS_ERR_ARMCMD_NULL_PARM,
                       getMykonosErrorMessage(MYKONOS_ERR_ARMCMD_NULL_PARM));
...
mykonosErr_t MYKONOS_loadAdcProfiles(mykonosDevice_t *device)
{
    ...
    if ((device->profilesValid & TX_PROFILE_VALID) > 0)
    {
        ...
        else
        {
            ...
            if (((device->profilesValid & TX_PROFILE_VALID) > 0) &&
                ((device->profilesValid & ORX_PROFILE_VALID) == 0))
            {
    ...
    if ((device->profilesValid & ORX_PROFILE_VALID) > 0)
    {
        ...
        if (((device->profilesValid & TX_PROFILE_VALID) == 0) &&
            ((device->profilesValid & RX_PROFILE_VALID) == 0))
        {
            retVal = MYKONOS_writeArmMem(device, MYKONOS_ADDR_ARM_START_DATA_ADDR + ARM_CONFIG_OFFSET + 96, &adcProfile[0], sizeof(adcProfile));
            if (retVal)
            {
...
static mykonosErr_t MYKONOS_calculateDigitalClocks(mykonosDevice_t *device, uint32_t *hsDigClk_kHz, uint32_t *hsDigClkDiv4or5_kHz)
{
    ...
    if (device == NULL)
    {
        CMB_writeToLog(ADIHAL_LOG_ERROR, 0, MYKONOS_ERR_CALCDIGCLK_NULLDEV_PARAM,
                       getMykonosErrorMessage(MYKONOS_ERR_CALCDIGCLK_NULLDEV_PARAM));
