...
struct adrv9002_ssi_fixed_delays {
	u8 rx_clk[ADRV9002_CHANN_MAX];
	u8 rx_data[ADRV9002_CHANN_MAX];
	u8 tx_clk[ADRV9002_CHANN_MAX];
	u8 tx_data[ADRV9002_CHANN_MAX];
	bool enabled;
};
...
struct adrv9002_rf_phy {
	...
	bool				mcs_run;
	bool				mcs_pulse_external;
	/*
	 * A sync signal signaling the FPGA to start sending the MCS pulses
	 * It only makes sense if @mcs_pulse_external is not set (IOW, the pulses
	 * are internally generated by the FPGA).
	 */
	bool				mcs_trigger_external;
	struct adrv9002_ssi_fixed_delays ssi_fixed_delays;
#ifdef CONFIG_DEBUG_FS
	struct adi_adrv9001_SsiCalibrationCfg ssi_delays;
#endif
};

static inline u8 adrv9002_ssi_fixed_clk(const struct adrv9002_rf_phy *phy, bool tx,
					int channel)
{
	return tx ? phy->ssi_fixed_delays.tx_clk[channel] :
		    phy->ssi_fixed_delays.rx_clk[channel];
}

static inline u8 adrv9002_ssi_fixed_data(const struct adrv9002_rf_phy *phy, bool tx,
					 int channel)
{
	return tx ? phy->ssi_fixed_delays.tx_data[channel] :
		    phy->ssi_fixed_delays.rx_data[channel];
}
