...
uint32_t TALISE_runInitCals(taliseDevice_t *device, uint32_t calMask)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_RUNINIT_OPCODE, &payload[0], 4);
    IF_ERR_RETURN_U32(retVal);

    device->devStateInfo.devState = (taliseStates_t)(device->devStateInfo.devState | TAL_STATE_CALS_RUN);
...
uint32_t TALISE_waitInitCals(taliseDevice_t *device, uint32_t timeoutMs, uint8_t *errorFlag)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_RUNINIT_OPCODE, &cmdStatusByte, (timeoutMs * TIMEOUT_MS_FACTOR), WAITINITCALS_INTERVAL_US);
    ...
    if (_errFlag > 0)
    {
        return (uint32_t)talApiErrHandler(device, TAL_ERRHDL_ARM_CMD_ERR,
                ARMCMD_ERRCODE(TALISE_ARM_RUNINIT_OPCODE, 0, cmdStatusByte), retVal, TALACT_ERR_RESET_ARM);
    }
    else
    {
        IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_checkInitCalComplete(taliseDevice_t *device, uint8_t *areCalsRunning, uint8_t *errorFlag)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_readArmCmdStatusByte(device, TALISE_ARM_RUNINIT_OPCODE, &cmdStatusByte);
...
uint32_t TALISE_abortInitCals(taliseDevice_t *device, uint32_t *calsCompleted)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_ABORT_OPCODE, &cmdStatusByte, ABORTINITCALS_TIMEOUT_US, ABORTINITCALS_INTERVAL_US);
    ...
    if (calsCompleted != NULL)
    {
        ...
        /* wait for command completion */
        retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, ABORTINITCALS_TIMEOUT_US, ABORTINITCALS_INTERVAL_US);
        ...
        retVal = (talRecoveryActions_t)TALISE_readArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR + talArmAddrOffset, &calCompleteBitField[0], 4, 0);
        IF_ERR_RETURN_U32(retVal);

        /* form the calsComplted word and return if requested */
        *calsCompleted = (((uint32_t)calCompleteBitField[3]) << 24) | (((uint32_t)calCompleteBitField[2]) << 16) | (((uint32_t)calCompleteBitField[1]) << 8) | (((uint32_t)calCompleteBitField[0]));
    }
...
uint32_t TALISE_getInitCalStatus(taliseDevice_t *device, uint32_t *calsSincePowerUp, uint32_t *calsLastRun, uint32_t *calsMinimum, uint8_t *initErrCal, uint8_t *initErrCode)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    uint8_t cmdStatusByte = 0;
    ...
    else
    {
        ...
        retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_GET_OPCODE, &talInitCalDoneObjectID, 1);
        IF_ERR_RETURN_U32(retVal);

        /* waiting for command completion */
        retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETINITCALSTATUS_TIMEOUT_US, GETINITCALSTATUS_INTERVAL_US);
        ...
        {
            return (uint32_t)talApiErrHandler(device, TAL_ERRHDL_ARM_CMD_ERR,
                    ARMCMD_ERRCODE(TALISE_ARM_GET_OPCODE, talInitCalDoneObjectID, cmdStatusByte), retVal, TALACT_ERR_RESET_ARM);
        }
        else
        {
            IF_ERR_RETURN_U32(retVal);
        ...
        retVal = (talRecoveryActions_t)TALISE_readArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR, &calBitField[0], byteCnt, 0);
        IF_ERR_RETURN_U32(retVal);

        /* getting the number of calibrations since power up */
        *calsSincePowerUp = (((uint32_t)calBitField[3]) << 24) | (((uint32_t)calBitField[2]) << 16) | (((uint32_t)calBitField[1]) << 8) | (((uint32_t)calBitField[0]));

        /* getting the number of calibrations since the last sequence */
        *calsLastRun = (((uint32_t)calBitField[7]) << 24) | (((uint32_t)calBitField[6]) << 16) | (((uint32_t)calBitField[5]) << 8) | (((uint32_t)calBitField[4]));

        /* getting the minimum number of initialization calibrations required */
        *calsMinimum = (((uint32_t)calBitField[11]) << 24) | (((uint32_t)calBitField[10]) << 16) | (((uint32_t)calBitField[9]) << 8) | (((uint32_t)calBitField[8]));
...
uint32_t TALISE_enableTrackingCals(taliseDevice_t *device, uint32_t enableMask)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_writeArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR, &armData[0], 4);
    IF_ERR_RETURN_U32(retVal);

    /* sending write config command */
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_WRITECFG_OPCODE, &extData[0], 4);
    IF_ERR_RETURN_U32(retVal);

    /* waiting for command to finish successfully */
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_WRITECFG_OPCODE, &cmdStatusByte, ENTRACKINGCALS_TIMEOUT_US, ENTRACKINGCALS_INTERVAL_US);
    /* SW Test */
    if (device->devStateInfo.swTest == CODECHECK_PARAM_ENABLETRACKINGCALS_ERR2)
    {
        cmdStatusByte = 3;
    ...
    {
        return (uint32_t)talApiErrHandler(device, TAL_ERRHDL_ARM_CMD_ERR,
                ARMCMD_ERRCODE(TALISE_ARM_WRITECFG_OPCODE, TALISE_ARM_OBJECTID_CAL_SCHEDULER, cmdStatusByte), retVal, TALACT_ERR_RESET_ARM);
    }
    else
    {
        IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getEnabledTrackingCals(taliseDevice_t *device, uint32_t *enableMask)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_readArmConfig(device, TALISE_ARM_OBJECTID_CAL_SCHEDULER, 0, &armData[0], 4);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getPendingTrackingCals(taliseDevice_t *device, uint32_t *pendingCalMask)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    adiHalErr_t halError = ADIHAL_OK;
...
#if TALISE_VERBOSE
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_getPendingTrackingCals()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    halError = talSpiReadByte(device->devHalInfo, TALISE_ADDR_ARM_CMD_STATUS_11, &readData[0]);
    ...
    halError = talSpiReadByte(device->devHalInfo, TALISE_ADDR_ARM_CMD_STATUS_12, &readData[1]);
    retVal = talApiErrHandler(device,TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_rescheduleTrackingCal(taliseDevice_t *device, taliseTrackingCalibrations_t trackingCal)
{
    ...
    /* sending ARM command */
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_SET_OPCODE, &extData[0], 3);
    IF_ERR_RETURN_U32(retVal);

    /* check for completion */
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_SET_OPCODE, &cmdStatusByte, RESCHEDULETRACKINGCALS_TIMEOUT_US, RESCHEDULETRACKINGCALS_INTERVAL_US);
    /* SW Test */
...
uint32_t TALISE_setAllTrackCalState(taliseDevice_t *device, uint32_t calSubsetMask, uint32_t resumeCalMask)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    uint8_t cfgData[8] = {0};
    ...
    retVal = (talRecoveryActions_t)TALISE_writeArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR, &cfgData[0], sizeof(cfgData));
    IF_ERR_RETURN_U32(retVal);

    /* sending ARM command */
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_SET_OPCODE, &extData[0], sizeof(extData));
    IF_ERR_RETURN_U32(retVal);

    /* check for completion */
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_SET_OPCODE,
                                                           &cmdStatusByte,
...
uint32_t TALISE_getAllTrackCalState(taliseDevice_t *device, uint32_t *resumeCalMask)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    /* sending ARM command */
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_GET_OPCODE, &extData[0], 1);
    IF_ERR_RETURN_U32(retVal);

    /* check for completion */
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE,
                                                           &cmdStatusByte,
                                                           PAUSERESUMETRACKINGCALS_TIMEOUT_US,
                                                           PAUSERESUMETRACKINGCALS_INTERVAL_US);
    /* SW Test */
    ...
    retVal = (talRecoveryActions_t)TALISE_readArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR, &armData[0], sizeof(armData), 0);
    IF_ERR_RETURN_U32(retVal);

    *resumeCalMask =  (uint32_t)armData[0] | ((uint32_t)armData[1] << 8) | ((uint32_t)armData[2] << 16) | ((uint32_t)armData[3] << 24);
...
uint32_t TALISE_getTxLolStatus(taliseDevice_t *device, taliseTxChannels_t channelSel, taliseTxLolStatus_t *txLolStatus)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_sendArmCommand(device, TALISE_ARM_GET_OPCODE, &extData[0], 3);
    IF_ERR_RETURN_U32(retVal);

    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETTXLOLSTATUS_TIMEOUT_US, GETTXLOLSTATUS_INTERVAL_US);
    ...
    retVal = (talRecoveryActions_t)TALISE_readArmMem(device, TALISE_ADDR_ARM_START_DATA_ADDR, &armReadBack[0], sizeof(armReadBack), 0);
    IF_ERR_RETURN_U32(retVal);

    /* Assign to data structure */
    txLolStatus->errorCode = (((uint32_t)armReadBack[3]) << 24) | (((uint32_t)armReadBack[2]) << 16) |
                             (((uint32_t)armReadBack[1]) <<  8) |  ((uint32_t)armReadBack[0]);
    txLolStatus->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                   (((uint32_t)armReadBack[5]) <<  8) |  ((uint32_t)armReadBack[4]);
    txLolStatus->varianceMetric = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                     (((uint32_t)armReadBack[ 9]) <<  8) |  ((uint32_t)armReadBack[ 8]);
    txLolStatus->iterCount = (((uint32_t)armReadBack[15]) << 24) | (((uint32_t)armReadBack[14]) << 16) |
                             (((uint32_t)armReadBack[13]) <<  8) |  ((uint32_t)armReadBack[12]);
    txLolStatus->updateCount = (((uint32_t)armReadBack[19]) << 24) | (((uint32_t)armReadBack[18]) << 16) |
...
uint32_t TALISE_getTxQecStatus(taliseDevice_t *device, taliseTxChannels_t channelSel, taliseTxQecStatus_t *txQecStatus)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETTXQECSTATUS_TIMEOUT_US, GETTXQECSTATUS_INTERVAL_US);
    ...
    /* Assign to data structure */
    txQecStatus->errorCode = (((uint32_t)armReadBack[3]) << 24) | (((uint32_t)armReadBack[2]) << 16) |
                             (((uint32_t)armReadBack[1]) <<  8) |  ((uint32_t)armReadBack[0]);
    txQecStatus->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                   (((uint32_t)armReadBack[5]) <<  8) |  ((uint32_t)armReadBack[4]);
    txQecStatus->correctionMetric = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                     (((uint32_t)armReadBack[ 9]) <<  8) |  ((uint32_t)armReadBack[ 8]);
    txQecStatus->iterCount = (((uint32_t)armReadBack[15]) << 24) | (((uint32_t)armReadBack[14]) << 16) |
                             (((uint32_t)armReadBack[13]) <<  8) |  ((uint32_t)armReadBack[12]);
    txQecStatus->updateCount = (((uint32_t)armReadBack[19]) << 24) | (((uint32_t)armReadBack[18]) << 16) |
                               (((uint32_t)armReadBack[17]) <<  8) |  ((uint32_t)armReadBack[16]);
...
uint32_t TALISE_getRxQecStatus(taliseDevice_t *device, taliseRxChannels_t channelSel, taliseRxQecStatus_t *rxQecStatus)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETRXQECSTATUS_TIMEOUT_US, GETRXQECSTATUS_INTERVAL_US);
    ...
    /* Assign to data structure */
    rxQecStatus->errorCode = (((uint32_t)armReadBack[3]) << 24) | (((uint32_t)armReadBack[2]) << 16) |
                             (((uint32_t)armReadBack[1]) <<  8) |  ((uint32_t)armReadBack[0]);
    rxQecStatus->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                   (((uint32_t)armReadBack[5]) <<  8) |  ((uint32_t)armReadBack[4]);
    rxQecStatus->selfcheckIrrDb = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                  (((uint32_t)armReadBack[ 9]) <<  8) |  ((uint32_t)armReadBack[ 8]);
    rxQecStatus->iterCount = (((uint32_t)armReadBack[15]) << 24) | (((uint32_t)armReadBack[14]) << 16) |
                             (((uint32_t)armReadBack[13]) <<  8) |  ((uint32_t)armReadBack[12]);
    rxQecStatus->updateCount = (((uint32_t)armReadBack[19]) << 24) | (((uint32_t)armReadBack[18]) << 16) |
                               (((uint32_t)armReadBack[17]) <<  8) |  ((uint32_t)armReadBack[16]);
...
uint32_t TALISE_getOrxQecStatus(taliseDevice_t *device, taliseObsRxChannels_t channelSel, taliseOrxQecStatus_t *orxQecStatus)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETORXQECSTATUS_TIMEOUT_US, GETORXQECSTATUS_INTERVAL_US);
    ...
    /* Assign to data structure */
    orxQecStatus->errorCode = (((uint32_t)armReadBack[3]) << 24) | (((uint32_t)armReadBack[2]) << 16) |
                              (((uint32_t)armReadBack[1]) <<  8) |  ((uint32_t)armReadBack[0]);
    orxQecStatus->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                    (((uint32_t)armReadBack[5]) <<  8) |  ((uint32_t)armReadBack[4]);
    orxQecStatus->selfcheckIrrDb = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                   (((uint32_t)armReadBack[ 9]) <<  8) |  ((uint32_t)armReadBack[ 8]);
    orxQecStatus->iterCount = (((uint32_t)armReadBack[15]) << 24) | (((uint32_t)armReadBack[14]) << 16) |
                              (((uint32_t)armReadBack[13]) <<  8) |  ((uint32_t)armReadBack[12]);
    orxQecStatus->updateCount = (((uint32_t)armReadBack[19]) << 24) | (((uint32_t)armReadBack[18]) << 16) |
                                (((uint32_t)armReadBack[17]) <<  8) |  ((uint32_t)armReadBack[16]);
...
uint32_t TALISE_getRxHd2Status(taliseDevice_t *device, taliseRxChannels_t channelSel, taliseRxHd2Status_t *rxHd2Status)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_GET_OPCODE, &cmdStatusByte, GETRXHD2STATUS_TIMEOUT_US, GETRXHD2STATUS_INTERVAL_US);
    ...
    /* Assign to data structure */
    rxHd2Status->errorCode = (((uint32_t)armReadBack[3]) << 24) | (((uint32_t)armReadBack[2]) << 16) |
                             (((uint32_t)armReadBack[1]) <<  8) |  ((uint32_t)armReadBack[0]);
    rxHd2Status->percentComplete = (((uint32_t)armReadBack[7]) << 24) | (((uint32_t)armReadBack[6]) << 16) |
                                   (((uint32_t)armReadBack[5]) <<  8) |  ((uint32_t)armReadBack[4]);
    rxHd2Status->confidenceLevel = (((uint32_t)armReadBack[11]) << 24) | (((uint32_t)armReadBack[10]) << 16) |
                                  (((uint32_t)armReadBack[ 9]) <<  8) |  ((uint32_t)armReadBack[ 8]);
    rxHd2Status->iterCount = (((uint32_t)armReadBack[15]) << 24) | (((uint32_t)armReadBack[14]) << 16) |
                             (((uint32_t)armReadBack[13]) <<  8) |  ((uint32_t)armReadBack[12]);
    rxHd2Status->updateCount = (((uint32_t)armReadBack[19]) << 24) | (((uint32_t)armReadBack[18]) << 16) |
                               (((uint32_t)armReadBack[17]) <<  8) |  ((uint32_t)armReadBack[16]);

    return (uint32_t)retVal;
}

uint32_t TALISE_waitForEvent(taliseDevice_t *device, taliseWaitEvent_t waitEvent, uint32_t timeout_us, uint32_t waitInterval_us)
{
...
#if TALISE_VERBOSE
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_waitForEvent()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
...
uint32_t TALISE_readEventStatus(taliseDevice_t *device, taliseWaitEvent_t waitEvent, uint8_t *eventDone)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    adiHalErr_t halError = ADIHAL_OK;
    uint16_t spiAddr = 0;
...
#if TALISE_VERBOSE
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_readEventStatus()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
...
uint32_t TALISE_resetExtTxLolChannel(taliseDevice_t *device, taliseTxChannels_t channelSel)
{
    ...
    retVal = (talRecoveryActions_t)TALISE_waitArmCmdStatus(device, TALISE_ARM_SET_OPCODE, &cmdStatusByte, GETTXLOLSTATUS_TIMEOUT_US, GETTXLOLSTATUS_INTERVAL_US);
...
uint32_t TALISE_setRxHd2Config(taliseDevice_t *device, taliseRxHd2Config_t *hd2CalConfig)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    byteOffset = 0;
    retVal = (talRecoveryActions_t)TALISE_writeArmConfig(device, TALISE_ARM_OBJECTID_RXHD2_TRACKING, byteOffset,
                                   &armFieldValue[0], 4);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getRxHd2Config(taliseDevice_t *device, taliseRxHd2Config_t *hd2CalConfig)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    byteOffset = 0;
    retVal = (talRecoveryActions_t)TALISE_readArmConfig(device, TALISE_ARM_OBJECTID_RXHD2_TRACKING,
...
uint32_t TALISE_setDigDcOffsetMShift(taliseDevice_t *device, taliseDcOffsetChannels_t channel, uint8_t mShift)
{
...
#if TALISE_VERBOSE
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_setDigDcOffsetMShift()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    halError = talSpiWriteField(device->devHalInfo, mshiftRegAddress, mShift, 0x1F, 0);
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getDigDcOffsetMShift(taliseDevice_t *device, taliseDcOffsetChannels_t channel, uint8_t *mShift)
{
...
#if TALISE_VERBOSE
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_getDigDcOffsetMShift()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    halError = talSpiReadField(device->devHalInfo, mshiftRegAddress, mShift, 0x1F, 0);
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_setDigDcOffsetEn(taliseDevice_t *device, uint8_t enableMask)
{
...
#if (TALISE_VERBOSE == 1)
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_setDigDcOffsetEn()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    REG_ADDRESS = TALISE_ADDR_DIGITAL_DC_OFFSET_CONFIG;                           /* Address for Rx Digital tracking Enable bits register */
    if (enableMask & ((uint8_t)TAL_DC_OFFSET_RX1 | (uint8_t)TAL_DC_OFFSET_RX2))   /* Enable / Disable  Rx1 and/or Rx2 */
    {
        dataToWrite |= (enableMask & ((uint8_t)TAL_DC_OFFSET_RX1 |(uint8_t) TAL_DC_OFFSET_RX2));
    }

    halError = talSpiWriteField(device->devHalInfo,  REG_ADDRESS, dataToWrite, 0x06, 1); /* Write RX enable bits to the register */
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);

    dataToWrite = 0x0;                                                             /* Reset dataToWrite */
    REG_ADDRESS = TALISE_DIGITAL_DC_OFFSET_ORX_LOOPBACK_CONFIG;                    /* Address for ORx Digital trackingEnable bits register */
    if (enableMask & ((uint8_t)TAL_DC_OFFSET_ORX1 | (uint8_t)TAL_DC_OFFSET_ORX2))  /* Check for channel  ORx and/or Sniffer */
    {
        dataToWrite |= ((enableMask & ((uint8_t)TAL_DC_OFFSET_ORX1 | (uint8_t)TAL_DC_OFFSET_ORX2)) >> 2);
    }

    halError = talSpiWriteField(device->devHalInfo,  REG_ADDRESS, dataToWrite, 0x03, 0);  /* Write Loopback, ORx1 and ORx2 enable bits to the register */
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getDigDcOffsetEn(taliseDevice_t *device,uint8_t *enableMask)
{
...
#if (TALISE_VERBOSE == 1)
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_getDigDcOffsetEn()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    REG_ADDRESS = TALISE_ADDR_DIGITAL_DC_OFFSET_CONFIG;  /* register address of Rx1 and Rx2 enable bits*/
    halError = talSpiReadField(device->devHalInfo, REG_ADDRESS, &readbackData, 0x06, 1);
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
    enableMaskData |= readbackData;  /* adjust bits to match channel :refer enum mykonosRfDcOffsettEn_t. */

    readbackData = 0x00;
    REG_ADDRESS = TALISE_DIGITAL_DC_OFFSET_ORX_LOOPBACK_CONFIG;  /* register address of Orx and sniffer enable bits*/
    halError = talSpiReadField(device->devHalInfo, REG_ADDRESS, &readbackData, 0x03, 0);
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_SPI, halError, retVal, TALACT_ERR_RESET_SPI);
    IF_ERR_RETURN_U32(retVal);
...
uint32_t TALISE_getTrackingCalsBatchSize(taliseDevice_t *device, taliseTrackingCalBatchSize_t* batchsize_us)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_getTrackingCalsBatchSize()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    retVal = (talRecoveryActions_t)TALISE_readArmConfig(device,
        ...
        16);
    IF_ERR_RETURN_U32(retVal);
    batchsizetime = (taliseTrackingCalBatchSize_t)((armFieldValue[1] << 2) + (armFieldValue[0]));
    switch (batchsizetime)
    {
        case 500: *batchsize_us = TAL_TRACK_BATCH_SIZE_500_US; break;
        case 200: *batchsize_us = TAL_TRACK_BATCH_SIZE_200_US; break;
        return (uint32_t)talApiErrHandler(device,
            TAL_ERRHDL_INVALID_PARAM,
...
uint32_t TALISE_setTrackingCalsBatchSize(taliseDevice_t *device, taliseTrackingCalBatchSize_t batchsize_us)
{
    talRecoveryActions_t retVal = TALACT_NO_ACTION;
    talRecoveryActions_t retValWarn = TALACT_NO_ACTION;
    ...
    halError = talWriteToLog(device->devHalInfo, ADIHAL_LOG_MSG, TAL_ERR_OK, "TALISE_setTrackingCalsBatchSize()\n");
    retVal = talApiErrHandler(device, TAL_ERRHDL_HAL_LOG, halError, retVal, TALACT_WARN_RESET_LOG);
#endif
    ...
    if (errSrc == TAL_ERRSRC_INITCALS)
    {
        switch (initErrCal)
        {
            ...
            case  TALISE_ARM_OBJECTID_TXBBF_INIT:
                switch (initErrCode)
                {
                    case 0x02: return "TxBBF Init Cal: Calibration Timed Out - Orx\n";
                    case 0x03: return "TxBBF Init Cal: DAC divider out of range\n";
                    case 0x04: return "TxBBF Init Cal: DAC divider out of range\n";
                    default: return  "Unknown Talise ARM TXBBF init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_ADCTUNER_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "ADC Tuner Init Cal: Calibration Timed Out\n";
                    case 0x02: return "ADC Tuner Init Cal: Invalid Channel Specified\n";
                    default: return  "Unknown Talise ARM ADC Tuner init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_TIA_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "TIA Init Cal: Error Configuring PLL - Orx\n";
                    case 0x02: return "TIA Init Cal: Error during TIA tuner calibration - Orx\n";
                    case 0x03: return "TIA Init Cal: Error configuring PLL - Rx\n";
                    case 0x04: return "TIA Init Cal: Error during TIA tuner calibration - Rx\n";
                    default: return  "Unknown Talise ARM TIA init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_DCOFFSET_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "DC Offset Init Cal: Calibration Timed Out - Rx\n";
                    case 0x02: return "DC Offset Init Cal: Calibration Timed Out - Orx\n";
                    case 0x03: return "DC Offset Init Cal: Calibration Timed Out - LBrx\n";
                    case 0x04: return "DC Offset Init Cal: Calibration Timed Out - Srx\n";
                    default: return  "Unknown Talise ARM DC Offset init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_ADCFLASH_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "ADC Flash Init Cal: Calibration Aborted\n";
                    case 0x02: return "ADC Flash Init Cal: Calibration Timed Out\n";
                    case 0x03: return "ADC Flash Init Cal: No Channel Selected\n";
                    case 0x04: return "ADC Flash Init Cal: Rx Disabled\n";
                    case 0x05: return "ADC Flash Init Cal: Orx Disabled\n";
                    default: return "Unknown Talise ARM ADC Flash init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_PATHDELAY_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "PathDelay Init Cal: Rx Disabled\n";
                    case 0x02: return "PathDelay Init Cal: Tx Disabled\n";
                    case 0x03: return "PathDelay Init Cal: Data capture timeout due to hardware setup\n";
                    case 0x04: return "PathDelay Init Cal: Data capture was aborted\n";
                    case 0x05: return "PathDelay Init Cal: NCO did not lock\n";
                    case 0x06: return "PathDelay Init Cal: Measurement error\n";
                    default: return "Unknown Talise ARM Path Delay init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_TXLOLEXT_INIT:
               switch (initErrCode)
                {
                    case 0x02: return "TxLol Init Cal: Tx Disabled\n";
                    case 0x03: return "TxLol Init Cal: Path delay invalid\n";
                    case 0x04: return "TxLol Init Cal: Initial calibration not completed\n";
                    case 0x05: return "TxLol Init Cal: Internal loopback tracking disabled\n";
                    case 0x06: return "TxLol Init Cal: Data capture timeout due to hardware setup\n";
                    case 0x07: return "TxLol Init Cal: Stitching for Orx2 not supported\n";
                    case 0x08: return "TxLol Init Cal: No Tx-Orx mapping\n";
                    default: return "Unknown Talise ARM Tx LOL init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_TXQEC_INIT:
                switch (initErrCode)
                {
                    case 0x02: return "TxQEC Init Cal: Tx Disabled\n";
                    case 0x03: return "TxQEC Init Cal: No path delay estimate present\n";
                    case 0x04: return "TxQEC Init Cal: NCO failed to lock\n";
                    default: return "Unknown Talise ARM TxQEC init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_LBLODELAY_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "LB LO Delay Init Cal: Rx Disabled\n";
                    case 0x02: return "LB LO Delay Init Cal: Tx Disabled\n";
                    case 0x03: return "LB LO Delay Init Cal: Error configuring Cal PLL\n";
                    case 0x04: return "LB LO Delay Init Cal: TCAL settling time\n";
                    case 0x05: return "LB LO Delay Init Cal: TCAL data capture error\n";
                    case 0x06: return "LB LO Delay Init Cal: Wrong frequency Bin\n";
                    default: return "Unknown Talise ARM LB LO Delay init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_LBRXQEC_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "LB RxQEC Init Cal: Rx Disabled\n";
                    case 0x02: return "LB RxQEC Init Cal: Tx Disabled\n";
                    case 0x03: return "LB RxQEC Init Cal: Error configuring Cal PLL\n";
                    case 0x04: return "LB RxQEC Init Cal: TCAL settling time\n";
                    case 0x05: return "LB RxQEC Init Cal: QFIR overflow\n";
                    case 0x06: return "LB RxQEC Init Cal: CFIR overflow\n";
                    case 0x07: return "LB RxQEC Init Cal: TCAL data capture error\n";
                    default: return "Unknown Talise ARM LB RxQEC init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_RXQEC_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "RxQEC Init Cal: Rx Disabled\n";
                    case 0x02: return "RxQEC Init Cal: Tx Disabled\n";
                    case 0x03: return "RxQEC Init Cal: Error configuring Cal PLL\n";
                    case 0x04: return "RxQEC Init Cal: TCAL settling time\n";
                    case 0x05: return "RxQEC Init Cal: QFIR overflow\n";
                    case 0x06: return "RxQEC Init Cal: CFIR overflow\n";
                    case 0x07: return "RxQEC Init Cal: TCAL data capture error\n";
                    default: return  "Unknown Talise ARM RxQEC init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_ORXQEC_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "ORxQEC Init Cal: Rx Disabled\n";
                    case 0x02: return "ORxQEC Init Cal: Tx Disabled\n";
                    case 0x03: return "ORxQEC Init Cal: Error configuring Cal PLL\n";
                    case 0x04: return "ORxQEC Init Cal: TCAL settling time\n";
                    case 0x05: return "ORxQEC Init Cal: QFIR overflow\n";
                    case 0x06: return "ORxQEC Init Cal: CFIR overflow\n";
                    case 0x07: return "ORxQEC Init Cal: TCAL data capture error\n";
                    default: return "Unknown Talise ARM ORx QEC init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_TXDAC_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "TxDAC Init Cal: Calibration time out\n";
                    case 0x02: return "TxDAC Init Cal: Tx Disabled\n";
                    default: return "Unknown Talise ARM TxDAC init cal error\n";
                }
                break;
            ...
            case  TALISE_ARM_OBJECTID_ADCSTITCHING_INIT:
                switch (initErrCode)
                {
                    case 0x01: return "ADC Stitching Init Cal: Excess level supported by hardware\n";
                    default: return "Unknown Talise ARM ADC stitching init cal error\n";
                }
                break;
            ...
            default: return "Unknown Init Calibration Error\n";
        }
    }
