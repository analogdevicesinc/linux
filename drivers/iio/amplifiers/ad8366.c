...
struct ad8366_info {
	...
	int gain_step;
	int num_channels;
	ssize_t (*pack_code)(struct ad8366_data *data, u8 *buf, size_t len);
};
...
static int ad8366_write_code(struct ad8366_state *st)
{
	...
	len = inf->pack_code(&st->data, st->buf, sizeof(st->buf));
	if (len < 0)
		return len;

	return spi_write(st->spi, st->buf, len);
}

static ssize_t ad8366_pack_code(struct ad8366_data *data, u8 *buf, size_t len)
{
	u8 ch_a = bitrev8(data->code[0] & 0x3F);
	u8 ch_b = bitrev8(data->code[1] & 0x3F);
...
static ssize_t simple_pack_code(struct ad8366_data *data, u8 *buf, size_t len)
{
	if (len < 1)
		return -EINVAL;
...
static ssize_t adrf5731_pack_code(struct ad8366_data *data, u8 *buf, size_t len)
{
	if (len < 1)
		return -EINVAL;
...
static ssize_t hmc271_pack_code(struct ad8366_data *data, u8 *buf, size_t len)
{
	if (len < 1)
		return -EINVAL;

	buf[0] = bitrev8(data->code[0] & 0x1F) >> 3;
	return 1;
}
			   ...
			   long mask)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		gain += st->data.code[chan->channel] * inf->gain_step;
		*val = gain / 1000;
		*val2 = (gain % 1000) * 1000;
			    ...
			    long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_HARDWAREGAIN:
		st->data.code[chan->channel] = code;
