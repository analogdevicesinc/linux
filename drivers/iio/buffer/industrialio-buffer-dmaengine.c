	...
	struct iio_dma_buffer_block *block)
{
	...
	if (block->sg_table) {
		...
		addrs = kmalloc(sizeof(*addrs) * nents, GFP_KERNEL);
		if (!addrs)
			return -ENOMEM;

		lenghts = kmalloc(sizeof(*lenghts) * nents, GFP_KERNEL);
		if (!lenghts) {
			kfree(addrs);
			return -ENOMEM;
		}
...
{
	return devm_iio_dmaengine_buffer_setup_with_ops(dev, indio_dev, channel, dir,
