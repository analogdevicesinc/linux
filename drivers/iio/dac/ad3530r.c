					 ...
					 const char *buf, size_t len)
{
	...
	guard(mutex)(&st->lock);
	switch (chan->channel) {
	case AD3530R_CHAN(0) ... AD3530R_CHAN(AD3531R_MAX_CHANNELS - 1):
		...
		return len;
	case AD3530R_CHAN(AD3531R_MAX_CHANNELS) ...
	     AD3530R_CHAN(AD3530R_MAX_CHANNELS - 1):
		ret = regmap_update_bits(st->regmap, AD3530R_OUTPUT_OPERATING_MODE_1,
...
static int ad3530r_setup(struct ad3530r_state *st)
{
	...
	st->channels = devm_kcalloc(dev, chip_info->num_channels,
