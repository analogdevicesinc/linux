...
static const struct iio_chan_spec ad5413_voltage_ch[] = { AD5413_DAC_CHAN(
	IIO_VOLTAGE) };
...
static int ad5413_init(struct ad5413_state *st)
{
	 ...
	 * In DT, reset-gpios is defined as active low,
	 * so default to HIGH = released (normal operation).
	 */
	st->gpio_reset =
	...
	if (st->gpio_reset) {
		...
		fsleep(100);
		gpiod_set_value(st->gpio_reset, 1); /* release reset */
	} else {
