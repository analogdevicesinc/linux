...
struct ad5706r_state {
	...
	u64 debug_spi_speed_hz_write;
	u64 debug_spi_speed_hz_read;
...
static int _ad5706r_set_clk_rate(struct ad5706r_state *st, u64 rate, u8 wr)
{
	int ret, current_rate;

	if (rate < SPI_MIN_SPEED_HZ)
		rate = SPI_MIN_SPEED_HZ;
	if (rate > SPI_MAX_SPEED_HZ)
		rate = SPI_MAX_SPEED_HZ;
