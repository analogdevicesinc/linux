...
static void ad5754_reset_dac(void *state)
{
...
static int ad5754_probe(struct spi_device *spi)
{
	...
	ret = devm_add_action_or_reset(dev, ad5754_reset_dac, st);
	if (ret)
		return ret;
...
static void ad5754_remove(struct spi_device *spi)
{
	struct ad5754_state *st = spi_get_drvdata(spi);
