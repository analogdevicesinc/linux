									...
									char *buf)
{
	...
	dev_info(&st->spi->dev, "ADMX: READ_FREQUENCY2 = %llu uHz -> %d.%06d Hz\n",
								...
								const char *buf, size_t len)
{
	...
	dev_info(&st->spi->dev,
	"ADMX: WRITE_AMPLITUDE2 - converted to %u uVrms (max: %u)\n",
	amplitude, ADMX_AMPLITUDE_MAX_UV);

	if (amplitude > ADMX_AMPLITUDE_MAX_UV) {
	dev_err(&st->spi->dev,
	"ADMX: WRITE_AMPLITUDE2 - INVALID range!\n");
	return -EINVAL;
	}
	...
	dev_info(&st->spi->dev,
	"ADMX: WRITE_AMPLITUDE2 - writing 0x%08x to reg 0x%03x\n",
	amplitude, ADMX_AMPLITUDE_SECONDARY);

	ret = regmap_write(st->regmap, ADMX_AMPLITUDE_SECONDARY, amplitude);
	if (ret) {
	dev_err(&st->spi->dev,
	"ADMX: WRITE_AMPLITUDE2 - failed to write register (ret=%d)\n",
	ret);
	goto out_unlock;
	}
