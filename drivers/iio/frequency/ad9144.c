		...
		struct ad9144_platform_data *pdata)
{
	...
	if (st->id == CHIPID_AD9144) {
		...
		regmap_write(map, 0x0e8, 0x01);	// read dac-0
		regmap_read(map, 0x0e9, &val);
		if ((val & 0xc0) != 0x80)
			dev_err(dev, "dac-0 calibration failed!!\n");

		regmap_write(map, 0x0e8, 0x02);	// read dac-1
		regmap_read(map, 0x0e9, &val);
		if ((val & 0xc0) != 0x80)
			dev_err(dev, "dac-1 calibration failed!!\n");
	...
	regmap_write(map, REG_PWRCNTRL0, PD_DAC_2 | PD_DAC_3);	// 0x11 dacs - power up everything
	regmap_write(map, REG_CLKCFG0, PD_CLK23);	// 0x80 clocks - power up everything
	regmap_write(map, REG_SYSREF_ACTRL0, 0x00);	// 0x81 sysref - power up/falling edge
	regmap_write(map, 0x2a7, 0x01);	// input termination calibration
	regmap_write(map, 0x2ae, 0x01);	// input termination calibration
	regmap_write(map, 0x2a4, 0x4b);	// clock termination (0x77 ?)
	regmap_write(map, 0x112, 0x00);	// interpolation (bypass)

	regmap_write(map, 0x200, 0x00);	// phy - power up

	for (i = 0; i < 2; i++) {
		regmap_write(map, 0x300, (i ? SEL_REG_MAP_1 : 0) | LINK_MODE);	// dual link - disabled
		regmap_write(map, 0x452, 0x04);	// lane-0, link id
		regmap_write(map, 0x453, 0x83);	// descrambling, 4 lanes
		regmap_write(map, 0x454, 0x00);	// octects per frame per lane (1)
		regmap_write(map, 0x476, 0x01);	// frame - bytecount (1)
		regmap_write(map, 0x455, 0x1f);	// mult-frame - framecount (32)
		...
		regmap_write(map, 0x45a, 0x80);	// HD mode, no CS bits
		regmap_write(map, 0x45d, 0x49);	// check-sum of 0x450 to 0x45c
		regmap_write(map, 0x478, 0x01);	// ilas mf count (4)
		regmap_write(map, 0x46c, 0x0f);	// enable deskew for all lanes
		regmap_write(map, 0x47d, 0x0f);	// enable all lanes
	}
	...
	regmap_write(map, 0x206, 0x00);	// cdr reset
	regmap_write(map, 0x206, 0x01);	// cdr reset
	regmap_write(map, 0x289, 0x00);	// data-rate == 10Gbps
	regmap_write(map, REG_SYNTH_ENABLE_CNTRL, SPI_ENABLE_SYNTH);	// enable serdes pll
	regmap_write(map, REG_SYNTH_ENABLE_CNTRL, SPI_ENABLE_SYNTH | SPI_RECAL_SYNTH);	// enable serdes calibration
	...
	regmap_write(map, 0x03a, 0x00);	// sync-disable
	regmap_write(map, 0x03a, 0x01);	// sync-oneshot mode
	regmap_write(map, 0x03a, 0x81);	// sync-enable
	regmap_write(map, 0x03a, 0xc1);	// sysref-armed
	regmap_write(map, 0x306, 0x0a);	// receive buffer delay
	...
	regmap_write(map, 0x304, 0x00);	// lmfc delay
	regmap_write(map, 0x305, 0x00);	// lmfc delay
	regmap_write(map, 0x0f7, 0x18);	// data path enable
	regmap_write(map, 0x033, 0x80);	// sync enable
	regmap_write(map, 0x034, 0x01);	// oneshot sysref
	regmap_write(map, 0x300, 0x03);	// enable link
#endif
