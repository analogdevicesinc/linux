...
/*
 * AD916x SPI DAC driver for AXI DDS PCORE/COREFPGA Module
 *
...
static int ad9162_setup(struct ad9162_state *st)
{
	 ...
	 */
	if (!device_property_read_u32(dev, "adi,full-scale-current-mircoamp",
				      &fsc))
...
int ad9162_reg_access(struct iio_dev *indio_dev, unsigned int reg,
...
static const struct of_device_id ad916x_dt_id[] = {
	{ .compatible = "adi,ad9162", .data = &ad916x_info[AD9162] },
	{ .compatible = "adi,ad9166", .data = &ad916x_info[AD9166] },
	{},
};
MODULE_DEVICE_TABLE(of, ad916x_dt_id);
#else
#define ad916x_dt_id	NULL
#endif
...
static int ad916x_standalone_probe(struct ad9162_state *st)
{
	...
	match = of_match_node(ad916x_dt_id, np);
	if (match && match->data) {
		st->ad916x_info = (struct ad916x_chip_info *)match->data;
	} else {
...
static int ad9162_probe(struct spi_device *spi)
{
	...
	if (device_property_read_bool(&spi->dev, "adi,standalone-pobe"))
