...
static int ad9739a_read(struct device *dev, unsigned reg)
{
...
static int ad9739a_write(struct device *dev, unsigned reg, unsigned val)
{
	struct spi_device *spi = to_spi_device(dev);
	unsigned char buf[3];
	int ret;
...
static int ad9739a_setup(struct cf_axi_converter *conv)
{
	...
	ad9739a_write(&spi->dev, REG_MODE, 0x00);

	/* Software reset to default SPI values. */
	ad9739a_write(&spi->dev, REG_MODE, MODE_RESET);

	/* Clear the reset bit. */
	ad9739a_write(&spi->dev, REG_MODE, 0x00);

	/* Set the common-mode voltage of DACCLK_P and DACCLK_N inputs */
	ad9739a_write(&spi->dev, REG_CROSS_CNT1, CROSS_CNT1_CLKP_OFFSET(0xF));
	ad9739a_write(&spi->dev, REG_CROSS_CNT2, CROSS_CNT2_CLKN_OFFSET(0xF));

	/* Configure the Mu controller. */
	ad9739a_write(&spi->dev, REG_PHS_DET,
		      PHS_DET_CMP_BST | PHS_DET_PHS_DET_AUTO_EN);
	ad9739a_write(&spi->dev, REG_MU_DUTY, MU_DUTY_MU_DUTY_AUTO_EN);
	ad9739a_write(&spi->dev, REG_MU_CNT2,
		      MU_CNT2_SRCH_MODE(2) | MU_CNT2_SET_PHS(4));
	ad9739a_write(&spi->dev, REG_MU_CNT3, MU_CNT3_MUDEL(0x6C));

	for (repeat = 0; repeat < 3; repeat++) {
		ad9739a_write(&spi->dev, REG_MU_CNT4,
				MU_CNT4_SEARCH_TOL | MU_CNT4_RETRY | MU_CNT4_GUARD(0xB));
		ad9739a_write(&spi->dev, REG_MU_CNT1, MU_CNT1_GAIN(1));
		/* Enable the Mu controller search and track mode. */
		ad9739a_write(&spi->dev, REG_MU_CNT1,
			      MU_CNT1_GAIN(1) | MU_CNT1_ENABLE);
		mdelay(10);
		status = ad9739a_read(&spi->dev, REG_MU_STAT1);
...
static int ad9739a_set_fsc(struct cf_axi_converter *conv, u16 fsc_ua)
{
	...
	ret = ad9739a_write(&spi->dev, REG_FSC_1, FSC_1_FSC_1(reg_val));
	ret |= ad9739a_write(&spi->dev, REG_FSC_2,
			     FSC_2_FSC_2((reg_val >> 8)));
...
static int ad9739a_set_op_mode(struct cf_axi_converter *conv, enum operation_mode op_mode)
{
	...
	if (op_mode == NORMAL_BASEBAND_OPERATION) {
		ret = ad9739a_write(&spi->dev, REG_DEC_CNT,
				    DEC_CNT_DAC_DEC(NORMAL_BASEBAND));
		phy->pdata->mix_mode_en = false;
	} else {
		ret = ad9739a_write(&spi->dev, REG_DEC_CNT,
				    DEC_CNT_DAC_DEC(MIX_MODE));
...
static int ad9739a_probe(struct spi_device *spi)
{
	...
	id = ad9739a_read(&spi->dev, REG_PART_ID);
	if (id != AD9739A_ID) {
		ret = -ENODEV;
