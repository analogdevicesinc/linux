...
#define AD9910_PLL_IN_MAX_FREQ_HZ	(60UL * HZ_PER_MHZ)
...
struct ad9910_state {
	...
	u8 instruction __aligned(IIO_DMA_MINALIGN);
};
				 ...
				 void *priv)
{
	...
	return devm_add_action_or_reset(dev, ad9910_backend_unregister,
					entry);
}
 ...
 *
 * Due to the lack of support under
...
static int ad9910_spi_read(struct ad9910_state *st, u8 reg, void *data, size_t len)
{
	struct spi_transfer t[] = {
		{ .tx_buf = &st->instruction, .len = 1, },
		{ .rx_buf = data, .len = len, },
	};

	st->instruction = AD9910_SPI_READ | (reg & AD9910_SPI_ADDR_MASK);
			    ...
			    size_t len, bool update)
{
	...
	st->instruction = reg & AD9910_SPI_ADDR_MASK;
...
{									\
	int ret;							\
									\
	ret = ad9910_spi_read(st, reg, &st->buf, sizeof(*data));	\
	if (ret)							\
		return ret;						\
									\
	*data = be##nb##_to_cpu(st->buf.be##nb);			\
	return ret;							\
}
...
{									\
	int ret;							\
									\
	st->buf.be##nb = cpu_to_be##nb(data);				\
	ret = ad9910_spi_write(st, reg, &st->buf, sizeof(data), update);\
	if (ret)							\
		return ret;						\
									\
...
static int ad9910_io_reset(struct ad9910_state *st)
{
	if (st->back)
		return ad9910_backend_op_call(st->back, io_reset);

	if (st->gpio_io_reset) {
		gpiod_set_value_cansleep(st->gpio_io_reset, 1);
		udelay(1);
...
static int ad9910_drg_control_mode_set(struct ad9910_state *st,
				       enum ad9910_drg_control_mode mode)
{
	if (st->back)
		return ad9910_backend_op_call(st->back, drg_control_mode_set, mode);

	if (mode > AD9910_DRG_CTL_MODE_OFF)
		return -EINVAL;

	return gpiod_set_value_cansleep(st->gpio_drctl, mode);
}

static int ad9910_drg_control_mode_get(struct ad9910_state *st,
				       enum ad9910_drg_control_mode *mode)
{
	int ret;

	if (st->back)
		return ad9910_backend_op_call(st->back, drg_control_mode_get, mode);

	ret = gpiod_get_value_cansleep(st->gpio_drctl);
	if (ret < 0)
		return ret;
	*mode = ret;
...
static int ad9910_reset(struct ad9910_state *st, bool enable)
{
	if (st->back)
		return ad9910_backend_op_call(st->back, reset, enable);

	return gpiod_set_value_cansleep(st->gpio_reset, enable);
}
...
{
	struct ad9910_state *st = iio_priv(indio_dev);
	enum ad9910_drg_control_mode ctl_mode;
	int ret, val;
	...
	case AD9910_POWERDOWN:
		val = !!FIELD_GET(AD9910_CFR1_SOFT_POWER_DOWN_MSK,
				  st->reg[AD9910_REG_CFR1].val32);
		break;
	case AD9910_DRG_CTL_MODE:
		ret = ad9910_drg_control_mode_get(st, &ctl_mode);
		if (ret)
			return ret;
		val = ctl_mode;
		break;
				     ...
				     const char *buf, size_t len)
{
	...
	case AD9910_POWERDOWN:
		...
		break;
	case AD9910_DRG_CTL_MODE:
		if (val32 > AD9910_DRG_CTL_MODE_OVERFLOW_TOGGLE)
			return -EINVAL;
		ret = ad9910_drg_control_mode_set(st, val32);
		break;
	case AD9910_PP_HOLD_LAST_EN:
...
static const struct iio_chan_spec_ext_info ad9910_drg_ext_info[] = {
	...
	AD9910_EXT_INFO("control_mode", AD9910_DRG_CTL_MODE, IIO_SEPARATE),
...
static int ad9910_setup(struct ad9910_state *st)
{
	...
	ret = ad9910_reset(st, false);
	if (ret)
		return ret;

	ret = ad9910_io_reset(st);
	if (ret)
		return ret;
				  ...
				  struct iio_backend *iio_back)
{
	...
	int ret, new_size, ch_idx;

	new_size = ARRAY_SIZE(ad9910_channels) + 2; /* adding 2 channels */
	channels = devm_kzalloc(dev, sizeof(*channels) * new_size, GFP_KERNEL);
	if (!channels)
		return -ENOMEM;
	...
	ch_idx = new_size - 2;
	channels[ch_idx].type = IIO_ALTVOLTAGE;
	channels[ch_idx].indexed = 1;
	channels[ch_idx].output = 1;
	channels[ch_idx].scan_index = -1;
	channels[ch_idx].channel = AD9910_CHANNEL_PARALLEL_PORT;
	ret = iio_backend_extend_chan_spec(iio_back, &channels[ch_idx]);
	if (ret)
		return ret;
	...
	channels[ch_idx].output = 1;
	channels[ch_idx].scan_index = -1;
	channels[ch_idx].channel = AD9910_CHANNEL_DRG;
	ret = iio_backend_extend_chan_spec(iio_back, &channels[ch_idx]);
	if (ret)
		return ret;

	indio_dev->channels = channels;
	indio_dev->num_channels = new_size;
...
static int ad9910_probe(struct spi_device *spi)
{
	...
	if (st->back) {
		...
		st->gpio_reset = devm_gpiod_get_index_optional(dev, "reset", 0,
							       GPIOD_OUT_HIGH);
		if (IS_ERR(st->gpio_reset))
			return dev_err_probe(dev, PTR_ERR(st->gpio_reset),
					     "failed to get master reset gpio\n");

		st->gpio_io_reset = devm_gpiod_get_index_optional(dev, "reset", 1,
								  GPIOD_OUT_HIGH);
		if (IS_ERR(st->gpio_io_reset))
			return dev_err_probe(dev, PTR_ERR(st->gpio_io_reset),
		...
		st->gpio_drctl = devm_gpiod_get_optional(dev, "drg",
							 GPIOD_OUT_LOW);
		if (IS_ERR(st->gpio_drctl))
			return dev_err_probe(dev, PTR_ERR(st->gpio_drctl),
					     "failed to get drg control gpio\n");
	}
