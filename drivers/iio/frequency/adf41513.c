...
static int adf41513_calc_pfd_frequency(struct adf41513_state *st,
				       ...
				       u64 fpfd_limit_uhz)
{
	/* set initial values from fw data */
	...
	/* set R counter starting with the div factor from fw data */
...
static int adf41513_parse_fw(struct adf41513_state *st)
{
	...
	ret = device_property_read_u32(dev, "adi,charge-pump-resistor-ohms", &tmp);
	if (!ret) {
		if (tmp >= ADF41513_MIN_R_SET && tmp <= ADF41513_MAX_R_SET)
			cp_resistance = tmp;
		else
...
static int adf41513_setup(struct adf41513_state *st)
{
	...
	/* assert load enable sync for later frequency changes */
	st->regs[ADF41513_REG12] |= ADF41513_REG12_LE_SELECT_MSK;
	return adf41513_sync_config(st, ADF41513_SYNC_DIFF);
}
