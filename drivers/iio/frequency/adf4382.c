...
/* ADF4382 REG002C Map */
#define ADF4382_LDWIN_PW_MSK			GENMASK(7, 5)
#define ADF4382_LD_COUNT_OPWR_MSK		GENMASK(4, 0)
...
/* ADF4382 REG0058 Map */
#define ADF4382_ADC_BUSY			BIT(2)

#define ADF4382_REF_MIN				10000000ULL	// 10MHz
#define ADF4382_REF_MAX				4500000000ULL	// 4.5GHz
...
static const struct reg_sequence adf4382_u2_reg_default[] = {
	{0x000A, 0x0A},
	{0x0200, 0x00},
	{0x0201, 0x00},
	{0x0202, 0x00},
...
static const struct reg_sequence adf4382_u4_reg_default[] = {
	{0x000A, 0xA5},
	{0x0200, 0x00},
	{0x0201, 0x00},
	{0x0202, 0x00},
	...
	{0x0107, 0x3F},
	{0x0108, 0x3F},
	// {0x0109, 0x25}, // workaournd for AD9084-EVAL
	{0x010A, 0x25},
	{0x010B, 0x3F},
	{0x010C, 0x3F},
	{0x010D, 0x3F},
	...
	{0x004F, 0x08},
	{0x004E, 0x06},
	{0x004D, 0x04}, // workaournd for AD9084-EVAL
	{0x004C, 0x2B},
	{0x004B, 0x5D},
	{0x004A, 0x00},
	{0x0048, 0x00},
	...
	{0x0037, 0x82},
	{0x0036, 0xC0},
	{0x0035, 0x25}, // workaournd for AD9084-EVAL
	{0x0034, 0x36},
	{0x0033, 0x00},
	{0x0032, 0x40},
	{0x0031, 0x6B},
...
static const struct reg_sequence adf4382_u5_reg_default[] = {
	{ 0x00A, 0xA5 },
	{ 0x200, 0x00 },
	{ 0x201, 0x00 },
	{ 0x202, 0x00 },
	...
	{ 0x107, 0x3F },
	{ 0x108, 0x3F },
	// { 0x109, 0x25 }, // workaournd for AD9084-EVAL
	{ 0x10A, 0x25 },
	{ 0x10B, 0x3F },
	{ 0x10C, 0x3F },
	{ 0x10D, 0x3F },
	...
	{ 0x04F, 0x08 },
	{ 0x04E, 0x06 },
	{ 0x04D, 0x04 }, // workaournd for AD9084-EVAL
	{ 0x04C, 0x2B },
	{ 0x04B, 0x5D },
	{ 0x04A, 0x00 },
	{ 0x048, 0x00 },
	...
	{ 0x037, 0xCA },
	{ 0x036, 0xC0 },
	{ 0x035, 0x25 }, // workaournd for AD9084-EVAL
	{ 0x034, 0x36 },
	{ 0x033, 0x00 },
	{ 0x032, 0x40 },
	{ 0x031, 0x63 },
			    ...
			    long mask)
{
	...
	case IIO_CHAN_INFO_PROCESSED:
		...
		ret = regmap_update_bits(st->regmap, 0x31, ADF4382_EN_ADC_CLK_MSK, ADF4382_EN_ADC_CLK_MSK);
		if (ret)
			return ret;

		ret = regmap_write(st->regmap, 0x54, ADF4382_ADC_ST_CNV_MSK);
		if (ret)
			return ret;
...
static int adf4382_set_defaults(struct adf4382_state *st)
{
	unsigned int val;
	int ret;
	
	ret = regmap_read(st->regmap, 0x67, &val);
	if (ret)
		return ret;

	switch (val) {
	case ADF4382_CHIP_VER_U2:
		return regmap_multi_reg_write(st->regmap, adf4382_u2_reg_default,
				     	      ARRAY_SIZE(adf4382_u2_reg_default));
	case ADF4382_CHIP_VER_U4:
		return regmap_multi_reg_write(st->regmap, adf4382_u4_reg_default,
				     	      ARRAY_SIZE(adf4382_u4_reg_default));
	...
	case ADF4382_CHIP_VER_U5_D:
		return regmap_multi_reg_write(st->regmap, adf4382_u5_reg_default,
				     	      ARRAY_SIZE(adf4382_u5_reg_default));
	default:
		dev_err(&st->spi->dev, "Unknown chip version.\n");
		return -EINVAL;
	}
}
