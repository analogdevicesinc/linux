...
#define ADL5960_OPERATING_MODE(x)	FIELD_PREP(ADL5960_OPERATING_MODE_MSK, x)
#define ADL5960_POWER_MODE_MSK		GENMASK(1, 0)
#define ADL5960_POWER_MODE(x)		FIELD_PREP(ADL5960_POWER_MODE_MSK, x)

/* ADL5960_REG_LO_CONFIG */
#define ADL5960_BYPASS_MSK		BIT(4)
#define ADL5960_BYPASS(x)		FIELD_PREP(ADL5960_BYPASS_MSK, x)
#define ADL5960_IFMODE_MSK		GENMASK(3, 2)
#define ADL5960_IFMODE(x)		FIELD_PREP(ADL5960_IFMODE_MSK, x)
#define ADL5960_LOMODE_MSK		GENMASK(1, 0)
#define ADL5960_LOMODE(x)		FIELD_PREP(ADL5960_LOMODE_MSK, x)

/* ADL5960_REG_CT2 */
#define ADL5960_CT2_MSK			GENMASK(4, 0)
#define ADL5960_CT2(x)			FIELD_PREP(ADL5960_CT2_MSK, x)

/* ADL5960_REG_CT4 */
#define ADL5960_CT4_MSK			GENMASK(4, 0)
#define ADL5960_CT4(x)			FIELD_PREP(ADL5960_CT4_MSK, x)

/* ADL5960_REG_IGAIN */
#define ADL5960_IGAIN_MSK		GENMASK(6, 0)
#define ADL5960_IGAIN(x)		FIELD_PREP(ADL5960_IGAIN_MSK, x)

/* ADL5960_REG_RGAIN */
#define ADL5960_RGAIN_MSK		GENMASK(6, 0)
#define ADL5960_RGAIN(x)		FIELD_PREP(ADL5960_RGAIN_MSK, x)

/* ADL5960_REG_CIF2_CIF1 */
#define ADL5960_CIF2_MSK		GENMASK(7, 4)
#define ADL5960_CIF2(x)			FIELD_PREP(ADL5960_CIF2_MSK, x)
#define ADL5960_CIF1_MSK		GENMASK(3, 0)
#define ADL5960_CIF1(x)			FIELD_PREP(ADL5960_CIF1_MSK, x)

/* ADL5960_REG_TDEG */
#define ADL5960_TDEG_MSK		GENMASK(4, 0)
#define ADL5960_TDEG(x)			FIELD_PREP(ADL5960_TDEG_MSK, x)
...
static unsigned int adl5960_update_cif_filters(u32 freq)
{
	...
	return ADL5960_CIF1(c1) | ADL5960_CIF2(c2);
}
				   ...
				   unsigned int mode)
{
	...
	case 0:
		...
		ret = regmap_update_bits(dev->regmap,
				...
				ADL5960_BYPASS(bypass) | ADL5960_LOMODE(val));
		break;
	case 1:
		switch (mode) {
		case 3:
		...
		ret = regmap_update_bits(dev->regmap,
				...
				ADL5960_BYPASS(bypass) | ADL5960_IFMODE(mode));
		break;
	default:
		ret = -EINVAL;
...
#define ADL5960_CHAN_OUT(_channel, _extend_name) {			\
	...
	.info_mask_shared_by_type = BIT(IIO_CHAN_INFO_LOW_PASS_FILTER_3DB_FREQUENCY), \
}
...
static int adl5960_dt_parse(struct adl5960_dev *dev)
{
	...
	dev->reg = devm_regulator_get(&spi->dev, "vcc");
	if (IS_ERR(dev->reg))
