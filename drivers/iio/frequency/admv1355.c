				       ...
				       char *buf, loff_t off, size_t count)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(kobj_to_dev(kobj));
	struct admv1355_priv *priv = iio_priv(indio_dev);
	unsigned int idx, entry_base, r0, r1;
	char *line, *ptr = buf;
	int ret;
	...
	while ((line = strsep(&ptr, "\n"))) {
		...
		ret = sscanf(line, "GLUT_%u %x %x", &idx, &r0, &r1);
		if (ret != 3)
			return -EINVAL;
		...
		ret = regmap_write(priv->regmap, entry_base, r0 & 0xFF);
		if (ret)
			return ret;

		ret = regmap_write(priv->regmap, entry_base + 1, r1 & 0x1F);
		if (ret)
			return ret;
	}
