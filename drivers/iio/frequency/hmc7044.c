			    ...
			    long mask)
{
	...
	case IIO_CHAN_INFO_PHASE:
		hmc7044_read(indio_dev, HMC7044_REG_CH_OUT_CRTL_3(ch->num),
			      &tmp);
		*val2 = (tmp & 0x1F) * 100000;
			     ...
			     long mask)
{
	...
	case IIO_CHAN_INFO_FREQUENCY:
		...
		break;
	case IIO_CHAN_INFO_PHASE:
		/* FIXME: PHASE is an angle, this uses the notation of
		 * coarse.fine delay
		 * Example coarse delay 3, fine delay 2: 3.2
		 */
		mutex_lock(&hmc->lock);
		hmc7044_write(indio_dev, HMC7044_REG_CH_OUT_CRTL_3(ch->num),
			      (val2 / 100000) & 0x1F);
