				      ...
				      const struct iio_chan_spec *chan, char *buf)
{
	...
	case FREQUENCY:
		...
		return sysfs_emit(buf, "%lu\n",
				  dev_clk / ((regval & AION_DEBUG_BSYNC_RATIO_MASK) >> 6) * 2);
...
static int axi_aion_debug_trig_set(void *arg, const u64 val)
{
	...
	return regmap_clear_bits(st->regmap, AION_CONTROL_REG, AION_CONTROL_DEBUG_TRIG);
}
...
static int axi_aion_jesd204_opt_setup_stage1(struct jesd204_dev *jdev,
					     enum jesd204_state_op_reason reason)
{
	u32 debug_msk = AION_DEBUG_BSYNC_READY_MASK | AION_DEBUG_BSYNC_CAPTURED_MASK;
	struct device *dev = jesd204_dev_to_device(jdev);
	...
	ret = regmap_write(st->regmap, AION_CONTROL_REG,
			   AION_CONTROL_SW_RESET | AION_CONTROL_BSYNC_DIRECTION_MASK);
	if (ret) {
	...
	ret = regmap_read_poll_timeout(st->regmap, AION_DEBUG_REG,
				       regval, (regval & debug_msk) == debug_msk, 100, 10000);
	if (ret) {
	...
	for (i = 0; i < ARRAY_SIZE(st->trigX_phase); i++) {
		ret = regmap_write(st->regmap, AION_TRIG_CH0_PHASE_REG + (i * 4),
				   st->trigX_phase[i]);
		if (ret)
			return ret;
	}
...
out:
	/*
	 * At this point we try put the HW in a sane way but there's no point in
	 * checking for errors.
	 */
...
static int axi_aion_iio_probe(struct platform_device *pdev)
{
	...
	st->iio_adf4030 = devm_fwnode_iio_channel_get_by_name(&pdev->dev,
							      dev_fwnode(&pdev->dev), "bsync");

	if (IS_ERR(st->iio_adf4030) && PTR_ERR(st->iio_adf4030) != -ENOENT)
		return dev_err_probe(&pdev->dev, PTR_ERR(st->iio_adf4030),
				     "bsync: error getting channel\n");
