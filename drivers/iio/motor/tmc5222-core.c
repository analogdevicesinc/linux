...
#define tmc5222_warn_zero(dev, name, val) \
	do { \
		if ((val) == 0) \
			dev_warn(dev, #name " is 0, motor may not move properly\n"); \
	} while (0)
...
struct tmc5222_priv {
	...
	/* Protects register access sequences */
	struct mutex lock;
...
static int tmc5222_clear_stall(struct iio_dev *indio_dev)
{
	...
	return tmc5222_update_bits(priv, TMC5222_RAMP_STAT,
...
static int tmc5222_set_position(struct iio_dev *indio_dev, s32 position)
{
	...
	tmc5222_warn_zero(&indio_dev->dev, a1, priv->a1);
	tmc5222_warn_zero(&indio_dev->dev, a2, priv->a2);
	tmc5222_warn_zero(&indio_dev->dev, amax, priv->amax);
	tmc5222_warn_zero(&indio_dev->dev, v1, priv->v1);
	tmc5222_warn_zero(&indio_dev->dev, v2, priv->v2);
	tmc5222_warn_zero(&indio_dev->dev, vmax, priv->vmax);
	tmc5222_warn_zero(&indio_dev->dev, d1, priv->d1);
	tmc5222_warn_zero(&indio_dev->dev, d2, priv->d2);
...
static IIO_DEVICE_ATTR_RW(sc2_vel_thrs, 0);
static IIO_DEVICE_ATTR_RW(cool_vel_thrs, 0);
static IIO_DEVICE_ATTR_RW(fullstep_vel_thrs, 0);

static struct attribute *tmc5222_attrs[] = {
	&iio_dev_attr_sc2_vel_thrs.dev_attr.attr,
	&iio_dev_attr_cool_vel_thrs.dev_attr.attr,
	&iio_dev_attr_fullstep_vel_thrs.dev_attr.attr,
	NULL,
};
			...
			if (state) {
				...
				if (tstep > priv->tcoolthrs && priv->tcoolthrs > 0) {
					dev_warn(&indio_dev->dev,
						 "TSTEP(%u) > TCOOLTHRS(%u): motor too slow, stallGuard inactive\n",
...
static int tmc5222_setup(struct iio_dev *indio_dev)
{
	...
	val = FIELD_PREP(GENMASK(31, 24), priv->irundelay) |
	      FIELD_PREP(GENMASK(23, 16), priv->iholddelay) |
	      FIELD_PREP(GENMASK(15, 8), priv->current_run) |
	      FIELD_PREP(GENMASK(7, 0), priv->current_hold);
	...
	/* PWM configuration - use values from DT */
	...
	ret = tmc5222_update_bits(priv, TMC5222_DIAG_CONF,
				  TMC5222_DIAG1_POS_REACH_MASK,
				  TMC5222_DIAG1_POS_REACH_MASK);
	if (ret)
		return ret;

	ret = tmc5222_update_bits(priv, TMC5222_DIAG_CONF,
				  TMC5222_DIAG0_STALL_MASK,
				  TMC5222_DIAG0_STALL_MASK);
	if (ret)
		return ret;
...
int tmc5222_probe_common(struct device *dev, struct regmap *regmap)
{
	...
	/* Current control */
	priv->current_run = 31;
	if (!device_property_read_u32(dev, "current-run", &val))
		priv->current_run = val;

	priv->current_hold = 8;
	if (!device_property_read_u32(dev, "current-hold", &val))
		priv->current_hold = val;

	/* Delay settings */
	priv->irundelay = 4;
	if (!device_property_read_u32(dev, "adi,irundelay", &val))
		priv->irundelay = val;

	priv->iholddelay = 1;
	if (!device_property_read_u32(dev, "adi,iholddelay", &val))
		priv->iholddelay = val;

	/* Driver configuration */
	priv->fs_gain = 1;
	if (!device_property_read_u32(dev, "adi,fs-gain", &val))
		priv->fs_gain = val;

	priv->fs_sense = 1;
	if (!device_property_read_u32(dev, "adi,fs-sense", &val))
		priv->fs_sense = val;

	priv->global_scaler_a = 0;
	priv->global_scaler_b = 0;
	if (!device_property_read_u32(dev, "adi,global-scaler", &val)) {
		priv->global_scaler_a = val;
		priv->global_scaler_b = val;
	}

	priv->tpowerdown = 10;
	device_property_read_u32(dev, "adi,tpowerdown", &priv->tpowerdown);

	/* Chopper configuration */
	priv->toff = 2;
	if (!device_property_read_u32(dev, "adi,toff", &val))
		priv->toff = val;

	priv->tbl = 2;
	if (!device_property_read_u32(dev, "adi,tbl", &val))
		priv->tbl = val;

	/* PWM configuration */
	priv->pwm_freewheel = TMC5222_FREEWHEEL_NORMAL;
	device_property_read_u32(dev, "pwm-freewheel", &priv->pwm_freewheel);

	priv->pwm_freq = TMC5222_PWM_FREQ_2_1024;
	device_property_read_u32(dev, "pwm-freq", &priv->pwm_freq);

	/* StallGuard configuration */
	priv->sgt = 0;
	if (!device_property_read_u32(dev, "adi,sgt", &val))
		priv->sgt = val;

	priv->sgt4 = 0;
	if (!device_property_read_u32(dev, "adi,sgt4", &val))
		priv->sgt4 = val;

	/* Microstep resolution */
	priv->microstep_res = TMC5222_MRES_256;
	if (!device_property_read_u32(dev, "microstep-res", &val))
		priv->microstep_res = val;

	/* Step angle in millidegrees (e.g., 1800 for 1.8°, 900 for 0.9°) */
	priv->step_angle_millidegrees = 1800;
	device_property_read_u32(dev, "step-angle-millidegrees",
				 &priv->step_angle_millidegrees);

	/* Target position */
	priv->xtarget = 0;
	device_property_read_u32(dev, "adi,xtarget", &priv->xtarget);

	/* Ramp parameters */
	priv->vstart = 0;
	device_property_read_u32(dev, "adi,vstart", &priv->vstart);

	priv->a1 = 10000;
	device_property_read_u32(dev, "adi,a1", &priv->a1);

	priv->v1 = 100000;
	device_property_read_u32(dev, "adi,v1", &priv->v1);

	priv->a2 = 30000;
	device_property_read_u32(dev, "adi,a2", &priv->a2);

	priv->v2 = 300000;
	device_property_read_u32(dev, "adi,v2", &priv->v2);

	priv->amax = 100000;
	device_property_read_u32(dev, "adi,amax", &priv->amax);

	priv->vmax = 500000;
	device_property_read_u32(dev, "adi,vmax", &priv->vmax);

	priv->dmax = 2000;
	device_property_read_u32(dev, "adi,dmax", &priv->dmax);

	priv->d2 = 1600;
	device_property_read_u32(dev, "adi,d2", &priv->d2);

	priv->d1 = 1000;
	device_property_read_u32(dev, "adi,d1", &priv->d1);

	priv->vstop = 10;
	device_property_read_u32(dev, "adi,vstop", &priv->vstop);
	...
	if (priv->irq_stallguard > 0) {
		...
		dev_info(dev, "Stallguard IRQ %d configured (disabled by default)\n",
			 priv->irq_stallguard);
	} else if (priv->irq_stallguard == -EPROBE_DEFER) {
		return -EPROBE_DEFER;
	}

	if (priv->irq_stallguard < 0 && priv->irq_stallguard != -EPROBE_DEFER) {
	...
	if (priv->irq_pos_reached > 0) {
		...
		dev_info(dev, "Position reached IRQ %d configured (disabled by default)\n",
			 priv->irq_pos_reached);
	} else if (priv->irq_pos_reached == -EPROBE_DEFER) {
		return -EPROBE_DEFER;
	}

	if (priv->irq_pos_reached < 0 && priv->irq_pos_reached != -EPROBE_DEFER) {
