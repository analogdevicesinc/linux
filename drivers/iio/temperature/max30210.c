...
struct max30210_state {
	...
	bool ext_cvt_pol;
	bool hwfifo_enabled;
	enum max30210_events_type events_type;
	u8 watermark;
};
...
enum max30210_int_drive_type {
	/** Open drain, up to 6V compliant, active low output */
	MAX30210_OPEN_DRAIN,
	/** Active drive to VDD & GND, the active level is a high output */
	MAX30210_ACTIVE_HIGH,
	/** Active drive to VDD & GND, the active level is a low output */
	MAX30210_ACTIVE_LOW,
};

/** maps adi,int-output-drive-type property value to enum */
static const char * const max30210_int_drive_types[] = {
	[MAX30210_OPEN_DRAIN] = "",
	[MAX30210_ACTIVE_HIGH] = "active-high",
	[MAX30210_ACTIVE_LOW] = "active-low",
};
			       ...
			       int val2, long mask)
{
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		if (st->ext_cvt_en) {
			...
			data = NANO;
			do_div(data, val);
			pwm_set_period(st->ext_cvt_pwm, data);
...
static int max30210_buffer_preenable(struct iio_dev *indio_dev)
{
	...
	if (st->ext_cvt_en) {
		...
		/** Match polarity of pwm and EXT CVT */
		st->ext_cvt_pwm->state.polarity = !st->ext_cvt_pol;
...
static int max30210_buffer_postdisable(struct iio_dev *indio_dev)
{
	...
	if (st->ext_cvt_en) {
		pwm_disable(st->ext_cvt_pwm);

		/**
		 * Make polarity active-low to function as powerdown pin again
		 */
		st->ext_cvt_pwm->state.polarity = 1;
...
static int max30210_show_serial_number(void *arg, u64 *val)
{
	...
	uval = (uval & GENMASK_ULL(MAX30210_UNIQUE_ID_LEN * 8 - 1, 0)) << 16;
...
static int max30210_show_roc_length(void *arg, u64 *val)
{
	struct iio_dev *indio_dev = arg;
	struct max30210_state *st = iio_priv(indio_dev);
	unsigned int uval;
	...
	ret = regmap_read(st->regmap, MAX30210_TEMP_CONF_1_REG, &uval);
	if (ret)
		return ret;

	uval = FIELD_GET(MAX30210_RATE_CHG_FILTER_MASK, uval);

	/** 0x6 and 0x7 corresponds to 65 samples */
	*val = uval < 0x6 ? (0x1 << uval) + 1 : 65;

	return 0;
}

static int max30210_store_roc_length(void *arg, u64 val)
{
	struct iio_dev *indio_dev = arg;
	struct max30210_state *st = iio_priv(indio_dev);
	...
	/** reg_value = log2(value - 1) */
	val = ilog2(val - 1);

	return regmap_update_bits(st->regmap, MAX30210_TEMP_CONF_1_REG,
				  MAX30210_RATE_CHG_FILTER_MASK, val);
}

DEFINE_DEBUGFS_ATTRIBUTE(max30210_roc_length_fops, max30210_show_roc_length,
			  max30210_store_roc_length, "%llu\n");

static void max30210_debugs_init(struct iio_dev *indio_dev)
{
	...
	if (st->chg_det_en) {
		debugfs_create_file_unsafe("roc", 0400, d, indio_dev,
					   &max30210_roc_fops);
		debugfs_create_file_unsafe("roc_length", 0600, d, indio_dev,
					   &max30210_roc_length_fops);
	}
}
			   ...
			   u8 *dir)
{
	...
	if (!IS_ERR(st->ext_cvt_pwm)) {
		...
		/** Make polarity active-low to function as powerdown pin */
		st->ext_cvt_pwm->state.polarity = 1;

		/**
		 * Default config of EXT CVT
		 * 6us duty cycle
		 * 1Hz frequency
		 */
		pwm_set_duty_cycle(st->ext_cvt_pwm, 6000);
		pwm_set_period(st->ext_cvt_pwm, NANO);
	...
	if (!IS_ERR(st->ext_cvt_pwm) && st->ext_cvt_pol)
	...
	if (!device_property_read_string(dev, "adi,int-output-drive-type",
					 &str_val)) {
		prop_val = sysfs_match_string(max30210_int_drive_types,
					      str_val);
		if (prop_val < 0)
			return dev_err_probe(dev, -EINVAL,
				     "unsupported adi,int-output-drive-type\n");

		ret = regmap_update_bits(st->regmap,
					 MAX30210_PIN_CONF_REG,
					 MAX30210_INT_OCFG_MASK,
					 FIELD_PREP(MAX30210_INT_OCFG_MASK,
					 prop_val));
		if (ret)
			return ret;
	} else {
...
static int max30210_probe(struct i2c_client *client)
{
	...
	if (st->events_type == MAX30210_DEFAULT_EVENTS)
		indio_dev->channels = &max30210_channels_default;
	else if (st->events_type == MAX30210_ROC_EVENTS)
		indio_dev->channels = &max30210_channels_roc;
	else if (st->events_type == MAX30210_COMP_MODE_EVENTS)
		indio_dev->channels = &max30210_channels_comp_mode;
	else if (st->events_type == MAX30210_COMP_MODE_ROC_EVENTS)
	...
	if (IS_ENABLED(CONFIG_DEBUG_FS))
		max30210_debugs_init(indio_dev);
