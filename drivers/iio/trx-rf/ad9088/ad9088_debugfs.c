...
static int ad9088_dbg(struct ad9088_phy *phy, int val, int val2, int val3, int val4)
{
	...
	switch (val) {
	case 1:
		adi_apollo_jrx_phase_adjust_set(device, val3 ? val3 :
						ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0, val2);
		break;
	case 2:
		adi_apollo_jtx_phase_adjust_set(device, val3 ? val3 :
						ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0, val2);
		break;
	case 3:
		adi_apollo_jtx_link_enable_set(device,
					       ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0, val2);
		adi_apollo_jrx_link_enable_set(device,
					       ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0, val2);
		break;
	case 4:
		/* Datapath reset */
	...
	case 9:
		...
		break;
	case 10:
		adi_apollo_adc_bgcal_unfreeze(device, device->dev_info.is_8t8r ?
					      ADI_APOLLO_ADC_ALL : ADI_APOLLO_ADC_ALL_4T4R);
		break;
	case 11:
		adi_apollo_adc_bgcal_freeze(device, device->dev_info.is_8t8r ?
					    ADI_APOLLO_ADC_ALL : ADI_APOLLO_ADC_ALL_4T4R);
		break;
	default:
		return -EINVAL;
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (entry->out_value) {
		...
		case DBGFS_BIST_PRBS_JRX_ERR:
			ret = adi_apollo_serdes_jrx_prbs_checker_status(&phy->ad9088,
									phy->jrx_lanes, prbs_stat,
									phy->jrx_lanes_used);
			if (ret) {
				dev_err(&phy->spi->dev, "adi_apollo_serdes_jrx_prbs_checker_status() failed (%d)",
					ret);
				break;
			}
			...
			for (i = 0; i < phy->jrx_lanes_used; i++)
				len += snprintf(phy->dbuf + len, sizeof(phy->dbuf) - len,
						...
						phy->jrx_lanes[i] > 11 ?
						phy->jrx_lanes[i] - 12 : phy->jrx_lanes[i],
						prbs_stat[phy->jrx_lanes[i]].err_count,
						prbs_stat[phy->jrx_lanes[i]].err_sticky);
			break;
		case DBGFS_BIST_JRX_2D_EYE:
			...
			if (phy->profile.jrx[0].common_link_cfg.lane_rate_kHz > 16000000) {
				ret = adi_apollo_serdes_jrx_bgcal_freeze(&phy->ad9088,
								ad9088_jrx_serdes_en_mask(phy));
			...
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
							"adi_apollo_serdes_jrx_horiz_eye_sweep");
			if (ret)
				return ret;

			msleep(duration); /* FIXME: Don't think this does anything */

			ret = adi_apollo_serdes_jrx_horiz_eye_sweep_resp_get(&phy->ad9088, lane,
									     &horz_resp);
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
						"adi_apollo_serdes_jrx_horiz_eye_sweep_resp_get");
			if (ret)
				return ret;

			ret = adi_apollo_serdes_jrx_vert_eye_sweep(&phy->ad9088, lane);
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
							"adi_apollo_serdes_jrx_vert_eye_sweep");
			if (ret)
				return ret;

			msleep(duration); /* FIXME: Don't think this does anything */

			ret = adi_apollo_serdes_jrx_vert_eye_sweep_resp_get(&phy->ad9088, lane,
									    &vert_resp);
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
						"adi_apollo_serdes_jrx_vert_eye_sweep_resp_get");
			if (ret)
				return ret;

			if (phy->profile.jrx[0].common_link_cfg.lane_rate_kHz > 16000000) {
				ret = adi_apollo_serdes_jrx_bgcal_unfreeze(&phy->ad9088,
								ad9088_jrx_serdes_en_mask(phy));
				ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
			...
			for (i = 0; i < ADI_APOLLO_SERDES_JRX_VERT_EYE_TEST_RESP_BUF_SIZE; i += 2)
				if (!(vert_resp.eye_heights_at_spo[i] == 127 &&
				      vert_resp.eye_heights_at_spo[i + 1] == -127))
		...
		case DBGFS_DEV_UUID_INFO:
			ret = adi_apollo_device_uuid_get(&phy->ad9088, uuid,
							 ADI_APOLLO_UUID_NUM_BYTES);
			if (ret)
				break;

			len = 0;
			for (i = 0; i < ADI_APOLLO_UUID_NUM_BYTES; i++)
				len += snprintf(phy->dbuf + len, sizeof(phy->dbuf) - len, "%02x",
						uuid[i]);
		...
		case DBGFS_DEV_CHIP_INFO:
			...
			len = snprintf(phy->dbuf, sizeof(phy->dbuf), "AD%X Rev. %u Grade %u\n",
				       ...
				       phy->chip_id.prod_grade);
			break;
		case DBGFS_CLK_PWR_STAT:
			ret = adi_apollo_clk_mcs_input_power_status_get(&phy->ad9088, &pwr_stat_a,
									&pwr_stat_b);
			if (ret)
				break;
		...
		case DBGFS_MCS_CAL_RUN: {
			...
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
							"adi_apollo_mcs_cal_init_status_get");
			if (ret)
				break;
		...
		case DBGFS_MCS_TRACK_STATUS: {
			...
			ret = adi_apollo_mcs_cal_tracking_status_get(&phy->ad9088,
								     &tracking_cal_status);
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
							"adi_apollo_mcs_cal_tracking_status_get");
			if (ret)
				break;

			len = ad9088_mcs_track_cal_status_print(phy, phy->dbuf,
								&tracking_cal_status, 1);
				    ...
				    const char __user *userbuf, size_t count, loff_t *ppos)
{
	...
	case DBGFS_BIST_PRBS_JRX:
		...
		adi_apollo_serdes_jrx_prbs_checker_enable(&phy->ad9088, phy->jrx_lanes,
							  phy->jrx_lanes_used, &prbs_chk);
		...
		if (prbs_chk.enable)
			adi_apollo_serdes_jrx_prbs_clear_error(&phy->ad9088, phy->jrx_lanes,
							       phy->jrx_lanes_used);
	...
	case DBGFS_BIST_PRBS_JTX:
		...
		adi_apollo_serdes_prbs_generator_enable(&phy->ad9088, (u8 *)lanes_all,
							ARRAY_SIZE(lanes_all), &prbs_gen);
		entry->val = val;
	...
	case DBGFS_HSCI_ENABLE:
		...
		if (phy->hsci) {
			if (val == 1)
				val2 = ADI_APOLLO_HAL_PROTOCOL_HSCI;
			else
				val2 = ADI_APOLLO_HAL_PROTOCOL_SPI0;
			ret = adi_apollo_hal_active_protocol_set(&phy->ad9088, val2);
		} else {
			return -ENODEV;
		}
		break;
	...
	case DBGFS_JRX_PHASE_ADJUST_CALC:
		ret  = adi_apollo_jrx_phase_adjust_calc(&phy->ad9088,
							ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0,
							ADI_APOLLO_JRX_PHASE_ADJ_MARGIN_DEFAULT,
							&phase);
		if (ret != API_CMS_ERROR_OK) {
			dev_err(&phy->spi->dev,
				"Error from adi_apollo_jrx_phase_adjust_calc() %d\n", ret);
			return ret;
		}
		entry->val = phase;
		return count;
	case DBGFS_JTX_LANE_DRIVE_SWING:
		if (ret < 3) {
			dev_err(&phy->spi->dev,
				"Attribute requires 3 arguments <link_side_mask> <lane> <swing>\n");
			return -EINVAL;
		}
		...
		dev_info(&phy->spi->dev,
			 "JTX Lane Drive Swing Set sides: 0x%X lane: %d swing: %d (%d)\n",
	...
	case DBGFS_MCS_FG_TRACK_CAL_RUN:
		if (val) {
			...
			ret = ad9088_check_apollo_error(&phy->spi->dev, ret,
							"adi_apollo_mcs_cal_fg_tracking_run");
		}
		break;
	case DBGFS_MCS_BG_TRACK_CAL_RUN:
		if (val) {
