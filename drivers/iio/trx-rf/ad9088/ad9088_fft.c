...
static const unsigned long ad9088_fft_sniffer_available_scan_masks[] = {
	BIT(AD9088_FFT_SNIFFER_SI_INDEX) | BIT(AD9088_FFT_SNIFFER_I_INDEX) |
...
enum {
	AD9088_FTT_MAX_THRESHOLD,
	AD9088_FTT_MIN_THRESHOLD,
};

static int ad9088_rx_sniffer_populate_default_params(adi_apollo_sniffer_mode_e mode,
						     adi_apollo_sniffer_param_t *config)
{
						...
						const struct iio_chan_spec *chan, char *buf)
{
	...
	switch (private) {
	case AD9088_FTT_MAX_THRESHOLD:
		return sysfs_emit(buf, "%u\n", st->sniffer_config.init.max_threshold);
	case AD9088_FTT_MIN_THRESHOLD:
		return sysfs_emit(buf, "%u\n", st->sniffer_config.init.min_threshold);
	default:
		return -EINVAL;
	}
}
						 ...
						 const char *buf, size_t len)
{
	...
	val = clamp_t(long long, readin, 0, 255);

	guard(mutex)(&phy->lock);

	switch (private) {
	case AD9088_FTT_MAX_THRESHOLD:
		ret = adi_apollo_hal_bf_set(&st->phy->ad9088,
					    BF_MAX_THRESHOLD_INFO(st->regmap_base), val);
		if (ret)
			return ret;
	...
	case AD9088_FTT_MIN_THRESHOLD:
		ret = adi_apollo_hal_bf_set(&st->phy->ad9088,
					    BF_MIN_THRESHOLD_INFO(st->regmap_base), val);
		if (ret)
			return ret;
					      ...
					      const struct iio_chan_spec *chan)
{
	...
	for (i = 0; i < ARRAY_SIZE(ad9088_fft_sniffer_adc_select); i++) {
		if (st->adc_select == BIT(i))
			return i;
	}
					       ...
					       unsigned int item)
{
	...
	/* Validate item range */
	if (item >= ARRAY_SIZE(ad9088_fft_sniffer_adc_select))
		return -EINVAL;
						       ...
						       char *buf)
{
	...
	for (i = 0; i < max_adc; i++)
		len += sysfs_emit_at(buf, len, "%s ", ad9088_fft_sniffer_adc_select[i]);
...
static struct iio_chan_spec_ext_info ad9088_fft_sniffer_ext_info[] = {
	...
	},
	{
		...
		.shared = IIO_SHARED_BY_ALL,
		.private = AD9088_FTT_MAX_THRESHOLD,
	},
	{
		.name = "min_threshold",
		.read = ad9088_fft_sniffer_ext_info_read,
		.write = ad9088_fft_sniffer_ext_info_write,
		.shared = IIO_SHARED_BY_ALL,
		.private = AD9088_FTT_MIN_THRESHOLD,
	},
	{},
};
					       ...
					       const unsigned long *active_scan_mask)
{
	...
	if (active_scan_mask[0] & BIT(AD9088_FFT_SNIFFER_MAGN_INDEX)) {
		st->sniffer_config_hw.pgm.sniffer_mode = st->mode ?
							 ADI_APOLLO_SNIFFER_INSTANT_MAGNITUDE :
...
static void ad9088_rx_sniffer_debugfs_init(struct iio_dev *indio_dev,
					   adi_apollo_sniffer_param_t *config)
{
	struct ad9088_fft_sniffer_state *st = iio_priv(indio_dev);
	struct dentry *d;
	...
	debugfs_create_u8("run_fft_engine_background", 0644, d,
...
static void ad9088_fft_sniffer_data_push(struct ad9088_fft_sniffer_state *st,
					 adi_apollo_sniffer_param_t *config, bool polling)
{
	adi_apollo_sniffer_fft_data_t *fft_data = &st->fft_data;
	adi_apollo_device_t *device = &st->phy->ad9088;
	unsigned int bit, j, i;
	bool iq_mode;
	int ret;

	if (polling)
		ret = adi_apollo_sniffer_data_get(device, st->side_sel, config, fft_data);
	else
		ret = adi_apollo_sniffer_fft_data_get(device, st->side_sel, config, fft_data);

	if (ret) {
		dev_err(st->dev, "Failed to get FFT data (polling:%u): %d\n", polling, ret);
		return;
	}

	if (config->pgm.sniffer_mode > ADI_APOLLO_SNIFFER_INSTANT_MAGNITUDE)
		iq_mode = true;
	else
		iq_mode = false;
	...
	for (i = 0; i < fft_data->valid_data_length; i++) {
		...
		iio_for_each_active_channel(st->indio_dev, bit) {
			switch (bit) {
			case AD9088_FFT_SNIFFER_SI_INDEX:
				st->buffer[j++] = iq_mode ? i : fft_data->bin_q_data[i];
				break;
			case AD9088_FFT_SNIFFER_I_INDEX:
				st->buffer[j++] = fft_data->mag_i_data[i];
				break;
			case AD9088_FFT_SNIFFER_Q_INDEX:
				st->buffer[j++] = fft_data->bin_q_data[i];
				break;
			case AD9088_FFT_SNIFFER_MAGN_INDEX:
				st->buffer[j++] = fft_data->mag_i_data[i];
				break;
			}
		}
...
static int ad9088_fft_sniffer_data_read(struct ad9088_fft_sniffer_state *st,
					adi_apollo_sniffer_param_t *config)
{
	adi_apollo_device_t *device = &st->phy->ad9088;
	u8 fft_done = 0;
	...
	ad9088_fft_sniffer_data_push(st, config, false);
...
{
	struct ad9088_fft_sniffer_state *st = container_of(work, struct ad9088_fft_sniffer_state,
							   sync_work.work);
	int ret;
	...
	if (st->irq) {
		...
		ad9088_fft_sniffer_data_push(st, &st->sniffer_config_hw, true);
	}
...
int ad9088_fft_sniffer_probe(struct ad9088_phy *phy, adi_apollo_side_select_e side_sel)
{
	...
	case ADI_APOLLO_SIDE_A:
		...
		ret = device_property_read_u32(dev, "adi,gpio-sniffer-a-export", &pin);
		if (ret)
	...
	case ADI_APOLLO_SIDE_B:
		...
		/* Read GPIO pin from devicetree */
		ret = device_property_read_u32(dev, "adi,gpio-sniffer-b-export", &pin);
		if (ret)
			pin = 18; /* Default fallback */
		break;
	default:
		return dev_err_probe(&phy->spi->dev, -EINVAL, "Invalid side selection\n");
	}

	ret = devm_ad9088_set_child_label(phy, indio_dev, label);
	if (ret)
		return ret;
	...
	ad9088_rx_sniffer_populate_default_params(ADI_APOLLO_SNIFFER_INSTANT_MAGNITUDE,
						  &st->sniffer_config);
