...
#define AD9088_REG_TX_SLICE_0_TX_DIGITAL0 \
	BF_SYNC_INPUT_COUNT_INFO(TXRX_PREFSRC_RECONF_TX_SLICE_0_TX_DIGITAL0)
#define AD9088_REG_TX_SLICE_1_TX_DIGITAL0 \
	BF_SYNC_INPUT_COUNT_INFO(TXRX_PREFSRC_RECONF_TX_SLICE_1_TX_DIGITAL0)
#define AD9088_REG_TX_SLICE_0_TX_DIGITAL1 \
	BF_SYNC_INPUT_COUNT_INFO(TXRX_PREFSRC_RECONF_TX_SLICE_0_TX_DIGITAL1)
#define AD9088_REG_TX_SLICE_1_TX_DIGITAL1 \
				     ...
				     enum jesd204_state_op_reason reason)
{
	...
	ret = adi_apollo_jtx_link_enable_set(device, ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0,
					     ADI_APOLLO_ENABLE);
	if (ret) {
		dev_err(dev, "Error enabling JTx links %d\n", ret);
		return ret;
	}
	ret = adi_apollo_jtx_link_enable_set(device, ADI_APOLLO_LINK_A1 | ADI_APOLLO_LINK_B1,
					     ADI_APOLLO_DISABLE);
	if (ret) {
		dev_err(dev, "Error enabling JTx links %d\n", ret);
		return ret;
	...
	ret = adi_apollo_jrx_link_enable_set(device, ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0,
					     ADI_APOLLO_ENABLE);
	if (ret) {
		dev_err(dev, "Error enabling JRx links %d\n", ret);
		return ret;
	}
	ret = adi_apollo_jrx_link_enable_set(device, ADI_APOLLO_LINK_A1 | ADI_APOLLO_LINK_B1,
					     ADI_APOLLO_DISABLE);
	if (ret) {
		dev_err(dev, "Error enabling JRx links %d\n", ret);
		return ret;
				       ...
				       enum jesd204_state_op_reason reason)
{
	...
	ret = adi_apollo_adc_nyquist_zone_set(device, n_adc, phy->rx_nyquist_zone);
	if (ret) {
	...
	ret = adi_apollo_jrx_phase_adjust_set(device, ADI_APOLLO_LINK_A0 | ADI_APOLLO_LINK_B0,
					      jrx_phase_adjust);
	if (ret) {
				       ...
				       enum jesd204_state_op_reason reason)
{
	...
	ret = adi_apollo_adc_init_cal_complete(device, device->dev_info.is_8t8r ?
					       ADI_APOLLO_ADC_ALL : ADI_APOLLO_ADC_ALL_4T4R);
	ret = ad9088_check_apollo_error(dev, ret, "adi_apollo_adc_init_cal_complete");
	if (ret)
		return ret;

	ret = adi_apollo_adc_nyquist_zone_set(device, device->dev_info.is_8t8r ?
					      ADI_APOLLO_ADC_ALL : ADI_APOLLO_ADC_ALL_4T4R,
					      phy->rx_nyquist_zone);
	if (ret) {
		dev_err(dev, "Error setting ADC Nyquist zone %d\n", ret);
					    ...
					    enum jesd204_state_op_reason reason)
{
	...
	if (__is_defined(DEBUG)) {
		ret = iio_read_channel_attribute(phy->iio_adf4030, &val, &val2,
						 IIO_CHAN_INFO_PHASE);
		if (ret < 0) {
			dev_err(dev, "Failed to read adf4030 phase\n");
			return ret;
	...
	/*
	 * MCS Tracking Calibration is only supported in single clock mode.
	 * Per UG-2300: "MCS Tracking Calibration restricted to Single Clock
	 * scheme due to HW limitation."
	 */
	if (device->dev_info.is_dual_clk) {
		dev_info(dev, "Dual clock mode: skipping MCS tracking calibration (HW limitation)\n");
	} else {
					    ...
					    enum jesd204_state_op_reason reason)
{
	...
	if (!IS_ERR_OR_NULL(phy->iio_adf4030) && jesd204_dev_is_top(jdev) &&
	    phy->aion_background_serial_alignment_en) {
		ret = ad9088_iio_write_channel_ext_info(phy, phy->iio_adf4030,
							"background_serial_alignment_en", 1);
		if (ret < 0) {
					    ...
					    enum jesd204_state_op_reason reason)
{
	...
	if (phy->trig_sync_en) {
		...
		ret = adi_apollo_clk_mcs_trig_phase_get(&phy->ad9088, ADI_APOLLO_TRIG_PIN_A0,
							&phase, &phase1);
		if (ret) {
		...
		if (__is_defined(DEBUG)) {
			/*
			 * Read the trigger sync count. It will increment by two each time the FPGA
			 * sysref sequencer is executed.
			 */
			u8 sync_input_count[4];

			adi_apollo_hal_bf_get(device, AD9088_REG_TX_SLICE_0_TX_DIGITAL0,
					      &sync_input_count[0], 1);
			adi_apollo_hal_bf_get(device, AD9088_REG_TX_SLICE_1_TX_DIGITAL0,
					      &sync_input_count[1], 1);
			adi_apollo_hal_bf_get(device, AD9088_REG_TX_SLICE_0_TX_DIGITAL1,
					      &sync_input_count[2], 1);
			adi_apollo_hal_bf_get(device, AD9088_REG_TX_SLICE_1_TX_DIGITAL1,
	...
	if (phy->iio_axi_fsrc) {
		...
		ret = ad9088_fsrc_reconfig_sequence_spi(phy, phy->profile.rx_path[0].rx_fsrc[0].enable);
		if (ret) {
			dev_err(dev, "Failed RX/TX FSRC SPI reconfig sequence: %d\n", ret);
		        return ret;
		}
	}
...
static int ad9088_jesd204_uninit(struct jesd204_dev *jdev, enum jesd204_state_op_reason reason)
{
	struct device *dev = jesd204_dev_to_device(jdev);
