...
int ad9088_delta_t_measurement_set(struct ad9088_phy *phy, u32 mode)
{
	...
	if (phy->profile.clk_cfg.clocking_mode == ADI_APOLLO_CLOCKING_MODE_SDR_DIV_8)
		bsync_divider = 8 * phy->profile.mcs_cfg.internal_sysref_prd_digclk_cycles_center;
	else
		bsync_divider = 4 * phy->profile.mcs_cfg.internal_sysref_prd_digclk_cycles_center;
	...
	ret = adi_apollo_mailbox_mcs_bsync_set_config(device, &bsync_set_config_cmd,
				 ...
				 adi_apollo_mcs_cal_init_status_t *cal_status)
{
	...
	if (cal_status->hdr.errorCode != 0) {
		adi_apollo_hal_log_write(device, ADI_CMS_LOG_ERR,
					 "MCS Init Cal Apollo CPU errorCode: 0x%X.\n",
					 cal_status->hdr.errorCode);
		ret = -EFAULT;
		goto end;
	}
	...
	if (cal_status->mcsErr != 0) {
		adi_apollo_hal_log_write(device, ADI_CMS_LOG_ERR,
					 "MCS Init Cal MCS errorCode: 0x%X.\n", cal_status->mcsErr);
		ret = -EFAULT;
		goto end;
	}
...
{
	adi_apollo_mcs_cal_status_t tracking_cal_status = { };
	...
	ret = ad9088_iio_read_channel_ext_info(phy, phy->iio_adf4382, "coarse_current",
					       &hw_coarse_current);
	if (ret)
		goto out_unfreeze;

	ret = ad9088_iio_read_channel_ext_info(phy, phy->iio_adf4382, "fine_current",
					       &hw_fine_current);
	if (ret)
		goto out_unfreeze;

	/* Get firmware-reported values */
	mcs_cal_status = &tracking_cal_status.mcs_tracking_cal_status;
	fw_bleed_pol = mcs_cal_status->adf4382_specific_status[0].bleed_pol;
	fw_coarse_current = mcs_cal_status->adf4382_specific_status[0].current_coarse_value;
	fw_fine_current = mcs_cal_status->adf4382_specific_status[0].current_fine_value;
...
out_unfreeze:
	if (need_unfreeze) {
		...
		ret = ad9088_check_apollo_error(dev, ret2,
...
int ad9088_mcs_track_cal_status_print(struct ad9088_phy *phy, char *buf,
				      ...
				      u8 print_full_state)
{
	adi_apollo_mcs_private_cal_status_t *mcs_cal_status = &cal_status->mcs_tracking_cal_status;
	int len = 0;
