...
int __adrv903x_dev_err(struct adrv903x_rf_phy *phy, const char *function,
		       const int line)
{
	int ret = 0;
				  ...
				  const char *buf, size_t len)
{
	...
	case ADRV903X_INIT_CAL:
		...
		if (val) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_InitCalsWait,
				     ...
				     const char *buf, size_t len)
{
	...
	case RX_DIG_DC:
		if (chan->channel <= CHAN_RX8) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_DigDcOffsetEnableSet, mask,
						enable ? 0xFF : 0x00);
		}
		break;
	case RX_ADC:
	...
	case RX_NCO_EN:
		if (chan->channel <= CHAN_RX8) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_RxNcoShifterSet, &rxNcoConfig);
			if (!ret)
	...
	case RX_NCO_FREQ:
		if (chan->channel <= CHAN_RX8) {
			...
			if (phy->rx_nco_en[chan->channel]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_RxNcoShifterSet, &rxNcoConfigFreq);
			}
		} else {
			ret = -EINVAL;
		}
	...
	case RX_NCO_PHASE:
		if (chan->channel <= CHAN_RX8) {
			...
			if (phy->rx_nco_en[chan->channel]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_RxNcoShifterSet, &rxNcoConfigPhase);
			}
		} else {
			ret = -EINVAL;
		}
	...
	case RX_NCO_BAND_SEL:
		if (chan->channel <= CHAN_RX8) {
			...
			if (phy->rx_nco_en[chan->channel]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_RxNcoShifterSet, &rxNcoConfigBand);
			}
		} else {
			ret = -EINVAL;
		}
	...
	case ORX_NCO_EN:
		if (chan->channel > CHAN_RX8) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_OrxNcoSet_v2, &orxNcoConfig);
			if (!ret)
	...
	case ORX_NCO_FREQ:
		if (chan->channel > CHAN_RX8) {
			...
			if (phy->orx_nco_en[orx_idx]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_OrxNcoSet_v2, &orxNcoConfigFreq);
			}
		} else {
			ret = -EINVAL;
		}
	...
	case ORX_NCO_PHASE:
		if (chan->channel > CHAN_RX8) {
			...
			if (phy->orx_nco_en[orx_idx]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_OrxNcoSet_v2, &orxNcoConfigPhase);
			}
		} else {
			ret = -EINVAL;
		}
	...
	case ORX_NCO_SEL:
		if (chan->channel > CHAN_RX8) {
			...
			if (phy->orx_nco_en[orx_idx]) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_OrxNcoSet_v2, &orxNcoConfigSel);
			}
		} else {
			ret = -EINVAL;
		}
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	case RX_DIG_DC:
		mask = ADI_ADRV903X_RX0 << chan->channel;
		ret = adrv903x_api_call(phy, adi_adrv903x_DigDcOffsetEnableGet, mask, &isEnabled);
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	switch (private) {
	case TX_QEC:
		ret = adrv903x_api_call(phy, adi_adrv903x_TrackingCalsEnableGet, enableMasks);
		if (ret == 0) {
			chan_cals = enableMasks->enableMask[chan->channel];
				     ...
				     const char *buf, size_t len)
{
	...
	case TX_TEST_TONE_FREQ: {
		...
		if (phy->tx_test_tone_en[chan->channel]) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_TxTestToneSet, &txNcoConfigFreq);
		}
		break;
	}
	case TX_TEST_TONE_PHASE: {
		...
		if (phy->tx_test_tone_en[chan->channel]) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_TxTestToneSet, &txNcoConfigPhase);
		}
		break;
	}
	case TX_TEST_TONE_NCO_SEL: {
		...
		if (phy->tx_test_tone_en[chan->channel]) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_TxTestToneSet, &txNcoConfigNco);
		}
		break;
	}
	case TX_TEST_TONE_ATTEN: {
		...
		if (phy->tx_test_tone_en[chan->channel]) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_TxTestToneSet, &txNcoConfigAtten);
		}
		break;
	}
	default:
				 ...
				 int *val2, long m)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		ret = adrv903x_api_call(phy, adi_adrv903x_RxTxEnableGet, &orxchan, &rxchan,
					&txchan);
		if (ret)
			break;
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output) {
			...
			ret = adrv903x_api_call(phy, adi_adrv903x_TxAttenGet,
			...
			*val2 = (txAtten.txAttenuation_mdB % 1000) * 1000;
			...
			if (chan->channel <= CHAN_RX8) {
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_RxGainGet,
				...
				ret = adrv903x_api_call(phy, adi_adrv903x_OrxAttenGet, chan->channel - CHAN_OBS_RX1, &attenDb);
				if (ret)
					break;
				*val = attenDb;
			}
...
static int adrv903x_write_tx_gain(struct adrv903x_rf_phy *phy,
				  const struct iio_chan_spec *chan,
				  int val, int val2)
{
	adi_adrv903x_TxAtten_t txAtten;
	...
	txAtten.txAttenuation_mdB = (abs(val) * 1000) + (abs(val2) / 1000);

	ret = adrv903x_api_call(phy, adi_adrv903x_TxAttenSet, &txAtten, 1);

	return ret;
}

static int adrv903x_write_rx_gain(struct adrv903x_rf_phy *phy,
				  const struct iio_chan_spec *chan,
				  int val, int val2)
{
	adi_adrv903x_RxGain_t rxGain;
	...
	ret = adrv903x_gain_to_gainindex(phy, chan->channel, val, val2, &code);
	if (ret < 0)
		return ret;
	...
	ret = adrv903x_api_call(phy, adi_adrv903x_RxGainSet, &rxGain, 1);

	return ret;
}

static int adrv903x_write_orx_gain(struct adrv903x_rf_phy *phy,
				   const struct iio_chan_spec *chan,
				   int val)
{
	int ret;

	ret = adrv903x_api_call(phy, adi_adrv903x_OrxAttenSet,
				chan->channel - CHAN_RX8, (u8)val);
				  ...
				  int val2, long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_ENABLE:
		ret = adrv903x_api_call(phy, adi_adrv903x_RxTxEnableGet, &orxchan_msk, &rxchan_msk,
					&txchan_msk);
		if (ret)
			break;
		...
		ret = adrv903x_api_call(phy, adi_adrv903x_RxTxEnableSet, orxchan_msk, orxchan_en, rxchan_msk, rxchan_en,
					txchan_msk, txchan_en);
		break;

	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (chan->output)
			ret = adrv903x_write_tx_gain(phy, chan, val, val2);
		else if (chan->channel <= CHAN_RX8)
			ret = adrv903x_write_rx_gain(phy, chan, val, val2);
		else
			ret = adrv903x_write_orx_gain(phy, chan, val);
		break;
	case IIO_CHAN_INFO_SAMP_FREQ:
		break;
	default:
				 ...
				 u32 source)
{
	...
	_parent_name[0] = adrv903x_clk_set_dev_name(phy, p_name[0], parent_name);
	_parent_name[1] = adrv903x_clk_set_dev_name(phy, p_name[1], parent_name2);
					...
					enum jesd204_state_op_reason reason)
{
	...
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
		dev_err(dev, "ERROR adi_adrv903x_HwOpen failed\n");
	...
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
		dev_err(dev, "ERROR adi_adrv903x_PreMcsInit failed\n");
		return JESD204_STATE_CHANGE_ERROR;
	}
	...
	recoveryAction = adi_adrv903x_PreMcsInit_NonBroadcast(phy->palauDevice,
							      &deviceInitStruct);
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
		dev_err(dev, "ERROR adi_adrv903x_PreMcsInit_NonBroadcast failed\n");
		return JESD204_STATE_CHANGE_ERROR;
	}
				      ...
				      struct jesd204_link *lnk)
{
	...
	case DEFRAMER1_LINK_TX: {
		...
		if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
			dev_err(dev, "ERROR adi_adrv903x_DeframerCfgGet failed\n");
			return JESD204_STATE_CHANGE_ERROR;
		}
	...
	case FRAMER2_LINK_RX: {
		...
		if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
			dev_err(dev, "ERROR adi_adrv903x_FramerCfgGet failed\n");
			return JESD204_STATE_CHANGE_ERROR;
		}
				       ...
				       enum jesd204_state_op_reason reason)
{
	...
	ret = adrv903x_api_call(phy, adi_adrv903x_MultichipSyncSet_v2, ADI_ENABLE);
	if (ret)
		return ret;
					 ...
					 enum jesd204_state_op_reason reason)
{
	...
	for (i = 0; i < 255; i++) {
		ret = adrv903x_api_call(phy, adi_adrv903x_MultichipSyncStatusGet,
					&mcsStatus);
		if (ret)
			return ret;
					 ...
					 enum jesd204_state_op_reason reason)
{
	...
	/* MCS end sequence*/
	ret = adrv903x_api_call(phy, adi_adrv903x_MultichipSyncSet_v2, ADI_DISABLE);
	if (ret)
		return ret;

	/* Post MCS */
	ret = adrv903x_api_call(phy, adi_adrv903x_PostMcsInit,
				&utilityInit);
	if (ret)
		return ret;

	ret = adrv903x_api_call(phy, adi_adrv903x_SerializerReset);
	if (ret)
		return ret;
					...
					struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		if (phy->palauDevice->devStateInfo.linkSharingEnabled == 1) {
			ret = adrv903x_api_call(phy, adi_adrv903x_FramerSysrefCtrlSet,
						ADI_ADRV903X_FRAMER_1,
						ADI_DISABLE);
			if (ret)
				return ret;

			ret = adrv903x_api_call(phy, adi_adrv903x_FramerLinkStateSet,
						ADI_ADRV903X_FRAMER_1,
						ADI_DISABLE);
			if (ret)
				return ret;
		...
		ret = adrv903x_api_call(phy, adi_adrv903x_DeframerSysrefCtrlSet,
					(uint8_t)ADI_ADRV903X_ALL_DEFRAMER,
					ADI_DISABLE);
		if (ret)
			return ret;

		ret = adrv903x_api_call(phy, adi_adrv903x_DeframerLinkStateSet,
					(uint8_t)ADI_ADRV903X_ALL_DEFRAMER,
					ADI_DISABLE);
		if (ret)
			return ret;
					...
					struct jesd204_link *lnk)
{
	...
	if (!priv->link[lnk->link_id].is_framer) { /* DEFRAMER */
		...
		ret = adrv903x_api_call(phy, adi_adrv903x_InitCalsRun, &serdesCal);
		if (ret)
			return ret;

		/* Wait up to 60 seconds for ARM */
		ret = adrv903x_api_call(phy, adi_adrv903x_InitCalsWait, 60000);
		if (ret) {
					 ...
					 struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		ret = adrv903x_api_call(phy, adi_adrv903x_FramerStatusGet,
					priv->link[lnk->link_id].source_id,
					&framerStatus);
		if (ret)
			return ret;
		...
		ret = adrv903x_api_call(phy, adi_adrv903x_DeframerStatusGet_v2,
					priv->link[lnk->link_id].source_id,
					&deframerStatus);
		if (ret)
			return ret;

		ret  = adrv903x_api_call(phy, adi_adrv903x_DfrmLinkConditionGet,
					 priv->link[lnk->link_id].source_id,
					       ...
					       enum jesd204_state_op_reason reason)
{
	...
	ret = adrv903x_api_call(phy, adi_adrv903x_RxTxEnableSet, orx_mask, orx_mask,
				rx_mask, rx_mask, tx_mask, tx_mask);
	if (ret)
		return ret;

	ret = adrv903x_api_call(phy, adi_adrv903x_RxTxEnableSet, orx_mask, 0x00,
	...
	ret = adrv903x_api_call(phy, adi_adrv903x_TxAttenSet, txAttenuation, 1);
	if (ret)
		return ret;

	memset(&initCalStatus, 0, sizeof(adi_adrv903x_InitCalStatus_t));

	ret = adrv903x_api_call(phy, adi_adrv903x_InitCalsDetailedStatusGet, &initCalStatus);
	if (ret)
		return ret;
...
static int adrv903x_probe(struct spi_device *spi)
{
	...
	if (!device_property_read_string(&spi->dev, "adi,device-config-name", &name)) {
		...
		if (ret < 0)
			return dev_err_probe(&spi->dev, ret, "device-config-name too long\n");
	} else {
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error missing property: adi,device-config-name\n");
	}
	...
	if (!device_property_read_string(&spi->dev, "adi,arm-firmware-name", &name)) {
		...
		if (ret < 0)
			return dev_err_probe(&spi->dev, ret, "arm-firmware-name too long\n");
	} else {
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error missing property: adi,arm-firmware-name\n");
	}
	...
	if (!device_property_read_string(&spi->dev, "adi,stream-firmware-name", &name)) {
		...
		if (ret < 0)
			return dev_err_probe(&spi->dev, ret, "stream-firmware-name too long\n");
	} else {
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error missing property: adi,stream-firmware-name\n");
	}

	{
		const char *names[ADI_ADRV903X_RX_GAIN_TABLE_ARR_MAX];
		u32 masks[ADI_ADRV903X_RX_GAIN_TABLE_ARR_MAX];
		int n;

		n = device_property_read_string_array(&spi->dev, "adi,rx-gaintable-names",
						      names, ARRAY_SIZE(names));
		if (n > 0 && !device_property_read_u32_array(&spi->dev,
							     "adi,rx-gaintable-channel-masks",
							     masks, n)) {
			for (i = 0; i < n; i++) {
				ret = strscpy(phy->trxBinaryInfoPtr.rxGainTable[i].filePath,
					      names[i],
					      sizeof(phy->trxBinaryInfoPtr.rxGainTable[i].filePath));
				if (ret < 0)
					return dev_err_probe(&spi->dev, ret,
							     "rx-gaintable-name[%d] too long\n", i);
				phy->trxBinaryInfoPtr.rxGainTable[i].channelMask = masks[i];
			}
		}
	}
	...
	if (adi_adrv903x_hal_PlatformSetup(ADI_LINUX) != ADI_HAL_ERR_OK)
		return dev_err_probe(&spi->dev, -EINVAL,
				     "error HAL function(s) not implemented.\n");

	phy->linux_hal.reset_gpio = devm_gpiod_get_optional(&spi->dev, "reset", GPIOD_OUT_LOW);
	...
	ret = adrv903x_api_call(phy, adi_adrv903x_HwOpen, &phy->spiSettings);
	if (ret)
		return ret;

	ret = adrv903x_api_call(phy, adi_adrv903x_SpiVerify);
	if (ret)
		return ret;

	ret = adrv903x_api_call(phy, adi_adrv903x_ApiVersionGet, &apiVersion);
	if (ret)
		return ret;
...
static const struct spi_device_id adrv903x_id[] = {
	{ "adrv9032", (kernel_ulong_t)&adrv9032_chip_info },
	{ "adrv9032r", (kernel_ulong_t)&adrv9032r_chip_info },
	{ }
};
