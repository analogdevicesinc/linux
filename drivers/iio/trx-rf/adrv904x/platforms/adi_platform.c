...
/* I2C Interface */
adi_hal_Err_e(*adrv904x_hal_I2cWrite)(   void* const     devHalCfg,
                                    const uint8_t   txData[],
                                    const uint32_t  numTxBytes) = NULL;

adi_hal_Err_e(*adrv904x_hal_I2cRead)(    void* const     devHalCfg,
                                    const uint8_t   txData[],
...
adi_hal_Err_e (*adrv904x_hal_BbicRegisterRead)(  void* const     devHalCfg,
                                            const uint32_t  addr,
                                            uint32_t* const data)           = NULL;

adi_hal_Err_e (*adrv904x_hal_BbicRegisterWrite)( void* const     devHalCfg,
                                            const uint32_t  addr,
                                            const uint32_t  data)           = NULL;

adi_hal_Err_e (*adrv904x_hal_BbicRegistersRead)( void* const     devHalCfg,
                                            const uint32_t  addr,
                                            uint32_t        data[],
                                            const uint32_t  numDataWords)   = NULL;

adi_hal_Err_e (*adrv904x_hal_BbicRegistersWrite)(void* const     devHalCfg,
                                            const uint32_t  addr,
                                            const uint32_t  data[],
                                            const uint32_t  numDataWords)   = NULL;
...
#ifndef __KERNEL__
/* Thread Interface */
adi_hal_thread_t (*adrv904x_hal_ThreadSelf)(void) = NULL;
#endif
...
adi_hal_Err_e(*adrv904x_hal_BoardIdentify)(char** boardNames, int32_t* numBoards) = NULL;
...
ADI_API adi_hal_Err_e adrv904x_hal_PlatformSetup(const adi_hal_Platforms_e platform)
{
    ...
    case ADI_ADS10_PLATFORM:
...
#endif
        adrv904x_hal_I2cWrite = NULL; /* ADS10 does not require I2C interface to any devices used in device API layer */
        adrv904x_hal_I2cRead = NULL;  /* ADS10 does not require I2C interface to any devices used in device API layer */
        ...
        adrv904x_hal_BbicRegisterRead   = ads10_BbicRegisterRead;
        adrv904x_hal_BbicRegisterWrite  = ads10_BbicRegisterWrite;
        adrv904x_hal_BbicRegistersRead  = ads10_BbicRegistersRead;
        adrv904x_hal_BbicRegistersWrite = ads10_BbicRegistersWrite;

        adrv904x_hal_ThreadSelf = posix_ThreadSelf;
        ...
        adrv904x_hal_BoardIdentify = ads10_BoardIdentify;
