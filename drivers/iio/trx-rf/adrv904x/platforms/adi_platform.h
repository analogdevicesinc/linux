 ...
 * it is the maximum number of threads that can have an adi_common_ErrData_t registered (using adrv904x_TlsSet)
 * at one time. */
 ...
 * \retval adi_hal_Err_e - ADI_HAL_ERR_OK if successful
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_SpiWrite)(void* const devHalCfg, const uint8_t txData[], const uint32_t numTxBytes);
 ...
 * \retval adi_hal_Err_e - ADI_HAL_ERR_OK if successful
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_SpiRead)(void* const     devHalCfg,
...
* \retval adi_hal_Err_e - ADI_HAL_ERR_OK if successful
*/
ADI_API_EX adi_hal_Err_e (*adrv904x_LogStatusGet)(void* const devHalCfg, adi_hal_LogStatusGet_t* const logStatus);
 ...
 *
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_LogConsoleSet)(void* const devHalCfg, const adi_hal_LogConsole_e logConsoleFlag);
 ...
 * ADI_HAL_ERR_OK.
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_MutexInit)(adi_hal_mutex_t* const mutex);
 ...
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_MutexLock)(adi_hal_mutex_t* const mutex);

/**
 * \brief Unlocks the mutex.
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_MutexUnlock)(adi_hal_mutex_t* const mutex);
 ...
 * Non-multithreaded HALs must implement this function but it can just simply return 0
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_MutexDestroy)(adi_hal_mutex_t* const mutex);

/**
 * \brief Store \param value on behalf of the calling thread and associate it with \param tlsType.
 * To retrieve the value later the same thread calls adrv904x_TlsGet using the same value of
 * \param tlsType.
 *
 * If adrv904x_TlsSet was previously called with a different value the new value overwrites the existing
 * value stored. Until a thread calls adrv904x_TlsSet the value associated with that thread is NULL for
 ...
 */
ADI_API_EX adi_hal_Err_e (*adrv904x_TlsSet)(const adi_hal_TlsType_e tlsType, void* const value);
 ...
 *
 * Further details of the HAL thread-local storage API are detailed by adrv904x_TlsSet.
 */
ADI_API_EX void* (*adrv904x_TlsGet)(const adi_hal_TlsType_e tlsType);
