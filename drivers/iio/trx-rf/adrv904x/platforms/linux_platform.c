					      ...
					      va_list argp)
{
	...
	case ADI_HAL_LOG_ALL:
		dev_info(&halCfg->spi->dev, "%s", logMessage);
		break;
	}

	dev_info(&halCfg->spi->dev, "MESSAGE: %s\n", logMessage);
					    ...
					    void *const value)
{
	if (tlsType == HAL_TLS_END) {
		...
		linux_adrv904x_TlsSet(HAL_TLS_ERR, NULL);
		linux_adrv904x_TlsSet(HAL_TLS_USR, NULL);
...
ADI_API adi_hal_Err_e linux_adrv904x_MutexInit(adi_hal_mutex_t __always_unused *mutex)
{
	return ADI_HAL_ERR_OK;
}

ADI_API adi_hal_Err_e linux_adrv904x_MutexDestroy(adi_hal_mutex_t __always_unused *mutex)
{
	return ADI_HAL_ERR_OK;
}

ADI_API adi_hal_Err_e linux_adrv904x_MutexLock(adi_hal_mutex_t __always_unused *mutex)
{
	return ADI_HAL_ERR_OK;
}

ADI_API adi_hal_Err_e linux_adrv904x_MutexUnlock(adi_hal_mutex_t __always_unused *mutex)
{
	return ADI_HAL_ERR_OK;
}
...
{
	FILE *stream = kzalloc(sizeof(*stream), GFP_KERNEL);
	int ret;
