...
static int adrv9104_live_profile_show(struct seq_file *s, void *ignored)
{
	struct adrv9104_rf_phy *phy = s->private;
	int n;

	char *buf __free(kfree) = kzalloc(ADRV9104_PROFILE_MAX_SZ, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	scoped_guard(mutex, &phy->lock) {
		n = adrv9104_profile_serialize(phy, buf, ADRV9104_PROFILE_MAX_SZ);
		if (n < 0)
			return n;
	}
...
DEFINE_SHOW_ATTRIBUTE(adrv9104_live_profile);
...
void adrv9104_debugfs_create(struct adrv9104_rf_phy *phy, struct iio_dev *indio_dev)
{
	...
	debugfs_create_file("live_profile", 0400, d, phy, &adrv9104_live_profile_fops);
