				     ...
				     const struct iio_chan_spec *iio_chan, int val, int val2)
{
	...
	carrier_freq_hz = iio_val_s64_compose(val, val2);
				     ...
				     const struct iio_chan_spec *iio_chan, int *val, int *val2)
{
	...
	iio_val_s64_decompose(carrier.carrierFrequency_Hz, val, val2);
...
static int adrv9104_digital_init(struct adrv9104_rf_phy *phy)
{
	...
	/* I need to rethink the below! It's just wrong! Works (for now) but wrong */
...
int adrv9104_init_with_profile(struct adrv9104_rf_phy *phy, const deviceProfileBundle_t *bundle)
{
	guard(mutex)(&phy->lock);
	if (bundle)
		memcpy(&phy->profile_bundle, bundle, sizeof(phy->profile_bundle));
...
static int adrv9104_probe(struct spi_device *spi)
{
	...
	ret = adrv9104_profile_register(phy, indio_dev);
	if (ret)
		return ret;
