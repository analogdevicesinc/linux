...
static ssize_t gt1x_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	...
	ret = raw_copy_from_user(&gesture_enabled, buff, 1);
	if (ret) {
...
static s32 io_iic_read(u8 * data, void __user * arg)
{
	...
	err = raw_copy_from_user(data, arg, CMD_HEAD_LENGTH);
	if (err) {
	...
	if (!err) {
		err = raw_copy_to_user(&((u8 __user *) arg)[CMD_HEAD_LENGTH], &data[CMD_HEAD_LENGTH], data_length);
		if (err) {
...
static long gt1x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd)) {
		...
		if (_IOC_DIR(cmd) & _IOC_WRITE) {
			err = raw_copy_from_user(data, (void __user *)arg, data_length);
			if (err) {
				GTP_ERROR("Can't access the memory.");
				kfree(data);
				return -1;
	...
	case IO_GET_VERISON:
		if ((u8 __user *) arg) {
			ret = raw_copy_to_user(((u8 __user *) arg), IO_VERSION, sizeof(IO_VERSION));
			if (!ret) {
