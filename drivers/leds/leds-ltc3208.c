...
struct ltc3208_dev {
	...
	struct gpio_desc *enrgbs_gpio;
	struct gpio_desc *camhl_gpio;
};
...
static ssize_t enrgbs_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	...
	gpiod_set_value(data->enrgbs_gpio, !!val);
	return count;
}

static ssize_t enrgbs_show(struct device *dev,
...
static ssize_t camhl_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct ltc3208_dev *data = dev_get_drvdata(dev);
	...
	gpiod_set_value(data->camhl_gpio, !!val);
	return count;
}

static ssize_t camhl_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ltc3208_dev *data = dev_get_drvdata(dev);
	int ret;
...
static int ltc3208_probe(struct i2c_client *client)
{
	...
	data->enrgbs_gpio = devm_gpiod_get_optional(&client->dev,
					"enrgbs", GPIOD_OUT_LOW);
	if (IS_ERR(data->enrgbs_gpio))
		return dev_err_probe(&client->dev, PTR_ERR(data->enrgbs_gpio),
				"Failed to get ENRGBS GPIO control\n");

	data->camhl_gpio = devm_gpiod_get_optional(&client->dev,
					"camhl", GPIOD_OUT_LOW);
	if (IS_ERR(data->camhl_gpio))
		return dev_err_probe(&client->dev, PTR_ERR(data->camhl_gpio),
				"Failed to get CAMHL GPIO control\n");

	ret = sysfs_create_group(&client->dev.kobj, &ltc3208_attr_group);
	if (ret)
		return dev_err_probe(&client->dev, ret,
			"Failed to create sysfs attributes for GPIO\n");
	...
	ret = device_property_match_property_string(&client->dev,
			"adi,force-cpo-level", ltc3208_cpo_opt,
			LTC3208_MAX_CPO_OPT);
	if (ret < 0)
		return dev_err_probe(&client->dev, ret,
	...
	for (i = 0; i <= LTC3208_NUM_AUX_LEDS; i++) {
		ret = device_property_match_property_string(&client->dev,
			ltc3208_dt_aux_channels[i], ltc3208_aux_opt,
			LTC3208_NUM_AUX_OPT);
		...
		if (ret == -EINVAL)
			...
			return dev_err_probe(&client->dev, ret,
				"Failed getting aux-channel name.\n");
	}
