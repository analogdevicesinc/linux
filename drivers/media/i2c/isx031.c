...
#define ISX031_WIDTH 1920
#define ISX031_HEIGHT 1536
...
static const char *const isx031_ctrl_test_pattern_options[] = {
	"Disabled",
	"Enabled",
};
...
static int isx031_write_mode_set_f_lock(struct isx031 *sensor, u8 val)
{
	int ret;

	fsleep(20000);

	ret = isx031_write_reg(sensor, 0x8a55, 0x06);
	if (ret) {
		return ret;
	}
	fsleep(20000);

	ret = isx031_write_reg(sensor, 0xbef0, val);
	if (ret) {
		return ret;
	}
	fsleep(20000);

	return isx031_write_reg(sensor, 0x8a55, 0x02);
}
...
static int isx031_set_response_mode(struct isx031 *sensor)
{
	...
	ret = isx031_write_mode_set_f_lock(sensor, 0x53);
	if (ret) {
		return ret;
	}
	fsleep(100000);
	...
	ret = isx031_read_reg(sensor, 0x8a55, &r_val);
	if (ret) {
		return ret;
	}
...
static int isx031_set_fsync_trigger_mode(struct isx031 *sensor)
{
	int ret;
	...
	ret = isx031_write_reg(sensor, 0x8af0, 0x02);
	if (ret) {
		return ret;
	}
	fsleep(120000);
	...
	/* APL mirror */
	ret = isx031_write_reg(sensor, 0xbf14, 0x02);
	if (ret) {
		return ret;
	}
	fsleep(240000);
...
static int isx031_set_crop(struct isx031 *sensor, u16 v_size, u16 v_offset)
{
	int ret;

	ret = isx031_write_reg(sensor, 0x8ada, 0x01);
	if (ret) {
		return ret;
	}
	...
	ret = isx031_write_reg16(sensor, 0x8ab0, v_offset);
	if (ret) {
		return ret;
	}

	ret = isx031_write_reg16(sensor, 0x8aae, v_size);
	if (ret) {
		return ret;
	}
	...
	ret = isx031_write_reg16(sensor, 0xbf0c, v_offset);
	if (ret) {
		return ret;
	}

	return isx031_write_reg16(sensor, 0xbf0a, v_size);
}
...
static int isx031_set_gain(struct isx031 *sensor, s64 val)
{
	...
	ret = isx031_write_reg(sensor, 0xabf0, 0x01);
	if (ret) {
		return ret;
	}

	ret = isx031_write_reg(sensor, 0xabf8, 0x01);
	if (ret) {
		return ret;
	}
	...
	//	ret = isx031_write_reg16(sensor, 0x6170, gain);
	//	if (ret)
	//    {
	//		return ret;
	//	}
...
static int isx031_set_tpg(struct isx031 *sensor, s32 val)
{
	u32 enabled = val ? 1 : 0;
	int ret;

	ret = isx031_write_reg(sensor, 0xbe14, enabled);
	if (ret) {
		return ret;
	}

	/* APL mirror */
	return isx031_write_reg(sensor, 0xbf60, enabled);
}
...
static int isx031_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct isx031 *sensor =
		container_of(ctrl->handler, struct isx031, ctrls);
	bool enable;
	...
	case V4L2_CID_TPG: {
		return isx031_set_tpg(sensor, ctrl->val);
	}
	default: {
		return -EINVAL;
	}
...
static const struct v4l2_ctrl_config isx031_ctrl_tpg = {
	.ops = &isx031_ctrl_ops,
	.id = V4L2_CID_TPG,
	.name = "Test Pattern Generator",
	.type = V4L2_CTRL_TYPE_MENU,
	.max = ARRAY_SIZE(isx031_ctrl_test_pattern_options) - 1,
	.def = 0,
	.qmenu = isx031_ctrl_test_pattern_options,
};
...
static int isx031_ctrls_init(struct isx031 *sensor)
{
	...
	v4l2_ctrl_new_std(&sensor->ctrls, &isx031_ctrl_ops, V4L2_CID_EXPOSURE,
			  0, 33000, 1, 11010);
	...
	ret = v4l2_ctrl_new_fwnode_properties(&sensor->ctrls, &isx031_ctrl_ops,
					      &props);
	if (ret) {
		goto free_ctrls;
	}
	...
	v4l2_ctrl_new_custom(&sensor->ctrls, &isx031_ctrl_tpg, NULL);
...
static int isx031_setup(struct isx031 *sensor, struct v4l2_subdev_state *state)
{
	return 0;
}
...
static int isx031_stream_on(struct isx031 *sensor)
{
	...
	if (sensor->trigger_mode) {
		ret = isx031_set_fsync_trigger_mode(sensor);
		if (ret) {
			return ret;
		}
		dev_dbg(sensor->dev,
			"Camera sensor configured for FSYNC slave mode\n");
	}
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->index >= 1 || fse->code != isx031_mbus_code(sensor)) {
		return -EINVAL;
	}

	fse->min_width = ISX031_WIDTH;
	fse->max_width = ISX031_WIDTH;
	fse->min_height = ISX031_HEIGHT;
	fse->max_height = ISX031_HEIGHT;

	return 0;
}

static int
isx031_enum_frame_interval(struct v4l2_subdev *sd,
			   struct v4l2_subdev_state *state,
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	format->width = ISX031_WIDTH;
	format->height = ISX031_HEIGHT;
...
static const struct dev_pm_ops isx031_pm_ops = { SET_RUNTIME_PM_OPS(
	isx031_runtime_suspend, isx031_runtime_resume, NULL) };
...
static int isx031_identify(struct isx031 *sensor)
{
	...
	if (chip_id != 0x6031) {
		dev_err(sensor->dev,
			"Unexpected chip ID 0x%04x (expected 0x2401)\n",
			chip_id);
