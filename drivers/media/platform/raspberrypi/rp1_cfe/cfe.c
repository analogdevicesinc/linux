...
static inline struct cfe_graph_entity *to_cfe_entity(struct v4l2_async_subdev *asd)
{
...
int cfe_graph_pipeline_start_stop(struct cfe_device *cfe, bool on)
{
	...
	list_for_each_entry(asd, &cfe->notifier.asd_list, asd_list) {
		struct cfe_graph_entity *entity;
		bool state;
...
static int cfe_probe_complete(struct cfe_device *cfe)
{
	...
	asd = list_first_entry(&cfe->notifier.asd_list, struct v4l2_async_subdev, asd_list);
	entity = to_cfe_entity(asd);
			   ...
			   struct v4l2_async_subdev *unused)
{
	...
	list_for_each_entry(asd, &cfe->notifier.asd_list, asd_list) {
		...
		if (entity->subdev) {
			cfe_err("duplicate subdev for node %p\n",
	...
	cfe_err("no entity for subdev %s\n", subdev->name);
	return -EINVAL;
}
...
static int cfe_graph_init(struct cfe_device *cfe)
{
	...
	if (list_empty(&cfe->notifier.asd_list)) {
