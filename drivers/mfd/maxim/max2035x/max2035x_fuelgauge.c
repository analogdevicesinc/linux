...
static int max2035x_load_ini_file(struct max2035x_fuelgauge *fuelgauge)
{
	...
	if (section_ptr &&
	    sscanf(section_ptr, "DesignCap=%hx", &bat->designcap) == 1) {
		dev_dbg(fuelgauge->dev, "DesingCap found, using DesignCap: 0x%04x\n", bat->designcap);
	} else {
		bat->designcap = 0x1194;
		dev_dbg(fuelgauge->dev, "DesingCap not found, using DesignCap: 0x%04x\n", bat->designcap);
	}

	section_ptr = _strcasestr(ptr, "dPacc=");
	if (section_ptr &&
	    sscanf(section_ptr, "dPacc=%hx", &bat->dpacc) == 1) {
		dev_dbg(fuelgauge->dev, "dPacc found, using dPacc: 0x%04x\n", bat->dpacc);
	} else {
		bat->dpacc = 0x0C80;
		dev_dbg(fuelgauge->dev, "dPacc not found, using dPacc: 0x%04x\n", bat->dpacc);
	}

	section_ptr = _strcasestr(ptr, "dQacc=");
	if (section_ptr &&
	    sscanf(section_ptr, "dQacc=%hx", &bat->dqacc) == 1) {
		dev_dbg(fuelgauge->dev, "dQacc found, using dQacc: 0x%04x\n", bat->dqacc);
	} else {
		bat->dqacc = 0x08CA;
		dev_dbg(fuelgauge->dev, "dQacc not found, using dQacc: 0x%04x\n", bat->dqacc);
	}

	section_ptr = _strcasestr(ptr, "ICHGTerm=");
	if (section_ptr &&
	    sscanf(section_ptr, "ICHGTerm=%hx", &bat->ichgterm) == 1) {
		dev_dbg(fuelgauge->dev, "ICHGTerm found, using ICHGTerm: 0x%04x\n", bat->ichgterm);
	} else {
		bat->ichgterm = 0x03C0;
		dev_dbg(fuelgauge->dev, "ICHGTerm not found, using ICHGTerm: 0x%04x\n", bat->ichgterm);
	}

	section_ptr = _strcasestr(ptr, "learncfg=");
	if (section_ptr &&
	    sscanf(section_ptr, "learncfg=%hx", &bat->learncfg) == 1) {
		dev_dbg(fuelgauge->dev, "learncfg found, using learncfg: 0x%04x\n", bat->learncfg);
	} else {
		bat->learncfg = 0x4486;
		dev_dbg(fuelgauge->dev, "learncfg not found, using learncfg: 0x%04x\n", bat->learncfg);
	}

	section_ptr = _strcasestr(ptr, "misccfg=");
	if (section_ptr &&
	    sscanf(section_ptr, "misccfg=%hx", &bat->misccfg) == 1) {
		dev_dbg(fuelgauge->dev, "misccfg found, using misccfg: 0x%04x\n", bat->misccfg);
	} else {
		bat->misccfg = 0x3870;
		dev_dbg(fuelgauge->dev, "misccfg not found, using misccfg: 0x%04x\n", bat->misccfg);
	}

	section_ptr = _strcasestr(ptr, "QRTable00=");
	if (section_ptr &&
	    sscanf(section_ptr, "QRTable00=%hx", &bat->qr_table00) == 1) {
		dev_dbg(fuelgauge->dev, "QRTable00 found, using QRTable00: 0x%04x\n", bat->qr_table00);
	} else {
		bat->qr_table00 = 0x2C04;
		dev_dbg(fuelgauge->dev, "QRTable00 not found, using QRTable00: 0x%04x\n", bat->qr_table00);
	}

	section_ptr = _strcasestr(ptr, "QRTable10=");
	if (section_ptr &&
	    sscanf(section_ptr, "QRTable10=%hx", &bat->qr_table10) == 1) {
		dev_dbg(fuelgauge->dev, "QRTable10 found, using QRTable10: 0x%04x\n", bat->qr_table10);
	} else {
		bat->qr_table10 = 0x1601;
		dev_dbg(fuelgauge->dev, "QRTable10 not found, using QRTable10: 0x%04x\n", bat->qr_table10);
	}

	section_ptr = _strcasestr(ptr, "QRTable20=");
	if (section_ptr &&
	    sscanf(section_ptr, "QRTable20=%hx", &bat->qr_table20) == 1) {
		dev_dbg(fuelgauge->dev, "QRTable20 found, using QRTable20: 0x%04x\n", bat->qr_table20);
	} else {
		bat->qr_table20 = 0x0B00;
		dev_dbg(fuelgauge->dev, "QRTable20 not found, using QRTable20: 0x%04x\n", bat->qr_table20);
	}

	section_ptr = _strcasestr(ptr, "QRTable30=");
	if (section_ptr &&
	    sscanf(section_ptr, "QRTable30=%hx", &bat->qr_table30) == 1) {
		dev_dbg(fuelgauge->dev, "QRTable30 found, using QRTable30: 0x%04x\n", bat->qr_table30);
	} else {
		bat->qr_table30 = 0x0A80;
		dev_dbg(fuelgauge->dev, "QRTable30 not found, using QRTable30: 0x%04x\n", bat->qr_table30);
	}

	section_ptr = _strcasestr(ptr, "RCOMP0=");
	if (section_ptr &&
	    sscanf(section_ptr, "RCOMP0=%hx", &bat->rcomp0) == 1) {
		dev_dbg(fuelgauge->dev, "RCOMP0 found, using RCOMP0: 0x%04x\n", bat->rcomp0);
	} else {
		bat->rcomp0 = 0x0023;
		dev_dbg(fuelgauge->dev, "RCOMP0 not found, using RCOMP0: 0x%04x\n", bat->rcomp0);
	}

	section_ptr = _strcasestr(ptr, "relaxcfg=");
	if (section_ptr &&
	    sscanf(section_ptr, "relaxcfg=%hx", &bat->relaxcfg) == 1) {
		dev_dbg(fuelgauge->dev, "relaxcfg found, using relaxcfg: 0x%04x\n", bat->relaxcfg);
	} else {
		bat->relaxcfg = 0x2039;
		dev_dbg(fuelgauge->dev, "relaxcfg not found, using relaxcfg: 0x%04x\n", bat->relaxcfg);
	}

	section_ptr = _strcasestr(ptr, "TempCo=");
	if (section_ptr &&
	    sscanf(section_ptr, "TempCo=%hx", &bat->tempco) == 1) {
		dev_dbg(fuelgauge->dev, "TempCo found, using TempCo: 0x%04x\n", bat->tempco);
	} else {
		bat->tempco = 0x171F;
		dev_dbg(fuelgauge->dev, "TempCo not found, using TempCo: 0x%04x\n", bat->tempco);
	}

	section_ptr = _strcasestr(ptr, "Vempty=");
	if (section_ptr &&
	    sscanf(section_ptr, "Vempty=%hx", &bat->vempty) == 1) {
		dev_dbg(fuelgauge->dev, "Vempty found, using Vempty: 0x%04x\n", bat->vempty);
	} else {
		bat->vempty = 0xA561;
		dev_dbg(fuelgauge->dev, "Vempty not found, using Vempty: 0x%04x\n", bat->vempty);
	}

	section_ptr = _strcasestr(ptr, "RCOMPSEG=");
	if (section_ptr &&
	    sscanf(section_ptr, "RCOMPSEG=%hx", &bat->rcompseg) == 1) {
		dev_dbg(fuelgauge->dev, "RCOMPSEG found, using RCOMPSEG: 0x%04x\n", bat->rcompseg);
	} else {
		bat->rcompseg = 0x0080;
		dev_dbg(fuelgauge->dev, "RCOMPSEG not found, using RCOMPSEG: 0x%04x\n", bat->rcompseg);
	}

	section_ptr = _strcasestr(ptr, "fullcaprep=");
	if (section_ptr &&
	    sscanf(section_ptr, "fullcaprep=%hx", &bat->fullcaprep) == 1) {
		dev_dbg(fuelgauge->dev, "fullcaprep found, using fullcaprep: 0x%04x\n", bat->fullcaprep);
	} else {
		bat->fullcaprep = 0x1194;
		dev_dbg(fuelgauge->dev, "fullcaprep not found, using fullcaprep: 0x%04x\n", bat->fullcaprep);
	}

	section_ptr = _strcasestr(ptr, "fullcapnom=");
	if (section_ptr &&
	    sscanf(section_ptr, "fullcapnom=%hx", &bat->fullcapnom) == 1) {
		dev_dbg(fuelgauge->dev, "fullcapnom found, using fullcapnom: 0x%04x\n", bat->fullcapnom);
	} else {
		bat->fullcapnom = 0x1194;
		dev_dbg(fuelgauge->dev, "fullcapnom not found, using fullcapnom: 0x%04x\n", bat->fullcapnom);
	}

	section_ptr = _strcasestr(ptr, "Cycles=");
	if (section_ptr &&
	    sscanf(section_ptr, "Cycles=%hx", &bat->cycles) == 1) {
		dev_dbg(fuelgauge->dev, "Cycles found, using Cycles: 0x%04x\n", bat->cycles);
	} else {
		bat->cycles = 0x0000;
		dev_dbg(fuelgauge->dev, "Cycles not found, using Cycles: 0x%04x\n", bat->cycles);
	}

	section_ptr = _strcasestr(ptr, "Mixcap=");
	if (section_ptr &&
	    sscanf(section_ptr, "Mixcap=%hx", &bat->mixcap) == 1) {
		dev_dbg(fuelgauge->dev, "Mixcap found, using Mixcap: 0x%04x\n", bat->mixcap);
	} else {
		bat->mixcap = 0x017F;
		dev_dbg(fuelgauge->dev, "Mixcap not found, using Mixcap: 0x%04x\n", bat->mixcap);
	}

	section_ptr = _strcasestr(ptr, "config=");
	if (section_ptr &&
	    sscanf(section_ptr, "config=%hx", &bat->config) == 1) {
		dev_dbg(fuelgauge->dev, "config found, using config: 0x%04x\n", bat->config);
	} else {
		bat->config = 0x2210;
		dev_dbg(fuelgauge->dev, "config not found, using config: 0x%04x\n", bat->config);
	}

	section_ptr = _strcasestr(ptr, "config2=");
	if (section_ptr &&
	    sscanf(section_ptr, "config2=%hx", &bat->config2) == 1) {
		dev_dbg(fuelgauge->dev, "config2 found, using config2: 0x%04x\n", bat->config2);
	} else {
		bat->config2 = 0x0658;
		dev_dbg(fuelgauge->dev, "config2 not found, using config2: 0x%04x\n", bat->config2);
	}

	section_ptr = _strcasestr(ptr, "FullSOCthr=");
	if (section_ptr &&
	    sscanf(section_ptr, "FullSOCthr=%hx", &bat->fullsocthr) == 1) {
		dev_dbg(fuelgauge->dev, "FullSOCthr found, using FullSOCthr: 0x%04x\n", bat->fullsocthr);
	} else {
		bat->fullsocthr = 0x5F00;
		dev_dbg(fuelgauge->dev, "FullSOCthr not found, using FullSOCthr: 0x%04x\n", bat->fullsocthr);
	}

	section_ptr = _strcasestr(ptr, "TGain=");
	if (section_ptr &&
	    sscanf(section_ptr, "TGain=%hx", &bat->tgain) == 1) {
		dev_dbg(fuelgauge->dev, "TGain found, using TGain: 0x%04x\n", bat->tgain);
	} else {
		bat->tgain = 0xEE56;
		dev_dbg(fuelgauge->dev, "TGain not found, using TGain: 0x%04x\n", bat->tgain);
	}

	section_ptr = _strcasestr(ptr, "TOff=");
	if (section_ptr &&
	    sscanf(section_ptr, "TOff=%hx", &bat->toff) == 1) {
		dev_dbg(fuelgauge->dev, "TOff found, using TOff: 0x%04x\n", bat->toff);
	} else {
		bat->toff = 0x1DA4;
		dev_dbg(fuelgauge->dev, "TOff not found, using TOff: 0x%04x\n", bat->toff);
	}

	section_ptr = _strcasestr(ptr, "Curve=");
	if (section_ptr &&
	    sscanf(section_ptr, "Curve=%hx", &bat->curve) == 1) {
		dev_dbg(fuelgauge->dev, "Curve found, using Curve: 0x%04x\n", bat->curve);
	} else {
		bat->curve = 0x3025;
		dev_dbg(fuelgauge->dev, "Curve not found, using Curve: 0x%04x\n", bat->curve);
	}
...
static void max2035x_custom_full_ini(struct max2035x_fuelgauge *fuelgauge)
{
	...
	for (i = 0x80; i <= 0x9F; i++) {
		...
		if (reg_val != 0x0000 && reg_val != 0xFFFF) {
			dev_warn(fuelgauge->dev, "%s: Model lock verify: 0x%02x reads 0x%04x\n",
				__func__, i, reg_val);
		}
	}
...
static int max2035x_fuelgauge_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "%s : INI load failed (%d), skipping FG init\n",
			__func__, ret);
	} else {
