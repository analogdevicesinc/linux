 ...
 *
 * This function writes a value to a register in the AXI HSCI device. It first prints a
 * debug message indicating the register address, physical address, and value being written.
 * Then, it uses the iowrite32() function to write the value to the register. !TODO: If the debug
 * flag is enabled, it reads the value from the register and compares it with the value that was
 * written. If the values do not match, it prints an error message indicating the register address,
 * physical address, and the expected and actual values.
 */
 ...
 *
 * Returns 0 in success, -ENODEV otherwise
 */
int axi_hsci_silent(struct axi_hsci_state *st, bool enable)
{
	guard(mutex)(&st->update_lock);
	if (!st->initialized)
		return -ENODEV;
 ...
 *
 * Return: Always returns 0 unless the device is not present (-ENODEV).
 */
 ...
 */
int axi_hsci_readm(struct axi_hsci_state *st, const u8 *tx_data, u8 *rx_data, u32 num_tx_rx_bytes,
		   u8 addr_len, u8 data_len, u32 stream_len)
{
	int ret = -EFAULT;
	u32 addr = 0;
	...
	if (!axi_hsci_wait_done(st)) {
		axi_hsci_write(st, AXI_HSCI_BRAM_ADDR, bram_addr);
		axi_hsci_write(st, bram_addr, addr);
		axi_hsci_write(st, AXI_HSCI_XFER_NUM, 1);
		axi_hsci_write(st, AXI_HSCI_ADDR_SIZE, 3);
		axi_hsci_write(st, AXI_HSCI_BYTE_NUM, len);
		axi_hsci_write(st, AXI_HSCI_CTRL, ((0 & 0x3) << 4) | (1 & 0x3));
		axi_hsci_write(st, AXI_HSCI_RUN, 1);
		...
		if (!axi_hsci_wait_done(st)) {
			if (stream_len >= 2) {
				...
				for (i = 0; i < byte_size; i++)
					rx_data[4 + (word_size * 4) + i] = rdData >> (8 * i);
...
int axi_hsci_writem(struct axi_hsci_state *st, const u8 *tx_data,
		    u32 num_tx_rx_bytes, u8 addr_len, u8 data_len, u32 stream_len)
{
	int ret = -EFAULT;
	u32 addr = 0;
	...
	if (axi_hsci_wait_done(st) == 0) {
		...
		if (stream_len >= 2) {
			...
			if (word_size != 0) {
				axi_hsci_write(st, AXI_HSCI_XFER_NUM, 1);
				axi_hsci_write(st, AXI_HSCI_ADDR_SIZE, 3);
				axi_hsci_write(st, AXI_HSCI_BYTE_NUM, (word_size * 4));
				axi_hsci_write(st, AXI_HSCI_CTRL, ((1 & 0x3) << 4) | (0 & 0x3));
				axi_hsci_write(st, AXI_HSCI_RUN, 1);
				if (axi_hsci_wait_done(st) == 0)
					ret = 0;
			}
			...
			if (byte_size != 0) {
				...
				axi_hsci_write(st, AXI_HSCI_XFER_NUM, 1);
				axi_hsci_write(st, AXI_HSCI_ADDR_SIZE, 3);
				axi_hsci_write(st, AXI_HSCI_BYTE_NUM, byte_size);
				axi_hsci_write(st, AXI_HSCI_CTRL, ((0 & 0x3) << 4) | (0 & 0x3));
				axi_hsci_write(st, AXI_HSCI_RUN, 1);
				if (axi_hsci_wait_done(st) == 0)
					ret = 0;
			}
			...
			axi_hsci_write(st, AXI_HSCI_XFER_NUM, 1);
			axi_hsci_write(st, AXI_HSCI_ADDR_SIZE, 3);
			/* bytes can be transferred for single transaction */
			axi_hsci_write(st, AXI_HSCI_BYTE_NUM, data_len);
			/* HSCI SLAVE AHB TSIZE and CMD SEL */
			axi_hsci_write(st, AXI_HSCI_CTRL, ((0 & 0x3) << 4) | (0 & 0x3));
			axi_hsci_write(st, AXI_HSCI_RUN, 1);
			if (axi_hsci_wait_done(st) == 0)
				ret = 0;
			    ...
			    int shift)
{
	...
	scoped_guard(mutex, &st->update_lock)
...
struct axi_hsci_state *devm_axi_hsci_get_optional(struct device *dev)
{
	...
	struct device_node *of_node __free(device_node) = of_parse_phandle(dev->of_node,
									   "adi,axi-hsci-connected",
									   0);
	if (!of_node)
		return NULL;

	guard(mutex)(&probed_devices_lock);
...
static void axi_hsci_clk_disable(void *clk)
{
...
static int axi_hsci_probe(struct platform_device *pdev)
{
	...
	ret = devm_mutex_init(&pdev->dev, &st->update_lock);
	if (ret)
		return ret;
	...
	ret = devm_add_action_or_reset(&pdev->dev, axi_hsci_clk_disable, st->pclk);
	if (ret)
		return ret;
	...
	if (reg_val != 0x3F)
		dev_warn(&pdev->dev, "HSCI PHY is 0x%X, expected 0x3F\n", reg_val);
	...
	/* Done */
	dev_dbg(&pdev->dev, "AXI HSCI IP core (%d.0.0) probed @ %u MHz\n",
