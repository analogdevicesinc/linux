...
#define ADI_PCIE_INTC_VEC_CLEAR		0x04c
...
static void adi_pcie_msi_chained_handler(struct irq_desc *desc)
{
	...
	adi_pcie_irq_dispatch(apo, readl(apo->intc + ADI_PCIE_INTC_VEC_CLAIM(vec->index)));
...
static irqreturn_t adi_pcie_intx_handler(int irq, void *data)
{
	...
	return adi_pcie_irq_dispatch(apo, readl(apo->intc + ADI_PCIE_INTC_SRC_CLAIM));
}
