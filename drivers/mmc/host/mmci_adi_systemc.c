...
static void adi_systemc_variant_init(struct mmci_host *host);
static void adi_systemc_init_card(struct mmc_host *host, struct mmc_card *card);
...
static struct variant_data variant_adi_systemc = {
        .fifosize               = 16 * 4,
        .fifohalfsize           = 8 * 4,
        .cmdreg_cpsm_enable     = MCI_CPSM_ENABLE,
        .cmdreg_lrsp_crc        = MCI_CPSM_RESPONSE | MCI_CPSM_LONGRSP,
...
{
	host->ops = &mmci_variant_ops;
	host->mmc_ops->init_card = adi_systemc_init_card;
...
static void adi_systemc_init_card(struct mmc_host *host, struct mmc_card *card)
{
	/* The Arm Fast Model for MMC doesn't seem to respond well to SEND_EXT_CSD,
	 * so we fake out a response from the chip for the sector size here.
	 * This should be sufficient for simulation purposes, but of course
	 * could never work in a production driver.
	 */
	/* Set number of sectors to equal a 2GB disk (512 bytes/sector) */
	card->ext_csd.sectors = 4194304;
}
...
static const struct amba_id mmci_ids[] = {
	...
	/* ADI variant for SystemC models */
	{
		.id	= 0xF0041180,
		.mask	= 0xff0fffff,
		.data	= &variant_adi_systemc,
	},
	{ 0, 0 },
};
