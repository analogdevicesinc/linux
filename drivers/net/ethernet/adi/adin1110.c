...
struct rtnl_link_stats64 *adin1110_ndo_get_stats64(struct net_device *netdev,
...
static const struct net_device_ops adin1110_netdev_ops = {
	...
	.ndo_get_phys_port_id	= adin1110_port_get_port_parent_id,
...
static int adin1110_check_spi(struct adin1110_priv *priv)
{
	...
	if (reset_gpio) {
		 ...
		 */
		spi_bus_lock(priv->spidev->master);
		...
		spi_bus_unlock(priv->spidev->master);
	}
...
static int adin1110_port_attr_get(struct net_device *dev,
				struct switchdev_attr *attr)
{
	...
	case SWITCHDEV_ATTR_ID_PORT_PARENT_ID:
		attr->u.ppid.id_len = strnlen(priv->mii_bus_name, MAX_PHYS_ITEM_ID_LEN);
		memcpy(&attr->u.ppid.id,  priv->mii_bus_name, attr->u.ppid.id_len);
		break;
	default:
		return -EOPNOTSUPP;
	}
...
static int adin1110_port_attr_set(struct net_device *dev, const struct switchdev_attr *attr,
				  struct switchdev_trans *trans)
{
	struct adin1110_port_priv *port_priv = netdev_priv(dev);
	int err = 0;
	...
	case SWITCHDEV_ATTR_ID_PORT_STP_STATE:
		err = adin1110_port_attr_stp_state_set(port_priv, attr->u.stp_state);
		break;
	default:
		err = -EOPNOTSUPP;
		break;
...
static int adin1110_port_obj_add(struct net_device *dev,
			         ...
			         struct switchdev_trans *trans)
{
	...
	case SWITCHDEV_OBJ_ID_PORT_FDB:
		err = adin1110_fdb_add(port_priv, SWITCHDEV_OBJ_PORT_FDB(obj), trans);
		break;
	default:
		err = -EOPNOTSUPP;
		break;
...
static int adin1110_port_obj_del(struct net_device *dev,
			       const struct switchdev_obj *obj)
{
	struct adin1110_port_priv *port_priv = netdev_priv(dev);
	int err = 0;
	...
	switch (obj->id) {
	case SWITCHDEV_OBJ_ID_PORT_FDB:
		err = adin1110_fdb_del(port_priv, SWITCHDEV_OBJ_PORT_FDB(obj));
		break;
	default:
		err = -EOPNOTSUPP;
		break;
...
static const struct switchdev_ops rocker_port_switchdev_ops = {
	.switchdev_port_attr_get	= adin1110_port_attr_get,
	.switchdev_port_attr_set	= adin1110_port_attr_set,
	.switchdev_port_obj_add		= adin1110_port_obj_add,
	.switchdev_port_obj_del		= adin1110_port_obj_del,
};
...
{
	return register_netdevice_notifier(&adin1110_netdevice_nb);
}
...
static int adin1110_probe_netdevs(struct adin1110_priv *priv)
{
	...
	for (i = 0; i < priv->cfg->ports_nr; i++) {
		ret = register_netdev(priv->ports[i]->netdev);
		if (ret < 0) {
