...
#define   ADIN_AN_ADV_ABILITY_L_FORCE_MS	BIT(12)

#define ADIN_AN_ADV_ABILITY_M			0x0203
#define   ADIN_AN_ADV_ABILITY_M_ADV_MST		BIT(4)
...
#define ADIN_AN_STATUS_EXTRA			0x8001
#define   ADIN_AN_MS_CONFIG_RSLTN_MSK		GENMASK(6, 5)
#define   ADIN_AN_MS_CONFIG_RSLTN_GET(x)	\
			FIELD_GET(ADIN_AN_MS_CONFIG_RSLTN_MSK, x)
...
static int adin_read_master_slave(struct phy_device *phydev)
{
	bool force, master;
	int cfg, state;
	int val;
	...
	val = phy_read_mmd(phydev, MDIO_MMD_AN, ADIN_AN_STATUS_EXTRA);
	if (val < 0)
		return val;

	switch (ADIN_AN_MS_CONFIG_RSLTN_GET(val)) {
	case 3:
	...
	val = phy_read_mmd(phydev, MDIO_MMD_AN, ADIN_AN_ADV_ABILITY_L);
	if (val < 0)
		return val;

	force = !!(val & ADIN_AN_ADV_ABILITY_L_FORCE_MS);


	val = phy_read_mmd(phydev, MDIO_MMD_AN, ADIN_AN_ADV_ABILITY_M);
	if (val < 0)
		return val;

	master = !!(val & ADIN_AN_ADV_ABILITY_M_ADV_MST);
...
static int adin_read_status(struct phy_device *phydev)
{
	...
	ret = adin_read_master_slave(phydev);
	if (ret)
		return ret;
...
static int adin_setup_master_slave(struct phy_device *phydev)
{
	bool force = false;
	bool master = false;
	int ret;
	...
	if (force)
		ret = phy_set_bits_mmd(phydev, MDIO_MMD_AN,
				       ADIN_AN_ADV_ABILITY_L,
				       ADIN_AN_ADV_ABILITY_L_FORCE_MS);
	else
	...
	if (master)
		return phy_set_bits_mmd(phydev, MDIO_MMD_AN,
					ADIN_AN_ADV_ABILITY_M,
					ADIN_AN_ADV_ABILITY_M_ADV_MST);

	return phy_clear_bits_mmd(phydev, MDIO_MMD_AN, ADIN_AN_ADV_ABILITY_M,
				  ADIN_AN_ADV_ABILITY_M_ADV_MST);
}
static int adin_config_aneg(struct phy_device *phydev)
{
	...
	ret = adin_setup_master_slave(phydev);
	if (ret < 0)
		return ret;
