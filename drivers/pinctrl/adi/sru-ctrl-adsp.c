...
static const struct dai_destination dai0_destinations[] = {
	...
	{ "SPDIF0_TX_CLK_I", 0x5, 0x02, REG_DAI_CLK2, REG_DAI_EXTD_CLK2,
	 GROUP_A },
	{ "PDM0_CLK0_I", 0x5, 0x03, REG_DAI_CLK2, REG_DAI_EXTD_CLK2,
	 GROUP_A },
	{ "PDM0_BCLK_I", 0x5, 0x04, REG_DAI_CLK2, REG_DAI_EXTD_CLK2,
	 GROUP_A },
	...
	{ "SPT3_BCLK_I", 0x5, 0x01, REG_DAI_CLK5, REG_DAI_EXTD_CLK5,
	 GROUP_A },
	{ "PCG0_SYNC_CLKE_I", 0x5, 0x02, REG_DAI_CLK5, REG_DAI_EXTD_CLK5,
	 GROUP_A },
	{ "PCG0_SYNC_CLKF_I", 0x5, 0x03, REG_DAI_CLK5, REG_DAI_EXTD_CLK5,
	 GROUP_A },
	{ "PCG0_EXTCLKE_I", 0x5, 0x04, REG_DAI_CLK5, REG_DAI_EXTD_CLK5,
	 GROUP_A },
	{ "PCG0_EXTCLKF_I", 0x5, 0x05, REG_DAI_CLK5, REG_DAI_EXTD_CLK5,
	 GROUP_A },
	...
	{ "DAI0_PB20_I", 0x7, 0x03, REG_DAI_PIN4, REG_DAI_EXTD_PIN4,
	 GROUP_D },
	{ "INV_DAI0_PB19_I", 0x1, 0x28, REG_DAI_PIN4, REG_DAI_EXTD_PIN4,
	 GROUP_D },
	{ "INV_DAI0_PB20_I", 0x1, 0x29, REG_DAI_PIN4, REG_DAI_EXTD_PIN4,
	 GROUP_D },
			       ...
			       unsigned int func, unsigned int group)
{
	...
	register_val &= ~GENMASK((width * input) + width - 1, (width * input));
	register_val |= selection_code << width * input;
	...
	if (adsp_sru_ctrl->has_extended) {
		if (func >= DAI0_SOURCE_COUNT
		    && func <= (DAI0_SOURCE_COUNT + DAI1_SOURCE_COUNT - 1)) {
			switch (width) {
			case 0x5:
				ext_bits = 0x3;
				break;
				     ...
				     struct pinctrl_desc *desc)
{
	...
	all_pins =
	    devm_kcalloc(dev, sizeof(*all_pins), adsp_sru_ctrl->dest_count,
			 GFP_KERNEL);

	adsp_sru_ctrl->pins =
	    devm_kcalloc(dev, sizeof(int), adsp_sru_ctrl->dest_count,
			 GFP_KERNEL);
		...
		if (p1 && p2) {
			...
			if (adsp_sru_ctrl->dai == 0) {
				if (destination >= DAI0_DST_DTS_BINDING &&
					...
					pos =
					    destination - DAI0_DST_DTS_BINDING;
					dest_name =
					    adsp_sru_ctrl->
			...
			if (adsp_sru_ctrl->dai == 1) {
				if (destination >= DAI1_DST_DTS_BINDING &&
					...
					pos =
					    destination - DAI1_DST_DTS_BINDING;
					dest_name =
					    adsp_sru_ctrl->
					    dest[pos].dest_signal;
				} else {
			...
			if (adsp_sru_ctrl->has_extended
				...
				if (source >= DAI0_SRC_DTS_BINDING
					...
					source_name =
					    dai0_sources[pos].source_signal;
				}
			}
			...
			if (adsp_sru_ctrl->has_extended
				...
				if (source >= DAI1_SRC_DTS_BINDING
					...
					source_name =
					    dai1_sources[pos].source_signal;
				}
			}
...
static const int adsp_sru_ctrl_probe(struct platform_device *pdev)
{
	...
	regmap =
	    system_config_regmap_lookup_by_phandle(np, "adi,system-config");
	if (IS_ERR(regmap)) {
