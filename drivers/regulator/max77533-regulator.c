	...
	if (!ret) {
		...
		if (val == 500000)
			...
			return dev_err_probe(dev, -EINVAL,
					     "Invalid adi,peak-current-limit-microamp: %u\n",
					     val);

		ret = regmap_update_bits(max77533->regmap, MAX77533_REG_CFG,
					 MAX77533_BIT_IPEAK,
...
static int max77533_probe(struct i2c_client *client)
{
	...
	if (max77533->pok_gpio) {
		...
		if (max77533->pok_irq < 0)
			return dev_err_probe(dev, max77533->pok_irq,
					     "Failed to get POK IRQ\n");
...
MODULE_AUTHOR("Sean Park <sean.park@analog.com>");
