				    ...
				    unsigned long parent_rate)
{
	...
	if (max31335->id == ID_MAX31331)
		ret = regmap_update_bits(max31335->regmap, MAX31331_RTC_CONFIG2,
					freq_mask, index);
	else
		ret = regmap_update_bits(max31335->regmap, MAX31335_RTC_CONFIG2,
					freq_mask, index);
...
static int max31335_clkout_enable(struct clk_hw *hw)
{
	...
	if (max31335->id == ID_MAX31331)
		return regmap_set_bits(max31335->regmap, MAX31331_RTC_CONFIG2,
...
static void max31335_clkout_disable(struct clk_hw *hw)
{
	...
	if (max31335->id == ID_MAX31331)
		regmap_clear_bits(max31335->regmap, MAX31331_RTC_CONFIG2,
			      MAX31335_RTC_CONFIG2_ENCLKO);
	else
