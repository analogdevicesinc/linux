...
static void adi_uart4_serial_stop_tx(struct uart_port *port)
{
	...
	if (uart->tx_dma_channel) {
		if (!uart->tx_done)
			dma_unmap_sg(uart->dev, &uart->tx_sgl, 1,
				     DMA_TO_DEVICE);
...
static void adi_uart4_serial_dma_rx_chars(struct adi_uart4_serial_port *uart)
{
	...
	for (i = uart->rx_dma_buf.tail;; i++) {
		...
		if (!uart_handle_sysrq_char
		    (&uart->port, uart->rx_dma_buf.buf[i]))
...
void adi_uart4_serial_rx_dma_timeout(struct timer_list *list)
{
	...
	status =
	    dmaengine_tx_status(uart->rx_dma_channel, uart->rx_cookie, &state);
...
static void adi_uart4_serial_dma_tx(void *data)
{
	...
	if (!(xmit->tail == 0 && xmit->head == 0)) {
		xmit->tail =
		    (xmit->tail + uart->tx_count) & (UART_XMIT_SIZE - 1);
...
static int adi_uart4_serial_startup(struct uart_port *port)
{
	...
	if (uart->tx_dma_channel) {
		// RX channel
		uart->rx_dma_buf.buf =
		    dmam_alloc_coherent(uart->dev, UART_XMIT_SIZE,
		...
		if (ret) {
			dev_err(uart->dev,
				"Error configuring RX DMA channel\n");
			return -EINVAL;
		}

		desc =
		    dmaengine_prep_dma_cyclic(uart->rx_dma_channel,
					      uart->rx_dma_phy, UART_XMIT_SIZE,
					      DMA_RX_XCOUNT, DMA_DEV_TO_MEM,
					      DMA_PREP_INTERRUPT);
		...
		if (ret) {
			dev_err(uart->dev,
				"Error configuring TX DMA channel\n");
			return -EINVAL;
		}
	}
...
static int adi_uart4_serial_probe(struct platform_device *pdev)
{
	...
	if (adi_uart4_serial_ports[uartid] == NULL) {
		...
		if (uart->hwflow_mode == ADI_UART_HWFLOW_PERI) {
			uart->hwflow_en_pin =
			    devm_gpiod_get(dev, "hwflow-en", GPIOD_OUT_HIGH);
...
static inline void adi_uart_write(struct uart_port *port, u32 val, u32 off)
{
