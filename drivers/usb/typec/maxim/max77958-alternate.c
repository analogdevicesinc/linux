...
/*
 * MAX77958 DisplayPort Alternate Mode Driver
 *
...
static void max77958_vdm_process_printf(char *type, char *vdm_data, int len)
{
...
static void max77958_request_vdm(struct max77958_usbc_platform_data *usbpd_data, struct vdm_info *vdm, bool in_lock)
{
	/* send the opcode */
	...
	if (in_lock)
		max77958_usbc_opcode_push_locked(usbpd_data, &write_data);
	else
		max77958_usbc_opcode_write(usbpd_data, &write_data);
}
...
void max77958_set_discover_identity(void *data)
{
	...
	max77958_request_vdm(usbpd_data, &vdm, false);
}

static void max77958_set_dp_enter_mode(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_info vdm;
	...
	max77958_request_vdm(usbpd_data, &vdm, true);
}
...
static void max77958_vdm_process_discover_identity(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
...
static void max77958_vdm_process_discover_svids(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	int i = 0, cnt = 0;
	uint16_t svid = 0;
...
static u8 max77958_vdm_choose_pin_assignment(DIS_MODE_DP_CAPA_Type *pDP_DIS_MODE)
{
...
static void max77958_vdm_process_discover_mode(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	DIS_MODE_DP_CAPA_Type *pDP_DIS_MODE = (DIS_MODE_DP_CAPA_Type *)&vdm_data[2];
	...
	if (DATA_MSG_VDM->BITS.Standard_Vendor_ID == TypeC_DP_SUPPORT && usbpd_data->svid_dp == TypeC_DP_SUPPORT) {
		...
		if (pDP_DIS_MODE->MSG_HEADER.BITS.Number_of_obj > 1)
			usbpd_data->pin_assignment = max77958_vdm_choose_pin_assignment(pDP_DIS_MODE);
	}

	max77958_set_dp_enter_mode(usbpd_data);
}

static void max77958_vdm_process_enter_mode(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	UND_DATA_MSG_VDM_HEADER_Type *DATA_MSG_VDM = (UND_DATA_MSG_VDM_HEADER_Type *)&vdm_data[4];
...
static int max77958_vdm_dp_select_pin(void *data, int multi_function_preference)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	uint32_t dp_pin = 0;
	int pin_sel = 0;
...
static void max77958_vdm_prepare_dp_configure(struct max77958_usbc_platform_data *usbpd_data, DP_STATUS_UPDATE_Type *DP_STATUS)
{
	uint8_t W_DATA = 0;
	...
	if (usbpd_data->is_sent_pin_configuration == 0) {
		...
		pin_sel = max77958_vdm_dp_select_pin(usbpd_data, multi_func);
		...
		max77958_set_dp_configure(usbpd_data, W_DATA);
...
static void max77958_vdm_announce_hpd(struct max77958_usbc_platform_data *usbpd_data, DP_STATUS_UPDATE_Type *DP_STATUS)
{
	null2term(usbpd_data);
...
static void max77958_vdm_process_dp_status_update(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	...
	if (usbpd_data->svid_dp == TypeC_DP_SUPPORT) {
		...
		max77958_vdm_prepare_dp_configure(usbpd_data, DP_STATUS);
		max77958_vdm_announce_hpd(usbpd_data, DP_STATUS);
	} else {
...
static void max77958_vdm_process_dp_attention(void *data, char *vdm_data)
{
	max77958_vdm_process_dp_status_update(data, vdm_data);
}

static void max77958_vdm_process_dp_configure(void *data, char *vdm_data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	UND_DATA_MSG_VDM_HEADER_Type *DATA_MSG_VDM = (UND_DATA_MSG_VDM_HEADER_Type *)&vdm_data[4];
...
void max77958_vdm_message_handler(struct max77958_usbc_platform_data *usbpd_data, char *opcode_data, int len)
{
	...
	case OPCODE_ID_VDM_DISCOVER_IDENTITY:
		max77958_vdm_process_printf("VDM_DISCOVER_IDENTITY::", vdm_data, len);
		max77958_vdm_process_discover_identity(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_DISCOVER_SVIDS:
		max77958_vdm_process_printf("VDM_DISCOVER_SVIDS::", vdm_data, len);
		max77958_vdm_process_discover_svids(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_DISCOVER_MODES:
		max77958_vdm_process_printf("VDM_DISCOVER_MODES::", vdm_data, len);
		vdm_data[0] = vdm_data[2];
		vdm_data[1] = vdm_data[3];
		max77958_vdm_process_discover_mode(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_ENTER_MODE:
		max77958_vdm_process_printf("VDM_ENTER_MODE::", vdm_data, len);
		max77958_vdm_process_enter_mode(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_SVID_DP_STATUS:
		max77958_vdm_process_printf("VDM_SVID_DP_STATUS::", vdm_data, len);
		vdm_data[0] = vdm_data[2];
		vdm_data[1] = vdm_data[3];
		max77958_vdm_process_dp_status_update(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_SVID_DP_CONFIGURE:
		max77958_vdm_process_printf("VDM_SVID_DP_CONFIGURE::", vdm_data, len);
		max77958_vdm_process_dp_configure(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_ATTENTION:
		max77958_vdm_process_printf("VDM_ATTENTION::", vdm_data, len);
		vdm_data[0] = vdm_data[2];
		vdm_data[1] = vdm_data[3];
		max77958_vdm_process_dp_attention(usbpd_data, vdm_data);
		break;
	case OPCODE_ID_VDM_EXIT_MODE:
		max77958_vdm_process_printf("VDM_EXIT_MODE::", vdm_data, len);
		break;
	default:
		break;
	}
...
static int max77958_get_vdm_response(struct max77958_usbc_platform_data *usbpd_data, struct vdm_response *vdm)
{
	/* send the opcode */
	usbc_cmd_data_t write_data;
	int ret = 0;
	...
	ret = max77958_usbc_opcode_write(usbpd_data, &write_data);
	if (ret)
...
static int max77958_get_discover_identity(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	...
	return max77958_get_vdm_response(usbpd_data, &vdm);
}

static int max77958_get_discover_svids(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static int max77958_get_discover_modes(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static int max77958_get_enter_mode(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static int max77958_get_attention(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static int max77958_get_dp_status_update(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static int max77958_get_dp_configure(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct vdm_response vdm;
...
static void max77958_process_alternate_mode(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	uint32_t mode;
	...
	if (mode) {
		...
		if (mode & VDM_DISCOVER_ID)
			if (max77958_get_discover_identity(usbpd_data))
				goto process_error;
		if (mode & VDM_DISCOVER_SVIDS)
			if (max77958_get_discover_svids(usbpd_data))
				goto process_error;
		if (mode & VDM_DISCOVER_MODES)
			if (max77958_get_discover_modes(usbpd_data))
				goto process_error;
		if (mode & VDM_ENTER_MODE)
			if (max77958_get_enter_mode(usbpd_data))
				goto process_error;
		if (mode & VDM_DP_STATUS_UPDATE)
			if (max77958_get_dp_status_update(usbpd_data))
				goto process_error;
		if (mode & VDM_DP_CONFIGURE)
			if (max77958_get_dp_configure(usbpd_data))
				goto process_error;
		if (mode & VDM_ATTENTION)
			if (max77958_get_attention(usbpd_data))
				goto process_error;
process_error:
...
void max77958_receive_alternate_message(struct max77958_usbc_platform_data *data)
{
	...
	for (i = 0; i < 8; i++) {
		dp_status = (1 << i);
		if (usbpd_data->max77958->dp_status & dp_status) {
			msg_info("%s", vdm_msg_irq_state_print[i + 1]);
			usbpd_data->alternate_state |= dp_status;
		}
	}

	max77958_process_alternate_mode(usbpd_data);
}

MODULE_DESCRIPTION("max77958 Alternate driver");
