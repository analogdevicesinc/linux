...
static void max77958_ccic_event_work(void *data, int event)
{
	...
	if (usbpd_data->partner == NULL) {
		if (event == USB_STATUS_NOTIFY_ATTACH_UFP) {
			mode = max77958_get_pd_support(usbpd_data);
...
static void max77958_dp_detach(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
...
static void max77958_notify_cci_vbus_current(struct max77958_usbc_platform_data *usbc_data)
{
#if IS_ENABLED(CONFIG_MAX77960_CHARGER)
	struct power_supply *psy_charger = power_supply_get_by_name("max77960-charger");

	if (psy_charger) {
		union power_supply_propval val;

		val.intval = (int)usbc_data->cc_cata->ccistat;
...
void max77958_notify_dr_status(struct max77958_usbc_platform_data *usbpd_data, uint8_t attach)
{
	...
	if (attach) {
		...
		if (pd_data->current_port_data == TYPEC_PORT_UFP) {
			if (usbpd_data->is_host == HOST_ON) {
				...
				if (usbpd_data->dp_is_connect == 1)
					max77958_dp_detach(usbpd_data);
				/* USB */
				max77958_ccic_event_work(usbpd_data, USB_STATUS_NOTIFY_DETACH);
				usbpd_data->is_host = HOST_OFF;
			}
			if (usbpd_data->is_client == CLIENT_OFF) {
				usbpd_data->is_client = CLIENT_ON;
				/* USB */
				max77958_ccic_event_work(usbpd_data, USB_STATUS_NOTIFY_ATTACH_UFP);
			}
			...
			if (usbpd_data->is_host == HOST_OFF) {
				...
				/* USB */
				max77958_ccic_event_work(usbpd_data, USB_STATUS_NOTIFY_ATTACH_DFP);
			}
		}
	} else {
		if (usbpd_data->dp_is_connect == 1)
		...
		usbpd_data->is_client = CLIENT_OFF;
		/* USB */
		max77958_ccic_event_work(usbpd_data, USB_STATUS_NOTIFY_DETACH/*drp*/);
	}
}

static irqreturn_t max77958_vconnocp_irq(int irq, void *data)
{
...
static void max77958_ccstat_irq_handler(void *data, int irq)
{
	...
	case cc_No_Connection:
		...
		if (psy_charger) {
			val.intval = 0;
			power_supply_set_property(psy_charger, POWER_SUPPLY_PROP_CHARGING_ENABLED, &val);
			power_supply_put(psy_charger);
		} else {
			msg_err("Fail to get psy charger");
		}
#endif
		break;
	case cc_SINK:
...
int max77958_cc_init(struct max77958_usbc_platform_data *usbc_data)
{
	...
	cc_data->ccstat_ws = wakeup_source_register(usbc_data->dev, "max77958-ccstat");
	if (!cc_data->ccstat_ws)
		pr_warn("%s : ccstat_ws not available (PM_SLEEP disabled?)\n", __func__);

	cc_data->irq_vconnocp = usbc_data->irq_base + MAX77958_CC_IRQ_VCONNCOP_INT;
	if (cc_data->irq_vconnocp) {
		ret = request_threaded_irq(cc_data->irq_vconnocp,
				NULL, max77958_vconnocp_irq,
				IRQF_ONESHOT, "cc-vconnocp-irq", usbc_data);
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_VCONNCOP_INT, ret);
			goto err_irq_vconnocp;
		}
	}

	cc_data->irq_vsafe0v = usbc_data->irq_base + MAX77958_CC_IRQ_VSAFE0V_INT;
	if (cc_data->irq_vsafe0v) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_VSAFE0V_INT, ret);
			goto err_irq_vsafe0v;
		}
	}

	cc_data->irq_vconnsc = usbc_data->irq_base + MAX77958_CC_IRQ_VCONNSC_INT;
	if (cc_data->irq_vconnsc) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_VCONNSC_INT, ret);
			goto err_irq_vconnsc;
		}
	}
	cc_data->irq_ccpinstat = usbc_data->irq_base + MAX77958_CC_IRQ_CCPINSTAT_INT;
	if (cc_data->irq_ccpinstat) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_CCPINSTAT_INT, ret);
			goto err_irq_ccpinstat;
		}
	}
	cc_data->irq_ccistat = usbc_data->irq_base + MAX77958_CC_IRQ_CCISTAT_INT;
	if (cc_data->irq_ccistat) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_CCISTAT_INT, ret);
			goto err_irq_ccistat;
		}
	}
	cc_data->irq_ccvcnstat = usbc_data->irq_base + MAX77958_CC_IRQ_CCVCNSTAT_INT;
	if (cc_data->irq_ccvcnstat) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_CCVCNSTAT_INT, ret);
	...
	if (cc_data->irq_ccstat) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_CC_IRQ_CCSTAT_INT, ret);
			goto err_irq_ccstat;
		}
	}
	/* check CC Pin state for cable attach booting scenario */
	max77958_ccstat_irq_handler(usbc_data, CCIC_IRQ_INIT_DETECT);
	max77958_read_reg(usbc_data->i2c, REG_CC_STATUS1, &cc_data->cc_status1);
...
MODULE_DESCRIPTION("max77958 CC driver");
