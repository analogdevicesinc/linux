...
/*
 * MAX77958 Power Delivery Driver
 *
...
#include <linux/usb/typec/maxim/max77958-alternate.h>
...
void __maybe_unused max77958_select_pdo(void *data, int num)
{
	...
	max77958_usbc_opcode_write(usbpd_data, &cmd_data);
}

static int max77958_check_pdo(void *data)
{
	struct max77958_usbc_platform_data *usbpd_data = data;
	struct max77958_usbc_command_data cmd_data;

	init_usbc_cmd_data(&cmd_data);
	cmd_data.opcode = OPCODE_CURRENT_SRC_CAP;
...
void max77958_current_pdo(struct max77958_usbc_platform_data *usbc_data, unsigned char *data)
{
	...
	int num_of_pdo = 0;
	int sel_pdo_pos = 0;
	union max77958_pdo_object pdo_obj;
...
static int max77958_get_chg_info(struct max77958_usbc_platform_data *usbc_data)
{
	usbc_cmd_data_t cmd_data;

	init_usbc_cmd_data(&cmd_data);
	cmd_data.opcode = OPCODE_SEND_GET_REQ;
	cmd_data.write_data[0] = 0;
	cmd_data.write_data[1] = 0;
	cmd_data.write_data[2] = 0;
	cmd_data.write_length = 3;
	cmd_data.read_length = 1;
	max77958_usbc_opcode_write(usbc_data, &cmd_data);

	return 0;
}

static void max77958_pd_check_pdmsg(struct max77958_usbc_platform_data *usbc_data, u8 pdmsg)
{
	switch (pdmsg) {
	...
	case PDMSG_SNK_PSRDY_RECEIVED:
		...
		max77958_get_chg_info(usbc_data);
		break;
	...
	case PDMSG_SRC_PSRDY_SENT:
		...
		if (usbc_data->pd_pr_swap == cc_SOURCE)
			max77958_vbus_turn_on_ctrl(usbc_data, VBUS_OFF);
		break;
	...
	case PDMSG_PRSWAP_SWAPTOSRC:
		max77958_vbus_turn_on_ctrl(usbc_data, VBUS_ON);
...
static irqreturn_t max77958_psrdy_irq(int irq, void *data)
{
	...
	msg_irq_handle_start(irq);
	max77958_read_reg(usbc_data->i2c, REG_PD_STATUS1, &usbc_data->pd_data->pd_status1);
...
static irqreturn_t max77958_vdm_irq(int irq, void *data)
{
	...
	max77958_receive_alternate_message(usbc_data);
...
int max77958_pd_init(struct max77958_usbc_platform_data *usbc_data)
{
	...
	pd_data->irq_pdmsg = usbc_data->irq_base + MAX77958_PD_IRQ_PDMSG_INT;
	if (pd_data->irq_pdmsg) {
		ret = request_threaded_irq(pd_data->irq_pdmsg,
				NULL, max77958_pdmsg_irq,
				IRQF_ONESHOT, "pd-pdmsg-irq", usbc_data);
		if (ret) {
			msg_irq_request_failed(MAX77958_PD_IRQ_PDMSG_INT, ret);
			goto err_irq_pdmsg;
		}
	}

	pd_data->irq_psrdy = usbc_data->irq_base + MAX77958_PD_IRQ_PS_RDY_INT;
	if (pd_data->irq_psrdy) {
		ret = request_threaded_irq(pd_data->irq_psrdy,
				NULL, max77958_psrdy_irq,
				IRQF_ONESHOT, "pd-psrdy-irq", usbc_data);
		if (ret) {
			msg_irq_request_failed(MAX77958_PD_IRQ_PS_RDY_INT, ret);
			goto err_irq_psrdy;
		}
	}

	pd_data->irq_datarole = usbc_data->irq_base + MAX77958_PD_IRQ_DATAROLE_INT;
	if (pd_data->irq_datarole) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_PD_IRQ_DATAROLE_INT, ret);
			goto err_irq_datarole;
		}
	}

	pd_data->irq_vdm = usbc_data->irq_base + MAX77958_PD_IRQ_DISPLAYPORT_INT;
	if (pd_data->irq_vdm) {
		ret = request_threaded_irq(pd_data->irq_vdm,
				NULL, max77958_vdm_irq,
				IRQF_ONESHOT, "pd-vdm-irq", usbc_data);
		if (ret) {
			msg_irq_request_failed(MAX77958_PD_IRQ_DISPLAYPORT_INT, ret);
			goto err_irq_vdm;
		}
	}
...
MODULE_DESCRIPTION("max77958 PD driver");
