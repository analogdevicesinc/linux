...
#include <linux/usb/typec/maxim/max77958-debug.h>
#endif
#include <linux/usb/typec/maxim/max77958-alternate.h>

/* Soft reset register and value */
#define MAX77958_SOFT_RESET_VAL		0x0F

struct max77958_usbc_platform_data *g_usbc_data;

static void max77958_get_version_info(struct max77958_usbc_platform_data *usbc_data);
int max77958_i2c_opcode_write(struct max77958_usbc_platform_data *usbc_data, u8 opcode, u8 length, u8 *values);
int max77958_i2c_opcode_read(struct max77958_usbc_platform_data *usbc_data, u8 opcode, u8 length, u8 *values);

static int max77958_current_pr_state(struct max77958_usbc_platform_data *usbc_data)
{
...
{
	struct max77958_usbc_platform_data *usbpd_data =
		container_of(work, struct max77958_usbc_platform_data,
			     vbus_hard_reset_work.work);
	int current_pr = max77958_current_pr_state(usbpd_data);

	msg_info("current_pr=%d", current_pr);

	if (current_pr == TYPEC_PORT_SRC)
		max77958_vbus_turn_on_ctrl(usbpd_data, VBUS_ON);
}

static void max77958_send_role_swap_message(struct max77958_usbc_platform_data *usbpd_data, u8 mode)
{
	usbc_cmd_data_t cmd_data;

	usbpd_data->role_swap_type = mode;
	max77958_usbc_clear_queue(usbpd_data);
	init_usbc_cmd_data(&cmd_data);
	cmd_data.opcode = OPCODE_SWAP_REQ;
...
static void max77958_send_role_swap_push(struct max77958_usbc_platform_data *usbpd_data, u8 mode)
{
	usbc_cmd_data_t cmd_data;
	...
	cmd_data.write_length = 0x1;
	cmd_data.read_length = 0x1;
	max77958_usbc_opcode_push(usbpd_data, &cmd_data);
}

static int max77958_dr_set(struct typec_port *port, enum typec_data_role role)
{
	struct max77958_usbc_platform_data *usbpd_data = typec_get_drvdata(port);
	...
	if (role == TYPEC_DEVICE) {
		...
		max77958_send_role_swap_message(usbpd_data, DATA_ROLE_SWAP); // for attaching UFP role
	} else if (role == TYPEC_HOST) {
		msg_info("try reversing, from UFP to DFP");
		usbpd_data->typec_try_state_change = TRY_ROLE_SWAP_DR;
		max77958_send_role_swap_message(usbpd_data, DATA_ROLE_SWAP); // for attaching DFP role
	} else {
...
static int max77958_pr_set(struct typec_port *port, enum typec_role role)
{
	struct max77958_usbc_platform_data *usbpd_data = typec_get_drvdata(port);
	...
	if (role == TYPEC_SINK) {
		...
		max77958_send_role_swap_message(usbpd_data, POWER_ROLE_SWAP); // for attaching SNK role
	} else if (role == TYPEC_SOURCE) {
		msg_info("try reversing, from Sink to Source");
		usbpd_data->typec_try_state_change = TRY_ROLE_SWAP_PR;
		max77958_send_role_swap_message(usbpd_data, POWER_ROLE_SWAP); // for attaching SRC role
	} else {
		msg_info("invalid typec_role");
		return -EIO;
	}
...
static void max77958_get_version_info(struct max77958_usbc_platform_data *usbc_data)
{
	...
	usbc_data->dev_id = chip_info[1];
	usbc_data->fw_revision = chip_info[2];
	usbc_data->fw_minor_revision = chip_info[3];

	msg_info("HWRev=%02Xh DevID=%02Xh FWRev=%02X.%02X",
...
static void max77958_irq_execute(struct max77958_usbc_platform_data *usbc_data, const struct max77958_usbc_command_data *cmd_data)
{
	...
	max77958_i2c_opcode_read(usbc_data, OPCODE_READ, len, data);
	...
	case OPCODE_GET_VDM_RESP:
		max77958_vdm_message_handler(usbc_data, data, len + OPCODE_SIZE);
		break;
				...
				if (vdm_response == SEC_UVDM_RESPONDER_ACK) {
					...
					case Attention:
						...
						/* Check requested vdm command and responded vdm command */
						{
							...
							max77958_read_reg(usbc_data->i2c, 0x23, &reqd_vdm_command);
							...
							if (reqd_vdm_command == Configure) {
								...
								max77958_set_dp_configure(usbc_data, W_DATA);
							}
						}
						break;
					case Discover_Identity:
	...
	case OPCODE_SWAP_REQ:
		...
		case ROLE_BUSY:
			if (usbc_data->role_swap_type == DATA_ROLE_SWAP) {
				...
				msleep(100);
				max77958_send_role_swap_push(usbc_data, DATA_ROLE_SWAP);
			}
			break;
		default:
			break;
...
static void max77958_usbc_dequeue_queue(struct max77958_usbc_platform_data *usbc_data)
{
	usbc_cmd_data_t cmd_data;
...
static void max77958_usbc_clear_fw_queue(struct max77958_usbc_platform_data *usbc_data)
{
	...
	max77958_usbc_opcode_write(usbc_data, &write_data);
}
...
void max77958_usbc_clear_queue(struct max77958_usbc_platform_data *usbc_data)
{
	...
	max77958_usbc_clear_fw_queue(usbc_data);
}

static void max77958_usbc_cmd_run(struct max77958_usbc_platform_data *usbc_data)
{
	usbc_cmd_queue_t *cmd_queue = NULL;
	usbc_cmd_data_t cmd_data;
	int ret = 0;
	...
	if (cmd_data.opcode == OPCODE_NONE) {/* Apcmdres isr */
		...
		max77958_irq_execute(usbc_data, &cmd_data);
		usbc_data->opcode_stamp = 0;
		max77958_usbc_cmd_run(usbc_data);
	} else { /* No ISR */
		msg_info("No ISR");
		copy_usbc_cmd_data(&cmd_data, &(usbc_data->last_opcode));
		ret = max77958_i2c_opcode_write(usbc_data, cmd_data.opcode, cmd_data.write_length, cmd_data.write_data);
		if (ret < 0) {
			msg_info("i2c write fail. dequeue opcode");
			max77958_usbc_dequeue_queue(usbc_data);
		}
	}
}
...
int max77958_i2c_opcode_write(struct max77958_usbc_platform_data *usbc_data, u8 opcode, u8 length, u8 *values)
{
	...
	ret = max77958_opcode_write(usbc_data->i2c, OPCODE_MAX_LENGTH, write_values);
	if (ret == 0)
...
int max77958_i2c_opcode_read(struct max77958_usbc_platform_data *usbc_data, u8 opcode, u8 length, u8 *values)
{
	...
	/* Read opcode data */
	return max77958_opcode_read(usbc_data->i2c, length + OPCODE_SIZE, values);
}
	...
	if (current_cmd_data.opcode == write_op->opcode)
		...
		if (usbc_data->opcode_stamp != 0 && current_cmd_data.opcode == OPCODE_NONE) {
			if (time_after(jiffies,
					usbc_data->opcode_stamp + MAX77958_MAX_APDCMD_TIME)) {
				usbc_data->opcode_stamp = 0;
...
int max77958_usbc_opcode_push_locked(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op)
{
	...
	/* the messages received From USBC. */
	init_usbc_cmd_data(&execute_cmd_data);
	execute_cmd_data.response = read_op->opcode;
	execute_cmd_data.read_length = read_op->read_length;
	execute_cmd_data.is_uvdm = read_op->is_uvdm;
...
int max77958_usbc_opcode_push(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op)
{
	...
	mutex_lock(&usbc_data->op_lock);
	ret = max77958_usbc_opcode_push_locked(usbc_data, read_op);
	mutex_unlock(&usbc_data->op_lock);

	return ret;
}

int max77958_usbc_opcode_rw(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op, usbc_cmd_data_t *write_op)
{
	usbc_cmd_queue_t *cmd_queue = &(usbc_data->cmd_queue);
	usbc_cmd_data_t execute_cmd_data;
	usbc_cmd_data_t current_cmd_data;
...
static void max77958_reset_ic(struct max77958_usbc_platform_data *usbc_data)
{
	msg_info("IN");
	max77958_write_reg(usbc_data->i2c, REG_RESET,
			   MAX77958_SOFT_RESET_VAL);
	msleep(100); /* need 100ms delay */
	msg_info("OUT");
}

static void max77958_usbc_check_sysmsg(struct max77958_usbc_platform_data *usbc_data, u8 sysmsg)
{
	usbc_cmd_queue_t *cmd_queue = &(usbc_data->cmd_queue);
	...
	case SYSMSG_BOOT_WDT:
		...
		max77958_write_reg(usbc_data->i2c, REG_UIC_INT_M, REG_UIC_INT_M_INIT);
		max77958_write_reg(usbc_data->i2c, REG_CC_INT_M, REG_CC_INT_M_INIT);
		max77958_write_reg(usbc_data->i2c, REG_PD_INT_M, REG_PD_INT_M_INIT);
		max77958_write_reg(usbc_data->i2c, REG_ACTION_INT_M, REG_ACTION_INT_M_INIT);
		/* clear UIC_INT to prevent infinite sysmsg irq*/
		max77958_read_reg(usbc_data->i2c, REG_UIC_INT, &interrupt);
		msg_info("UIC_INT:0x%02x", interrupt);
		max77958_usbc_clear_queue(usbc_data);
		break;
	case SYSMSG_BOOT_SWRSTREQ:
		msg_info("[0x%02X] SYSMSG_BOOT_SWRSTREQ", sysmsg);
		break;
	...
	case SYSMSG_AFC_DONE:
		msg_info("[0x%02X] SYSMSG_AFC_DONE", sysmsg);
		break;
	...
	case SYSMSG_APCMD_FAIL:
		...
		if (!is_empty_queue) {
			...
			if ((cmd_data.opcode != OPCODE_NONE) && (cmd_data.opcode == next_opcode)) {
				if (next_opcode != OPCODE_SEND_VDM) {
					int ret = max77958_i2c_opcode_write(usbc_data, cmd_data.opcode, cmd_data.write_length, cmd_data.write_data);
					if (ret) {
	...
	case SYSMSG_CCx_5V_SHORT:
		msg_info("[0x%02X] SYSMSG_CCx_5V_SHORT", sysmsg);
		break;
	case SYSMSG_SBUx_GND_SHORT:
		msg_info("[0x%02X] SYSMSG_SBUx_GND_SHORT", sysmsg);
		break;
	case SYSMSG_SBUx_5V_SHORT:
		msg_info("[0x%02X] SYSMSG_SBUx_5V_SHORT", sysmsg);
		break;
	default:
...
static irqreturn_t max77958_apcmd_irq(int irq, void *data)
{
	...
	max77958_read_reg(usbc_data->i2c, REG_USBC_STATUS2, &usbc_data->usbc_status2);
...
static irqreturn_t max77958_sysmsg_irq(int irq, void *data)
{
	...
	for (i = 0; i < 3; i++) {
		usbc_status2 = 0;
		max77958_read_reg(usbc_data->i2c, REG_USBC_STATUS2, &usbc_status2);
	...
	max77958_usbc_check_sysmsg(usbc_data, sysmsg);
...
static int max77958_init_irq_handler(struct max77958_usbc_platform_data *usbc_data)
{
	int ret = 0;

	usbc_data->irq_apcmd = usbc_data->irq_base + MAX77958_UIC_IRQ_APC_INT;
	if (usbc_data->irq_apcmd) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_UIC_IRQ_APC_INT, ret);
			return ret;
		}
	}

	usbc_data->irq_sysmsg = usbc_data->irq_base + MAX77958_UIC_IRQ_SYSM_INT;
	if (usbc_data->irq_sysmsg) {
		...
		if (ret) {
			msg_irq_request_failed(MAX77958_UIC_IRQ_SYSM_INT, ret);
...
static void max77958_usbc_free_irq(struct max77958_usbc_platform_data *usbc_data)
{
	free_irq(usbc_data->irq_apcmd, usbc_data);
...
int max77958_usbc_init(struct max77958_dev *max77958)
{
	...
	msg_info("init start with irq %d", max77958->irq);
	usbc_data = kzalloc(sizeof(struct max77958_usbc_platform_data), GFP_KERNEL);
	if (!usbc_data) {
	...
	usbc_data->pd_data = devm_kzalloc(max77958->dev, sizeof(*usbc_data->pd_data), GFP_KERNEL);
	...
	usbc_data->cc_data = devm_kzalloc(max77958->dev, sizeof(*usbc_data->cc_data), GFP_KERNEL);
	...
	usbc_data->bc12_data = devm_kzalloc(max77958->dev, sizeof(*usbc_data->bc12_data), GFP_KERNEL);
...
#if IS_ENABLED(CONFIG_MAX77960_CHARGER)
	usbc_data->chg_data = kzalloc(sizeof(struct max77960_charger_data), GFP_KERNEL);
	if (!usbc_data->chg_data) {
		msg_err("chg_data is null");
		ret = -ENOMEM;
		goto err_nomem;
	}
#endif
	...
	usbc_data->typec_cap.ops = &max77958_ops;
...
void max77958_usbc_deinit(struct max77958_usbc_platform_data *usbc_data)
{
	...
	max77958_usbc_free_irq(usbc_data);
	if (usbc_data->port)
...
static void max77958_usbc_disable_irq(struct max77958_usbc_platform_data *usbc_data)
{
...
void max77958_usbc_shutdown(struct max77958_usbc_platform_data *usbc_data)
{
	...
	if (!usbc_data->i2c) {
		msg_err("no max77958 i2c client");
		return;
	}
	...
	max77958_usbc_disable_irq(usbc_data);
	max77958_usbc_free_irq(usbc_data);
	usbc_data->shut_down = 1;

	/* send the reset command */
	if (usbc_data->current_connstat == WATER)
		msg_info("Skip the max77958_reset_ic function");
	else {
		max77958_reset_ic(usbc_data);
		max77958_write_reg(usbc_data->i2c, REG_PD_INT_M, 0xFF);
		max77958_write_reg(usbc_data->i2c, REG_CC_INT_M, 0xFF);
...
MODULE_DESCRIPTION("max77958 USBC driver");
