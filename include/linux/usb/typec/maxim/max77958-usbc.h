...
/*
 * MAX77958 USB Type-C Controller Header
 *
 * Copyright (c) 2024 Analog Devices, Inc.
 */

#ifndef __MAX77958_USBC_H__
#define __MAX77958_USBC_H__
#include <linux/usb/typec.h>

#include <linux/usb/typec/maxim/max77958.h>
#include <linux/usb/typec/maxim/max77958-pd.h>
#include <linux/usb/typec/maxim/max77958-cc.h>
#include <linux/usb/typec/maxim/max77958-bc12.h>
#define MAX77958_MAX_APDCMD_TIME (7*HZ)
...
union max77958_custm_info_pass2 {
	uint8_t CustmInfoArry[11];
	...
	struct {
		...
		uint16_t srcpdovolt : 7;
		uint16_t srcpdomaxcurr : 9;
...
union max77958_custm_info_pass3 {
...
typedef struct max77958_usbc_command_data {
	u8	opcode;
	u8  prev_opcode;
	u8	response;
...
typedef struct max77958_usbc_command_node {
	struct max77958_usbc_command_data cmd_data;
	struct max77958_usbc_command_node *next;
} usbc_cmd_node_t;

typedef struct max77958_usbc_command_queue {
...
struct max77958_usbc_platform_data {
	...
	enum max77958_connstat prev_connstat;
	enum max77958_connstat current_connstat;

	struct max77958_usbc_command_queue apcmd_queue;
...
#if IS_ENABLED(CONFIG_MAX77960_CHARGER)
	struct max77963_charger_data *chg_data;
#endif
...
void init_usbc_cmd_data(struct max77958_usbc_command_data *cmd_data);
bool is_empty_usbc_cmd_queue(usbc_cmd_queue_t *cmd_queue);

void max77958_usbc_clear_queue(struct max77958_usbc_platform_data *usbc_data);

int max77958_usbc_opcode_rw(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *opcode_r, usbc_cmd_data_t *opcode_w);
int max77958_usbc_opcode_write(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *write_op);
int max77958_usbc_opcode_read(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op);
int max77958_usbc_opcode_push(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op);
int max77958_usbc_opcode_push_locked(struct max77958_usbc_platform_data *usbc_data, usbc_cmd_data_t *read_op);

#if IS_ENABLED(CONFIG_MAX77960_CHARGER)
void max77958_notify_cci_vbus_current(struct max77958_usbc_platform_data *usbc_data);
int __maybe_unused max77960_chg_init(struct max77958_usbc_platform_data *usbc_data);
void __maybe_unused max77960_chg_irq_apcmd_handler(struct max77958_usbc_platform_data *usbpd_data, char *apcmd_data, int irq_routine);
#endif
#define VBUS_OFF	(0)
#define VBUS_ON		(1)
void max77958_vbus_turn_on_ctrl(struct max77958_usbc_platform_data *usbc_data, bool enable);
void max77958_notify_dr_status(struct max77958_usbc_platform_data *usbpd_data, uint8_t attach);
...
void max77958_detach_pd(struct max77958_usbc_platform_data *usbc_data);
void max77958_current_pdo(struct max77958_usbc_platform_data *usbc_data, unsigned char *data);
void __maybe_unused max77958_select_pdo(void *data, int num);
...
int max77958_get_pd_support(struct max77958_usbc_platform_data *usbc_data);

/* Driver init/deinit functions */
struct max77958_dev;
int max77958_usbc_init(struct max77958_dev *max77958);
void max77958_usbc_deinit(struct max77958_usbc_platform_data *usbc_data);
void max77958_usbc_shutdown(struct max77958_usbc_platform_data *usbc_data);

/* Global USBC data pointer */
extern struct max77958_usbc_platform_data *g_usbc_data;

#endif /* __MAX77958_USBC_H__ */
