...
The Universal Asynchronous Receiver/Transmitter module (UART) is a full-duplex peripheral compatible with PCstyle industry-standard UARTs. The UART converts data between serial and parallel formats. The serial communication follows an asynchronous protocol that supports various word length, stop bit, parity, and interrupt generation
...
Table 17-2: ADSP-SC58x UART Register List
...
## ADSP-SC58x UART Interrupt List

Table 17-3: ADSP-SC58x UART Interrupt List

|   Interrupt ID | Name            | Description             | Sensitivity   |   DMA Channel |
|----------------|-----------------|-------------------------|---------------|---------------|
|            114 | UART0_TXDMA     | UART0 TransmitDMA       | Level         |            20 |
|            115 | UART0_RXDMA     | UART0 ReceiveDMA        | Level         |            21 |
|            116 | UART0_STAT      | UART0 Status            | Level         |               |
|            117 | UART1_TXDMA     | UART1 TransmitDMA       | Level         |            34 |
|            118 | UART1_RXDMA     | UART1 ReceiveDMA        | Level         |            35 |
|            119 | UART1_STAT      | UART1 Status            | Level         |               |
|            120 | UART2_TXDMA     | UART2 TransmitDMA       | Level         |            37 |
|            121 | UART2_RXDMA     | UART2 ReceiveDMA        | Level         |            38 |
|            122 | UART2_STAT      | UART2 Status            | Level         |               |
|            202 | UART0_TXDMA_ERR | UART0 Transmit DMAError | Level         |               |

Table 17-3: ADSP-SC58x UART Interrupt List (Continued)

|   Interrupt ID | Name            | Description             | Sensitivity   | DMA Channel   |
|----------------|-----------------|-------------------------|---------------|---------------|
|            203 | UART0_RXDMA_ERR | UART0 Receive DMAError  | Level         |               |
|            204 | UART1_TXDMA_ERR | UART1 Transmit DMAError | Level         |               |
|            205 | UART1_RXDMA_ERR | UART1 Receive DMAError  | Level         |               |
|            206 | UART2_TXDMA_ERR | UART2 Transmit DMAError | Level         |               |
|            207 | UART2_RXDMA_ERR | UART2 Receive DMAError  | Level         |               |
...
Table 17-6: ADSP-SC58x UART DMA Channel List (Continued)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000000_bce32395e49babbfc425612e9891a7dc1e8b21825b519512045ee108b66d069b.png)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000001_2abd249306193d922c8850d1c3d22bc08b7202539d8a053698cdf27fed4bb180.png)
...
The peripheral clock (SCLK0\_0) and the 16-bit divisor in the UART\_CLK register characterize the sample clock. The UART uses the UART\_CTL.EN bit to enable the clock. By default, every serial bit is oversampled 16 times.

The bit clock is 1/16th of the sample clock. If not in IrDA mode, the bit clock can equal the sample clock if the UART\_CLK.EDBO bit is set, so that the following equation applies:

Bit Rate = SCLK0\_0/16  (1-EDBO)  × Divisor

## ADSP-SC5xx Processor Example

The UART Bit Rate Examples with 100 MHz SCLK0\_0 table provides example divide factors required to support standard baud rates at an SCLK0\_0 of 100 MHz.

Table 17-7: UART Bit Rate Examples with 100 MHz SCLK0\_0

| Bit Rate (bits/ sec)   | DFactor = 16   | DFactor = 16   | DFactor = 16   | DFactor = 1   | DFactor = 1   | DFactor = 1   |
|------------------------|----------------|----------------|----------------|---------------|---------------|---------------|
...
NOTE: Careful selection of SCLK0\_0 frequencies-that is, even multiples of desired bit rates- can result in lower error percentages.

Setting the bit clock equal to the sample clock ( UART\_CLK.EDBO =1) improves bit rate granularity and enables the bit clock to more closely match the bit rate of the communication partner. The disadvantage to this configuration is that the power dissipation is higher and the sample points are not always as accurate. Therefore, use UART\_CLK.EDBO =1 mode only when bit rate accuracy is not acceptable in UART\_CLK.EDBO =0 mode.

The UART\_CLK.EDBO =1 mode is not intended to increase operation speed beyond the electrical limitations of the UART transfer protocol.

## Autobaud Detection

At the chip level, the UART\_RX pin is typically routed to an alternate capture input ( TIMER\_ACI[n] ) of a general-purpose timer. When working in width capture mode, the processor uses this general-purpose timer to detect the
...
A software routine can detect the pulse widths of serial stream bit cells. The sample base of the timer is synchronous with the UART operation (all derived from the same SCLK0\_0). The UART uses pulse widths to calculate the bit rate divider as follows:
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000002_04a99c77e8596b47867018644e7e9e5acefb868265aeea13bef01fd8caf48dd4.png)
...
Divisor = TIMER\_TMR[n]\_PER /16 (1-EDBO)  × 8 or: · Divisor = TIMER\_TMR[n]\_PER&gt;&gt; 7, if UART\_CLK.EDBO =0
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000003_41caf4f6c40abf9bf3942a7421ff0823a88ce54ea42c4d8ab241295ed2e301d9.png)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000004_951fc6fe368aa9a158f910da5949a98f44224615d8e52e9ca3c32c9dc7616ee3.png)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000005_9369b88e9da6da5e10b5cfed88681c8b58e34fc313e1915c6638d96fb6cca645.png)
...
The UART receiver can detect break commands through the break indicator ( UART\_STAT.BI ) flag. This flag reports that an entire UART frame has been received in low state. It does not report whether the duration of the received low pulse was exact or at least 13 bit-times as LIN masters transmit. Typically, the break indicator meets LIN requirements. The processor can use GP timers to determine the pulse width more precisely, if necessary.
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000006_851713ca676a634f7426a3b6249be5e718dbfbde225f4dffa8fbc2b2af3b15c7.png)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000007_02c86c5a0a6c49d712842534bbc8f1e2004940859ea53f0af47f08a88d66c89f.png)
...
In DMA transmit mode, the UART\_IMSK.ETBEI bit enables the peripheral request to the DMA FIFO. The DMA\_CFG.EN bit enables the strobe on the memory side. If the DMA count is less than the DMA FIFO depth, which is 4, then the DMA interrupt can be requested before the UART\_IMSK.ETBEI bit is set. If this behavior is unwanted, set the DMA\_CFG.SYNC bit.
...
On reset, when the UART is not yet enabled and the port multiplexing has not been programmed, the UART\_RTS pin is not driven. Some applications require a resistor pull the UART\_RTS signal to either state during reset.
...
See the Direct Memory Access (DMA) chapter on for information on DMA multiplexing. Several interrupts and DMA channels in thr UART can be multiplexed.
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000008_e39bece3b112a6d8d412a0d9dd531418e72e7d7fcb16abe6507a064f2baa07c9.png)
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000009_2efe9354c6811314b2a4048973773808d32ad4958b26d163e1070bb865a1c712.png)
...
Several status bits and interrupt features in the UART\_STAT and UART\_IMSK registers facilitate efficient data handling in multi-drop bus mode. These features include the address ( UART\_STAT.ADDR ) bit, the address sticky ( UART\_STAT.ASTKY ) bit and the enable address word interrupt ( UART\_IMSK.EAWI ). One of the key features of the multi-drop bus protocol is its address bit. The address bit signifies to the slaves that the master is transmitting an address word (all read it) or a data word (only the addressed slave reads its). The UART hardware provides an efficient method of handling the situation described with the use of UART\_STAT.ASTKY bit.
...
Refer to Autobaud Detection section for more information. The required steps are:

1. Ensure that the timer is disabled.

2. Configure the following bits: UART\_CTL.MOD =00, UART\_CTL.LOOP\_EN =1, UART\_CTL.WLS =11 (8bit data), and UART\_CTL.EN =1
...
Table 17-9: ADSP-SC58x UART Register List
...
The UART\_CLK register divides the system clock ( SCLK0\_0) down to the bit clock.

Figure 17-11: UART\_CLK Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000010_a27ab2dac7e091bc805ba12ea35c7af6764d45f81a847ba9f517ecc3cf08e97f.png)

Table 17-10: UART\_CLK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                               |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 31 (R/W)           | EDBO       | Enable Divide By One. The UART_CLK.EDBO bit enables the bypassing of the divide-by-16 prescaler in bit clock generation. This functionality improves bit rate granularity, especially at high bit rates. Do not set this bit in IrDA mode. 0 Bit clock prescaler = 16 |
| 15:0 (R/W)         | DIV        | Divisor. The UART_CLK.DIV provides the divisor for the UART's clock bit rate calculation. The bit rate is defined by: Bit Rate = SCLK0_0 / (16 (1-EDBo) x UART_CLK.DIV )                                                                                              |

## Control Register

The UART\_CTL register provides enable and disable control for internal UART and for the IrDA mode of operation. This register also provides UART line control, permitting selection of the format of received and transmitted character frames. Modem feature control also is available from this register, including partial modem functionality to allow for hardware flow control and loopback mode.

Figure 17-12: UART\_CTL Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000011_eabc12fd351dfaab34656428e61d256afb342b7f68f1182c0e18f8396c678c56.png)

Table 17-11: UART\_CTL Register Fields
...
| 19 (R/W)           | SB         | Set Break. If set, the UART_CTL.SB bit forces the UART_TX pin to low asynchronously, re- gardless of whether or not data is currently transmitted. This bit functions even when the UART clock is disabled. Because the UART_TX pin normally drives high, it can be used as a flag output pin, if the UART is not used. (For example, if UART_CTL.TPOLC is cleared, drive UART_TX pin low; or if UART_CTL.TPOLC is set, drive UART_TX pin high.) 0 No force |
...
| Bit No. (Access) Bit Name                                    | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                | Description/Enumeration                                                                                                                                                                                    |
|--------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 16 STP                                                       | Sticky Parity.                                                                                                                                                                                                                                                                                                                                                                         | Sticky Parity.                                                                                                                                                                                             |
| (R/W) The UART_CTL.STP bit on the data bits or whether it is | controls whether the set to a fixed value. set. Then, the count of logical-1 bits must be a even value. If the UART_CTL.STP bit is set, hardware parity calculation is disabled. In this case, the sent and received par- ity equals the inverted UART_CTL.EPS bit. 0 No forced parity 1 Force (Stick) parity to defined value (if PEN=1) Even Parity Select. 0 Odd parity             | parity is generated by hardware based If this bit is cleared, the hardware                                                                                                                                 |
| 14                                                           | UART_CTL.PEN bit enables parity transmission and bit inserts one additional bit between the the first stop bit. The polarity of this so-called parity bit and UART_CTL.EPS control bits. Both the parity value. The receiver compares the received pected value and issues a parity error if they do not match. If the bit is cleared, the UART_CTL.STP and the UART_CTL.EPS 0 Disable | bits are ignored.                                                                                                                                                                                          |
| (R/W) PEN                                                    | 1 Even parity Parity Enable. The parity check. The UART_CTL.PEN most significant data bit and depends on data and the UART_CTL.STP transmitter and receiver calculate parity bit with the ex- UART_CTL.PEN                                                                                                                                                                             | 1 Even parity Parity Enable. The parity check. The UART_CTL.PEN most significant data bit and depends on data and the UART_CTL.STP transmitter and receiver calculate parity bit with the ex- UART_CTL.PEN |
| 13 (R/W) STBH Stop Bits (Half Bit Time).                     | 1 Enable parity transmit and check                                                                                                                                                                                                                                                                                                                                                     |                                                                                                                                                                                                            |
| 12 (R/W)                                                     |                                                                                                                                                                                                                                                                                                                                                                                        |                                                                                                                                                                                                            |
...
Figure 17-13: UART\_IMSK Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000012_635f8f192b1983a8391d175530423ed004311c620d751b68265931286971e5df.png)

Table 17-12: UART\_IMSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                        |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 9 (R/W)            | ETXS       | Enable TX to Status Interrupt Mask Status. If set (interrupt unmasked), the UART_IMSK.ETXS bit indicates re-direction of the TX interrupt requests to status interrupt output. If cleared, TX interrupt requests are routed to normal interrupt outputs. 0 Interrupt is masked |
| 8 (R/W)            | ERXS       | Enable RX to Status Interrupt Mask Status. If set (interrupt unmasked), the UART_IMSK.ERXS bit indicates re-direction of RX interrupt requests to status interrupt output. If cleared, RX interrupt requests are rout- ed to normal interrupt outputs. 0 Interrupt is masked   |
| 8 (R/W)            |            | 1 Interrupt is unmasked                                                                                                                                                                                                                                                        |
| 8 (R/W)            |            |                                                                                                                                                                                                                                                                                |

Table 17-12: UART\_IMSK Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7 (R/W)            | EAWI       | Enable Address Word Interrupt Mask Status. If set (interrupt unmasked), the UART_IMSK.EAWI bit indicates generation of a sta- tus interrupt request when an Address word in MDB-mode is present in the UART_RBR . A received word is an address word if the UART_STAT.ADDR bit is set.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 7 (R/W)            | EAWI       | 0 Interrupt is masked                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 1 (R/W)            |            | 0 Interrupt is masked                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 1 (R/W)            |            | 1 Interrupt is unmasked                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 0 (R/W)            | ERBFI      | Enable Receive Buffer Full Interrupt Mask Status. If set (interrupt unmasked), the UART_IMSK.ERBFI indicates generation of an RX interrupt request if the UART_STAT.DR bit is set.                                                                                                                                                                                                                                                                            |
| 0 (R/W)            |            | 0 Interrupt is masked                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000013_eb4fc32ede32622dc2ff0c0089aa308f9713610e7380ad58cfe1171e1d089cf4.png)
...
Figure 17-14: UART\_IMSK\_CLR Register Diagram

Table 17-13: UART\_IMSK\_CLR Register Fields
...
Table 17-13: UART\_IMSK\_CLR Register Fields (Continued)
...
Figure 17-15: UART\_IMSK\_SET Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000014_d448dc786a8164a9f491d78e5ba42a9d7c83b9f25bda862237b3e91be9b84b60.png)

Table 17-14: UART\_IMSK\_SET Register Fields
...
Table 17-14: UART\_IMSK\_SET Register Fields (Continued)
...
Figure 17-16: UART\_RBR Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000015_3f8ddd570c288e274c6452afb54600ba7c989f1910033628cf56b1757c409127.png)

Table 17-15: UART\_RBR Register Fields
...
Figure 17-17: UART\_RSR Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000016_6cb4199198f49964e9e0e08aefbd9387a61546f65f09c1adedc20b51b52234f2.png)

Table 17-16: UART\_RSR Register Fields
...
Figure 17-18: UART\_RXCNT Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000017_04c09ba4332ebffd7bb99ad8a18d3bce5be880b0d46d96a20c071dea689a04b9.png)

Table 17-17: UART\_RXCNT Register Fields
...
Figure 17-19: UART\_SCR Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000018_4b53d1a790988ac4b9e51c700b7dbfa79474090f3d21c84f226448d6480860a4.png)

Table 17-18: UART\_SCR Register Fields
...
Figure 17-20: UART\_STAT Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000019_7b871cc9173c22b2181fa27f6d203636cd8edbeb72bb8953e6cdaba6c9fbb801.png)

Table 17-19: UART\_STAT Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|--------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 17 (R/NW)          | RFCS       | Receive FIFO Count Status. The UART_STAT.RFCS bit is set when the receive buffer holds more or equal entries than a certain threshold. The threshold is controlled by the UART_CTL.RFIT bit. If UART_CTL.RFIT is cleared, the threshold is four entries. If UART_CTL.RFIT is set, the threshold is seven entries. The UART_STAT.RFCS bit is cleared when the UART_RBR register is read sufficient times until the buffer is drained below the threshold. The UART_STAT.RFCS bit can trigger a status interrupt request if ena- bled by the UART_IMSK_SET.ERFCI bit. | Receive FIFO Count Status. The UART_STAT.RFCS bit is set when the receive buffer holds more or equal entries than a certain threshold. The threshold is controlled by the UART_CTL.RFIT bit. If UART_CTL.RFIT is cleared, the threshold is four entries. If UART_CTL.RFIT is set, the threshold is seven entries. The UART_STAT.RFCS bit is cleared when the UART_RBR register is read sufficient times until the buffer is drained below the threshold. The UART_STAT.RFCS bit can trigger a status interrupt request if ena- bled by the UART_IMSK_SET.ERFCI bit. |
...
| 4 (R/W1C)          | BI         | 1 Empty THR/TAIP Break Indicator. The UART_STAT.BI bit indicates that the first stop bit is sampled low and the en- tire data word, including parity bit, consists of low bits only. (This condition indicates that UART_RX was held low for more than the maximum word length.) The UART_STAT.BI bit is updated simultaneously with the UART_STAT.DR bit, that is, by the time the first stop bit is received or when data is loaded from the receive FIFO to the UART_RBR register. The bit is sticky and can be cleared by W1C opera- tions. |
| 4 (R/W1C)          | BI         | 0 No break interrupt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 3 (R/W1C)          | FE         | maximum word length Framing Error. The UART_STAT.FE bit indicates that the first stop bit is sampled. This bit is up- dated simultaneously with the UART_STAT.DR bit, that is, by the time the first stop bit is received or when data is loaded from the receive FIFO to the UART_RBR regis- ter. The UART_STAT.FE bit is sticky and can be cleared by W1C operations. Note that invalid stop bits can be simulated by setting the UART_CTL.FFE bit.                                                                                           |
| 3 (R/W1C)          | FE         | 0 No error                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 3 (R/W1C)          | FE         | 1 Invalid stop bit error                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2 (R/W1C)          | PE         | Parity Error. The UART_STAT.PE bit indicates that the received parity bit does not match the ex- pected value. This bit is updated simultaneously with the UART_STAT.DR bit, that is, by the time the first stop bit is received or when data is loaded from the receive FIFO to the UART_RBR register. The UART_STAT.PE bit is sticky and can be cleared by W1C operations. Note that invalid parity bits can be simulated by setting the UART_CTL.FPE bit.                                                                                    |
...
| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1                  | OE         | Overrun Error. The UART_STAT.OE bit indicates that further data is received while the internal re- ceive buffer was full. This bit is set when sampling the stop bit of the sixth data word. To avoid overruns, read the UART_RBR register in time. In DMAreceive mode, over- runs are very unlikely to happen ever. After an overrun occurs, the UART_RBR and receive FIFO are protected from being overwritten by new data until the UART_STAT.OE bit is cleared by software. The content of the UART_RSR register is lost as soon as the overrun occurs. The UART_STAT.OE bit is sticky and can be cleared by W1C operations. | Overrun Error. The UART_STAT.OE bit indicates that further data is received while the internal re- ceive buffer was full. This bit is set when sampling the stop bit of the sixth data word. To avoid overruns, read the UART_RBR register in time. In DMAreceive mode, over- runs are very unlikely to happen ever. After an overrun occurs, the UART_RBR and receive FIFO are protected from being overwritten by new data until the UART_STAT.OE bit is cleared by software. The content of the UART_RSR register is lost as soon as the overrun occurs. The UART_STAT.OE bit is sticky and can be cleared by W1C operations. |
...
Figure 17-21: UART\_TAIP Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000020_33c3134de0d4ef00311ded0e942df4e4fa30e2183fda075729725df2560f512c.png)

Table 17-20: UART\_TAIP Register Fields
...
Figure 17-22: UART\_THR Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000021_21258ce951a0f0c8388d4a83ad5aa4bf0a96219184ada2987d69485473e795fd.png)

Table 17-21: UART\_THR Register Fields
...
Figure 17-23: UART\_TSR Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000022_1755ed88a7fd2bb8fcf2486b2d03e054ef25ca59ef4870c5a7601c397a4ccaff.png)

Table 17-22: UART\_TSR Register Fields
...
Figure 17-24: UART\_TXCNT Register Diagram

![Image](20_Universal_Asynchronous_ReceiverTransmitter_(UART)_artifacts/image_000023_04c09ba4332ebffd7bb99ad8a18d3bce5be880b0d46d96a20c071dea689a04b9.png)

Table 17-23: UART\_TXCNT Register Fields
