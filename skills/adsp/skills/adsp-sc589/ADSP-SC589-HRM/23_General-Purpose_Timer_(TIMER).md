## 20   General-Purpose Timer (TIMER)
...
## ADSP-SC58x TIMER Register List
...
Table 20-1: ADSP-SC58x TIMER Register List
...
## ADSP-SC58x TIMER Interrupt List

Table 20-2: ADSP-SC58x TIMER Interrupt List

|   Interrupt ID | Name        | Description    | Sensitivity   | DMA Channel   |
|----------------|-------------|----------------|---------------|---------------|
|             26 | TIMER0_TMR0 | TIMER0 Timer 0 | Level         |               |
|             27 | TIMER0_TMR1 | TIMER0 Timer 1 | Level         |               |
|             28 | TIMER0_TMR2 | TIMER0 Timer 2 | Level         |               |
|             29 | TIMER0_TMR3 | TIMER0 Timer 3 | Level         |               |
...
## ADSP-SC58x TIMER Trigger List

Table 20-3: ADSP-SC58x TIMER Trigger List Masters
...
Table 20-4: ADSP-SC58x TIMER Trigger List Slaves
...
Table 20-4: ADSP-SC58x TIMER Trigger List Slaves (Continued)
...
![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000000_ef101681f69a6e3146fd242aa48c598642a95f43d219799a3ecc4292fdfea734.png)
...
Table 20-5: Timer Signal Routing

| Timer Signal   | Connection   |
|----------------|--------------|
| TM0_ACLK0      | SYS_CLKIN1   |

Table 20-5: Timer Signal Routing (Continued)
...
Table 20-6: Timer Mode Descriptions
...
Table 20-6: Timer Mode Descriptions (Continued)
...
Table 20-7: Usage of the Period, Width, and Delay Registers in Different Timer Modes
...
Table 20-7: Usage of the Period, Width, and Delay Registers in Different Timer Modes  (Continued)

| Timer Mode   | TIMER_TMR[n]_PER   | TIMER_TMR[n]_WID   | TIMER_TMR[n]_DLY   |
|--------------|--------------------|--------------------|--------------------|
| PININT       | Not used           | Not used           | Not used           |

If any of the period, pulse width, and delay registers are not used, then programs cannot write into that register. For example, in WIDCAP mode, the delay registers are not used. So, the program is not allowed to write any value to the TIMER\_TMR[n]\_DLY register. T o prevent undesired operation, program the

TIMER\_TMR[n]\_CFG.TMODE bits before programming the period, width, or delay registers.
...
Use this feature carefully, as it can corrupt the PWM pattern generated at the TIMER\_TMR[n] pin, though after such a stop the pin deasserts automatically. Each timer samples its TIMER\_RUN.TMR[nn] bit at the end of each period. It stops cleanly at the end of the first period after the TIMER\_RUN.TMR[nn] bit is low. A timer that is disabled and then restarted (before the end of the current period), continues to run as if nothing happened. Typically, the program disables a PWMOUT timer and then waits for it to stop itself.

## Width Capture (WIDCAP) Mode

The timer uses WIDCAP mode, often called capture mode, to measure pulse widths on the TIMER\_TMR[n] or TIMER\_ACI[n] inputs. The polarity (active high or low) of the input signal can be selected with the TIMER\_TMR[n]\_CFG.PULSEHI bit. The Timer Signal Flow in Width Capture Mode figure shows the control signal flow for WIDCAP\_CAP mode.

Figure 20-3: Timer Signal Flow in Width Capture Mode

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000002_d58fce99f2d4b92b1f22a09401133696fffc54c343174415db795b643eab81f6.png)

NOTE: SCLK in the Timer Signal Flow in Width Capture Mode figure is SCLK0\_0.

In this mode, the timer uses the TIMER\_TMR[n]\_CFG.TINSEL bit to select between the TIMER\_TMR[n] or TIMER\_ACI[n] input. The internally clocked timer is used to determine the period and pulse width of the externally applied rectangular waveforms.
...
The TIMER\_TMR[n]\_CFG.TMODE bit 0 controls the point in time at which this set of events is executed. Taken together, these four events are called a measurement report. The TIMER\_STAT\_ILAT register is not set at a measurement report. A measurement report occurs, at most, once per input signal period. The current TIMER\_TMR[n]\_CNT value is always copied to the width buffer and period buffer registers at the trailing and leading edges of the input signal, respectively. But, these values are not visible to software. A measurement report event samples the captured values into visible registers and sets the timer interrupt request to signal that the TIMER\_TMR[n]\_PER and the TIMER\_TMR[n]\_WID registers are ready to be read.
...
Figure 20-4: Example of Width Capture Deasserted Mode (TMODE=b#1011)

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000003_009566cc5e547a361e1069ab4eb0274dec836e2a434e3bb8fd41bfa9e3d4f8cc.png)

NOTE: SCLK in the Example of Width Capture Deasserted Mode (TMODE=b#1011) figure is SCLK0\_0.
...
Figure 20-5: Example of Width Capture Asserted Mode (TMODE=b#1010)

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000004_5d69f85e6eee81a69d800053af7178445c177320dedd85adfa0e558dcb47c672.png)

NOTE: SCLK in the Example of Width Capture Asserted Mode (TMODE=b#1010) figure is SCLK0\_0.

To measure the pulse width of a waveform that has only one leading edge and one trailing edge, set TIMER\_TMR[n]\_CFG.TMODE = b#1011. If TIMER\_TMR[n]\_CFG.TMODE = b#1010 for this case, no period value is captured in the period buffer register. Instead, the timer genererates an error report interrupt request (if enabled) when the TIMER\_TMR[n]\_CNT range is exceeded and the counter wraps around. In this case, both the TIMER\_TMR[n]\_PER and TIMER\_TMR[n]\_WID registers read 0 (because no measurement report occurred to copy the value captured in the width buffer register to the TIMER\_TMR[n]\_WID register).
...
Figure 20-6: Example Timing for Width Capture Followed by Period Overflow (TMR\_CFG.TMODE=b#1010)

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000005_0a0793e674dd1d2067ea35e573a0bf9f8e9e6f3c7fc0ab24d46ac3c645327fc1.png)

NOTE: SCLK in the Example Timing for Width Capture Followed by Period Overflow figure is SCLK0\_0.

The waveform applied to the TIMER\_TMR[n] (or TIMER\_ACI[n] ) pin is not required to have a 50% duty cycle. The minimum input low time is little more than one SCLK0\_0 period. The minimum input high time is a little more than one SCLK0\_0 period. (Refer to the product data sheet for details.) The maximum TIMER\_TMR[n] input frequency is less than SCLK0\_0/2, with a 50% duty cycle. Under these conditions, the WIDCAP mode timer measures: period =2 and pulse width =1.

Figure 20-7: Example Timing for Width Capture Followed by Period Overflow (TMR\_CFG.TMODE=b#1011)

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000006_be4281bf166d43045a390f8d20998669983c2bf8a88c6705516341dc874c5e50.png)

NOTE: SCLK in the Example Timing for Width Capture Followed by Period Overflow figure is SCLK0\_0.

## Windowed Watchdog (WATCHDOG) Modes

In windowed watchdog (WATCHDOG) modes, a timer can take inputs from either the TIMER\_TMR[n] pin or the TIMER\_ACI[n] pin. With this mode, the timer can monitor pulse width (width watchdog mode) or pulse period (period watchdog mode) on the input line. It also compares the measured value against a minimum required value and maximum allowed value and generates an interrupt request appropriately. The timer uses the TIMER\_TMR[n]\_CFG.PULSEHI bit to select polarity of the input signal.
...
Figure 20-8: Watchdog Width Mode Timing

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000007_f96037b4b4f0425d42072c72af2f58eee122ab4b53a5aaf0adf1f12a0682280f.png)

NOTE: SCLK in the Watchdog Width Mode Timing figure is SCLK0\_0.
...
Table 20-8: Windowed Watchdog Width Mode Interpretation
...
Figure 20-9: Watchdog Period Mode Timing

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000008_7734c341e001dae57955062a28b3c16a37ce439252131816b66f7e9ac36a26a3.png)

NOTE: SCLK in the Watchdog Period Mode Timing figure is SCLK0\_0.
...
Table 20-9: Windowed Watchdog Period Mode Interpretation
...
Table 20-9: Windowed Watchdog Period Mode Interpretation (Continued)
...
Figure 20-10: EXTCLK Mode Control Flow

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000009_e765f1cc89dede2c4f32686e403418a9682a410ec027ad7fa4c0dfb603c3b3f4.png)
...
3. To stop one or more timers, first program the TIMER\_STOP\_CFG register to determine whether to stop immediately or gracefully upon receiving a stop command. ADDITIONAL INFORMATION: PWMOUT modes are the only modes where a timer can be configured for
...
```
· In mode __ at event __, · if TIMER_TMR[n]_PER is __ and TIMER_TMR[n]_WID is __ and TIMER_TMR[n]_DLY is __, · then TIMER_ERR_TYPE is __ and TIMER_STAT_ILAT is __.
```

Startup error conditions do not prevent the timer from starting. Similarly, overflow and rollover error conditions do not stop the timer. Illegal cases can cause unwanted behavior of the TIMER\_TMR[n] pin.

NOTE: For PININT mode, the timer does not use error functionality.

## Continuous PWMOUT Mode

Table 20-10: Startup Event
...
|---------------------|-------------------------------------------------|------------------------|-----------------------------------------|---------------------------------------|------------------------------------|
| ≤ 1                 | Anything other than peri- od[8]                 | Anything               | Anything                                | b#10                                  | Set                                |
| ≥ 2                 | Anything including 0, ex- cluding TMR_PER value | Anything in- cluding 0 | ≤ PERIOD                                | Unchanged                             | Unchanged                          |
|                     | Anything including 0                            | Anything in- cluding 0 | > PERIOD                                | Unchanged[9] (De- tected at rollover) | Unchanged (Detected at roll- over) |
|                     | Anything                                        | Anything               | > 2 32 - 1                              | b#11                                  | Set                                |
|                     | =Period                                         | =0                     | =Period                                 | No error                              | Unchanged (Detected at roll- over) |

Table 20-11: Rollover Event

| TIMER_TMR[ n]_PER   | TIMER_TMR[n]_DLY                                | TIMER_TMR[ n]_WID      | TIMER_TMR[ n]_WID + TIMER_TMR[ n]_DLY   | TIMER_ERR_ TYPE                                                 | TIMER_STAT_ILAT (if enabled)   |
|---------------------|-------------------------------------------------|------------------------|-----------------------------------------|-----------------------------------------------------------------|--------------------------------|
| ≥ 1                 | Anything                                        | Anything               | Anything                                | b#10[timer running at SCLK0_0] b#11 [timer running at ALT_CLKx] | Set                            |
| ≥ 2                 | Anything including 0, ex- cluding TMR_PER value | Anything in- cluding 0 | ≤ PERIOD                                | Unchanged                                                       | Unchanged                      |
| ≥ 2                 | Anything including 0, ex- cluding TMR_PER value | Anything >0            | >PERIOD                                 | b#11                                                            | Set                            |
| ≥ 2                 | Anything                                        | Anything               | > 2 32 - 1                              | b#11                                                            | Set                            |
| ≥ 2                 | = Period[10]                                    | =0                     | =Period                                 | b#11                                                            | Set                            |
| ≥ 2                 | >Period                                         | =0                     | >Period                                 | Unchanged                                                       | Unchanged                      |

Table 20-12: Overflow Event (On TMR\_PER Register Programming Error Only)
...
Table 20-14: Overflow Event (On another error, such as DELAY + WIDTH ≥ 2 32 - 1)
...
Table 20-18: Overflow Event (On TMR\_PER Register = 0 Only)
...
Table 20-22: ADSP-SC58x TIMER Register List
...
Figure 20-11: TIMER\_BCAST\_DLY Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000010_589428b35dc7e0d4032f8430b7c8065658d69c6d9c29f3ba6a0e1c572f2c6dc7.png)

Table 20-23: TIMER\_BCAST\_DLY Register Fields
...
Figure 20-12: TIMER\_BCAST\_PER Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000011_5adf544f6dd3b3a1b07c86d70308f7b9b8a28371c2e2539dbedc26f334bee74d.png)

Table 20-24: TIMER\_BCAST\_PER Register Fields
...
Figure 20-13: TIMER\_BCAST\_WID Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000012_e9c845e6643cd95f5ea2d18295e114aecc55db4b66d32ac3b8f07d07b8e35aa6.png)

Table 20-25: TIMER\_BCAST\_WID Register Fields
...
Figure 20-14: TIMER\_DATA\_ILAT Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000013_7c3e8b2b83a6345262e8e4fbca3725ac2b2fe73c24a1aa212d81fd6e785939b0.png)

Table 20-26: TIMER\_DATA\_ILAT Register Fields
...
Figure 20-15: TIMER\_DATA\_IMSK Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000014_2727e30a50c2f483369cae7b6603c4ad48e8ba02cea82ec0ddbcc85b0accd136.png)

Table 20-27: TIMER\_DATA\_IMSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                   |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | TMR[nn]    | Data Interrupt Mask. For all TIMER_DATA_IMSK.TMR[nn] bits, write =0 unmasks (enables) the corre- sponding data interrupt request, and write =1 masks (disables) the corresponding data interrupt request. |
...
Figure 20-16: TIMER\_ERR\_TYPE Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000015_ef0fed54ff3bebfbd5d580d77da29539360d2472e83bfd80ada8cad86ae09515.png)

Table 20-28: TIMER\_ERR\_TYPE Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration   | Description/Enumeration               |
|--------------------|------------|---------------------------|---------------------------------------|
| 15:14              | TERR7      | Error Type for Timer 7.   | Error Type for Timer 7.               |
| (R/NW)             |            | 0                         | No Error                              |
|                    |            | 1                         | Counter Overflow Error                |
|                    |            | 2                         | PER Register Programming Error        |
|                    |            | 3                         | WID or DLY Register Programming Error |
| 13:12              | TERR6      | Error Type for Timer 6.   | Error Type for Timer 6.               |
| (R/NW)             |            | 0                         | No Error                              |
|                    |            | 1                         | Counter Overflow Error                |
|                    |            | 2                         | PER Register Programming Error        |
|                    |            | 3                         | WID or DLY Register Programming Error |

Table 20-28: TIMER\_ERR\_TYPE Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration   | Description/Enumeration               |
|--------------------|------------|---------------------------|---------------------------------------|
| 11:10 (R/NW)       | TERR5      | Error Type for Timer 5.   | Error Type for Timer 5.               |
| 11:10 (R/NW)       | TERR5      | 0                         | No Error                              |
| 11:10 (R/NW)       | TERR5      | 1                         | Counter Overflow Error                |
| 11:10 (R/NW)       | TERR5      | 2                         | PER Register Programming Error        |
| 11:10 (R/NW)       | TERR5      | 3                         | WID or DLY Register Programming Error |
| 9:8 (R/NW)         | TERR4      | Error Type for Timer 4.   | Error Type for Timer 4.               |
|                    |            | 0                         | No Error                              |
|                    |            | 1                         | Counter Overflow Error                |
|                    |            | 2                         | PER Register Programming Error        |
|                    |            | 3                         | WID or DLY Register Programming Error |
| 7:6 (R/NW)         | TERR3      | Error Type for Timer 3.   | Error Type for Timer 3.               |
|                    |            | 0                         | No Error                              |
|                    |            | 1                         | Counter Overflow Error                |
|                    |            | 2                         | PER Register Programming Error        |
|                    |            | 3                         | WID or DLY Register Programming Error |
| 5:4 (R/NW)         |            | Error Type for Timer 2.   | Error Type for Timer 2.               |
|                    | TERR2      | 0                         | No Error                              |
|                    | TERR2      | 1                         | Counter Overflow Error                |
|                    | TERR2      | 2                         | PER Register Programming Error        |
|                    | TERR2      | 3                         | WID or DLY Register Programming Error |
| 3:2 (R/NW)         | TERR1      | Error Type for Timer 1.   | Error Type for Timer 1.               |
|                    |            | 0                         | No Error                              |
|                    |            | 1                         | Counter Overflow Error                |
|                    |            | 2                         | PER Register Programming Error        |
|                    |            | 3                         | WID or DLY Register Programming Error |
...
Figure 20-17: TIMER\_RUN Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000016_2c54fee293f1a5b3f9b71c68a2c47208569e106a05ee1153385533875baf276c.png)

Table 20-29: TIMER\_RUN Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                       |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | TMR[nn]    | Start/Stop Timer n. For all TIMER_RUN.TMR[nn] bits, write =0 for stop, and write =1 for start. Read =1 when timer is running. |
...
Figure 20-18: TIMER\_RUN\_CLR Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000017_e29e60d7fe4deb77bd87382f37629a80c81d63b8cd9c2038861dce2bb3a2867c.png)

Table 20-30: TIMER\_RUN\_CLR Register Fields
...
Figure 20-19: TIMER\_RUN\_SET Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000018_3082e5a10d8995e6027b2b773eca027ea93ac563835cb391aec5a4b56fb1c8ce.png)

Table 20-31: TIMER\_RUN\_SET Register Fields
...
Figure 20-20: TIMER\_STAT\_ILAT Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000019_d68fabeeca6129a5b38470f7a400dcf7b9c4adc092d93050796ad35d56644248.png)

Table 20-32: TIMER\_STAT\_ILAT Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                          |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W1C)        | TMR[nn]    | Status Interrupt Latch. For all TIMER_STAT_ILAT.TMR[nn] bits, status of 0 indicates no error interrupt request is latched, and status of 1 indicates a timer counter overflow or programming error interrupt request is latched. |
...
Figure 20-21: TIMER\_STAT\_IMSK Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000020_f6f214a8cf61ed397d0f4d60ca02904653717dcc08f62a848d3da1217c596167.png)

Table 20-33: TIMER\_STAT\_IMSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                           |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | TMR[nn]    | Status Interrupt Mask. For all TIMER_STAT_IMSK.TMR[nn] bits, write =0 unmasks (enables) the corre- sponding status interrupt request, and write =1 masks (disables) the corresponding sta- tus interrupt request. |
...
Figure 20-22: TIMER\_STOP\_CFG Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000021_8d0eba3b04ef4cc775fb4b99c501961749e7472470f57e0f4fcc646f52d278e2.png)

Table 20-34: TIMER\_STOP\_CFG Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                         |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | TMR[nn]    | Stop Mode Select. For all TIMER_STOP_CFG.TMR[nn] bits, write =0 for graceful termination (PWMOUT modes only), and write =1 for abrupt (immediate halt) on stop. |
...
Figure 20-23: TIMER\_STOP\_CFG\_CLR Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000022_5141634a3cf62dbd42076f5a27734e9cd0d8355385c3d1a0b1159b1e365da133.png)

Table 20-35: TIMER\_STOP\_CFG\_CLR Register Fields
...
Figure 20-24: TIMER\_STOP\_CFG\_SET Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000023_5141634a3cf62dbd42076f5a27734e9cd0d8355385c3d1a0b1159b1e365da133.png)

Table 20-36: TIMER\_STOP\_CFG\_SET Register Fields
...
Figure 20-25: TIMER\_TMR[n]\_CFG Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000024_4534cfec40b2aebb6c3d883cc27383e3a7162b63b8130ffeaa42a6742bb6104f.png)

Table 20-37: TIMER\_TMR[n]\_CFG Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                              | Description/Enumeration                                                                                                                              |
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| 15 (R/W)           | EMURUN     | Emulation Run. The TIMER_TMR[n]_CFG.EMURUN bit causes the timer to run (count) during emulation.                                                     | Emulation Run. The TIMER_TMR[n]_CFG.EMURUN bit causes the timer to run (count) during emulation.                                                     |
| 15 (R/W)           | EMURUN     | 0                                                                                                                                                    | Stop Timer During Emulation                                                                                                                          |
| 15 (R/W)           | EMURUN     | 1                                                                                                                                                    | Run Timer During Emulation                                                                                                                           |
| 14 (R/W)           | BPEREN     | Broadcast Period Enable. The TIMER_TMR[n]_CFG.BPEREN bit enables updates to the TIMER_TMR[n]_PER register simultaneously across more than one timer. | Broadcast Period Enable. The TIMER_TMR[n]_CFG.BPEREN bit enables updates to the TIMER_TMR[n]_PER register simultaneously across more than one timer. |
| 14 (R/W)           | BPEREN     | 0                                                                                                                                                    | Disable Broadcast to PER Register                                                                                                                    |
| 14 (R/W)           | BPEREN     | 1                                                                                                                                                    | Enable Broadcast to PER Register                                                                                                                     |

Table 20-37: TIMER\_TMR[n]\_CFG Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                           |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 13 (R/W)           | BWIDEN     | Broadcast Width Enable. The TIMER_TMR[n]_CFG.BWIDEN bit enables updates to the TIMER_TMR[n]_WID register simultaneously across more than one timer.                                                                                                                                                               |
| 12 (R/W)           | BDLYEN     | Broadcast Delay Enable. The TIMER_TMR[n]_CFG.BDLYEN bit enables updates to the TIMER_TMR[n]_DLY register simultaneously across more than one timer. 0 Disable Broadcast to DLY Register                                                                                                                           |
| 11 (R/W)           | OUTDIS     | Output Disable. The TIMER_TMR[n]_CFG.OUTDIS bit enables or disables the timer pin output buffer. 0 Enable TMR Pin Output Buffer                                                                                                                                                                                   |
| 10 (R/W)           | TINSEL     | Input Select (for WIDCAP, WATCHDOG, PININT modes). 0 Use TMR Pin Input                                                                                                                                                                                                                                            |
| 9:8 (R/W)          |            | Timer 1 Use TMR Alternate Capture Input                                                                                                                                                                                                                                                                           |
|                    | CLKSEL     | Clock Select. The TIMER_TMR[n]_CFG.CLKSEL bit field selects the TIMER clock to use. 0 Use SCLK0_0 1 Use TMR_ALT_CLK0 as TMR Clock 3 Use TMR_ALT_CLK1 as TMR Clock                                                                                                                                                 |
| 7 (R/W)            | PULSEHI    | Polarity Response Select. The TIMER_TMR[n]_CFG.PULSEHI bit defines specific behaviors of the timer based on the operating mode. For more information, see the specific operating mode in the Programming Concepts section. 0 Negative Response or Pulse. A Negative Edge or Negative Action Pulse on the TMR pin. |
|                    |            | Response 1 Positive Response or Pulse. A Positive Edge Response or                                                                                                                                                                                                                                                |

Table 20-37: TIMER\_TMR[n]\_CFG Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                          | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                          |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 6 (R/W)            | SLAVETRIG  | Slave Trigger Response.. The TIMER_TMR[n]_CFG.SLAVETRIG bit controls the trigger response. The trigger pulse has no effect (to stop or start the timer) if the timer is already in the re- quested state.                                                                                                                                                                                        | Slave Trigger Response.. The TIMER_TMR[n]_CFG.SLAVETRIG bit controls the trigger response. The trigger pulse has no effect (to stop or start the timer) if the timer is already in the re- quested state.                                                                                                                                                                                        |
| 6 (R/W)            | SLAVETRIG  | 0                                                                                                                                                                                                                                                                                                                                                                                                | Pulse Stops Timer if it is Running                                                                                                                                                                                                                                                                                                                                                               |
| 6 (R/W)            | SLAVETRIG  | 1                                                                                                                                                                                                                                                                                                                                                                                                | Pulse Starts Timer if it is Stopped                                                                                                                                                                                                                                                                                                                                                              |
| 5:4 (R/W)          | IRQMODE    | Interrupt Modes. The TIMER_TMR[n]_CFG.IRQMODE bit field selects the interrupt request mode. Note that any mismatched combination of the TIMER_TMR[n]_CFG.IRQMODE and the TIMER_TMR[n]_CFG.TMODE bits results in no interrupt being generated. In WIDCAP modes, the position of the interrupt is controlled with the TIMER_TMR[n]_CFG.TMODE bit, and the TIMER_TMR[n]_CFG.IRQMODE bit is ignored. | Interrupt Modes. The TIMER_TMR[n]_CFG.IRQMODE bit field selects the interrupt request mode. Note that any mismatched combination of the TIMER_TMR[n]_CFG.IRQMODE and the TIMER_TMR[n]_CFG.TMODE bits results in no interrupt being generated. In WIDCAP modes, the position of the interrupt is controlled with the TIMER_TMR[n]_CFG.TMODE bit, and the TIMER_TMR[n]_CFG.IRQMODE bit is ignored. |
...
| 5:4 (R/W)          | IRQMODE    | 2                                                                                                                                                                                                                                                                                                                                                                                                | Width Plus Delay Expired Mode. The timer generates an interrupt when the TIMER_TMR[n]_CNT value reaches the value in the TIMER_TMR[n]_WID regis- ter plus the value in the TIMER_TMR[n]_DLY regis- ter. (PWMOUT modes only) The timer generates an interrupt if the deasserting edge is within the specified window. (WATCHDOG modes only)                                                       |
| 5:4 (R/W)          | IRQMODE    | 3                                                                                                                                                                                                                                                                                                                                                                                                | Period Expired Mode. The timer generates an interrupt when the TIMER_TMR[n]_CNT value reaches the val- ue in the TIMER_TMR[n]_PER register. (Continuous PWMOUTand EXTCLK modes only) The timer generates an interrupt if the deasserting edge is outside the specified window.(WATCHDOG modes only)                                                                                              |
...
Depending on the timer operation mode, the counter increment can be clocked by a number of sources, including SCLK0\_0, the TMR or alternate capture input pins, TIMER\_ACLK[n] . The counter retains its value after the timer is disabled.

Figure 20-26: TIMER\_TMR[n]\_CNT Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000025_ba620b025ae4121662358fce240b93aab450f9618f52e773363ec678ae271b5f.png)

Table 20-38: TIMER\_TMR[n]\_CNT Register Fields
...
Figure 20-27: TIMER\_TMR[n]\_DLY Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000026_de8e2e99ceff55ec87f782ef1e3b5cbe3522a048c9d6c0f451063ae9bfcfaa4e.png)

Table 20-39: TIMER\_TMR[n]\_DLY Register Fields
...
Figure 20-28: TIMER\_TMR[n]\_PER Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000027_f0630e344e5b66568f08dd562faa9fff9cbe95c88f39f7559e8c5402a4766ec5.png)

Table 20-40: TIMER\_TMR[n]\_PER Register Fields
...
Figure 20-29: TIMER\_TMR[n]\_WID Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000028_f092925d004b72b61b93d1ceb6de290f7507063f5f50432d6b8b03f0953bf3f2.png)

Table 20-41: TIMER\_TMR[n]\_WID Register Fields
...
As a trigger slave, each timer can generate a unique data trigger pulse signal. The TIMER\_TRG\_IE contains trigger input enable bits for these signals, disabling or enabling the triggers as programmed. The reset value of the TIMER\_TRG\_IE register is 0xFFFF , masking these triggers after reset.

Figure 20-30: TIMER\_TRG\_IE Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000029_45d68f56a062ac20465bcb53141772b48eed22c29748c6d749c7e1045178c25c.png)

Table 20-42: TIMER\_TRG\_IE Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                           |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0                | TMR[nn]    | Trigger Input Enable. For all TIMER_TRG_IE.TMR[nn] bits, write =0 disables the corresponding trigger input, and write =1 enables the corresponding trigger input. |
| (R/W)              |            |                                                                                                                                                                   |
...
Figure 20-31: TIMER\_TRG\_MSK Register Diagram

![Image](23_General-Purpose_Timer_(TIMER)_artifacts/image_000030_8c6967402159ee1f028b8d43cc753a55230b03a56b512f1d23ad617fedf0e0c3.png)

Table 20-43: TIMER\_TRG\_MSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                           |
|--------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | TMR[nn]    | Trigger Output Mask. For all TIMER_TRG_MSK.TMR[nn] bits, write =0 unmasks (enables) the corre- sponding data trigger output, and write =1 masks (disables) the corresponding data trigger output. |