## 36   Asynchronous Sample Rate Converter (ASRC)
...
Table 36-1: ADSP-SC58x ASRC Register List
...
Table 36-2: ASRC Interrupt List
...
| DAI1_IRQH        | ASRC Sample rate change |                       |                       |       25 |
| DAI0_IRQL        |                         |                       |                       |      145 |
| DAI1_IRQL        |                         |                       | 146                   |          |
...
Figure 36-1: ASRC Block Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000000_a45698b94a34c17d7a274c482707051eb275600d0fe1e8d5beb8841d7710e863.png)

Figure 36-2: ASRC Core Architecture

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000001_e1ad0f9599d4bc640ce1c60e67c248b75c0f97496aa1f0e22672149df1bc2376.png)
...
multiple SRCs to form a frame. The serial output data is dithered down to 20, 18, or 16 bits when 20-, 18-, or 16bit output data is selected.
...
![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000002_85be98f6ea95d5553176868e915f6dd759fca95f753aa9abfba3a4b20e815276.png)
...
As shown in the TDM Input/Output Modes figure, using the TDM output port several ASRCs can be daisy-chained together and connected to the SPORT of this or another processor. The ASRC OP contains a 64-bit parallel load shift register. When the SRCx\_FS\_OP\_I pulse arrives, each ASRC loads its left and right data into the 64-bit shift register. The input to the shift register is connected to SRCx\_TDM\_OP\_I , and the output is connected to

SRCx\_DAT\_OP\_O . By connecting the SRCx\_DAT\_OP\_O to the SRCx\_TDM\_OP\_I of the next ASRC, a large shift register is created, which is clocked by SRCx\_CLK\_OP\_I .
...
Figure 36-4: TDM Input/Output Modes

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000003_e99d56126b5354199a5b8103c59d8a015ff8d78a00823c6ce2eaf934e6f8e604.png)
...
Figure 36-5: Typical Configuration for Matched-Phase Mode Operation

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000004_a5aa8a449713d3ababd04cba5ae65b6c2c5d1418a9573125b5caa86d23009acb.png)
...
filter length of the ASRC, which corresponds directly with the group delay. Thus, the magnitude in the phase difference depends upon the resolution of the SRCx\_FS\_OP and SRCx\_FS\_IP counters. The greater the resolution of the counters, the smaller the phase difference error.
...
The phase master ASRC device transmits its SRCx\_FS\_OP / SRCx\_FS\_IP ratio through the data output pin ( SRCx\_DAT\_OP\_O ) to the slave's ASRC's data input pins ( SRCx\_TDM\_OP\_I ). The transmitted data (32-bit subframe) contains 24-bit data and 8-bits matched phase (see the ASRC Data Frame Format by Protocol figure).
...
NOTE: By default, matched phased data is sent on the SRCx\_DAT\_OP\_O pin, but only if the SRCx\_TDM\_OP\_I pin is tied low. The slaves simply ignore the matched phased data if their ASRC\_CTL01.MPHASE1 through ASRC\_CTL23.MPHASE3 bits are cleared (= 0).

## Bypass Mode

When the ASRC\_CTL01.BYP0 , ASRC\_CTL01.BYP1 , ASRC\_CTL23.BYP2 and ASRC\_CTL23.BYP3 bits are set (=1), the input data bypasses the sample rate converter and is sent directly to the serial output port. Dithering is disabled. This mode is ideal when the input and output sample rates are the same and the SRCx\_FS\_IP\_I and SRCx\_FS\_OP\_I signals are synchronous with respect to each other. In matched phase bypass mode, the SRCx\_FS\_OP\_I signal should come at least one SRCx\_CLK\_xx\_I period before SRCx\_FS\_IP\_I . Cases where this is not met could result in data loss. For example, if internal SPORTS are used then the SRCx\_FS\_OP\_I and SRCx\_FS\_IP\_I signals could be driven by different SPORTs so that the timing of these signals can be controlled by enabling them at different times. This mode can also be used for passing through non-audio data since no processing is performed on the input data.
...
word length. The 21BIT\_DITHER signal is used for the consumer version of the SRC to reduce the dynamic range performance to approximately 128 dB.

- NOTE: The ASRC can be programmed to add the triangular Probability Distribution Function (PDF) dither to the digital audio samples. It is advisable to add dither when the input word width exceeds the output word width, for example the input word is 20 bits and the output word is 16 bits. T riangular PDF is generally considered to create the most favorable noise shaping of the residual quantization noise.
...
Table 36-3: ADSP-SC58x ASRC Register List
...
Figure 36-6: ASRC\_CTL01 Register Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000005_9ebb522bd6ee25c949dd5823c11d0bcc21b664e0983d7e8085ca44c28f9e7794.png)

Table 36-4: ASRC\_CTL01 Register Fields
...
| 30 (R/W)           | MPHASE1    | a minimum of 5 SCLK0_0 cycles. Matched-phase Mode 1. The ASRC_CTL01.MPHASE1 bit configures SRC1 to not use its own internally- generated sample rate ratio but use an externally-generated ratio. Used with TDMda- ta.                                                                                                                                                                                 | a minimum of 5 SCLK0_0 cycles. Matched-phase Mode 1. The ASRC_CTL01.MPHASE1 bit configures SRC1 to not use its own internally- generated sample rate ratio but use an externally-generated ratio. Used with TDMda- ta.                                                                                                                                                                                 |
| 30 (R/W)           | MPHASE1    | 0                                                                                                                                                                                                                                                                                                                                                                                                      | Matched phase slave disabled                                                                                                                                                                                                                                                                                                                                                                           |
| 30 (R/W)           | MPHASE1    | 1                                                                                                                                                                                                                                                                                                                                                                                                      | Matched phase slave enabled                                                                                                                                                                                                                                                                                                                                                                            |
...
Table 36-4: ASRC\_CTL01 Register Fields (Continued)
...
Figure 36-7: ASRC\_CTL23 Register Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000006_a32d8ec2faabf15555e672f34323e79d6217b8416c5b8f3a8f69ea25cb8672c1.png)

Table 36-5: ASRC\_CTL23 Register Fields
...
| 30 (R/W)           | MPHASE3    | 0                                                                                                                                                                                                                                                                                                                                                                                                     | Matched phase slave disabled                                                                                                                                                                                                                                                                                                                                                                          |
| 30 (R/W)           | MPHASE3    | 1                                                                                                                                                                                                                                                                                                                                                                                                     | Matched phase slave enabled                                                                                                                                                                                                                                                                                                                                                                           |
...
Table 36-5: ASRC\_CTL23 Register Fields (Continued)
...
Figure 36-8: ASRC\_MUTE Register Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000007_d7ccea7b3a591cecae22eae3e28cbe5a2eccf29ac9cb3f357ba5e57759063d1e.png)

Table 36-6: ASRC\_MUTE Register Fields
...
Figure 36-9: ASRC\_RAT01 Register Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000008_700d7af3fadc7b4c5e3830b365e901bc96eddcb2fdd647957c32a31444f04465.png)

Table 36-7: ASRC\_RAT01 Register Fields
...
Table 36-7: ASRC\_RAT01 Register Fields (Continued)
...
Figure 36-10: ASRC\_RAT23 Register Diagram

![Image](39_Asynchronous_Sample_Rate_Converter_(ASRC)_artifacts/image_000009_baf61a1ef849e27c583fa6d8f08746da9db4cc946e9bb5226597dcf5c2294db9.png)

Table 36-8: ASRC\_RAT23 Register Fields
...
Table 36-8: ASRC\_RAT23 Register Fields (Continued)
