## 37   Sony/Philips Digital Interface (S/PDIF)
...
## ADSP-SC58x SPDIF Register List
...
Table 37-1: ADSP-SC58x SPDIF Register List
...
Table 37-1: ADSP-SC58x SPDIF Register List (Continued)
...
Program the corresponding SRU registers to connect the outputs to the required destinations (see the DAI Routing Capabilities section). The biphase encoded data and the external PLL clock inputs to the receiver are routed through the SRU. The extracted clock, frame sync, and data are also routed through the SRU.

## S/PDIF Interrupt List

Table 37-2: S/PDIF Interrupt List
...
| DAI1_IRQH        | Validity              |                       |                       |       25 |
| DAI0_IRQL        | Non Audio             |                       |                       |      145 |
| DAI1_IRQL        | Locked/Loss of Lock   |                       |                       |      146 |

## Clocking

The module clock of the S/PDIF transceiver is SCLK0\_0. The clock source for the S/PDIF receiver reference clock is CLKO5 from the CDU. When CLKO5 is configured, it supports sampling frequencies of 24 kHz to 192 kHz. The clock to this module may be shut off for power savings.
...
The S/PDIF transmitter, shown in the S/PDIF Transmitter Block Diagram , resides within the DAI, and its inputs and outputs can be routed via the SRU. It receives audio data in serial format, encloses the specified user status information, and converts it into the biphase encoded signal. The serial data input to the transmitter can be formatted as left-justified, I 2 S, or right-justified with word widths of 16, 18, 20 or 24 bits. AES3 Output Block shows the detail of the AES block.
...
Figure 37-1: S/PDIF Transmitter Block Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000000_4f3e861853e7512b43d985f048bad66bde0740ba1bcff68a2310b8889585d6e8.png)
...
Figure 37-2: AES3 Output Block

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000001_5e001c01052a096ca0cf3b1722451377b08ad0bf3f82e269bb8e09fd181546d5.png)
...
Figure 37-3: I 2 S and Left-Justified Formats

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000002_fd833476e6919164289a3daf91a421e9968ce02bbf7de66a633fe086b8c90179.png)

Figure 37-4: Right-Justified Formats
...
![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000003_634dcc535ca7266c2f483151f8cd6c0bbd914a7700cd6ad90e67f61aa429e712.png)
...
In SCDF mode, the transmitter sends successive audio samples of the same signal across both sub frames, instead of channel A and B. The transmitter will transmit at half the sample rate of the input bit stream. The SPDIF\_TX\_CTL.SCDF bit selects SCDF mode. When in SCDF mode, the SPDIF\_TX\_CTL.SCDFLR bit determines whether left or right channel data is transmitted.
...
Figure 37-5: S/PDIF Receiver Block Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000004_a4f432300b8930506d827a5a6ee82bf2fb624e4f3a7017c047086f0e4faedd1f.png)
...
To be AES11 compliant, the recovered left/right clock must be aligned with the preambles within a + or - 5% of the frame period. Since the PLL clock generates a clock 512 times the frame rate clock (512 f SCLK ), this clock can be used and divided down to create the phase aligned jitter-free left/right clock.
...
Figure 37-6: I2S Format

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000005_0c4a72bf327c78a1def22d31661d6cb0a580bc60f3a65523bf89feec12aff8e3.png)
...
MPEG-2, AC-3, DTS, and AAC compressed data may be transmitted without setting either the SPDIF\_RX\_STAT.VALID bit or bit 1 of byte 0. T o detect this data, the IEC61937 and SPMTE 337M standards dictate that there be a 96-bit sync code in the 16-, 20- or 24-bit audio data stream. This sync code consists of four

words of zeros followed by a word consisting of 0xF872 and another word consisting of 0x4E1F . When this sync code is detected, the SPDIF\_RX\_STAT.COMPMODE bits hold the information regarding type of compression.
...
Unlike previous processors, support for single-channel, double-frequency mode (SCDF) is not supported through specific bits within the SPDIF\_RX\_CTL register, but rather have to be implemented in software using the information provided by the CS (channel status) bits.
...
The receiver can recover the clock from the biphase encoded stream using an on-chip NCO shown in the following figure. Note the dedicated NCO is separate from the PLL that supplies the clock to the processor core and which is the default operation of the receiver.

Figure 37-7: S/PDIF Clock Recovery Mechanism

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000006_288ffc3d1c39091453db6b9550530545cb11a348d241d1b3605b1e2db1325c24.png)
...
The INTR\_DAI\_IRQH and INTR\_DAI\_IRQL signals are routed to the SEC and GIC.
...
## ADSP-SC58x SPDIF Register Descriptions

The S/PDIF module (SPDIF) contains the following registers.

Table 37-3: ADSP-SC58x SPDIF Register List
...
Table 37-3: ADSP-SC58x SPDIF Register List (Continued)
...
Figure 37-8: SPDIF\_RX\_CTL Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000007_d77420b0f33c6887f4371638523000881f25daad41618aff488af7e96948d28a.png)

Table 37-4: SPDIF\_RX\_CTL Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                  | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                  |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 4 (R/W)            | RST        | Reset SPDIF Receiver. The SPDIF_RX_CTL.RST bit resets the receiver.                                                                                                                                                                                                                                                                                                                                      | Reset SPDIF Receiver. The SPDIF_RX_CTL.RST bit resets the receiver.                                                                                                                                                                                                                                                                                                                                      |
| 4 (R/W)            | RST        | 0                                                                                                                                                                                                                                                                                                                                                                                                        | Takes the receiver out of reset                                                                                                                                                                                                                                                                                                                                                                          |
| 4 (R/W)            | RST        | 1                                                                                                                                                                                                                                                                                                                                                                                                        | Resets the SPDIF receiver                                                                                                                                                                                                                                                                                                                                                                                |
| 3 (R/W)            | RSTRTAUDIO | Restart Audio. The SPDIF_RX_CTL.RSTRTAUDIO bit restarts the audio once a re-lock has oc- curred. When the S/PDIF receiver loses lock the audio output is set to 0. This bit de- termines the behavior of the audio once lock is re-established. Audio can be manually restarted by toggling this bit high and then low.                                                                                  | Restart Audio. The SPDIF_RX_CTL.RSTRTAUDIO bit restarts the audio once a re-lock has oc- curred. When the S/PDIF receiver loses lock the audio output is set to 0. This bit de- termines the behavior of the audio once lock is re-established. Audio can be manually restarted by toggling this bit high and then low.                                                                                  |
| 3 (R/W)            | RSTRTAUDIO | 0                                                                                                                                                                                                                                                                                                                                                                                                        | Manually restart audio                                                                                                                                                                                                                                                                                                                                                                                   |
...
Figure 37-9: SPDIF\_RX\_STAT Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000008_7ac205560b245dc583b1b8e8f0eb0fc6834cf85d4027aceff36479c4275401f6.png)

Table 37-5: SPDIF\_RX\_STAT Register Fields
...
Table 37-5: SPDIF\_RX\_STAT Register Fields (Continued)
...
Figure 37-10: SPDIF\_RX\_STAT0\_A Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000009_2bb5035fbcbfa7a52890a5d2e3ac0480f7f23efba31936abdaf3556763dceb4a.png)

Table 37-6: SPDIF\_RX\_STAT0\_A Register Fields
...
Figure 37-11: SPDIF\_RX\_STAT0\_B Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000010_2bb5035fbcbfa7a52890a5d2e3ac0480f7f23efba31936abdaf3556763dceb4a.png)

Table 37-7: SPDIF\_RX\_STAT0\_B Register Fields
...
Figure 37-12: SPDIF\_RX\_STAT1\_A Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000011_d90fc033df42926dd78d093fdf14c852fac155cd2c38b4e5e0afa98de2acbe19.png)

Table 37-8: SPDIF\_RX\_STAT1\_A Register Fields
...
Figure 37-13: SPDIF\_RX\_STAT1\_B Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000012_d90fc033df42926dd78d093fdf14c852fac155cd2c38b4e5e0afa98de2acbe19.png)

Table 37-9: SPDIF\_RX\_STAT1\_B Register Fields
...
Figure 37-14: SPDIF\_TX\_CTL Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000013_58ba55e4e49df1863a69d0d5536e7f809cf2f5e1e421b802e7dc6d569920f079.png)

Table 37-10: SPDIF\_TX\_CTL Register Fields
...
Table 37-10: SPDIF\_TX\_CTL Register Fields (Continued)
...
Figure 37-15: SPDIF\_TX\_STAT\_A0 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000014_8721bf053bd41c8ec484eddb2a5518465ef8b2210b4770ecce1ab1dd6654b6c5.png)

Table 37-11: SPDIF\_TX\_STAT\_A0 Register Fields
...
Figure 37-16: SPDIF\_TX\_STAT\_A1 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000015_822fa3bf1c4487aea28f2f56296c81ba3df6498ea290e337504adea3d6a86901.png)

Table 37-12: SPDIF\_TX\_STAT\_A1 Register Fields
...
Figure 37-17: SPDIF\_TX\_STAT\_A2 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000016_22f608772b506c16a623c62f97c1ad3d755875cc2b89e10fcc3389250e39bf0d.png)

Table 37-13: SPDIF\_TX\_STAT\_A2 Register Fields
...
Figure 37-18: SPDIF\_TX\_STAT\_A3 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000017_033edf202be94448efb88855b8fb4feb59e3555ebffd21c539d8100a329db77d.png)

Table 37-14: SPDIF\_TX\_STAT\_A3 Register Fields
...
Figure 37-19: SPDIF\_TX\_STAT\_A4 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000018_828e8b1dddb0a7c1f023d78ea290f5ff6c58c08ffaa1b991d7d20de614ba06da.png)

Table 37-15: SPDIF\_TX\_STAT\_A4 Register Fields
...
Figure 37-20: SPDIF\_TX\_STAT\_A5 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000019_1b8103d61aae88543aa5676f1103e0245695170b55c3493837250baae19c4bbf.png)

Table 37-16: SPDIF\_TX\_STAT\_A5 Register Fields
...
Figure 37-21: SPDIF\_TX\_STAT\_B0 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000020_0577dbe9231c0b8217c7123ae9000f1dfd970870e0339689efd27190b6c8a292.png)

Table 37-17: SPDIF\_TX\_STAT\_B0 Register Fields
...
Figure 37-22: SPDIF\_TX\_STAT\_B1 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000021_42b6c298355a41308e0369b31d1535d6802080c4e70e08f3ab74f8e7c7f8cb7e.png)

Table 37-18: SPDIF\_TX\_STAT\_B1 Register Fields
...
Figure 37-23: SPDIF\_TX\_STAT\_B2 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000022_f6cb42362551977513f6a4b4af71e78adfa8cdc6ad1f7e6e96302db7bc5ebab5.png)

Table 37-19: SPDIF\_TX\_STAT\_B2 Register Fields
...
Figure 37-24: SPDIF\_TX\_STAT\_B3 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000023_c1c74fa63e97d4e550157ba9d5f1456535a84ef37b3f923c066f80d9cf61498a.png)

Table 37-20: SPDIF\_TX\_STAT\_B3 Register Fields
...
Figure 37-25: SPDIF\_TX\_STAT\_B4 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_000024_468ebc76af41361e5adcd9a13b5cf350d8fdcdd2859314b7f62c2c68e0afad13.png)

Table 37-21: SPDIF\_TX\_STAT\_B4 Register Fields
...
Figure 37-26: SPDIF\_TX\_STAT\_B5 Register Diagram

![Image](40_SonyPhilips_Digital_Interface_(SPDIF)_artifacts/image_