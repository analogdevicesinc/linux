...
NOTE: Unlike previous SHARC processors which contained external port DMA, the current EMDMA module can access all memory locations (L1/L2/L3) for source and destination DMA operations.
...
## ADSP-SC58x EMDMA Register List
...
Table 39-1: ADSP-SC58x EMDMA Register List
...
## ADSP-SC58x EMDMA Interrupt List

Table 39-2: ADSP-SC58x EMDMA Interrupt List

|   Interrupt ID | Name        | Description          | Sensitivity   | DMA Channel   |
|----------------|-------------|----------------------|---------------|---------------|
|            170 | EMDMA0_DONE | EMDMA0 Transfer Done | Edge          |               |
|            171 | EMDMA1_DONE | EMDMA1 Transfer Done | Edge          |               |

## ADSP-SC58x EMDMA Trigger List

Table 39-3: ADSP-SC58x EMDMA Trigger List Masters
...
Table 39-4: ADSP-SC58x EMDMA Trigger List Slaves
...
NOTE: Similar to EPDMA on the previous generation of SHARC processors, EMDMA on the ADSP-SC58x processors also has both internal and external sets of DMA parameter registers. The register sets on the new ADSP-SC58x processors can point to either internal or external memory and are referred to as channel 0 and channel 1 in this manual. These channels should not be considered as two EPDMA channels (which is equivalent to two EMDMA module instances) of the previous SHARC processors.
...
The EMDMA supports many different types of DMA, resulting in different lengths of TCBs. The TCB size varies from six locations (chained DMA) to 13 locations (delay line DMA). The EMDMA TCBs table shows the required TCBs for chained DMA.

In the following tables, TCB refers to the start address of the TCB array.

Table 39-5: EMDMA TCBs
...
The order the descriptors are fetched with circular buffering enabled is shown in the EMDMA TCBs for Circular DMA table.

Table 39-6: EMDMA TCBs for Circular DMA
...
The order the descriptors are fetched for scatter/gather DMA with circular buffering enabled is shown in the EMDMA TCBs for Scatter/Gather DMA table and the EMDMA TCBs for Circular Scatter/Gather DMA Address Register table.

Table 39-8: EMDMA TCBs for Scatter/Gather DMA
...
Table 39-9: EMDMA TCBs for Circular Scatter/Gather DMA Address Register

| Address   | Register     |
|-----------|--------------|
| TCB       | EMDMA_CHNPTR |

Table 39-9: EMDMA TCBs for Circular Scatter/Gather DMA Address Register (Continued)
...
Table 39-10: EMDMA\_CTL Register Bit to Operating Modes
...
Table 39-10: EMDMA\_CTL Register Bit to Operating Modes (Continued)
...
Figure 39-1: Circular Buffering Write DMA

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000000_077974fcb9d33b6596613f5020a166ad225b525b0dcbdd7a23066843c2a0ff6d.png)

Figure 39-2: Circular Buffering Read DMA

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000001_31a0b1cffb36269981720bbee4aa24e84107014388f494d0b89cedf95ef9ab76.png)

Figure 39-3: Standard Write DMA
...
![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000002_60263d8696c839903de87e40b2ace88661e1ed5c606841d0c10a2f3b37a814c0.png)
...
The scatter/gather DMA type is a fixed block size scatter/gather DMA that relies on tap list entries in system memory to calculate the (Channel 1 DMA Address) to scatter/gather the DMA. If the DMA direction is Channel 1 write ( EMDMA\_CTL.TRAN =1) then it is a scatter DMA. If EMDMA\_CTL.TRAN =0 then it is a gather DMA. This mode also supports chained and circular buffer chained DMAs.

See the Scatter DMA (Writes) , Gather DMA (Reads) , Circular Buffering Scatter DMA (Writes) , and Circular Gather DMA (Reads) figures.

Figure 39-6: Circular Buffering Scatter DMA (Writes)

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000003_e1ca536659b0eb3203b32d56d3ddda299b74dc6657fd1331d9c7d69edaacd054.png)

Figure 39-7: Circular Gather DMA (Reads)

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000004_216e16050c5f7e0c812a5d1029e073e1211dd5bce0faf719c6c6b37f6a4c52eb.png)
...
Table 39-11: Read/Write Index Pre-Modify (Scatter/Gather DMA)

| Pre-Modify Address Equation EMDMA_INDX1 + EMDMA_TPTR   | Result     | Result   |
|--------------------------------------------------------|------------|----------|
| [ EMDMA_TCNT ] + ( EMDMA_MOD1                          | Block Size | Tap      |
| EMDMA_INDX1 + EMDMA_TPTR [0] + EMDMA_MOD1 × 1          | N          | 0        |
| EMDMA_INDX1 + EMDMA_TPTR [0] + EMDMA_MOD1 × 2          | N          | 0        |
| EMDMA_INDX1 + EMDMA_TPTR [0] + EMDMA_MOD1 × 3          | N          | 0        |
| EMDMA_INDX1 + EMDMA_TPTR [0] + EMDMA_MOD1 ×N           | N          | 0        |
| EMDMA_INDX1 + EMDMA_TPTR [1] + EMDMA_MOD1 × 1          | N          | 1        |
| EMDMA_INDX1 + EMDMA_TPTR [1] + EMDMA_MOD1 × 2          | N          | 1        |
| EMDMA_INDX1 + EMDMA_TPTR [1] + EMDMA_MOD1 × 3          | N          | 1        |
...
| EMDMA_INDX1 + EMDMA_TPTR [M] + EMDMA_MOD1 × 3          | N          | M        |
| EMDMA_INDX1 + EMDMA_TPTR [M] + EMDMA_MOD1 ×N           | N          | M        |

## Pre Modified Read/Write Index

For scatter/gather DMA, the tap list modifiers are employed and the number of taps is determined by the tap list count register ( EMDMA\_TCNT ). The number of sequential reads (block size) from every tap is determined by the

internal count register ( EMDMA\_CNT0 ), and is the same for every tap. The read/write pointer in external index register ( EMDMA\_INDX1 ) serves as the index address for these read/writes.
...
Once the EMDMA\_CNT0 register for the final tap decrements to zero (both EMDMA\_TCNT and EMDMA\_CNT0 are zero), then the tap list DMA access is complete and the DMA completion interrupt is generated (if chaining is enabled the interrupt depends on the EMDMA\_CHNPTR.PCI bit setting).
...
Figure 39-8: Write to Delay Line Buffer

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000005_f39003271fec326d797d20729385484113a9600314722e269649af1901b9ae51.png)
...
Reads from the delay line. For reads, the tap list (TL) modifiers are used and the number of reads is determined by the EMDMA\_CNT0 register. The write pointer in the EMDMA\_INDX0 register serves as the index address for these reads (reads start from where writes end). The EMDMA\_INDX0 register, along with tap list modifiers, are used in a pre-modify addressing mode to create the external address for the reads. Therefore, for each read, the DMA controller fetches the external modifier ( EMDMA\_TCNT register) from the tap list and the reads are circular buffered (if enabled). See the Read From Delay Line Buffer figure.

Figure 39-9: Read From Delay Line Buffer

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000006_0664b0054b753e0a2509b9790f05605a5562a015ab213d50961b213f77e46a02.png)
...
NOTE: The channel 1 DMA address generation follows pre-modify addressing for reads in delay line DMA and therefore the EMDMA\_INDX1 register values are not updated. Also the EMDMA\_MOD1 register does not

have any effect during these delay line reads. Once the read count completes, the EMDMA\_CNT0 register decrements to zero (both EMDMA\_CNT0 and EMDMA\_TCNT are zero) for the final tap. Finally, the delay line DMA access completes and the DMA completion interrupt is generated. If chaining is enabled, the interrupt is dependent on the EMDMA\_CHNPTR.PCI bit setting. The delay line DMA can only be initialized using the TCB. In order to use the delay line DMA for a single DMA sequence, initialize the EMDMA\_CHNPTR register to zero in the TCB.

For each 32-bit tap read, the channel 1 read index is shown in the Read/Write Index Pre-Modify (Scatter/Gather DMA) table. Note that one tap list entry starts multiple reads.

Table 39-12: Read/Write Index Pre-Modify (Scatter/Gather DMA)
...
| EMDMA_INDX0 + EMDMA_TPTR [0] + EMDMA_MOD1 × 3          | N          | 0        |
...
| EMDMA_INDX0 + EMDMA_TPTR [1] + EMDMA_MOD1 × 3          | N          | 1        |
...
| EMDMA_INDX0 + EMDMA_TPTR [M] + EMDMA_MOD1 × 3          | N          | M        |
| EMDMA_INDX0 + EMDMA_TPTR [M] + EMDMA_MOD1 ×N           | N          | M        |

## ADSP-SC58x EMDMA Register Descriptions

Extended Memory DMA (EMDMA) contains the following registers.

Table 39-13: ADSP-SC58x EMDMA Register List
...
Table 39-13: ADSP-SC58x EMDMA Register List (Continued)
...
Figure 39-10: EMDMA\_BASE Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000007_8b18d824931c4e958a37154518b5cd137ae3f26c43b40004ef7a4b1e05bd4ee4.png)

Table 39-14: EMDMA\_BASE Register Fields
...
Figure 39-11: EMDMA\_BUFLEN Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000008_9e10ca6209e9dabd630621ca5d14012fd29f3ce796607bc3f9bf506934e031b1.png)

Table 39-15: EMDMA\_BUFLEN Register Fields
...
The EMDMA\_CHNPTR register contains the address of the next descriptor in memory when the EMDMA\_CTL.CHEN bit =1. This register also has bits to change DMA directions for each descriptor and the PCI bit. Note that the lower 30-bits of this register are to be written with 30 MSB's of the Word Aligned Byte addresses of the corresponding next descriptor address.

Figure 39-12: EMDMA\_CHNPTR Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000009_390550b634ce42b05850ce1d064b5eb4011bca001627fb89017e2f1026aab384.png)

Table 39-16: EMDMA\_CHNPTR Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                         |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 31 (R/W)           | CPDR       | CPDR DMADirection for the next TCB. The EMDMA_CHNPTR.CPDR bit configures whether the DMAis a write to internal memory or a read from internal memory. Note that this setting is applicable only if EMDMA_CTL.OFCEN =1 and is not applicable for Delay Line DMA. |
| 30 (R/W)           | PCI        | Program Controlled Interrupt. The EMDMA_CHNPTR.PCI bit PCI sets whether an interrupt is generated after the current TCB or if no interrupt is generated. (Only affects DMAif chaining is enabled)                                                               |
...
The EMDMA\_CNT0 register contains the number of words to be transferred for channel 0 DMA. Note: If Delay Line DMA is enabled then the EMDMA\_CNT0 register serves as the count register for the Delay Line writes.

Figure 39-13: EMDMA\_CNT0 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000010_19eecb51fb3bb8373314a6fbc4dc7e40d880588adcc0f5e6fbeb5ead1e5e7707.png)

Table 39-17: EMDMA\_CNT0 Register Fields
...
The EMDMA\_CNT1 register contains the number of words to be transferred for channel 1 DMA. Note: If Delay Line DMA is enabled then the EMDMA\_CNT1 register serves as the count register for the Delay Line writes.

Figure 39-14: EMDMA\_CNT1 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000011_59265cabb64c759f33576004634fed380fff09aedb6764cd86a22cb08bccfdff.png)

Table 39-18: EMDMA\_CNT1 Register Fields
...
Figure 39-15: EMDMA\_CTL Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000012_e9156faa285e276274e861e8a93608a61ab91d85271ff55cc62a5a79ec868109.png)

Table 39-19: EMDMA\_CTL Register Fields
...
Table 39-19: EMDMA\_CTL Register Fields (Continued)
...
| 8 (R/W)            | OFCEN      | On the Fly Control Loading Enable. The control bits in EMDMA_CHNPTR register are used to describe the next TCB be- havior if the EMDMA_CTL.OFCEN bit is set and therefore the DMAcontrols can be changed from TCB to TCB. 0 = disables the control bits in the EMDMA_CHNPTR register 1 = Enables the control bits in the EMDMA_CHNPTR register. If chaining is enabled with EMDMA_CTL.OFCEN bit set then the EMDMA_CTL.TRAN bit has no effect, and direction is determined by EMDMA_CHNPTR.CPDR bit. | On the Fly Control Loading Enable. The control bits in EMDMA_CHNPTR register are used to describe the next TCB be- havior if the EMDMA_CTL.OFCEN bit is set and therefore the DMAcontrols can be changed from TCB to TCB. 0 = disables the control bits in the EMDMA_CHNPTR register 1 = Enables the control bits in the EMDMA_CHNPTR register. If chaining is enabled with EMDMA_CTL.OFCEN bit set then the EMDMA_CTL.TRAN bit has no effect, and direction is determined by EMDMA_CHNPTR.CPDR bit. |
...
| 5 (R/W)            | DFLSH      | Flush DMAFIFO. The EMDMA_CTL.DFLSH bit flushes the DMAFIFO. The buffer is only flushed if this bit is set. It can be set with the enable bit. Setting this bit also clears the EMDMA_CTL.DFS bit.                                                                                                                                                                                                                                                                                                    | Flush DMAFIFO. The EMDMA_CTL.DFLSH bit flushes the DMAFIFO. The buffer is only flushed if this bit is set. It can be set with the enable bit. Setting this bit also clears the EMDMA_CTL.DFS bit.                                                                                                                                                                                                                                                                                                    |
| 4 (R/W)            | CBEN       | Circular Buffering Enable. The EMDMA_CTL.CBEN bit enables circular buffering. Circular buffering can be used with normal DMAas well, if circular buffering is enabled with chaining for nor- mal DMAthen ELEP and EBEP should be part of the TCB.                                                                                                                                                                                                                                                    | Circular Buffering Enable. The EMDMA_CTL.CBEN bit enables circular buffering. Circular buffering can be used with normal DMAas well, if circular buffering is enabled with chaining for nor- mal DMAthen ELEP and EBEP should be part of the TCB.                                                                                                                                                                                                                                                    |
...
| 2 (R/W)            | CHEN       | Enable Chaining. The EMDMA_CTL.CHEN bit enables DMAchaining. 0 Chaining disabled 1 Chaining enabled                                                                                                                                                                                                          |
| 1 (R/W)            | TRAN       | DMADirection. The EMDMA_CTL.TRAN bit determines the DMAdata direction. Note: If delay line DMAis enabled then this bit does not have any effect. For delay line DMA, transfer direction depends on the state of delay line transfers. For Internal-Internal or External- External DMAthis bit has to be set. |
| 0 (R/W)            | EN         | DMAEnable. The EMDMA_CTL.EN bit enables DMA.                                                                                                                                                                                                                                                                 |

## Internal Index Register

The EMDMA\_INDX0 register contains the start address of the buffer for Channel 0 DMA. Note: For delay line DMA the EMDMA\_INDX0 register serves as the delay line write index which is the start address of the channel 0 DMA buffer for the channel 1 write data.

Figure 39-16: EMDMA\_INDX0 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000013_465f4595dbd11481ffec224ec24689809d5f0b93e1d0072510f9cc48e9e0c793.png)

Table 39-20: EMDMA\_INDX0 Register Fields
...
Figure 39-17: EMDMA\_INDX1 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000014_a0b91b0b951d35c8ef3ee5666885b0a0d6f3d0162dc979cf08b69275edcefce5.png)

Table 39-21: EMDMA\_INDX1 Register Fields
...
Figure 39-18: EMDMA\_MOD0 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000015_d36a44181098b269dd04d8462225158bdf79332095476b2a3e6ff88978a4f564.png)

Table 39-22: EMDMA\_MOD0 Register Fields
...
Figure 39-19: EMDMA\_MOD1 Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000016_2a56d3582b15ca81e282aaf22f5ffd8ef1bea8866fd071e9de87fc9426b6fc86.png)

Table 39-23: EMDMA\_MOD1 Register Fields
...
Figure 39-20: EMDMA\_TCNT Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000017_d703178b7a0b2e3ce0705b96927673a920751186a683d878c33769630b406d76.png)

Table 39-24: EMDMA\_TCNT Register Fields
...
The EMDMA\_TPTR register holds the address of an array in memory which holds offsets to be used when accessing a Delay-line in system memory. The offset represents the first address of each read block. Note that the lower 30-bits of this register are to be written with 30 MSBs of the Word Aligned Byte address of the Array.

Figure 39-21: EMDMA\_TPTR Register Diagram

![Image](42_Extended_Memory_DMA_(EMDMA)_artifacts/image_000018_8cff5e8c17c22f755b900185c5fa32ba48c7767ffbfffdf3fcc5967071a97432.png)

Table 39-25: EMDMA\_TPTR Register Fields
