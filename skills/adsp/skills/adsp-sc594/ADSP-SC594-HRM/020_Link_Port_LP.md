...
<!-- source: 020_Link_Port_LP.pdf | original pages 927–952 -->
...
NOTE: Reference to CLKO8 in this chapter can be considered as OCLK0\_0.
...
## ADSP-2159x\_SC592\_SC594 LP Register List
...
Table 18-1: ADSP-2159x\_SC592\_SC594 LP Register List
...
## ADSP-2159x\_SC592\_SC594 LP Interrupt List

Table 18-2: ADSP-2159x\_SC592\_SC594 LP Interrupt List
...
## ADSP-2159x\_SC592\_SC594 LP Trigger List

Table 18-3: ADSP-2159x\_SC592\_SC594 LP Trigger List Generators

|   Trigger ID | Name    | Description    | Sensitivity   |
|--------------|---------|----------------|---------------|
|           48 | LP0_DMA | LP0 DMAChannel |               |
|           49 | LP1_DMA | LP1 DMAChannel |               |

Table 18-4: ADSP-2159x\_SC592\_SC594 LP Trigger List Receivers
...
## ADSP-2159x\_SC592\_SC594 LP DMA Channel List

Table 18-5: ADSP-2159x\_SC592\_SC594 LP DMA Channel List
...
- When configured as a transmitter, the port drives both the data and the clock while LP\_ACK is three-stated. In this mode, LP\_CLK is always synchronous with OCLK0\_0.
- When configured as a receiver, the link port drives the acknowledge signal and the data and clock lines are three-stated. In this case, the external LP\_CLK signal can either be synchronous or asynchronous with OCLK0\_0.
- When the link port is disabled, the data, clock, and acknowledge signals are three-stated.

Figure 18-3: Link Port Communication and Handshake Waveform
...
The transmitter samples the LP\_ACK signal. If the signal is high, the transmitter gives out the falling edges of LP\_CLK for data sampling. The LP\_ACK signal is first sampled at the rising edge of OCLK0\_0. One more OCLK0\_0 stage synchronizes the signal further. This synchronized signal is given to the subsequent logic. The LP\_CLK falling edge is aligned with OCLK0\_0 falling edge in a 1:1 clock ratio mode and with the OCLK0\_0 rising edge for the rest of the clock ratios. The following figures explain how the synchronization is maintained between the LP\_ACK and LP\_CLK signals.

In the following figure, synchronizing time is guaranteed to be 1.5 OCLK0\_0 cycles.

Figure 18-4: LP\_ACK Synchronization for OCLK0\_0: LP\_CLK =1:1

<!-- image -->

In the following figure, synchronizing time is guaranteed to be 2 OCLK0\_0 cycles.

Figure 18-5: LP\_ACK Synchronization for OCLK0\_0: LP\_CLK =1:2, 1:4 and Up
...
Figure 18-6: Relationship Between Internal Link Port Clock and Link Port Clock at the Pins
...
Figure 18-7: Transmit FIFO path
...
Figure 18-8: LACK Generation Based on Receive FIFO Status
...
Figure 18-9: Enable the Transmitter Before the Receiver
...
The link port clock ( LP\_CLK ) is derived from the internal system clock (OCLK0\_0). The link port clock to system clock ratio can be configured in the LP\_DIV register. This value applies to the transmitter only. The receiver can operate at any asynchronous frequency up to the maximum frequency, independent of the ratio programmed. The following formula describes the relationship between the frequency of the link port clock, the OCLK0\_0 frequency, and the LP\_DIV value.

- f LP\_CLK = f OCLK0\_0 &lt; or = f LP\_CLK-MAX  if DIV = 0
- f LP\_CLK = f OCLK0\_0 /(2 × DIV) if DIV &gt; 0
...
Figure 18-10: Central Processor-Based Model

Figure 18-12: Link Port Ring Model
...
Figure 18-11: Link Port Full-Duplex Transfer Model
...
6. ADDITIONAL INFORMATION: An example configuration is: DMA\_ADDRSTART , DMA\_XCNT , DMA\_XMOD , and DMA\_CFG registers (Stop/Auto, DMA\_CFG.PSIZE =1, DMA\_CFG.MSIZE =4, interrupt generation and memory read).
...
## ADSP-2159x\_SC592\_SC594 LP Register Descriptions

Link Port (LP) contains the following registers.

Table 18-6: ADSP-2159x\_SC592\_SC594 LP Register List
...
Figure 18-13: LP\_CTL Register Diagram
...
Figure 18-14: LP\_DIV Register Diagram
...
Figure 18-15: LP\_RX Register Diagram
...
Figure 18-16: LP\_STAT Register Diagram
...
Figure 18-17: LP\_TX Register Diagram
...
Figure 18-18: LP\_TXIN\_SHDW Register Diagram
...
Figure 18-19: LP\_TXOUT\_SHDW Register Diagram
