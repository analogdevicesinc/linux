...
<!-- source: 025_Universal_Asynchronous_ReceiverTransmitter_UART.pdf | original pages 1319–1371 -->
...
Table 23-1: UART Specifications
...
## ADSP-2159x\_SC592\_SC594 UART Register List
...
Table 23-2: ADSP-2159x\_SC592\_SC594 UART Register List
...
## ADSP-2159x\_SC592\_SC594 UART Interrupt List

Table 23-3: ADSP-2159x\_SC592\_SC594 UART Interrupt List

|   Interrupt ID | Name        | Description       | Sensitivity   |   DMA Channel |
|----------------|-------------|-------------------|---------------|---------------|
|            138 | UART0_TXDMA | UART0 TransmitDMA | Level         |            20 |
|            139 | UART0_RXDMA | UART0 ReceiveDMA  | Level         |            21 |
|            140 | UART0_STAT  | UART0 Status      | Level         |               |
|            141 | UART1_TXDMA | UART1 TransmitDMA | Level         |            34 |
|            142 | UART1_RXDMA | UART1 ReceiveDMA  | Level         |            35 |
|            143 | UART1_STAT  | UART1 Status      | Level         |               |
|            144 | UART2_TXDMA | UART2 TransmitDMA | Level         |            37 |

Table 23-3: ADSP-2159x\_SC592\_SC594 UART Interrupt List (Continued)

|   Interrupt ID | Name            | Description             | Sensitivity   |   DMA Channel |
|----------------|-----------------|-------------------------|---------------|---------------|
|            145 | UART2_RXDMA     | UART2 ReceiveDMA        | Level         |            38 |
|            146 | UART2_STAT      | UART2 Status            | Level         |               |
|            147 | UART3_TXDMA     | UART3 TransmitDMA       | Level         |            53 |
|            148 | UART3_RXDMA     | UART3 ReceiveDMA        | Level         |            54 |
|            149 | UART3_STAT      | UART3 Status            | Level         |               |
|            269 | UART0_TXDMA_ERR | UART0 Transmit DMAError | Level         |               |
|            270 | UART0_RXDMA_ERR | UART0 Receive DMAError  | Level         |               |
|            271 | UART1_TXDMA_ERR | UART1 Transmit DMAError | Level         |               |
|            272 | UART1_RXDMA_ERR | UART1 Receive DMAError  | Level         |               |
|            273 | UART2_TXDMA_ERR | UART2 Transmit DMAError | Level         |               |
|            274 | UART2_RXDMA_ERR | UART2 Receive DMAError  | Level         |               |
|            275 | UART3_TXDMA_ERR | UART3 Transmit DMAError | Level         |               |
|            276 | UART3_RXDMA_ERR | UART3 Receive DMAError  | Level         |               |

## ADSP-2159x\_SC592\_SC594 UART Trigger List

Table 23-4: ADSP-2159x\_SC592\_SC594 UART Trigger List Generators

|   Trigger ID | Name        | Description       | Sensitivity   |
|--------------|-------------|-------------------|---------------|
|          160 | UART0_TXDMA | UART0 TransmitDMA | Edge          |
|          161 | UART0_RXDMA | UART0 ReceiveDMA  | Edge          |
|          162 | UART1_TXDMA | UART1 TransmitDMA | Edge          |
|          163 | UART1_RXDMA | UART1 ReceiveDMA  | Edge          |
|          164 | UART2_TXDMA | UART2 TransmitDMA | Edge          |
|          165 | UART2_RXDMA | UART2 ReceiveDMA  | Edge          |
|          166 | UART3_TXDMA | UART3 TransmitDMA | Edge          |
|          167 | UART3_RXDMA | UART3 ReceiveDMA  | Edge          |

Table 23-5: ADSP-2159x\_SC592\_SC594 UART Trigger List Receivers
...
Table 23-5: ADSP-2159x\_SC592\_SC594 UART Trigger List Receivers (Continued)
...
## ADSP-2159x\_SC592\_SC594 UART DMA Channel List

Table 23-6: ADSP-2159x\_SC592\_SC594 UART DMA Channel List
...
- The status interrupt output connects directly to the SEC . On many processors, the alternative capture input (( TIMER\_ACI[nn] )) of one of the GP timers also senses the UART\_RX pin. When configured in capture mode, the processor can then use the GP timer to detect the bit rate of the received signal.

## External Interface

Each UART features a UART\_RX (receive) pin and a UART\_TX (transmit) pin available through the general-purpose ports. These two pins usually connect to an external transceiver device that meets the electrical requirements of full-duplex or half-duplex standards. For example, EIA-232, EIA-422, 4-wire EIA-485 for full-duplex or 2-wire EIA-485, LIN for half-duplex. Additionally, the UART features a pair of clear-to-send, input pins ( UART\_CTS ),

and request-to-send, output pins ( UART\_RTS ) for hardware flow control. UART signals are multiplexed with other functions at the pin level.
...
Figure 23-2: UART Hardware Flow Control
...
Table 23-7: UART Bit Rate Examples with 100 MHz SCLK0

| Bit Rate   | EDBO = 0   | EDBO = 0   | EDBO = 0   | EDBO = 1   | EDBO = 1   | EDBO = 1   |
|------------|------------|------------|------------|------------|------------|------------|
| (bits/sec) | DL         | Actual     | %Error     | DL         | Actual     | %Error     |
| 2400       | 2604       | 2400.15    | 0.006      | 41667      | 2399.98    | 0.001      |
| 4800       | 1302       | 4800.31    | 0.006      | 20833      | 4800.08    | 0.002      |

Table 23-7: UART Bit Rate Examples with 100 MHz SCLK0 (Continued)
...
NOTE: Carefully select the SCLK0 frequencies. Even multiples of the bit rate can decrease the error percentage.
...
Figure 23-3: Autobaud Detection
...
Divisor = TIMER\_TMR[n]\_PER /(16 (1-EDBO)  × 8)

or:

- Divisor = TIMER\_TMR[n]\_PER&gt;&gt; 7, if UART\_CLK.EDBO =0
...
Figure 23-4: Autobaud Detection Character 0x40
...
Figure 23-5: Bit Stream on a UART TX Pin Transmitting an 'S' Character (0x53)
...
Figure 23-6: UART Frame with Address Bit
...
In core mode, the processor core moves data to and from the UART. A write to the UART\_THR register initiates the transmit operation. If no former operation is pending, the UART\_THR register passes the data immediately to the

UART\_TSR register. There, it is shifted out at the bit rate characterized by the UART\_CLK register, with start, stop, and parity bits appended as defined by the UART\_CTL register.
...
Figure 23-7: IrDA Transmit Pulse
...
Figure 23-8: IrDA Receiver Pulse Detection
...
The ISRs evaluate the status bits in the UART\_STAT register to determine the signaling interrupt source. The system event controller for the processor assigns and unmasks interrupts. The ISRs must clear the interrupt latches explicitly. T o reduce interrupt frequency on the receive side in core mode, use the UART\_IMSK.ERFCI status interrupt as an alternative to the regular UART\_IMSK.ERBFI receive interrupt. Hardware must ensure that at least two (if UART\_CTL.RFIT =0) or four (if UART\_CTL.RFIT =1) words are available in the receive buffer by the time the interrupt is requested.
...
Figure 23-9: Transmit Interrupts
...
Figure 23-10: Receive Interrupts
...
The UART module uses the UART\_IMSK\_SET.ERFCI bit to enable the receive FIFO count interrupt. If set, a status interrupt request is generated when the UART\_STAT.RFCS is active. The UART\_STAT.RFCS bit indicates a receive buffer threshold level. If the UART\_CTL.RFIT bit is cleared, software can safely read two words out of the UART\_RBR register by the time the UART\_STAT.RFCS interrupt occurs.
...
## ADSP-2159x\_SC592\_SC594 UART Register Descriptions

UART (UART) contains the following registers.

Table 23-9: ADSP-2159x\_SC592\_SC594 UART Register List

| Name     | Description         |
|----------|---------------------|
| UART_CLK | Clock Rate Register |

Table 23-9: ADSP-2159x\_SC592\_SC594 UART Register List (Continued)
...
Figure 23-12: UART\_CTL Register Diagram

<!-- image -->

Table 23-11: UART\_CTL Register Fields
...
Table 23-11: UART\_CTL Register Fields (Continued)
...
Figure 23-13: UART\_IMSK Register Diagram

<!-- image -->

<!-- image -->

Table 23-12: UART\_IMSK Register Fields
...
Table 23-12: UART\_IMSK Register Fields (Continued)
...
Figure 23-14: UART\_IMSK\_CLR Register Diagram

Table 23-13: UART\_IMSK\_CLR Register Fields
...
Table 23-13: UART\_IMSK\_CLR Register Fields (Continued)
...
Figure 23-15: UART\_IMSK\_SET Register Diagram

<!-- image -->

Table 23-14: UART\_IMSK\_SET Register Fields
...
Table 23-14: UART\_IMSK\_SET Register Fields (Continued)
...
Figure 23-16: UART\_RBR Register Diagram

<!-- image -->

Table 23-15: UART\_RBR Register Fields
...
Figure 23-17: UART\_RSR Register Diagram

<!-- image -->

Table 23-16: UART\_RSR Register Fields
...
Figure 23-18: UART\_RXCNT Register Diagram

<!-- image -->

Table 23-17: UART\_RXCNT Register Fields
...
Figure 23-19: UART\_SCR Register Diagram

<!-- image -->

Table 23-18: UART\_SCR Register Fields
...
Figure 23-20: UART\_STAT Register Diagram

<!-- image -->

Table 23-19: UART\_STAT Register Fields
...
Table 23-19: UART\_STAT Register Fields (Continued)
...
Figure 23-21: UART\_TAIP Register Diagram

<!-- image -->

Table 23-20: UART\_TAIP Register Fields
...
Figure 23-22: UART\_THR Register Diagram

<!-- image -->

Table 23-21: UART\_THR Register Fields
...
Figure 23-23: UART\_TSR Register Diagram

<!-- image -->

Table 23-22: UART\_TSR Register Fields
...
Figure 23-24: UART\_TXCNT Register Diagram

<!-- image -->

Table 23-23: UART\_TXCNT Register Fields
