...
<!-- source: 313_System_Security.pdf | original pages 3131–3134 -->

## 39   System Security

The requirement to protect content, keys, intellectual property, and other sensitive information have become increasingly prevalent. The processor contains several modules and system elements that contribute to creating a secure operating environment for trusted code to execute.
...
When security is not enabled, OTP can be accessed freely. When a user decides to set the LOCK bit in OTP memory to enable security, portions of the OTP , specifically the first 6K bits and another 1K bits for the customer boot information are also locked. From then on, only secure requesters can access those memory regions.
