...
<!-- source: 319_FIR_Accelerator_FIR.pdf | original pages 3383–3434 -->

## 45   FIR Accelerator (FIR)
...
NOTE: The FIR accelerator module has a local memory that the core cannot access during regular operation. Unlike previous SHARC processors, the FIR accelerator modules each have access to the system memory (onchip or off-chip).
...
Table 45-1: ADSP-SC59x FIR Register List
...
## ADSP-SC59x FIR Interrupt List

Table 45-2: ADSP-SC59x FIR Interrupt List

|   Interrupt ID | Name         | Description               | Sensitivity   | DMA Channel   |
|----------------|--------------|---------------------------|---------------|---------------|
|            166 | FIR0_DMA     | FIR0 Core1DMA             | Edge          |               |
|            167 | FIR0_STAT    | FIR0 Core 1 Status        | Edge          |               |
|            176 | FIR1_DMA     | FIR1 Core2DMA             | Edge          |               |
...
## ADSP-SC59x FIR Trigger List

Table 45-3: ADSP-SC59x FIR Trigger List Generators
...
Table 45-4: ADSP-SC59x FIR Trigger List Receivers
...
Table 45-5: Pipeline Operation for Window Size = 1

<!-- image -->

Table 45-5: Pipeline Operation for Window Size = 1 (Continued)
...
Figure 45-3: Multi-Iteration Filtering Flow
...
Figure 45-4: Filter Output Calculation
...
Table 45-6: TCBs for Chained DMA in Legacy Mode
...
Table 45-7: TCBs for Chained DMA in Auto Configuration Mode
...
Figure 45-6: Wait for Core Intervention ≥ Idle (if auto channel iterate bit = 0)
...
```
Coefficients = b0, b1, b2,……, b4095 Input data = x[n], x [n - 1],……….x[n - 4095] Filter 2 Coefficients = b4096, b4097,…………..,b8191
```
...
Table 45-8: FIR Interrupt Overview
...
## ADSP-SC59x FIR Register Descriptions

The FIR accelerator (FIR) contains the following registers.

Table 45-9: ADSP-SC59x FIR Register List
...
Figure 45-8: FIR\_CHNPTR Register Diagram

<!-- image -->

Table 45-10: FIR\_CHNPTR Register Fields
...
Figure 45-9: FIR\_COEFCNT Register Diagram

<!-- image -->

Table 45-11: FIR\_COEFCNT Register Fields
...
Figure 45-10: FIR\_COEFIDX Register Diagram

<!-- image -->

Table 45-12: FIR\_COEFIDX Register Fields
...
Figure 45-11: FIR\_COEFMOD Register Diagram

<!-- image -->

Table 45-13: FIR\_COEFMOD Register Fields
...
Figure 45-12: FIR\_CTL1 Register Diagram

<!-- image -->

Table 45-14: FIR\_CTL1 Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                |
|--------------------|------------|----------------------------------------------------------------------------------------|
| 31 (R/W)           | ACM        | Auto Configuration Mode. The FIR_CTL1.ACM bit configures the mode for loading the TCB. |

Table 45-14: FIR\_CTL1 Register Fields (Continued)

| Bit No. (Access)   | Bit Name     | Description/Enumeration                                                                                                                                                                                                                                                                    |
|--------------------|--------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 30 (R/W)           | HALT         | HALT (Pause Accelerator). This bit is only valid in Auto Configuration mode (ACM)                                                                                                                                                                                                          |
| 29 (R/W)           | BURST16_DIS  | Disable 16 Burst. The FIR_CTL1.BURST16_DIS bit configures the accelerator to disable 16 burst when in burst mode.                                                                                                                                                                          |
| 28 (R/W)           | DCP_DIS      | Disable Data Coefficient Load in Parallel. FIR_CTL1.DCP_DIS bit enables data and coefficient loading to occur in                                                                                                                                                                           |
| 27 (R/W)           |              | The par- allel. 0 Enable 1 Disable                                                                                                                                                                                                                                                         |
|                    | PFB_EN       | Enable PFB on Both FIR Channels. 0- FIR Pre-Fetch Buffer Disabled 1- FIR Pre-Fetch Buffer Enabled                                                                                                                                                                                          |
| 26 (R/W)           | SMQ_LIUPS_EN | Legacy I/P and O/P Index Update Scheme in SMART_Q Mode. The FIR_CTL1.SMQ_LIUPS_EN bit configures the scheme the accelerator uses to update the input index (II) and output index (OI). This bit is only valid in ACM. 0 Update the II with all '0' and the OI with all 'F', respec- tively |
| 14 (R/W)           | RND          | Rounding Mode. The FIR_CTL1.RND bit configures the accelerator to use a rounding mode.                                                                                                                                                                                                     |
|                    |              | 0 Round to Nearest                                                                                                                                                                                                                                                                         |
...
| 9 (R/W)            | CAI        | Channel Auto Iterate. The FIR_CTL1.CAI bit, if cleared, causes the TDMprocessing to stop (idle) once all channels are done. If set, processing moves to the first channel and continuesTDM processing in a loop when all channels are done. Channel Auto Iterate is not available in Auto Configuration Mode (ACM). |
...
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5:1 (R/W)          | CH         | Number of Channels. The FIR_CTL1.CH bit field configures the number of channels and is programma- ble from 0 to 31. This bit field is only valid in Legacy Mode. There is no channel number limitation in Auto Configuration Mode (ACM) as the accelerator keeps proc- essing the TCBs until the chain pointer becomes null. | Number of Channels. The FIR_CTL1.CH bit field configures the number of channels and is programma- ble from 0 to 31. This bit field is only valid in Legacy Mode. There is no channel number limitation in Auto Configuration Mode (ACM) as the accelerator keeps proc- essing the TCBs until the chain pointer becomes null. |
|                    |            | 0                                                                                                                                                                                                                                                                                                                            | Channel 1                                                                                                                                                                                                                                                                                                                    |
|                    |            | 1-30                                                                                                                                                                                                                                                                                                                         | Channel 2-31                                                                                                                                                                                                                                                                                                                 |
|                    |            | 31                                                                                                                                                                                                                                                                                                                           | Channel 32                                                                                                                                                                                                                                                                                                                   |
...
Figure 45-13: FIR\_CTL2 Register Diagram

<!-- image -->

Table 45-15: FIR\_CTL2 Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                               |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| 31 (R/W)           | TMASK      | Trigger Mask. The FIR_CTL2.TMASK bit enables trigger generation for the channel. It is only val- id in Auto Configuration Mode (ACM). 0 Enable 1 Mask |
| 30 (R/W)           | UPSAMP     | Up Sampling Enable. The FIR_CTL2.UPSAMP bit enables up sampling. 0 Down Sampling 1 Up Sampling                                                        |

Table 45-15: FIR\_CTL2 Register Fields (Continued)
...
Figure 45-14: FIR\_DBG\_ADDR Register Diagram

<!-- image -->

Table 45-16: FIR\_DBG\_ADDR Register Fields
...
Figure 45-15: FIR\_DBG\_CTL Register Diagram

<!-- image -->

Table 45-17: FIR\_DBG\_CTL Register Fields
...
Figure 45-16: FIR\_DBG\_RDDAT Register Diagram

<!-- image -->

Table 45-18: FIR\_DBG\_RDDAT Register Fields
...
Figure 45-17: FIR\_DBG\_WRDAT Register Diagram

<!-- image -->

Table 45-19: FIR\_DBG\_WRDAT Register Fields
...
Figure 45-18: FIR\_DMASTAT Register Diagram

<!-- image -->

Table 45-20: FIR\_DMASTAT Register Fields
...
Table 45-20: FIR\_DMASTAT Register Fields (Continued)
...
Figure 45-19: FIR\_INBASE Register Diagram

<!-- image -->

Table 45-21: FIR\_INBASE Register Fields
...
Figure 45-20: FIR\_INCNT Register Diagram

<!-- image -->

Table 45-22: FIR\_INCNT Register Fields
...
Figure 45-21: FIR\_INIDX Register Diagram

<!-- image -->

Table 45-23: FIR\_INIDX Register Fields
...
Figure 45-22: FIR\_INMOD Register Diagram

<!-- image -->

Table 45-24: FIR\_INMOD Register Fields
...
Figure 45-23: FIR\_MACSTAT Register Diagram

<!-- image -->

Table 45-25: FIR\_MACSTAT Register Fields
...
Table 45-25: FIR\_MACSTAT Register Fields (Continued)
...
Figure 45-24: FIR\_OUTBASE Register Diagram

<!-- image -->

Table 45-26: FIR\_OUTBASE Register Fields
...
Figure 45-25: FIR\_OUTCNT Register Diagram

<!-- image -->

Table 45-27: FIR\_OUTCNT Register Fields
...
Figure 45-26: FIR\_OUTIDX Register Diagram

<!-- image -->

Table 45-28: FIR\_OUTIDX Register Fields
...
Figure 45-27: FIR\_OUTMOD Register Diagram

<!-- image -->

Table 45-29: FIR\_OUTMOD Register Fields
...
Figure 45-28: FIR\_SCTL1 Register Diagram

<!-- image -->

Table 45-30: FIR\_SCTL1 Register Fields
...
Figure 45-29: FIR\_SCTL2 Register Diagram

<!-- image -->

Table 45-31: FIR\_SCTL2 Register Fields
...
Figure 45-30: FIR\_SGCTL Register Diagram

<!-- image -->

Table 45-32: FIR\_SGCTL Register Fields
