...
<!-- source: 321_Boot_ROM_and_Booting_the_Processor.pdf | original pages 3484–3629 -->

## 47    Boot ROM and Booting the Processor
...
Table 47-1: Boot Process SRAM Requirements
...
For the ADSP-SC598 variant, core 0 is responsible for controlling the boot process. Cores 1 and 2 are held in reset until core 0 releases them from reset during the preboot phase of the boot process.

The initial sequence of events after the completion of the reset sequence are shown below.

Figure 47-1: Core Reset Sequencing - ADSP-SC598

<!-- image -->

The ADSP-SC596 variant contains two cores with IDs of 0 and 1. In this device, core 0 is responsible for controlling the boot process; core 1 is held in reset until released by core 0 during the preboot phase of the boot process. The initial sequence of events after the completion of the reset sequence are shown.

Figure 47-2: Core Reset Sequencing - ADSP-SC596
...
- Initialize the register banks X0 to X30 to zero
- Initialize SCTLR\_EL3
- Update the vector base address to point to AARCH64 exceptions in VBAR\_EL3/EL2/EL1
- Determine if the CPU MIDR is A55. If not jump to forever WFI loop.
- Disable power down bit for Cortex A55 CPU in CORTEX\_A55\_CPUPWRCTLR\_EL1 register.
- Check if the CPU is 0. If not, then jump to forever WFI loop.
- Initialize the translation table control registers.
- Set memory attribute for all the address space as Normal Non-Cacheable. Only MMR and OTP space are configured as Device memory.
- Initialize SCTLR\_EL3 without disturbing setting done above:
- Initialize SCR\_EL3

- Initialize MDCR\_EL3
- Initialize PMCR\_EL0
- Enable External Aborts, Error Interrupts, IRQ and FIQ.
- Initialize CPTR\_EL3
- Check If Data Independent Timing (DIT) functionality is implemented, always enable DIT in EL3
- Install Faults
- Setup the memory and stack
- Initialize system control register HCR\_EL2 to zero
- Initialize SCTLR\_EL2/EL1 to 0x30C50838
- Read the RCU\_STAT register and store locally in register set
...
ADSP-SC598 Processor - Core 1 is not responsible for the booting of the processor from reset and thus enters a safe IDLE state allowing access to the cores L1 resources by any other core.
...
Table 47-2: Primary/Secondary DAG Configuration for C Run-Time Setup
...
In the ADSP-SC598 Processor , core 1 is not responsible for the booting of the processor from reset and thus enters a safe IDLE state allowing access to the cores L1 resources by any other core. Core 2 has no entry point into the main boot rom to execute the preboot process or controller boot process from reset of the device.
...
Table 47-3: Initial Faults installed during Preboot
...
- NOTE: In the ADSP-SC598 processor, the SYSFAULT pin is dedicated. However the SYSFAULT signal also appears on the PC\_07 in pin mux.
...
Table 47-4: SPU Secure Controllers During Boot

|   SPU Endpoint ID | Controller Name              |
|-------------------|------------------------------|
|               147 | MDMA0 Source DMAChannel      |
|               148 | MDMA0 Destination DMAChannel |
|               149 | MDMA1 Source DMAChannel      |
|               150 | MDMA1 Destination DMAchannel |
|               145 | CRC0                         |
|               146 | CRC1                         |
|               178 | PKTE                         |
...
Table 47-5: SMPU Configuration

| SMPU Instance          |   SMPU Instance | Open SMPU_SECURECTL Value                        | Locked SMPU_SECURECTL Value      |
|------------------------|-----------------|--------------------------------------------------|----------------------------------|
| SMC                    |               0 | SMPU_SECURECTL.WNSEN &#124; SMPU_SECURECTL.RNSEN | 0x00000000 (Default Reset Value) |
...
Table 47-6: Secure Debug Key Load Procedure
...
NOTE: In the ADSP-SC59x processor family, the boot ROM supports two 128-bit secure debug keys, secure\_emu\_key0 and secure\_emu\_key1 , where only one of them is used by the boot ROM at a time. Each of the secure debug keys is associated with a bypass key ( emu\_key0\_disable or emu\_key1\_disable ) which decides the validity of the corresponding key. Any non-zero value written into the 16-bit bypass key invalidates that particular secure debug key.

The validity of the secure debug keys can be determined as shown in the Secure Debug Key Validation table:

Table 47-7: Secure Debug Key Validation
...
Table 47-8: ADI\_ROM\_OTP\_BOOT\_CGU\_INFO Members
...
| uint32_t | reserved1:1 (bitfield)            | Reserved                                             |
| uint32_t | ctl_S1SELEXEN:1 (bitfield)        | CGU_CTL.S1SELEXEN value                              |
| uint32_t | auto_disable:1 (bitfield)         | Disable auto-alignment                               |
...
| uint32_t | reserved2:8 (bitfield)            | Reserved                                             |
| uint32_t | divex_S1SELEX:8 (bitfield)        | CGU_DIVEX.S1SELEX value                              |
...
Table 47-8: ADI\_ROM\_OTP\_BOOT\_CGU\_INFO Members (Continued)
...
Table 47-9: L1 Memory Initialization
...
Table 47-10: Default Entry Point
...
NOTE: NO-BOOT processing is usually only entered because of the boot mode pin sampling that results in execution of the No-Boot boot mode. This processing can also be optionally enabled by setting RCU\_BCODE.HALT field. The RCU\_BCODE.HALT setting can be especially useful for debug sessions to force the execution of the NO-BOOT mode regardless of the SYS\_BMODE[n] state allowing a user application to be loadedusing the debug tools without fear of the image being corrupted because of attempting to boot through another source.
...
Table 47-68 ADI\_ROM\_OTP\_DMC\_CONFIG Members provides details of the OTP region that is used to store the DMC configuration. In the table, the ADI\_ROM\_OTP\_BOOT\_CFG::dmcEn must be set for the settings to apply.

Table 47-11: ADI\_ROM\_OTP\_DMC\_CONFIG Members
...
Table 47-11: ADI\_ROM\_OTP\_DMC\_CONFIG Members (Continued)
...
Table 47-13: Boot Mode Specific SPU Configuration

| Boot Mode                                                            | SPU Endpoint ID   | Controller Name                                                         |
|----------------------------------------------------------------------|-------------------|-------------------------------------------------------------------------|
| SPI Controller Boot (Memory- Mapped Mode) and OSPI Con- troller Boot | 147, 148          | MDMA0 Source DMAChannel, MDMA0 Destination DMAChan- nel                 |
| SPI Controller Boot (Peripheral Mode)                                | 100, 98, 96, 94   | SPI3 Receive DMA, SPI2 Receive DMA, SPI1 Receive DMA, SPI0 Re- ceiveDMA |
| eMSI Controller Boot                                                 | 175               | EMSI0                                                                   |
| SPI Target Boot                                                      | 100, 98, 96, 94   | SPI3 Receive DMA, SPI2 Receive DMA, SPI1 Receive DMA, SPI0 Re- ceiveDMA |

Table 47-13: Boot Mode Specific SPU Configuration (Continued)
...
NOTE: In the Boot Mode Specific SPU Configuration table, for a given boot mode, all the SPU resources are not configured. Only a single peripheral instance is enabled according the peripheral instance selected for boot. For example if the boot command for the boot mode indicates boot from UART0 only the UART0 Receive DMA is configured, the other UART Receive DMAs are not configured for secure access.
...
Table 47-14: Default Boot ROM API Parameters
...
| eMMC Controller Boot | 0x00000023 | 0x00000000 | 0x00000000   | Points to an empty routine in ROM | 0x00082009 |
...
NOTE: For SPI controller boot and OSPI controller boot modes, start address (pAddress) is set to 0x60000000, This corresponds to the first memory location in the respective flash device (0x0). Similarly, the eMMC

controller boot start address (pAddress) is set to 0x00000023, which corresponds to the default 34th bock of memory location in the eMMC device.
...
|              110 | eMMC Controller Boot | Boot from eMMC card through eMSI peripheral.                                                                                                     |
...
Figure 47-3: SPI Memory Connections
...
NOTE: If modifying the slave select signal used during the boot process, configure the pin multiplexing to enable the correct functionality for the pin. Once the boot process has proceeded past the configuration function

and the boot process has actually started, the boot kernel will not perform any further pin multiplexing operations.
...
Figure 47-4: SPI Device Detection Principle

<!-- image -->

Table 47-16: SPI Master BCODE Configuration Lookup Table
...
Table 47-17: SPI Master BCODE Configuration Lookup Table
...
Table 47-17: SPI Master BCODE Configuration Lookup Table (Continued)
...
Table 47-18: SPI Master Boot Command Descriptions
...
Table 47-18: SPI Master Boot Command Descriptions (Continued)
...
| 14:12  | ROM_BCMD_SPIM_SSEL    | 101       | Reserved                                                                                                                                                                         |
| 14:12  | ROM_BCMD_SPIM_SSEL    | 110       | Reserved                                                                                                                                                                         |
| 14:12  | ROM_BCMD_SPIM_SSEL    | 111       | Reserved                                                                                                                                                                         |
...
Figure 47-5: Connection between Host (SPI Master) and Processor (SPI Slave)

<!-- image -->

Figure 47-6: Connection between Host (SPI Master) and Processor (SPI Slave) DIOM

<!-- image -->

Figure 47-7: Connection between Host (SPI Master) and Processor (SPI Slave) QSPI
...
Table 47-19: SPICMD Descriptions
...
Figure 47-8: SPI Program Flow on the Host Side
...
Table 47-20: SPI Slave Boot Command Bit Descriptions
...
performed under the control of DMA. The maximum supported operating frequency of the link port is 125 MHz for which the controller boot source is responsible for deriving the clock frequency. The link port receiver operates at an asynchronous frequency up to the maximum supported operating frequency.
...
Table 47-21: LINKPORT Target Boot command Bit Descriptions
...
Table 47-21: LINKPORT Target Boot command Bit Descriptions (Continued)
...
Figure 47-9: Connection Between Host and Processor
...
Figure 47-10: Host Relying on RTS
...
Figure 47-11: UART Autobaud Detection Waveform
...
Table 47-22: UART Target Boot command Bit Descriptions
...
Table 47-22: UART Target Boot command Bit Descriptions (Continued)
...
NOTE: The ADSP-SC598 processor family supports OSPI controller boot through PC\_XX pins, while SPI controller boot is supported through PA\_XX pins. Separate flash devices are required for both the boot modes, as they don't share the common PORT pins.
...
Figure 47-12: SPI Memory Connections
...
Figure 47-13: SPI Device Detection Principle

<!-- image -->

Table 47-23: OSPI Controller BCODE Configuration Lookup Table
...
Table 47-24: OSPI Controller BCODE Configuration Lookup Table
...
Table 47-25: OSPI Controller BCODE Configuration Lookup Table
...
Table 47-25: OSPI Controller BCODE Configuration Lookup Table (Continued)
...
Table 47-26: OSPI Controller BCODE Configuration Lookup Table (Applicable to Secure Streams Only)
...
Table 47-27: OSPI Controller Boot Command Bit Descriptions
...
Table 47-27: OSPI Controller Boot Command Bit Descriptions (Continued)
...
## eMSI Controller Boot Mode

eMSI boot mode allows the processor to boot from the onboard eMMC devices. Boot support for eMMC devices is available, but, SD card boot is not supported. Data is loaded from the eMMC device in page mode; it is read 1024 bytes at a time. This process optimizes boot time by loading the data in two buffers in a ping-pong fashion, filling one buffer while another buffer is being processed by the boot kernel. In eMSI boot, there is support for booting from either from the user area of eMMC device or the boot area of eMMC device. The user can choose between the reliability provided by the boot area memory of eMMC device and the abundance of space to store a large boot stream in the user area.

The boot flow for eMSI boot mode is shown in the figure:

Figure 47-14: eMSI Boot Mode Flow

<!-- image -->

After the eMSI controller is initialized to read from the eMMC device, the ROM\_BCMD\_EMSI\_BOOTENABLE field in dBootCommand determines whether the boot is attempted from the user area or the boot area of eMMC device. (The field is read from the OTP).

When ROM\_BCMD\_EMSI\_BOOTENABLE = 0 (default), boot is attempted from the user area of the eMMC device. (An engineer can program the OTP to update the ROM\_BCMD\_EMSI\_BOOTENABLE field).

Booting from the user area includes:

- Enumerating the connected eMMC devices -the process allocates RCA to the card.
- Loading boot data-eMSI can be configured to read the data either in SDR or DDR mode based on the ROM\_BCMD\_EMSI\_SDRMODE bit in the OTP. By default, the reads are done in 8-bit DDR mode, unless

this bit is configured to initiate the reads in SDR mode. A bus width of 8-bit is used by default to get the maximum throughput. The bus width can be reconfigured to 1-bit or 4-bit through the ROM\_BCMD\_EM-SI\_BUSWID field in the dBootCommand . In user area boot (default), the boot stream is read from block 34 (LBA-34). It is expected that the user has programmed the boot stream to start at this address. The user can implement MBR or GUID because the initial 33 blocks of eMMC device are available. However, the boot can be done from any other block including block 0 by programming a second dBootCommand ( dBootCommand1 ) to indicate the block. If the field is left unprogrammed, the boot will happen from block 34 of the user area. To boot from any other block user must program a value of Block Number +1 in dBootCommand1 . For example to boot from block 20, program a value of 21; to boot from block 0, program a value of 1.

When ROM\_BCMD\_EMSI\_BOOTENABLE =1 in OTP, the boot kernel expects the boot stream to be stored in the boot partition 1 of the eMMC device.

Booting from the boot area includes:

- Configuring the controller for boot operation-in this case, boot kernel expects the eMMC device to have the eMMC device configured for 8-bit SDR access to the boot partition for the best throughput for reading the boot stream (DDR mode is not supported by the eMSI controller during boot operation so DDR mode should not be set by user for eMMC boot operation).
- Reading the boot data-the boot kernel reads 1024 bytes in page mode until the end of boot stream and then terminates the read from boot area.

## eMSI Boot Configuration

eMSI boot configuration ensures that the eMSI controller is set up properly and the eMMC device is in a state to read the data out. The configuration process can vary depending on the value programmed in the dBootCommand in OTP or passed through the ROM API call.

EMSI\_BOOTENABLE = 0. Booting from the user area. Configure as follows:

1. Configure the eMSI clock to less than 400 kHz for enumeration.
2. Perform eMMC device enumeration and set up the device RCA.
3. Set up the eMSI and eMMC device bus width and DDR mode as per the BCMD\_EMSI\_BUSWID and BCMD\_EMSI\_SDRMODE values in dBootCommand .
4. When BCMD\_EMSI\_SDRMODE = 1 (SDR mode), the BCMD\_EMSI\_HISPEEDENABLE bit is used to determine whether to put card in high speed mode or at the default speed.
5. Configure the frequency of the eMSI clock as per the clock settings in BCMD\_EMSI\_SPEED field in dBootCommand .

NOTE: For power up case DDR mode in 8-bit bus width with clocks to configured to maximum frequency possible as per default CGU clocks.

6. Call the load function to read 512 bytes of data from loader stream.

7. Identify the type of boot stream (secure/non-secure).

EMSI\_BOOTENABLE =1. Booting from the boot area. Configure as follows:

1. Configure the frequency of the eMSI clock as per the clock settings in the BCMD\_EMSI\_SPEED field in dBootCommand .
2. Configure the bus mode as per the BCMD\_EMSI\_BUSWID field in dbootcommand . When booting from the boot area, only the SDR mode of operation is supported. Therefore, DDR mode should not be configured.
3. Call the load function to read 512 bytes of data from the loader stream.
4. Identify the type of boot stream (secure/non-secure).

## eMSI Data Loading

The eMSI load routine is responsible for reading the boot data stored in the eMMC device. It is called by the boot kernel to request data from the device. Boot ROM employs different load routines based on whether user has requested boot from boot area or user area of the eMMC device. For boot from user area (EMSI\_BOOTENABLE= 0), the load routines used depend on state of the BCMD\_EMSI\_SAFEBOOT bit in dBootCommand . The BCMD\_EMSI\_SAFEBOOT bit is provided as an optional way to load the boot stream using multi-block read commands, each reading 1024 bytes of data. This method of loading is slower. Sending multi-block read commands to read each set of 1024 byte adds some delay in reading the data. Each read command has latency that varies across eMMC devices.

When BCMD\_EMSI\_SAFEBOOT =1, the load function does the following:

- Sets up the block length on device to 512 bytes.
- Sends CMD23 to predefine the length of transfer as 1024 bytes.
- Sets up the descriptor for ADMA2 to read 1024 bytes.
- Issues a multi-block read every time the load function is called to read 1024 bytes.

When BCMD\_EMSI\_SAFEBOOT = 0 (default), the eMSI load routine is set up to use the unterminated multiblock read command. Data is read from the eMMC device as long as the end of the boot stream is not detected.

On the first call of the load function:

1. The eMSI is configured for a AMDA2 read.
2. Descriptors are set up.
3. A multi-block read command is issued.

For every subsequent call of the load function, no further multi-block read command is issued; only the ADMA2 descriptor is updated to update the destination buffer (where the data is read). Once the descriptor is ready, the ADMA2 requests the data, and the controller continues to read the data from the device for the multi-block read issued on the first call of the load function. The controller does not have to issue the read command every time the data must be read from the eMMC device. This process reduces the latency incurred by the read command. Once

the end of the loader stream is detected, boot is terminated by sending out the stop command to end the multiblock read from the device.

When EMSI\_BOOTENABLE= 1 (booting from a boot area), the boot ROM uses a separate subroutine to handle the loading of data from boot area.

Loading from boot area includes:

- Setting up the eMSI controller to read from the boot area.
- Setting up the ADMA2 descriptor.
- Issuing a CMD0 with the 0xf0f0f0f0 argument.

Once the CMD line from controller side is driven low, after some delay, the eMMC device starts driving the data. Each subsequent load function involves updating the ADMA 2 descriptor to read 1024 bytes. Once the end of the loader stream is detected, boot is terminated by deasserting the CMD line.

Page mode is used to read data from the eMMC device. Page mode optimizes memory reads for block-organized devices by always reading a page, rather than reading data on demand. The page size of the physical source device is defined by the dPageByteCount variable.

## Run-Time API

The following table provides descriptions of the adi\_rom\_Boot() command parameter.

Table 47-28: eMSI Controller Boot Command Bit Descriptions

| Bits   | Name                          | Setting    | Description                                                                              |
|--------|-------------------------------|------------|------------------------------------------------------------------------------------------|
| 29:22  | ROM_BCMD_EMSI_SPEED           | 0x00- 0xFF | eMSI clock divider to be used. This value is written to the eMSI clock divider register. |
| 21:20  | Reserved                      | Reserved   | Reserved                                                                                 |
| 19     | ROM_BCMD_EMSI_HISPEE- DENABLE | 0          | Disable high speed mode for eMMC device                                                  |
| 19     | ROM_BCMD_EMSI_HISPEE- DENABLE | 1          | Enable high speed mode for eMMC device                                                   |
| 18     | ROM_BCMD_EMSI_SAFE- BOOT      | 0          | Boot whole loader stream using single multiblock read command                            |
| 18     | ROM_BCMD_EMSI_SAFE- BOOT      | 1          | Enable boot using multiple read commands                                                 |
| 17     | ROM_BCMD_EMSI_CMD0DE- LAY     | 0          | No additional delay after CMD0                                                           |
| 17     | ROM_BCMD_EMSI_CMD0DE- LAY     | 1          | Additional delay after CMD0                                                              |
| 16     | ROM_BCMD_EMSI_BOOTEN- ABLE    | 0          | Boot from user area of eMMC device                                                       |
| 16     | ROM_BCMD_EMSI_BOOTEN- ABLE    | 1          | Boot from boot area of eMMC device                                                       |
| 15     | Reserved                      | Reserved   | Reserved                                                                                 |

Table 47-28: eMSI Controller Boot Command Bit Descriptions (Continued)

| Bits   | Name                    | Setting     | Description                                                                                                                                                                                                                                                               |
|--------|-------------------------|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 14     | ROM_BCMD_EM- SI_SDRMODE | 0           | Use DDR mode (default for user area boot                                                                                                                                                                                                                                  |
| 14     | ROM_BCMD_EM- SI_SDRMODE | 1           | Use SDR mode (default for boot area boot). For boot from the boot area of the eMMC device always set this bit to 1.                                                                                                                                                       |
| 13:12  | ROM_BCMD_EMSI_BUSWID    | 00b         | 1-bit mode                                                                                                                                                                                                                                                                |
| 13:12  | ROM_BCMD_EMSI_BUSWID    | 01b         | 4-bit mode                                                                                                                                                                                                                                                                |
| 13:12  | ROM_BCMD_EMSI_BUSWID    | 10b         | 8-bit mode                                                                                                                                                                                                                                                                |
| 13:12  | ROM_BCMD_EMSI_BUSWID    | 11b         | Reserved                                                                                                                                                                                                                                                                  |
| 11:8   | ROM_BCMD_EMSI_DEVE- NUM | 0 to 16     | Specifies the boot peripheral instance. Configure to 0 when there is only one instance of eMSI.                                                                                                                                                                           |
| 7      | Reserved                | Reserved    | Reserved                                                                                                                                                                                                                                                                  |
| 6      | ROM_BCMD_EMSI_NOCFG     | 0           | Has no effect on eMSI boot                                                                                                                                                                                                                                                |
| 6      | ROM_BCMD_EMSI_NOCFG     | 1           | Has no effect on eMSI boot                                                                                                                                                                                                                                                |
| 5      | ROM_BCMD_EMSI_NOCFG     | 0           | Instructs the config routine to perform pin-muxing configuration and full peripheral configuration                                                                                                                                                                        |
| 5      | ROM_BCMD_EMSI_NOCFG     | 1           | The config routine will not change the configuration of the peripheral and it will not enable pin-muxing. This can be used if the boot software does not support specific features of a peripheral, users can configure the peripheral prior to calling the boot routine. |
| 4      | ROM_BCMD_EMSI_HOST      | 0           | Controller boot mode                                                                                                                                                                                                                                                      |
| 4      | ROM_BCMD_EMSI_HOST      | 1           | Not to be used for eMSI boot                                                                                                                                                                                                                                              |
| 3:0    | ROM_BCMD_EMSI_DEVICE    | 0000b-1000B | Not to be used for eMSI boot                                                                                                                                                                                                                                              |
| 3:0    | ROM_BCMD_EMSI_DEVICE    | 1001b       | eMSI Controller boot mode                                                                                                                                                                                                                                                 |
| 3:0    | ROM_BCMD_EMSI_DEVICE    | 1010b-111B  | Not to be used for eMSI boot                                                                                                                                                                                                                                              |

Table 47-29: eMSI Controller Start Address Bit Descriptions

| Bits   | Name             | Setting               | Description                                                                                                                                                                                                     |
|--------|------------------|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 31:0   | eMMCStartAddress | 0x00000000-0xFFFFFFFF | Number of Block+1 in user area from where to start reading the boot stream. If this is configured to zero, by default, a boot from LBA-34 will happen. To boot from LBA-0, configure this field to 1 and so on. |
...
Table 47-30: SPI2 Default Pin-Mux
...
Table 47-31: OSPI0 Default Pin-Mux
...
Table 47-32: UART0 Default Pin-Mux
...
Figure 47-17: Loader Stream Block Structure
...
Table 47-34: Block Code Flags (Continued)
...
To register a callback, create an Init Block whose Initcode modifies the

ADI\_ROM\_BOOT\_CONFIG::pCallBackFunction with the address of the callback function to execute. A

callback function must be registered prior to processing a callback block.
...
```
ROM_BOOT_RESULT callback( ADI_ROM_BOOT_CONFIG * pBootConfig,
```

```
ADI_ROM_BOOT_BUFFER * pBuffer, uint32_t nFlags )
```
...
Table 47-44: Initial Header for Single-Block Stream
...
Table 47-45: Example Direct Code Execution Header

| Field      | Value      | Comments                                                                                    |
|------------|------------|---------------------------------------------------------------------------------------------|
| BLOCK_CODE | 0xAD7BD006 | 0xAD000000&#124;XORSUM&#124; BFLAG_FINAL &#124; BFLAG_FIRST &#124; BFLAG_IGNORE &#124;BCODE |

Table 47-45: Example Direct Code Execution Header (Continued)
...
Figure 47-18: Multi-Application Boot Stream Example
...
Figure 47-19: Multi-Application Direct Code Execution Example
...
The Figure 47-20 Secure Boot Image shows that the image attributes are encapsulated within the secure boot header. The image attributes are actually integrity protected along with the image section. The image section contains a standard boot loader stream. Some block types are not allowed as described in Unsupported Boot Stream Blocks.

Figure 47-20: Secure Boot Image

<!-- image -->

## Secure Boot Header

Table 47-46: Secure Boot Header

|          |            |                                                                                                            | Values                                                                                                                                     | Values                                                                                                                                     | Values                                                                                                                                     |
|----------|------------|------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------|
| Bytes    | Name       | Description                                                                                                | Keyless Format BLx                                                                                                                         | Wrapped Format BLw                                                                                                                         | Plain Text Format BLp                                                                                                                      |
| 3:0      | Type       | Format and version of the image. Upper 24 bits are the image format and lower 8 bits are the image version | 0x424c7803                                                                                                                                 | 0x424c7703                                                                                                                                 | 0x424c7003                                                                                                                                 |
...
Figure 47-21: Secure Boot Processing

<!-- image -->

## Pre-Hash Authentication

Boot ROM in the ADSP-SC59x family uses the pre-calculated hash (SHA-224 or SHA-256) stored in the secured header to start the authentication process earlier and thereby reduce the boot rom. With pre-calculated hash of the whole boot image available, authentication starts in parallel with rest of the boot process. Since the boot uses the pre-calculate hash for authentication, the last step is to verify that the hash calculated by the boot ROM from the actual image read from the boot source matches the pre-calculated hash. If the hash calculated by boot ROM does not match the pre-calculates hash, the result of ECDSA authentication is ignored and the secure boot fails.

However, if the hash calculated by boot ROM matches the pre-calculated hash, the result of ECDSA authentication is used to decide whether the secure boot happened successfully or not.
...
Table 47-47: Secure Boot Image Attributes
...
| 0x80000002 | BCODE       | Used by the boot mode drivers that support auto- detection to configure the                                                                                                    | Values between 0x00000000 and 0x0000000F sup- ported. | Values between 0x00000000 and 0x0000000F sup- ported.                                                 |

Table 47-47: Secure Boot Image Attributes (Continued)

| ID         | Name       | Description                                                                       | Values                                                                                        |
|------------|------------|-----------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
|            |            | device from a range of preconfigured settings                                     |                                                                                               |
| 0x80000003 | ECDSA Type | Used by boot ROMto perform Authentication using ECDSA-224 or ECDSA-256 algorithm. | 224 : Authentication using ECDSA-224 algorithm 256 : Authentication using ECDSA-256 algorithm |
...
When the core is used to boot a separately generated boot stream for another core, the ROM\_BFLAG\_RETURN flag should be used when calling adi\_rom\_Boot() to instruct the boot kernel to return to the calling application, which in this case is back to the core that was running the second stage loader. Failure to set this flag results in the core vectoring to the location in its own ( RCU\_SVECT0 -RCU\_SVECT2 ) register which is not the intention.
...
## Anti -rollback NV Counter

Use an anti-rollback counter to prevent loading of older firmware during secure boot. Boot ROM implements the counter using the OTPC module to allow easy reading or writing of the counter, without dealing with the complexities of rewriting OTP memory sections that are ECC protected.

The implementation involves two functional APIs for the counter, which are functionally unique in the way that they set and retrieve data as counters in OTP memory.

It uses a different method to count bits because each bit in OTP memory can only be set =1 once, and the ECC protects each 16-bit unit. This functionality essentially means that each 16-bit unit can only be written to once. Therefore, a counter that can count 0-31 requires 32 × 16 bits of memory.

The counter API receives and returns the value of the counter as a uint8\_t binary number. Providing a value less than the current value of the counter or greater than the maximum value results in an error. A greater value will result in updating the counter value equal to the one provided.

To implement this functionality, the driver counts by shifting 1's from the right, treating each block as 1 bit. A three-bit counter is encoded as follows.
...
Table 47-48: Secure Boot Header
...
Table 47-48: Secure Boot Header (Continued)
...
void ErrorFunction(ADI\_ROM\_BOOT\_CONFIG* pBootStruct, void *pFailingAddress);
...
Table 47-49: ROM\_HOOK\_CALL\_CAUSE Members
...
Table 47-50: Application Entry Point Registers

| Core ID   | Corresponding RCU_SVECTn Register   |
|-----------|-------------------------------------|
|           | RCU_SVECT0                          |

Table 47-50: Application Entry Point Registers (Continued)
...
ROM\_DMA\_MDMA\_CONFIG object passed as the first argument. The second argument is a descriptor that provides access to the DMA channel's MMR registers and the associated CRC peripheral. When called from the higher level adi\_rom\_MemDma() routine this object is retrieved from the ROM.
...
The configuration provided as the only argument to the function is provided in Table 47-72 ROM\_DMA\_MDMA\_CONFIG Members .
...
```
ROM_DMA_MDMA_CONFIG::eOperation type as . If an MDMA channel is specified that does
```

ROM\_DMA\_MDMA\_OPERATION::ROM\_DMA\_CRC\_LUT\_INIT not support CRC function, an error result is returned.
...
ROM\_BOOT\_RESULT adi\_rom\_PeriphDma(ROM\_DMA\_PDMA\_CONFIG * pDmaCfg)
...
```
Pointer to Crypto descriptors Ecdsa_Word ECDSA word count for 224/256 bit operation output_p Pointer to the output buffer input_p Pointer to the input buffer size Size of the input buffer. It must be a multiple of 512 finalFlag Final block or not
```
...
```
(_bootrom_jumptable_aes128_key_unwrap_t) (FUNC_ROM_AES128_KEY_UNWRAP); return (*pTmp)(pCrypto, WrappedIn_p, KeyOut_p, KeyBits, KWK_p); }
```

## /* points to bootrom\_jumptable\_aes128\_cbc\_loadkey */

```
typedef void * (*_bootrom_jumptable_aes128_cbc_loadkey_t)(sa_t * pSARecord, uint32_t *key_p); inline static void * adi_rom_Aes128CbcLoadkey(sa_t * pSARecord, uint32_t *key_p) { _bootrom_jumptable_aes128_cbc_loadkey_t pTmp = (_bootrom_jumptable_aes128_cbc_loadkey_t) (FUNC_ROM_AES128_CBC_LOADKEY); return (*pTmp)(pSARecord,key_p); }
```
...
```
(_bootrom_jumptable_ecdsa_verify_init_t) (FUNC_ROM_ECDSA_VERIFY_INIT); return (*pTmp)(Verify_p,PublicKey_p,Signature_p,EcdsaWord); }
```

/* points to bootrom\_jumptable\_ecdsa\_verify\_set\_digest */

```
typedef void * (*_bootrom_jumptable_ecdsa_verify_set_digest_t) (SBHYBRID_EcdsaContext_t * const Verify_p, uint8_t * Digest_p); inline static void * adi_rom_EcdsaVerifySetDigest(SBHYBRID_EcdsaContext_t * const Verify_p, uint8_t * Digest_p) { _bootrom_jumptable_ecdsa_verify_set_digest_t pTmp = (_bootrom_jumptable_ecdsa_verify_set_digest_t) (FUNC_ROM_ECDSA_VERIFY_SET_DIGEST); return (*pTmp)(Verify_p,Digest_p); }
```
...
```
Structure Type Declaration: ADI_ROM_BOOT_BUFFER Boot Buffer.
```

A basic buffer type consisting of a pointer to the buffer and its size

Table 47-52: ADI\_ROM\_BOOT\_BUFFER Members
...
Table 47-53: ADI\_ROM\_BOOT\_CONFIG Members
...
Table 47-53: ADI\_ROM\_BOOT\_CONFIG Members (Continued)
...
Table 47-54: ADI\_ROM\_BOOT\_HEADER Members
...
Table 47-55: ADI\_ROM\_BOOT\_INTER\_BUFFER Members
...
Table 47-56: ADI\_ROM\_BOOT\_INTER\_BUFFERS Members
...
Table 47-56: ADI\_ROM\_BOOT\_INTER\_BUFFERS Members (Continued)
...
Table 47-57: ADI\_ROM\_BOOT\_LINKPORT Members
...
Table 47-58: ADI\_ROM\_BOOT\_MODES Members
...
| ADI_ROM_BOOT_EMSI     | pEmsi    | Access to all EMSI boot mode resources     |
...
Access to all EMSI boot mode resources
...
Table 47-59: ADI\_ROM\_BOOT\_REGISTRY Members
...
Table 47-60: ADI\_ROM\_BOOT\_SPI Members
...
Table 47-61: ADI\_ROM\_BOOT\_OSPI Members
...
Table 47-62: ADI\_ROM\_BOOT\_UART Members
...
This is a 192-bit structure that is allocated to one contiguous region in the OTP memory array. The functionality allows for individual flags to enable or disable specific features of the boot process. Each flag is allocated in a separate 16-bit word so that each flag can be set at different times and the ECC information will not impact the setting of another flag.

Table 47-63: ADI\_ROM\_OTP\_BOOT\_CFG Members

| Type     | Name          | Description                                                                 |
|----------|---------------|-----------------------------------------------------------------------------|
| uint32_t | lockMonitor:1 | Flag to enable the status checking of ADI private OTP area                  |
| uint32_t | reserved0:15  | Reserved                                                                    |
| uint32_t | dstCompare: 1 | Flag to enable the data compare from the destination address for data block |

Table 47-63: ADI\_ROM\_OTP\_BOOT\_CFG Members (Continued)

| Type     | Name          | Description                              |
|----------|---------------|------------------------------------------|
| uint32_t | reserved1:15  | Reserved                                 |
| uint32_t | pubkey0Inv:1  | Invalidate public key0                   |
| uint32_t | reserved2:15  | Reserved                                 |
| uint32_t | pubkey1Inv:1  | Invalidate public key1                   |
| uint32_t | reserved3:15  | Reserved                                 |
| uint32_t | privkey0Inv:1 | Invalidate private key0                  |
| uint32_t | reserved4:15  | Reserved                                 |
| uint32_t | privkey1Inv:1 | Invalidate private key1                  |
| uint32_t | reserved5:15  | Reserved                                 |
| uint32_t | privkey2Inv:1 | Invalidate private key2                  |
| uint32_t | reserved6:15  | Reserved                                 |
| uint32_t | privkey3Inv:1 | Invalidate private key3                  |
| uint32_t | reserved7:15  | Reserved                                 |
| uint32_t | dmcEn:1       | Enables configuration of theDMC          |
| uint32_t | reserved8:15  | Reserved                                 |
| uint32_t | dmcInv:1      | Invalidates the DMCvalues                |
| uint32_t | reserved9:15  | Reserved                                 |
| uint32_t | dmcDecDis:1   | Disabled Decryption from off-chip memory |
| uint32_t | reserved10:31 | Reserved                                 |
...
Flag to enable the data compare from the destination address for data block
...
Disabled decryption from off-chip memory
...
Table 47-64: ADI\_ROM\_OTP\_BOOT\_CGU\_INFO Members
...
Table 47-64: ADI\_ROM\_OTP\_BOOT\_CGU\_INFO Members (Continued)

| Type     | Name                              | Description                                          |
|----------|-----------------------------------|------------------------------------------------------|
| uint32_t | ctl_MSEL:7 (bitfield)             | CGU_CTL.MSEL value                                   |
| uint32_t | Reserved1:1 (bitfield)            | Reserved                                             |
| uint32_t | ctl_SISELEXEN:1 (bitfield)        | CGU_CTL.S1SELEXEN value                              |
| uint32_t | auto_disable:1 (bitfield)         | Disable auto alignment                               |
| uint32_t | clkoutsel_CLKOUTSEL:5 (bitfield)  | CGU_CLKOUTSEL.CLKOUTSEL value                        |
| uint32_t | clkoutsel_WEN:1 (bitfield)        | Enable write to the CGU_CLKOUTSEL register           |
| uint32_t | Reserved2:8 (bitfield)            | Reserved                                             |
...
```
Enable write to the CGU_DIV register div_DSEL CGU_DIV.DSEL value div_CSEL CGU_DIV.CSEL value div_S0SEL CGU_DIV.S0SEL value div_SYSSEL CGU_DIV.SYSSEL value div_S1SEL CGU_DIV.S1SEL value div_OSEL CGU_DIV.OSEL value ctl_DF CGU_CTL.DF value ctl_MSEL CGU_CTL.MSEL value auto_disable disable polling on auto-alignment of clocks, NOT RECOMMENDED! clkoutsel_CLKOUTSEL CGU_CLKOUTSEL.CLKOUTSEL value clkoutsel_WEN Enable write to the CGU_CLKOUTSEL register oscwctl0_WEN Enable write to the CGU_OSCWDCTL instance 0 register oscwctl0_HODF CGU_OSCWDCTL.HODF value oscwctl0_HODEN CGU_OSCWDCTL.HODEN value
```
...
Table 47-65: ADI\_ROM\_OTP\_BOOT\_CMD\_INFO Members
...
| uint32_t | emmcMasterBootCmd0 | eMSI Controller Boot Mode    |
| uint32_t | emmcMasterBootCmd1 | eMSI Controller Boot Mode    |
| uint32_t | lpddrBootCmd       | Linkport Slave DDR Boot Mode |
...
Linkport Slave DDR Boot Mode

## struct ADI\_ROM\_BOOT\_EMSI

Structure Type Declaration: ADI\_ROM\_BOOT\_EMSI

The EMSI boot mode specific structure.

This structure contains all the boot context information that is specific to the EMSI master boot mode.

Table 47-66: ROM\_BOOT\_EMSI Members

| Type        | Name             | Description                                                                                    |
|-------------|------------------|------------------------------------------------------------------------------------------------|
| bool        | bFirstLoadCall   | Indicates first call to EMSI load function                                                     |
| bool        | bFinalLoadCall   | Indicates final call to EMSI load function                                                     |
| uint32_t    | Reserved0        | reserved                                                                                       |
| uint32_t[2] | nRead_Adma2_Des0 | Descriptor 0 which handles the Data load to buffer                                             |
| uint32_t[2] | nRead_Adma2_Des1 | Descriptor 1 which together with Descriptor 2 creates pause in data read by continuous linking |
| uint32_t[2] | nRead_Adma2_Des2 | Descriptor 2 which together with Descriptor 1 creates pause in data read by continuous linking |

## bFirstLoadCall

Indicates first call to EMSI load function
...
Descriptor 0 which handles the Data load to buffer

## nRead\_Adma2\_Des1

Descriptor 1 which together with Descriptor 2 creates pause in data read by continuous linking

## nRead\_Adma2\_Des2

Descriptor 2 which together with Descriptor 1 creates pause in data read by continuous linking

## struct ADI\_ROM\_OTP\_BOOT\_INFO

Structure Type Declaration: ADI\_ROM\_OTP\_BOOT\_INFO

The 704-bit boot info object located in OTP for boot customization.

This is a 704-bit structure that is allocated to one contiguous region in the OTP memory array. The content in OTP is stored in the format of this structure so boot can read the contents directly into this object.

Table 47-67: ADI\_ROM\_OTP\_BOOT\_INFO Members
...
Table 47-68: ADI\_ROM\_OTP\_DMC\_CONFIG Members
...
Table 47-69: ROM\_BOOT\_DMA\_INSTANCE Members
...
Table 47-70: ROM\_BOOT\_MDMA Members
...
Table 47-71: ROM\_BOOT\_MDMA\_REGS Members
...
Table 47-72: ROM\_DMA\_MDMA\_CONFIG Members
...
Table 47-72: ROM\_DMA\_MDMA\_CONFIG Members (Continued)
...
Type of operation to perform eId
...
Table 47-73: ROM\_DMA\_PDMA\_CONFIG Members
...
Table 47-74: otp\_data Members (Continued)
...
Table 47-75: ADI\_ROM\_BOOT\_KEY\_TYPE Members
...
Table 47-76: ADI\_ROM\_BOOT\_TYPE Members
...
Table 47-77: ROM\_BOOT\_MDMA\_CRC\_SUPPORT Members
...
Table 47-78: ROM\_BOOT\_RESULT Members
...
Table 47-78: ROM\_BOOT\_RESULT Members (Continued)
...
Table 47-79: ROM\_CORE\_ID Members
...
Table 47-80: ROM\_DMA\_DONE\_DETECT\_METHOD Members
...
Table 47-81: ROM\_DMA\_MDMA\_ID Members

| Enumerator    | Description        |
|---------------|--------------------|
| ROM_DMA_MDMA0 | Memory DMAStream 0 |

Table 47-81: ROM\_DMA\_MDMA\_ID Members (Continued)
...
Table 47-82: ROM\_DMA\_MDMA\_OPERATION Members
...
Enumeration Type Declaration: ROM\_DMA\_PDMA\_OPERATION

Table 47-83: ROM\_DMA\_PDMA\_OPERATION Members
...
Table 47-84: ROM\_GETADDR\_VALUE Members
...
Table 47-84: ROM\_GETADDR\_VALUE Members (Continued)
...
Table 47-85: ROM\_HOOK\_CALL\_CAUSE Members
...
Table 47-85: ROM\_HOOK\_CALL\_CAUSE Members (Continued)
...
Table 47-86: ROM\_SB\_IMAGE\_TYPE Members
...
Table 47-87: ROM\_SPI\_PROTOCOL Members
