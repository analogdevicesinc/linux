...
## ADSP-2184x TMU Register List

Thermal monitoring unit

Table 14-1: ADSP-2184x TMU Register List
...
Table 14-1: ADSP-2184x TMU Register List (Continued)
...
## ADSP-2184x TMU Interrupt List

Table 14-2: ADSP-2184x TMU Interrupt List
...
## ADSP-2184x TMU Trigger List

Table 14-3: ADSP-2184x TMU Trigger List Generators
...
Table 14-4: ADSP-2184x TMU Trigger List Receivers
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000000_d946e13a234552aa6725ff50b5e26242d015481c5f0fc40a487611e1e39e0e1b.png)
...
The typical value of blanking period is 1.1× 10 6  system clock (SCLK) cycles.

The typical value of refresh period is 21×10 6  SCLK cycles.

So, for every 21×10 6 cycles, 1.1× 10 6  cycles are used for temperature measurement. Program these values for blanking period and refresh period through the TMU registers.
...
- The alert high limit (0,1,2) is configured in the TMU\_ALRT\_LIM\_HI register. The interrupt is generated when the temperature value is greater than or equal to these value.
...
The fault and alert events are sent to the core through the status register ( TMU\_STAT ). Both the TMU\_STAT.FLTHI and the TMU\_STAT.ALRT1HI / TMU\_STAT.ALRT2HI / TMU\_STAT.ALRT3HI bits are sticky bits and are cleared by a W1C operation by the core.
...
To get the best performance and accuracy from the TMU, initialize the TMU\_GAIN and TMU\_OFFSET registers before enabling the module. After the registers are programmed, the rest of TMU initialization can take place. This includes setting up the fault and alert limits and then enabling the TMU.

## Reading Temperature Data

After enabling the TMU, the temperature data can only be read after a finite period of time referred to as startup time.

If the HADC module is used, apply the following mechanism:

1. Initialize the HADC module.
2. Wait for the ADC ready interrupt.
3. Perform TMU conversions.

The minimum time required before reading the first valid data is ~100us for a typical SCLK frequency of 125Mhz and using the default register values.

If the HADC module is not used, complete the following steps

1. Initialize the HADC module (dummy initialization).
2. Wait for the ADC ready interrupt.
3. Perform TMU conversions.
...
## ADSP-2184x TMU Register Descriptions

Thermal monitoring unit (TMU) contains the following registers.

Table 14-6: ADSP-2184x TMU Register List
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000001_1ffa1cd64cd15db537e41b97d129044c505a9769ec833378a97c675c50dc8835.png)

Table 14-7: TMU\_ALRT\_LIM\_HI Register Fields

| Bit No. (Access)   | Bit Name      | Description/Enumeration                                                                                                                                                                                                                                         |
|--------------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 28:20 (R/W)        | ALERT3_LIM_HI | Value. The TMU_ALRT_LIM_HI.ALERT3_LIM_HI bit field configures the alert temper- ature high limit as an integer value stored in two's complement format. If the tempera- ture value is greater than or equal to the high limit, the TMU_STAT.ALRT3HI bit is set. |
| 18:10 (R/W)        | ALERT2_LIM_HI | Value. The TMU_ALRT_LIM_HI.ALERT2_LIM_HI bit field configures the alert temper- ature high limit as an integer value stored in two's complement format. If the tempera- ture value is greater than or equal to the high limit, the TMU_STAT.ALRT2HI bit is set. |
| 8:0 (R/W)          | ALERT1_LIM_HI | Value. The TMU_ALRT_LIM_HI.ALERT1_LIM_HI bit field configures the alert temper- ature high limit as an integer value stored in two's complement format. If the tempera- ture value is greater than or equal to the high limit, the TMU_STAT.ALRT1HI bit is set. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000002_9e96f15d0be1503e1afcfdf4c99903d2c497952456edde8608595c57225c52df.png)
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000003_a22e9d664d65529574cda0c6b08e147e0fa7a9182fe418c54feeb6980afd1aa2.png)

Table 14-9: TMU\_AVG Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:4 (R/W)          | AVG_COUNT  | Value. The TMU_AVG.AVG_COUNT bit field indicates the number of values (N) to be averaged whereN=2 AVG_COUNT . The minimum value of TMU_AVG.AVG_COUNT is 1 and maximum value is 12. The fault value is 5, hence 2 5 = 32 values are averaged, by default. A maximum 2 12 =4096 values can be averaged.                                                                                                                                                                                                                                                                                                    |
| 0 (R/W)            | EN         | Averaging Enable. The TMU_AVG.EN bit enables averaging on the TMU. Moving average is calculated for the past Nvalues, where Nis a power of 2. Nvalue can be programmed by a register. Nis defined by the TMU_AVG.AVG_COUNT register The average value is calculated using the equation: {(N-1)*previous_average+current_sampe}/N. The first Nsamples are summed and divided by Nto calculate the first average value. Hence, the temperature value will not be updated for the first Nconversions, when averaging is enabled. Averaging is enabled by default. It can be disabled when it is not needed. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000004_d07f910824ea623dd557cf2049fb3000a4762a3a221f1fac6dd6e739a8460dae.png)

Table 14-10: TMU\_CNV\_BLANK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                              |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5:0 (R/W)          | PERIOD     | Blanking Period. Blanking period is (CNV_BLANK+1)*150k SCLK cycles. So the minimum blanking period is 150k SCLK cycles. Default blanking period is 1650k (1.65 million) SCLK cycles. For this duration, The system cannot use HADC for data conversion and HADC will not take input for data-conversion. The status will be indicated by TMHADC_BUSY bit of HADC block. More details can be found in TMUProgram- ming model section. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000005_260df3d4ea4f04fec80d780f8550efa5b108c5d098af9a1366f84f02ed16f021.png)
...
| 11:4 (R/W)         | SCLKDIV    | System Clock Divide. The TMU_CTL.SCLKDIV bit selects the division ratio for the system clock (SCLK). SCLK is divided by (11+2*SCLKDIV). Thus, by default the SCLK is divided by 11.                                                                                                  |
| 3 (R/W)            | TMEN       | Periodic Enable. Asserting the TMU_CTL.TMEN bit enables the TMUperiodically. TMUis enabled once in every refresh period defined by the TMU_REFR_CNTR register. And in each refresh period, TMUmeasures temperature for the blanking period (Tblank). It is defined by CNV_BLANK bit. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000006_16dc503dea15ce4ca9a4a3efee8ec6b324317583dfbf3872de3c3e53c6cc7247.png)

Table 14-12: TMU\_FLT\_LIM\_HI Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                          |
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 8:0 (R/W)          | VALUE      | Value. The TMU_FLT_LIM_HI.VALUE bit field sets the temperature high limit as an integer in two's complement format. If the limit value is greater than or equal to the high limit the TMU_STAT.FLTHI bit is set. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000007_9e96f15d0be1503e1afcfdf4c99903d2c497952456edde8608595c57225c52df.png)
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000008_07dfd61f31f144d80893ff7ca87c4ae77534c71a4e514338456d4bf4741710e8.png)

Table 14-14: TMU\_GAIN Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                          |
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 9:0 (R/W)          | VALUE      | Value. The TMU_GAIN.VALUE bit field configures the gain value in two's complement format. This value is used to correct the gain error in the TMU_TEMP register. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000009_e916f8652462993e63643e66235fa69ff152c2dbc564ac202671b8ad3fef2f87.png)

Table 14-15: TMU\_IMSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                   |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------|
| 5 (R/W)            | ALRT3HI    | Alert1 High Mask.                                                                                         |
| 4 (R/W)            | ALRT2HI    | Alert1 High Mask.                                                                                         |
...
| 1 (R/W)            | ALRT1HI    | Alert1 High Mask.                                                                                         |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000010_320909f1884d73484ee10705bb16c7e400fd312e6d66232f86e79b97f6528332.png)
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000011_b611f82d4bfb7bb6e29c80693eb99c640c78b86ecb343fbc913c1222e9df2df2.png)

Table 14-17: TMU\_REFR\_CNTR Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                            |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5:0 (R/W)          | VALUE      | Refresh Period. The TMU_REFR_CNTR.VALUE bit field defines the period to refresh the temper- ature value. Temperature is refreshed for every (VALUE+1)*(21+4*SCLKDIV)*1M system-clock cycles. SCLKDIV is defined in TMU_CTL register. So, by default, the period is 21 million system-clock cycles. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000012_e752c28b800fb32f75638d8a456cf8a4ee07097f9f170f55a46db2c494f50402.png)
...
| 5 (R/W1C)          | ALRT3HI    | Alert High. The TMU_STAT.ALRT3HI bit is set when the temperature value is greater than or equal to the setting in the TMU_ALRT_LIM_HI register. A Write1toClear is needed to reset the bit if it is asserted. The bit assertion is masked when TMU_IMSK.ALRT3HI =1. |
| 4 (R/W1C)          | ALRT2HI    | Alert High. The TMU_STAT.ALRT2HI bit is set when the temperature value is greater than or equal to the setting in the TMU_ALRT_LIM_HI register. A Write1toClear is needed to reset the bit if it is asserted. The bit assertion is masked when TMU_IMSK.ALRT2HI =1. |

Table 14-18: TMU\_STAT Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                               |
|--------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 3 (R/W1C)          | ALRT1HI    | Alert High. The TMU_STAT.ALRT1HI bit is set when the temperature value is greater than or equal to the setting in the TMU_ALRT_LIM_HI register. A Write1toClear is needed to reset the bit if it is asserted. The bit assertion is masked when TMU_IMSK.ALRT1HI =1.                                                                   |
| 2 (R/W1C)          | FLTHI      | Fault High. The TMU_STAT.FLTHI bit is set when the temperature value is greater than or equal to the setting in the TMU_FLT_LIM_HI register. The temperature value regis- ter is set to 16'h0D80 (effective temperature value of 27 degrees C) when a W1C is done on this bit. The bit assertion is masked if TMU_IMSK.FLTHI is HIGH. |
...
![Image](17_Thermal_Monitoring_Unit_(TMU)_artifacts/image_000013_934b299bafd33fae5f54eaa6036b4f9937a769fe2f6898013ff7b4ef2fff2661.png)

Table 14-19: TMU\_TEMP Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                 |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 15:0 (R/NW)        | VALUE      | Value. The TMU_TEMP.VALUE bit field is the temperature value from A/D converter. This value is stored in two's complement fixed point Q8.7 format where the 9 MSB's represent the integer part in 2's complement, and the remaining 7 LSB's represent the decimal part. |