...
- Auto sequencing capability provides up to 15 auto-conversions in a single session. Each conversion can be programmed to select any of the available input channels.
...
Table 15-1: ADSP-2184x HADC Register List
...
## ADSP-2184x HADC Interrupt List

Table 15-2: ADSP-2184x HADC Interrupt List
...
## ADSP-2184x HADC Trigger List

Table 15-3: ADSP-2184x HADC Trigger List Generators

|   Trigger ID | Name      | Description                   | Sensitivity   |
|--------------|-----------|-------------------------------|---------------|
|           57 | HADC0_EOC | HADC0 HADC0 End of Conversion | Edge          |

Table 15-4: ADSP-2184x HADC Trigger List Receivers
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000000_556f7affe63b0a34ce5f24b73d78bd6c0922a9eb59105ffe39a154dc31110ef5.png)
...
|---------------|----------------------------------------------------------------------------------|
| AVDD          | ADC I/O supply                                                                   |
| G             | I/O ground for analog blocks - shorted with other ground pins within the package |
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000001_cff47ec50524bca6e2ec369ca2dd5c10bf151792c98288715cf0fb5203e825c1.png)
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000002_854c8ccdfd17760a2c58181ff9c9f2bc6e9c4bac37ad261c265f23087661a270.png)
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000003_e0f415589ec1a0c33bb15051af998fd7639a20ea7209ab3938faabb0a6d658f3.png)

## Results

The HADC takes 20 cycles of f SAMPLE  for one channel conversion. (The value of the HADC\_CTL.FDIV bit field determines f SAMPLE ). The time taken to complete one sequence depends on the number of channels in the auto-scan chain. There is a latency of 1 cycle from the time the channel is selected internally to sample to the time the data is ready. After the end of each channel conversion, the data is written into the corresponding data register. An interrupt is generated (if the interrupt mask is not enabled) to signal that the data is ready for that particular channel.
...
## ADSP-2184x HADC Register Descriptions

Housekeeping ADC (HADC) contains the following registers.

Table 15-6: ADSP-2184x HADC Register List
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000004_ce259eff7cf138db88e457625852547ece507f4c49954cc6cf106251faf1c0c2.png)
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000005_e20b9eaada9913ac991a19a9413ca5e5e450292c5995ab46dfc7b45086f09ea0.png)

Table 15-8: HADC\_CTL Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                              |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 13 (R/W)           | ENLS       | Enable Level Shifters. Setting the HADC_CTL.ENLS bit enables the level shifters, allowing the HADC analog side (which works in the VDD_EXT domain) to work with the digital core and interface is (in the VDD_INT domain). 0 Disable down level shifters                                                                             |
| 12 (R/W)           | DOUTOREOCB | Serial data on DOUT. If the HADC_CTL.DOUTOREOCB bit =1, serial data arrives on the EOC_DOUT pin. If this bit =0 (default) it acts as an EOC only if the external multiplexer is connected.                                                                                                                                           |
...
| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                         |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7 (R/W)            | CONT       | Continuous Conversion. When the HADC_CTL.CONT bit =0, the ADC converts a sequence for a fixed number of times. This number is configured using the HADC_CTL.FIXEDCNV bit field. When the HADC_CTL.CONT bit =1, the ADC continuously converts a given se- quence, provided the HADC_CTL.STARTCNV is held high. 0 ADC converts sequence for fixed number of times |
| 6:3 (R/W)          | FDIV       | Frequency Divider. The HADC_CTL.FDIV bit field configures the f_SAMPLE=f_CLK/(FDIV+1). Select f_CLK and HADC_CTL.FDIV values so that f_SAMPLE is in range of 50 kHz to 22.5 MHz. The minimum value for HADC_CTL.FDIV is 1. Before changing the HADC_CTL.FDIV bits, clear the HADC_CTL.NRST bit.                                                                 |
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000006_3969d88268756db3d6aec20c4291ce8db00a51018fe5e7e29558230b66816c9f.png)

Table 15-9: HADC\_DATA[nn] Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                           |
|--------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------|
| 11:0               | VALUE      | Converted Data.                                                                                                                   |
| (R/NW)             |            | The HADC_DATA[nn].VALUE bit field contains the digital code for the sampled analog value. Each channel has its own data register. |
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000007_736876d4cd65802e88a7a0c12e995124e206bf9db5f4badf477168ea9f3da674.png)

Table 15-10: HADC\_IMSK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                      |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 17 (R/W)           | RDY        | Mask Interrupt when ADC Ready. The HADC_IMSK.RDY bit masks the interrupt generated when ADC is ready to convert.                                                                                                                                             |
| 16 (R/W)           | SEQ        | Mask Interrupt at End of Sequence. The HADC_IMSK.SEQ bit masks the interrupt which is generated at the end of sequence completion.                                                                                                                           |
...
![Image](18_Housekeeping_ADC_(HADC)_artifacts/image_000008_de7fc4830814827d5027f11c5f9238f9706b190604d30bd2fc21c02d61d9353c.png)
