...
Table 42-1: ADSP-2184x TRNG Register List

| Name        | Description                 |
|-------------|-----------------------------|
| TRNG_ALMCNT | TRNG Alarm Counter Register |

Table 42-1: ADSP-2184x TRNG Register List (Continued)
...
2. After taking a snapshot of the LFSR, a number of FRO output samples defined by the TRNG\_CFG.MINREFCYC bit field, are gathered in the main LFSR. If the random data buffer is full, sample-taking continues until the number of samples (counting from the snapshot) matches the number of samples defined by the TRNG\_CFG.MAXREFCYC bit field. At that point, the TRNG switches off the FROs and powers down.
...
![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000000_9ba39e0750e01bb8c6505825368bc7e3e576c1fceb3c449f5805442a2daba224.png)
...
|           1 | SHDNOVR   | Shutdown Overflow. The number of FRO's automatically shut down due to failures or errors have gone above the threshold specified in TRNG_ALMCNT.SHDNTHRESH .                      |
...
## ADSP-2184x TRNG Register Descriptions

True Random Number Generator (TRNG) contains the following registers.

Table 42-4: ADSP-2184x TRNG Register List
...
Table 42-4: ADSP-2184x TRNG Register List (Continued)
...
Figure 42-2: TRNG\_ALMCNT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000001_c0b63c81c58a00494b3bb9f82d7011e83d19529747ad67675cec35b81e415178.png)

Table 42-5: TRNG\_ALMCNT Register Fields
...
| 20:16 (R/W)        | SHDNTHRESH  | Shutdown Threshold. The TRNG_ALMCNT.SHDNTHRESH bit field provides the threshold setting for generating the shutdown overflow (SHDNOVR) interrupt, which is activated when the shutdown count ( TRNG_ALMCNT.SHDNCNT ) value in this register exceeds the threshold value set here.                                                                              |
...
Table 42-5: TRNG\_ALMCNT Register Fields (Continued)
...
Figure 42-3: TRNG\_ALMMSK Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000002_bf1ebce221c20326c85c73f49c735ffaef18043e8ac8f0e1215c8fc7789d070d.png)

Table 42-6: TRNG\_ALMMSK Register Fields
...
Figure 42-4: TRNG\_ALMSTP Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000003_821c48d55c70c5d0a1b5350ec0bc595c1d30c021a06c9c7c602a8078ad144dd8.png)

Table 42-7: TRNG\_ALMSTP Register Fields
...
Figure 42-5: TRNG\_BLKCNT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000004_df9ca708801dabd866013824b9d52eb39d348d82b74923875736df2454f63580.png)

Table 42-8: TRNG\_BLKCNT Register Fields
...
Figure 42-6: TRNG\_CFG Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000005_d153fadf26c60255792dcebee59f0ed1ac259dae7726ab534679626e1ff4af33.png)

Table 42-9: TRNG\_CFG Register Fields
...
| 15:12 (R/W)        | RDTIMEOUT  | Read Timeout. The TRNG_CFG.RDTIMEOUT bit field controls the secure reading mode. When this field is 0, secure reading mode is disabled. Values in the range 1-15 enable secure reading and set a read gate closure timeout of approximately (read_timeout + 1) x 16 clock input cycles. This field can only be modified while the TRNG_CTL.TRNGEN bit =0.                                                                      |

Table 42-9: TRNG\_CFG Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|--------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 11:8 (R/W)         | SAMPLEDIV  | Sample Div. The TRNG_CFG.SAMPLEDIV bit field directly controls the number of input cycles between samples taken from the FROs. The default value 0 indicates that samples are taken every cycle, maximum value 15 (decimal) takes one sample every 16 cycles. This field must be set to a value such that the slowest FRO (even under worst-case condi- tions) has a cycle time less than twice the sample period. The default configuration of the FROs allows this field to remain 0. This field can only be modified while TRNG_CTL.TRNGEN =0. Note: In order to avoid oversampling the FRO, do not to use a TRNG_CFG.SAMPLEDIV value of 0.                                            |
...
Figure 42-7: TRNG\_CNT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000006_229645c132f7a33c700e2cb1d76ebd1dbd8828393d72d4b4e7938bcce2c8ed1c.png)

Table 42-10: TRNG\_CNT Register Fields
...
Figure 42-8: TRNG\_CTL Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000007_10dea7ece6c9e8141a7e8168688f1910a10f547d922638e7bc43c89a09167508.png)

Table 42-11: TRNG\_CTL Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                      |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 31:16 (R/W)        | STARTUPCYC | Startup Cycles. The TRNG_CTL.STARTUPCYC bit field determines the number of samples (be- tween 2 8 and 2 24 ) taken to gather entropy from the FROs during startup. If the written value of this field is zero, the number of samples is 2 24 , otherwise the number of samples equals the written value times 2 8 . This field can only be written when TRNG_CTL.TRNGEN =0 before the write. |

Table 42-11: TRNG\_CTL Register Fields (Continued)

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 15 (R/W)           | RESEED     | Re-seed. The TRNG_CTL.RESEED bit is set-only, writing a 1 starts a re-seed cycle and writing a 0 has no effect. A re-seed cycle entails loading the TRNG_KEY[n] and TRNG_V[n] registers with random values generated internally; these values are not visible outside the TRNG core. This bit falls back to 0 automatically after the re-seed operation is complete; at that time the TRNG_BLKCNT register is reset to zero and the random data buffer is zeroized so that any new data read from the TRNG will use the new seed values. This bit is only present when post-processing is available and can only be set to 1 when TRNG_CTL.TRNGEN =1 before the write. Note that re-seeding can be done with the post-processor disabled (normally used to seed the post-processor before enabling it). When writing a 1 to this bit, all other bits in this register remain unchanged.                                                                                                                                                                                                            |
...
Figure 42-9: TRNG\_FRODETUNE Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000008_2695a4155247b49f44ef650498a90c6763ed6d504cf183939da9e0fbfd236f73.png)

Table 42-12: TRNG\_FRODETUNE Register Fields
...
Figure 42-10: TRNG\_FROEN Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000009_da3b5b316492819dd4bb89feb97b5aa0b8c7ab113af127fa77a96fcdd9ed1953.png)

Table 42-13: TRNG\_FROEN Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                                                                                                |
|--------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7:0 (R/W)          | FROS       | Enable Free-Running Oscillators. The TRNG_FROEN.FROS bits are the enables for the individual FROs. A 1 in bit [n] enables FRO n. The default state is all ones to enable all FROs after power-up. Note that the FROs are not actually started up before the TRNG_CTL.TRNGEN bit is set to 1. These bits are automatically forced to 0 (and cannot be written to 1) when the corresponding bit in the TRNG_ALMSTP register has value 1. |
...
Figure 42-11: TRNG\_INPUT[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000010_64b434aff67379f72b01854d54aeab4ae766e075052efbc779964288afcc54a7.png)

Table 42-14: TRNG\_INPUT[n] Register Fields
...
Figure 42-12: TRNG\_INTACK Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000011_e64563535cb84d62c8d1e1b9b1b73f483a7180475ce52a44b621d2a26e1569dc.png)

Table 42-15: TRNG\_INTACK Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                             |
|--------------------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 7 (RX/W1C)         | MBITFAIL   | Monobit Fail Acknowledge. Set the TRNG_INTACK.MBITFAIL bit to acknowledge the Monobit Fail In- terrupt. This also resets all counter and state bits in the TRNG_RUN[n] , TRNG_MONOBITCNT and the TRNG_POKER[n] registers (except for the TRNG_RUNCNT.LENMAX field). |
...
Table 42-15: TRNG\_INTACK Register Fields (Continued)
...
Figure 42-13: TRNG\_KEY[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000012_9d957fb33a39c5f2a26a28f96b6367badcd258582e95ce4fae1b4b19e93f852f.png)

Table 42-16: TRNG\_KEY[n] Register Fields
...
Figure 42-14: TRNG\_LFSR\_H Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000013_fe0119fc952200bb9ae7268e295ed528338ea1156a2ed886116fe549ff4c639c.png)

Table 42-17: TRNG\_LFSR\_H Register Fields
...
Figure 42-15: TRNG\_LFSR\_L Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000014_6369e896c9f4aa00d65bba247496865ee83e55ed3099ed4118ce6f81ca2cabda.png)

Table 42-18: TRNG\_LFSR\_L Register Fields
...
Figure 42-16: TRNG\_LFSR\_M Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000015_f3e3cfaf5e49799f27396417cf35563eec60dd6983634de889bf072f2c9e568b.png)

Table 42-19: TRNG\_LFSR\_M Register Fields
...
Figure 42-17: TRNG\_MONOBITCNT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000016_572913b1b2052cfb1571a430ff40577526d6907fef74b3b9dc1b605cea823736.png)

Table 42-20: TRNG\_MONOBITCNT Register Fields
...
Figure 42-18: TRNG\_OUTPUT[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000017_69cd83960080af67947d96736f7adcf03d2e410218453969967f5238a506e816.png)

Table 42-21: TRNG\_OUTPUT[n] Register Fields
...
Poker test result registers are read-only; writing them accesses the registers mapped at these same addresses. All counters in these registers are reset when writing a 1 to either the monobit fail acknowledge ( TRNG\_INTACK.MBITFAIL ), run fail acknowledge ( TRNG\_INTACK.RUNFAIL ) or the poker fail acknowledge ( TRNG\_INTACK.PKRFAIL ) bits.

Figure 42-19: TRNG\_POKER[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000018_0ef7cfc0131315a038ee52e4d3dcb60806eb73ea17c818105cdf527229670e5d.png)

Table 42-22: TRNG\_POKER[n] Register Fields
...
The TRNG\_RUNCNT registers are read-only; writing them accesses the other registers which are mapped at the same addresses. Unless otherwise indicated, all counters and state bits in these registers are reset when writing a 1 to either the Monobit Fail acknowledge ( TRNG\_INTACK.MBITFAIL ), Run Fail acknowledge ( TRNG\_INTACK.RUNFAIL ) or the Poker Fail acknowledge ( TRNG\_INTACK.PKRFAIL ) bits.

Figure 42-20: TRNG\_RUNCNT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000019_2c5bce395aa4ba85819f5a1f0251d09ac787a3f4b67b709fd4884a1b32d8696b.png)

Table 42-23: TRNG\_RUNCNT Register Fields
...
Figure 42-21: TRNG\_RUN[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000020_67cdc557bf13b50d6a24e38b50daef5468f78340f1bd38e73f9bfff83bb04b10.png)

Table 42-24: TRNG\_RUN[n] Register Fields
...
Table 42-24: TRNG\_RUN[n] Register Fields (Continued)
...
Figure 42-22: TRNG\_STAT Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000021_0aa765c1f2ad3f77e7e9f1dba2c02a357a8cbb29814516e250f3f50f4ac73868.png)

Table 42-25: TRNG\_STAT Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                                                                                                                                                                                                                                                                          |
|--------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 24 (R/NW)          | NEEDCLK    | Need Clock. When the TRNG_STAT.NEEDCLK bit is set, it indicates that the TRNG is busy generating entropy or is in one of its test modes; the module clock may not be turned off.                                                                                                                                                                                 |
...
| 8 (R/NW)           | TSTRDY     | Test Ready. When the TRNG_STAT.TSTRDY bit is set, it indicates that data for known-answer tests on the monobit test, run test, poker test and post-processor functions can be written to the TRNG_INPUT[n] registers. When testing the post-processor, result data can be read from those same registers when this bit has become 1 again (after dropping to 0). |

Table 42-25: TRNG\_STAT Register Fields (Continued)
...
| 4 (R/NW)           | RUNFAIL    | Run Fail. When the TRNG_STAT.RUNFAIL bit is set, the run test logic monitoring data shifted into the main LFSR detected an out-of-bounds value for at least one of the TRNG_RUN[n].CNTZEROS or TRNG_RUN[n].CNTONES counters after check- ing 20,000 bits (test T3 as specified in the AIS-31 standard).                        |
...
Figure 42-23: TRNG\_TEST Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000022_f0df91eaa53efa32ffeb661d79a91a18402164f73f6f025feacd909a55ad1209.png)

Table 42-26: TRNG\_TEST Register Fields

| Bit No. (Access)   | Bit Name   | Description/Enumeration                                                                                      |
|--------------------|------------|--------------------------------------------------------------------------------------------------------------|
| 31 (R/W)           | IRQ        | Test IRQ. When the TRNG_TEST.IRQ bit is set force irq output HIGH for interrupt signal connectivity testing. |
| 31 (R/W)           | IRQ        | 0 Do not force IRQ high                                                                                      |
| 31 (R/W)           | IRQ        | 1 Force IRQ output high                                                                                      |

Table 42-26: TRNG\_TEST Register Fields (Continued)
...
| 4 (R/W)            | CONTPKR    | Continue Poker. When the TRNG_TEST.CONTPKR bit is set, Monobit Test and Poker Test keep running continuously by not resetting the Monobit count ( TRNG_MONOBITCNT ) and poker counters ( TRNG_POKER[n] register) at the end of each 20,000 bits test block. This bit can only be set to 1 when TRNG_CTL.TSTMODE =1. 0 Do not continue poker test                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 2 (R/W)            | PATTDET Test When      | Pattern Detect. the TRNG_TEST.PATTDET bit is set, it repeatedly feeds test pattern (PATTERN) into the error detection circuit of the FRO selected by the                                                                                                                                                                                 |
| 1 (R/W)            | PATTFRO                | 1 Repeat feed test pattern Test Pattern FRO. When the TRNG_TEST.PATTFRO bit is set, it repeatedly feeds test pattern (PATTERN) into the delay chain of the FRO selected by the test select ( TRNG_TEST.SEL ) field by forcing the corresponding FRO enable (FROEN) out- put LOW. This bit can only be set to 1 when TRNG_CTL.TSTMODE =1. |
| 0 (R/W)            | ENOUT Test Enable Out. | When the TRNG_TEST.ENOUT bit is set, it enables the tst_fro_clk_out output, connecting to the FRO selected by the test select ( TRNG_TEST.SEL ) field. This bit can only be set to 1 when TRNG_CTL.TSTMODE =1. 0 Disable tst_fro_clk_out                                                                                                 |
...
Figure 42-24: TRNG\_V[n] Register Diagram

![Image](45_True_Random_Number_Generator_(TRNG)_artifacts/image_000023_691974d2a7c4c3bea08741618e6d64949dbb48fbaee4081feb9fdd8cc9f82c6e.png)

Table 42-27: TRNG\_V[n] Register Fields
