	...
	struct tdmc_slave *slave)
{
	...
	dai_cfg.out_active_channels = 0x1;
...
/*
 * There is no mixer in front of the DSP, the DSP has access to each channel
 * induvidually, but we want to be able to tell the DAPM framework that a
 * channel is currently not used inside the DSP, so those are all virtual
 * switches.
 */
static const struct snd_kcontrol_new ssm4329_dsp_mixer_ctrls[] = {
	SOC_DAPM_SINGLE_VIRT("ADC Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Input SRC Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Output SRC Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Serial Port 1 Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Serial Port 2 Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Voltage Sense Switch", 1),
	SOC_DAPM_SINGLE_VIRT("Current Sense Switch", 1),
	SOC_DAPM_SINGLE_VIRT("VBAT Switch", 1),
};
...
static const struct snd_soc_dapm_widget ssm4329_dapm_widgets[] = {
	...
	SOC_MIXER_ARRAY("DSP", SND_SOC_NOPM, 0, 0, ssm4329_dsp_mixer_ctrls),
...
static const struct snd_soc_dapm_route ssm4329_dapm_routes[] = {
	...
	{ "DSP", "Voltage Sense Switch", "Voltage Sense" },
	{ "DSP", "Current Sense Switch", "Current Sense" },
	{ "DSP", "VBAT Switch", "VBAT" },
	{ "DSP", "Serial Port 1 Switch", "SP1 IN" },
	{ "DSP", "Serial Port 2 Switch", "SP2 IN" },
	{ "DSP", "ADC Switch", "ADC" },
	{ "DSP", "Input SRC Switch", "Input SRC Channel 1 Mux" },
	{ "DSP", "Input SRC Switch", "Input SRC Channel 2 Mux" },
	{ "DSP", "Output SRC Switch", "Output SRC Channel 1 Mux" },
	{ "DSP", "Output SRC Switch", "Output SRC Channel 2 Mux" },

	{ "DSP Processor", NULL, "DSP" },
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	unsigned int reg_base = dai->driver->base;
...
static int ssm4329_set_tristate(struct snd_soc_dai *dai, int tristate)
{
	...
	return regmap_update_bits(ssm4329->regmap,
		dai->driver->base + SSM4329_REG_SPT_CTRL2,
