...
#define DEFINE_CONTAINER_FN(name, type, member) \
type *name(typeof(((type *)0)->member) *member) \
{ \
	return container_of(member, type, member); \
}

static DEFINE_CONTAINER_FN(dev_to_tdmc_master, struct tdmc_master, dev);
static DEFINE_CONTAINER_FN(dev_to_tdmc_slave, struct tdmc_slave, dev);
static DEFINE_CONTAINER_FN(driver_to_tdmc_slave_driver,
...
#endif
		slave = tdmc_slave_alloc(master, 1);
		if (!slave) {
//			dev_err(&master->dev, "Failed to allocate slave for node %s\n",
//				np->full_name);
//			continue;
		}
...
//			dev_err(&master->dev, "Failed to register slave for node %s\n",
//				np->full_name);
//			continue;
		}
//	}
...
static int tdmc_regmap_write(void *context, unsigned int reg,
	unsigned int val)
{
	struct tdmc_slave *slave = context;
...
static int tdmc_regmap_read(void *context, unsigned int reg,
	unsigned int *val)
{
	struct tdmc_slave *slave = context;
	struct tdmc_master *master = slave->master;
...
static const struct regmap_bus tdmc_regmap_bus = {
	.reg_write = tdmc_regmap_write,
	.reg_read = tdmc_regmap_read,
};
...
{
	return devm_regmap_init(&slave->dev, &tdmc_regmap_bus, slave, config);
}
